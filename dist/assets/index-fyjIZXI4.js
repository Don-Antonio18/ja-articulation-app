(function () {
	const o = document.createElement("link").relList;
	if (o && o.supports && o.supports("modulepreload")) return;
	for (const d of document.querySelectorAll('link[rel="modulepreload"]')) f(d);
	new MutationObserver((d) => {
		for (const m of d)
			if (m.type === "childList")
				for (const S of m.addedNodes)
					S.tagName === "LINK" && S.rel === "modulepreload" && f(S);
	}).observe(document, { childList: !0, subtree: !0 });
	function s(d) {
		const m = {};
		return (
			d.integrity && (m.integrity = d.integrity),
			d.referrerPolicy && (m.referrerPolicy = d.referrerPolicy),
			d.crossOrigin === "use-credentials"
				? (m.credentials = "include")
				: d.crossOrigin === "anonymous"
					? (m.credentials = "omit")
					: (m.credentials = "same-origin"),
			m
		);
	}
	function f(d) {
		if (d.ep) return;
		d.ep = !0;
		const m = s(d);
		fetch(d.href, m);
	}
})();
function bh(c) {
	return c && c.__esModule && Object.prototype.hasOwnProperty.call(c, "default")
		? c.default
		: c;
}
var Of = { exports: {} },
	Du = {};
/**
 * @license React
 * react-jsx-runtime.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var Pd;
function ov() {
	if (Pd) return Du;
	Pd = 1;
	var c = Symbol.for("react.transitional.element"),
		o = Symbol.for("react.fragment");
	function s(f, d, m) {
		var S = null;
		if (
			(m !== void 0 && (S = "" + m),
			d.key !== void 0 && (S = "" + d.key),
			"key" in d)
		) {
			m = {};
			for (var A in d) A !== "key" && (m[A] = d[A]);
		} else m = d;
		return (
			(d = m.ref),
			{ $$typeof: c, type: f, key: S, ref: d !== void 0 ? d : null, props: m }
		);
	}
	return ((Du.Fragment = o), (Du.jsx = s), (Du.jsxs = s), Du);
}
var th;
function sv() {
	return (th || ((th = 1), (Of.exports = ov())), Of.exports);
}
var V = sv(),
	Df = { exports: {} },
	F = {};
/**
 * @license React
 * react.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var eh;
function dv() {
	if (eh) return F;
	eh = 1;
	var c = Symbol.for("react.transitional.element"),
		o = Symbol.for("react.portal"),
		s = Symbol.for("react.fragment"),
		f = Symbol.for("react.strict_mode"),
		d = Symbol.for("react.profiler"),
		m = Symbol.for("react.consumer"),
		S = Symbol.for("react.context"),
		A = Symbol.for("react.forward_ref"),
		b = Symbol.for("react.suspense"),
		v = Symbol.for("react.memo"),
		C = Symbol.for("react.lazy"),
		R = Symbol.for("react.activity"),
		H = Symbol.iterator;
	function w(g) {
		return g === null || typeof g != "object"
			? null
			: ((g = (H && g[H]) || g["@@iterator"]),
				typeof g == "function" ? g : null);
	}
	var K = {
			isMounted: function () {
				return !1;
			},
			enqueueForceUpdate: function () {},
			enqueueReplaceState: function () {},
			enqueueSetState: function () {},
		},
		L = Object.assign,
		G = {};
	function j(g, x, q) {
		((this.props = g),
			(this.context = x),
			(this.refs = G),
			(this.updater = q || K));
	}
	((j.prototype.isReactComponent = {}),
		(j.prototype.setState = function (g, x) {
			if (typeof g != "object" && typeof g != "function" && g != null)
				throw Error(
					"takes an object of state variables to update or a function which returns an object of state variables.",
				);
			this.updater.enqueueSetState(this, g, x, "setState");
		}),
		(j.prototype.forceUpdate = function (g) {
			this.updater.enqueueForceUpdate(this, g, "forceUpdate");
		}));
	function I() {}
	I.prototype = j.prototype;
	function $(g, x, q) {
		((this.props = g),
			(this.context = x),
			(this.refs = G),
			(this.updater = q || K));
	}
	var yt = ($.prototype = new I());
	((yt.constructor = $), L(yt, j.prototype), (yt.isPureReactComponent = !0));
	var dt = Array.isArray;
	function Tt() {}
	var k = { H: null, A: null, T: null, S: null },
		Dt = Object.prototype.hasOwnProperty;
	function wt(g, x, q) {
		var X = q.ref;
		return {
			$$typeof: c,
			type: g,
			key: x,
			ref: X !== void 0 ? X : null,
			props: q,
		};
	}
	function Ne(g, x) {
		return wt(g.type, x, g.props);
	}
	function ye(g) {
		return typeof g == "object" && g !== null && g.$$typeof === c;
	}
	function Jt(g) {
		var x = { "=": "=0", ":": "=2" };
		return (
			"$" +
			g.replace(/[=:]/g, function (q) {
				return x[q];
			})
		);
	}
	var xe = /\/+/g;
	function ve(g, x) {
		return typeof g == "object" && g !== null && g.key != null
			? Jt("" + g.key)
			: x.toString(36);
	}
	function xt(g) {
		switch (g.status) {
			case "fulfilled":
				return g.value;
			case "rejected":
				throw g.reason;
			default:
				switch (
					(typeof g.status == "string"
						? g.then(Tt, Tt)
						: ((g.status = "pending"),
							g.then(
								function (x) {
									g.status === "pending" &&
										((g.status = "fulfilled"), (g.value = x));
								},
								function (x) {
									g.status === "pending" &&
										((g.status = "rejected"), (g.reason = x));
								},
							)),
					g.status)
				) {
					case "fulfilled":
						return g.value;
					case "rejected":
						throw g.reason;
				}
		}
		throw g;
	}
	function D(g, x, q, X, P) {
		var lt = typeof g;
		(lt === "undefined" || lt === "boolean") && (g = null);
		var st = !1;
		if (g === null) st = !0;
		else
			switch (lt) {
				case "bigint":
				case "string":
				case "number":
					st = !0;
					break;
				case "object":
					switch (g.$$typeof) {
						case c:
						case o:
							st = !0;
							break;
						case C:
							return ((st = g._init), D(st(g._payload), x, q, X, P));
					}
			}
		if (st)
			return (
				(P = P(g)),
				(st = X === "" ? "." + ve(g, 0) : X),
				dt(P)
					? ((q = ""),
						st != null && (q = st.replace(xe, "$&/") + "/"),
						D(P, x, q, "", function (ja) {
							return ja;
						}))
					: P != null &&
						(ye(P) &&
							(P = Ne(
								P,
								q +
									(P.key == null || (g && g.key === P.key)
										? ""
										: ("" + P.key).replace(xe, "$&/") + "/") +
									st,
							)),
						x.push(P)),
				1
			);
		st = 0;
		var Wt = X === "" ? "." : X + ":";
		if (dt(g))
			for (var Ct = 0; Ct < g.length; Ct++)
				((X = g[Ct]), (lt = Wt + ve(X, Ct)), (st += D(X, x, q, lt, P)));
		else if (((Ct = w(g)), typeof Ct == "function"))
			for (g = Ct.call(g), Ct = 0; !(X = g.next()).done; )
				((X = X.value), (lt = Wt + ve(X, Ct++)), (st += D(X, x, q, lt, P)));
		else if (lt === "object") {
			if (typeof g.then == "function") return D(xt(g), x, q, X, P);
			throw (
				(x = String(g)),
				Error(
					"Objects are not valid as a React child (found: " +
						(x === "[object Object]"
							? "object with keys {" + Object.keys(g).join(", ") + "}"
							: x) +
						"). If you meant to render a collection of children, use an array instead.",
				)
			);
		}
		return st;
	}
	function B(g, x, q) {
		if (g == null) return g;
		var X = [],
			P = 0;
		return (
			D(g, X, "", "", function (lt) {
				return x.call(q, lt, P++);
			}),
			X
		);
	}
	function W(g) {
		if (g._status === -1) {
			var x = g._result;
			((x = x()),
				x.then(
					function (q) {
						(g._status === 0 || g._status === -1) &&
							((g._status = 1), (g._result = q));
					},
					function (q) {
						(g._status === 0 || g._status === -1) &&
							((g._status = 2), (g._result = q));
					},
				),
				g._status === -1 && ((g._status = 0), (g._result = x)));
		}
		if (g._status === 1) return g._result.default;
		throw g._result;
	}
	var vt =
			typeof reportError == "function"
				? reportError
				: function (g) {
						if (
							typeof window == "object" &&
							typeof window.ErrorEvent == "function"
						) {
							var x = new window.ErrorEvent("error", {
								bubbles: !0,
								cancelable: !0,
								message:
									typeof g == "object" &&
									g !== null &&
									typeof g.message == "string"
										? String(g.message)
										: String(g),
								error: g,
							});
							if (!window.dispatchEvent(x)) return;
						} else if (
							typeof process == "object" &&
							typeof process.emit == "function"
						) {
							process.emit("uncaughtException", g);
							return;
						}
						console.error(g);
					},
		bt = {
			map: B,
			forEach: function (g, x, q) {
				B(
					g,
					function () {
						x.apply(this, arguments);
					},
					q,
				);
			},
			count: function (g) {
				var x = 0;
				return (
					B(g, function () {
						x++;
					}),
					x
				);
			},
			toArray: function (g) {
				return (
					B(g, function (x) {
						return x;
					}) || []
				);
			},
			only: function (g) {
				if (!ye(g))
					throw Error(
						"React.Children.only expected to receive a single React element child.",
					);
				return g;
			},
		};
	return (
		(F.Activity = R),
		(F.Children = bt),
		(F.Component = j),
		(F.Fragment = s),
		(F.Profiler = d),
		(F.PureComponent = $),
		(F.StrictMode = f),
		(F.Suspense = b),
		(F.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = k),
		(F.__COMPILER_RUNTIME = {
			__proto__: null,
			c: function (g) {
				return k.H.useMemoCache(g);
			},
		}),
		(F.cache = function (g) {
			return function () {
				return g.apply(null, arguments);
			};
		}),
		(F.cacheSignal = function () {
			return null;
		}),
		(F.cloneElement = function (g, x, q) {
			if (g == null)
				throw Error(
					"The argument must be a React element, but you passed " + g + ".",
				);
			var X = L({}, g.props),
				P = g.key;
			if (x != null)
				for (lt in (x.key !== void 0 && (P = "" + x.key), x))
					!Dt.call(x, lt) ||
						lt === "key" ||
						lt === "__self" ||
						lt === "__source" ||
						(lt === "ref" && x.ref === void 0) ||
						(X[lt] = x[lt]);
			var lt = arguments.length - 2;
			if (lt === 1) X.children = q;
			else if (1 < lt) {
				for (var st = Array(lt), Wt = 0; Wt < lt; Wt++)
					st[Wt] = arguments[Wt + 2];
				X.children = st;
			}
			return wt(g.type, P, X);
		}),
		(F.createContext = function (g) {
			return (
				(g = {
					$$typeof: S,
					_currentValue: g,
					_currentValue2: g,
					_threadCount: 0,
					Provider: null,
					Consumer: null,
				}),
				(g.Provider = g),
				(g.Consumer = { $$typeof: m, _context: g }),
				g
			);
		}),
		(F.createElement = function (g, x, q) {
			var X,
				P = {},
				lt = null;
			if (x != null)
				for (X in (x.key !== void 0 && (lt = "" + x.key), x))
					Dt.call(x, X) &&
						X !== "key" &&
						X !== "__self" &&
						X !== "__source" &&
						(P[X] = x[X]);
			var st = arguments.length - 2;
			if (st === 1) P.children = q;
			else if (1 < st) {
				for (var Wt = Array(st), Ct = 0; Ct < st; Ct++)
					Wt[Ct] = arguments[Ct + 2];
				P.children = Wt;
			}
			if (g && g.defaultProps)
				for (X in ((st = g.defaultProps), st))
					P[X] === void 0 && (P[X] = st[X]);
			return wt(g, lt, P);
		}),
		(F.createRef = function () {
			return { current: null };
		}),
		(F.forwardRef = function (g) {
			return { $$typeof: A, render: g };
		}),
		(F.isValidElement = ye),
		(F.lazy = function (g) {
			return { $$typeof: C, _payload: { _status: -1, _result: g }, _init: W };
		}),
		(F.memo = function (g, x) {
			return { $$typeof: v, type: g, compare: x === void 0 ? null : x };
		}),
		(F.startTransition = function (g) {
			var x = k.T,
				q = {};
			k.T = q;
			try {
				var X = g(),
					P = k.S;
				(P !== null && P(q, X),
					typeof X == "object" &&
						X !== null &&
						typeof X.then == "function" &&
						X.then(Tt, vt));
			} catch (lt) {
				vt(lt);
			} finally {
				(x !== null && q.types !== null && (x.types = q.types), (k.T = x));
			}
		}),
		(F.unstable_useCacheRefresh = function () {
			return k.H.useCacheRefresh();
		}),
		(F.use = function (g) {
			return k.H.use(g);
		}),
		(F.useActionState = function (g, x, q) {
			return k.H.useActionState(g, x, q);
		}),
		(F.useCallback = function (g, x) {
			return k.H.useCallback(g, x);
		}),
		(F.useContext = function (g) {
			return k.H.useContext(g);
		}),
		(F.useDebugValue = function () {}),
		(F.useDeferredValue = function (g, x) {
			return k.H.useDeferredValue(g, x);
		}),
		(F.useEffect = function (g, x) {
			return k.H.useEffect(g, x);
		}),
		(F.useEffectEvent = function (g) {
			return k.H.useEffectEvent(g);
		}),
		(F.useId = function () {
			return k.H.useId();
		}),
		(F.useImperativeHandle = function (g, x, q) {
			return k.H.useImperativeHandle(g, x, q);
		}),
		(F.useInsertionEffect = function (g, x) {
			return k.H.useInsertionEffect(g, x);
		}),
		(F.useLayoutEffect = function (g, x) {
			return k.H.useLayoutEffect(g, x);
		}),
		(F.useMemo = function (g, x) {
			return k.H.useMemo(g, x);
		}),
		(F.useOptimistic = function (g, x) {
			return k.H.useOptimistic(g, x);
		}),
		(F.useReducer = function (g, x, q) {
			return k.H.useReducer(g, x, q);
		}),
		(F.useRef = function (g) {
			return k.H.useRef(g);
		}),
		(F.useState = function (g) {
			return k.H.useState(g);
		}),
		(F.useSyncExternalStore = function (g, x, q) {
			return k.H.useSyncExternalStore(g, x, q);
		}),
		(F.useTransition = function () {
			return k.H.useTransition();
		}),
		(F.version = "19.2.6"),
		F
	);
}
var lh;
function Xf() {
	return (lh || ((lh = 1), (Df.exports = dv())), Df.exports);
}
var M = Xf();
const hv = bh(M);
var Cf = { exports: {} },
	Cu = {},
	Uf = { exports: {} },
	Nf = {};
/**
 * @license React
 * scheduler.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var ah;
function mv() {
	return (
		ah ||
			((ah = 1),
			(function (c) {
				function o(D, B) {
					var W = D.length;
					D.push(B);
					t: for (; 0 < W; ) {
						var vt = (W - 1) >>> 1,
							bt = D[vt];
						if (0 < d(bt, B)) ((D[vt] = B), (D[W] = bt), (W = vt));
						else break t;
					}
				}
				function s(D) {
					return D.length === 0 ? null : D[0];
				}
				function f(D) {
					if (D.length === 0) return null;
					var B = D[0],
						W = D.pop();
					if (W !== B) {
						D[0] = W;
						t: for (var vt = 0, bt = D.length, g = bt >>> 1; vt < g; ) {
							var x = 2 * (vt + 1) - 1,
								q = D[x],
								X = x + 1,
								P = D[X];
							if (0 > d(q, W))
								X < bt && 0 > d(P, q)
									? ((D[vt] = P), (D[X] = W), (vt = X))
									: ((D[vt] = q), (D[x] = W), (vt = x));
							else if (X < bt && 0 > d(P, W))
								((D[vt] = P), (D[X] = W), (vt = X));
							else break t;
						}
					}
					return B;
				}
				function d(D, B) {
					var W = D.sortIndex - B.sortIndex;
					return W !== 0 ? W : D.id - B.id;
				}
				if (
					((c.unstable_now = void 0),
					typeof performance == "object" &&
						typeof performance.now == "function")
				) {
					var m = performance;
					c.unstable_now = function () {
						return m.now();
					};
				} else {
					var S = Date,
						A = S.now();
					c.unstable_now = function () {
						return S.now() - A;
					};
				}
				var b = [],
					v = [],
					C = 1,
					R = null,
					H = 3,
					w = !1,
					K = !1,
					L = !1,
					G = !1,
					j = typeof setTimeout == "function" ? setTimeout : null,
					I = typeof clearTimeout == "function" ? clearTimeout : null,
					$ = typeof setImmediate < "u" ? setImmediate : null;
				function yt(D) {
					for (var B = s(v); B !== null; ) {
						if (B.callback === null) f(v);
						else if (B.startTime <= D)
							(f(v), (B.sortIndex = B.expirationTime), o(b, B));
						else break;
						B = s(v);
					}
				}
				function dt(D) {
					if (((L = !1), yt(D), !K))
						if (s(b) !== null) ((K = !0), Tt || ((Tt = !0), Jt()));
						else {
							var B = s(v);
							B !== null && xt(dt, B.startTime - D);
						}
				}
				var Tt = !1,
					k = -1,
					Dt = 5,
					wt = -1;
				function Ne() {
					return G ? !0 : !(c.unstable_now() - wt < Dt);
				}
				function ye() {
					if (((G = !1), Tt)) {
						var D = c.unstable_now();
						wt = D;
						var B = !0;
						try {
							t: {
								((K = !1), L && ((L = !1), I(k), (k = -1)), (w = !0));
								var W = H;
								try {
									e: {
										for (
											yt(D), R = s(b);
											R !== null && !(R.expirationTime > D && Ne());
										) {
											var vt = R.callback;
											if (typeof vt == "function") {
												((R.callback = null), (H = R.priorityLevel));
												var bt = vt(R.expirationTime <= D);
												if (((D = c.unstable_now()), typeof bt == "function")) {
													((R.callback = bt), yt(D), (B = !0));
													break e;
												}
												(R === s(b) && f(b), yt(D));
											} else f(b);
											R = s(b);
										}
										if (R !== null) B = !0;
										else {
											var g = s(v);
											(g !== null && xt(dt, g.startTime - D), (B = !1));
										}
									}
									break t;
								} finally {
									((R = null), (H = W), (w = !1));
								}
								B = void 0;
							}
						} finally {
							B ? Jt() : (Tt = !1);
						}
					}
				}
				var Jt;
				if (typeof $ == "function")
					Jt = function () {
						$(ye);
					};
				else if (typeof MessageChannel < "u") {
					var xe = new MessageChannel(),
						ve = xe.port2;
					((xe.port1.onmessage = ye),
						(Jt = function () {
							ve.postMessage(null);
						}));
				} else
					Jt = function () {
						j(ye, 0);
					};
				function xt(D, B) {
					k = j(function () {
						D(c.unstable_now());
					}, B);
				}
				((c.unstable_IdlePriority = 5),
					(c.unstable_ImmediatePriority = 1),
					(c.unstable_LowPriority = 4),
					(c.unstable_NormalPriority = 3),
					(c.unstable_Profiling = null),
					(c.unstable_UserBlockingPriority = 2),
					(c.unstable_cancelCallback = function (D) {
						D.callback = null;
					}),
					(c.unstable_forceFrameRate = function (D) {
						0 > D || 125 < D
							? console.error(
									"forceFrameRate takes a positive int between 0 and 125, forcing frame rates higher than 125 fps is not supported",
								)
							: (Dt = 0 < D ? Math.floor(1e3 / D) : 5);
					}),
					(c.unstable_getCurrentPriorityLevel = function () {
						return H;
					}),
					(c.unstable_next = function (D) {
						switch (H) {
							case 1:
							case 2:
							case 3:
								var B = 3;
								break;
							default:
								B = H;
						}
						var W = H;
						H = B;
						try {
							return D();
						} finally {
							H = W;
						}
					}),
					(c.unstable_requestPaint = function () {
						G = !0;
					}),
					(c.unstable_runWithPriority = function (D, B) {
						switch (D) {
							case 1:
							case 2:
							case 3:
							case 4:
							case 5:
								break;
							default:
								D = 3;
						}
						var W = H;
						H = D;
						try {
							return B();
						} finally {
							H = W;
						}
					}),
					(c.unstable_scheduleCallback = function (D, B, W) {
						var vt = c.unstable_now();
						switch (
							(typeof W == "object" && W !== null
								? ((W = W.delay),
									(W = typeof W == "number" && 0 < W ? vt + W : vt))
								: (W = vt),
							D)
						) {
							case 1:
								var bt = -1;
								break;
							case 2:
								bt = 250;
								break;
							case 5:
								bt = 1073741823;
								break;
							case 4:
								bt = 1e4;
								break;
							default:
								bt = 5e3;
						}
						return (
							(bt = W + bt),
							(D = {
								id: C++,
								callback: B,
								priorityLevel: D,
								startTime: W,
								expirationTime: bt,
								sortIndex: -1,
							}),
							W > vt
								? ((D.sortIndex = W),
									o(v, D),
									s(b) === null &&
										D === s(v) &&
										(L ? (I(k), (k = -1)) : (L = !0), xt(dt, W - vt)))
								: ((D.sortIndex = bt),
									o(b, D),
									K || w || ((K = !0), Tt || ((Tt = !0), Jt()))),
							D
						);
					}),
					(c.unstable_shouldYield = Ne),
					(c.unstable_wrapCallback = function (D) {
						var B = H;
						return function () {
							var W = H;
							H = B;
							try {
								return D.apply(this, arguments);
							} finally {
								H = W;
							}
						};
					}));
			})(Nf)),
		Nf
	);
}
var uh;
function yv() {
	return (uh || ((uh = 1), (Uf.exports = mv())), Uf.exports);
}
var xf = { exports: {} },
	$t = {};
/**
 * @license React
 * react-dom.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var nh;
function vv() {
	if (nh) return $t;
	nh = 1;
	var c = Xf();
	function o(b) {
		var v = "https://react.dev/errors/" + b;
		if (1 < arguments.length) {
			v += "?args[]=" + encodeURIComponent(arguments[1]);
			for (var C = 2; C < arguments.length; C++)
				v += "&args[]=" + encodeURIComponent(arguments[C]);
		}
		return (
			"Minified React error #" +
			b +
			"; visit " +
			v +
			" for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
		);
	}
	function s() {}
	var f = {
			d: {
				f: s,
				r: function () {
					throw Error(o(522));
				},
				D: s,
				C: s,
				L: s,
				m: s,
				X: s,
				S: s,
				M: s,
			},
			p: 0,
			findDOMNode: null,
		},
		d = Symbol.for("react.portal");
	function m(b, v, C) {
		var R =
			3 < arguments.length && arguments[3] !== void 0 ? arguments[3] : null;
		return {
			$$typeof: d,
			key: R == null ? null : "" + R,
			children: b,
			containerInfo: v,
			implementation: C,
		};
	}
	var S = c.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
	function A(b, v) {
		if (b === "font") return "";
		if (typeof v == "string") return v === "use-credentials" ? v : "";
	}
	return (
		($t.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE = f),
		($t.createPortal = function (b, v) {
			var C =
				2 < arguments.length && arguments[2] !== void 0 ? arguments[2] : null;
			if (!v || (v.nodeType !== 1 && v.nodeType !== 9 && v.nodeType !== 11))
				throw Error(o(299));
			return m(b, v, null, C);
		}),
		($t.flushSync = function (b) {
			var v = S.T,
				C = f.p;
			try {
				if (((S.T = null), (f.p = 2), b)) return b();
			} finally {
				((S.T = v), (f.p = C), f.d.f());
			}
		}),
		($t.preconnect = function (b, v) {
			typeof b == "string" &&
				(v
					? ((v = v.crossOrigin),
						(v =
							typeof v == "string"
								? v === "use-credentials"
									? v
									: ""
								: void 0))
					: (v = null),
				f.d.C(b, v));
		}),
		($t.prefetchDNS = function (b) {
			typeof b == "string" && f.d.D(b);
		}),
		($t.preinit = function (b, v) {
			if (typeof b == "string" && v && typeof v.as == "string") {
				var C = v.as,
					R = A(C, v.crossOrigin),
					H = typeof v.integrity == "string" ? v.integrity : void 0,
					w = typeof v.fetchPriority == "string" ? v.fetchPriority : void 0;
				C === "style"
					? f.d.S(b, typeof v.precedence == "string" ? v.precedence : void 0, {
							crossOrigin: R,
							integrity: H,
							fetchPriority: w,
						})
					: C === "script" &&
						f.d.X(b, {
							crossOrigin: R,
							integrity: H,
							fetchPriority: w,
							nonce: typeof v.nonce == "string" ? v.nonce : void 0,
						});
			}
		}),
		($t.preinitModule = function (b, v) {
			if (typeof b == "string")
				if (typeof v == "object" && v !== null) {
					if (v.as == null || v.as === "script") {
						var C = A(v.as, v.crossOrigin);
						f.d.M(b, {
							crossOrigin: C,
							integrity: typeof v.integrity == "string" ? v.integrity : void 0,
							nonce: typeof v.nonce == "string" ? v.nonce : void 0,
						});
					}
				} else v == null && f.d.M(b);
		}),
		($t.preload = function (b, v) {
			if (
				typeof b == "string" &&
				typeof v == "object" &&
				v !== null &&
				typeof v.as == "string"
			) {
				var C = v.as,
					R = A(C, v.crossOrigin);
				f.d.L(b, C, {
					crossOrigin: R,
					integrity: typeof v.integrity == "string" ? v.integrity : void 0,
					nonce: typeof v.nonce == "string" ? v.nonce : void 0,
					type: typeof v.type == "string" ? v.type : void 0,
					fetchPriority:
						typeof v.fetchPriority == "string" ? v.fetchPriority : void 0,
					referrerPolicy:
						typeof v.referrerPolicy == "string" ? v.referrerPolicy : void 0,
					imageSrcSet:
						typeof v.imageSrcSet == "string" ? v.imageSrcSet : void 0,
					imageSizes: typeof v.imageSizes == "string" ? v.imageSizes : void 0,
					media: typeof v.media == "string" ? v.media : void 0,
				});
			}
		}),
		($t.preloadModule = function (b, v) {
			if (typeof b == "string")
				if (v) {
					var C = A(v.as, v.crossOrigin);
					f.d.m(b, {
						as: typeof v.as == "string" && v.as !== "script" ? v.as : void 0,
						crossOrigin: C,
						integrity: typeof v.integrity == "string" ? v.integrity : void 0,
					});
				} else f.d.m(b);
		}),
		($t.requestFormReset = function (b) {
			f.d.r(b);
		}),
		($t.unstable_batchedUpdates = function (b, v) {
			return b(v);
		}),
		($t.useFormState = function (b, v, C) {
			return S.H.useFormState(b, v, C);
		}),
		($t.useFormStatus = function () {
			return S.H.useHostTransitionStatus();
		}),
		($t.version = "19.2.6"),
		$t
	);
}
var ih;
function gv() {
	if (ih) return xf.exports;
	ih = 1;
	function c() {
		if (
			!(
				typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
				typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
			)
		)
			try {
				__REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(c);
			} catch (o) {
				console.error(o);
			}
	}
	return (c(), (xf.exports = vv()), xf.exports);
}
/**
 * @license React
 * react-dom-client.production.js
 *
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */ var ch;
function pv() {
	if (ch) return Cu;
	ch = 1;
	var c = yv(),
		o = Xf(),
		s = gv();
	function f(t) {
		var e = "https://react.dev/errors/" + t;
		if (1 < arguments.length) {
			e += "?args[]=" + encodeURIComponent(arguments[1]);
			for (var l = 2; l < arguments.length; l++)
				e += "&args[]=" + encodeURIComponent(arguments[l]);
		}
		return (
			"Minified React error #" +
			t +
			"; visit " +
			e +
			" for the full message or use the non-minified dev environment for full errors and additional helpful warnings."
		);
	}
	function d(t) {
		return !(!t || (t.nodeType !== 1 && t.nodeType !== 9 && t.nodeType !== 11));
	}
	function m(t) {
		var e = t,
			l = t;
		if (t.alternate) for (; e.return; ) e = e.return;
		else {
			t = e;
			do ((e = t), (e.flags & 4098) !== 0 && (l = e.return), (t = e.return));
			while (t);
		}
		return e.tag === 3 ? l : null;
	}
	function S(t) {
		if (t.tag === 13) {
			var e = t.memoizedState;
			if (
				(e === null && ((t = t.alternate), t !== null && (e = t.memoizedState)),
				e !== null)
			)
				return e.dehydrated;
		}
		return null;
	}
	function A(t) {
		if (t.tag === 31) {
			var e = t.memoizedState;
			if (
				(e === null && ((t = t.alternate), t !== null && (e = t.memoizedState)),
				e !== null)
			)
				return e.dehydrated;
		}
		return null;
	}
	function b(t) {
		if (m(t) !== t) throw Error(f(188));
	}
	function v(t) {
		var e = t.alternate;
		if (!e) {
			if (((e = m(t)), e === null)) throw Error(f(188));
			return e !== t ? null : t;
		}
		for (var l = t, a = e; ; ) {
			var u = l.return;
			if (u === null) break;
			var n = u.alternate;
			if (n === null) {
				if (((a = u.return), a !== null)) {
					l = a;
					continue;
				}
				break;
			}
			if (u.child === n.child) {
				for (n = u.child; n; ) {
					if (n === l) return (b(u), t);
					if (n === a) return (b(u), e);
					n = n.sibling;
				}
				throw Error(f(188));
			}
			if (l.return !== a.return) ((l = u), (a = n));
			else {
				for (var i = !1, r = u.child; r; ) {
					if (r === l) {
						((i = !0), (l = u), (a = n));
						break;
					}
					if (r === a) {
						((i = !0), (a = u), (l = n));
						break;
					}
					r = r.sibling;
				}
				if (!i) {
					for (r = n.child; r; ) {
						if (r === l) {
							((i = !0), (l = n), (a = u));
							break;
						}
						if (r === a) {
							((i = !0), (a = n), (l = u));
							break;
						}
						r = r.sibling;
					}
					if (!i) throw Error(f(189));
				}
			}
			if (l.alternate !== a) throw Error(f(190));
		}
		if (l.tag !== 3) throw Error(f(188));
		return l.stateNode.current === l ? t : e;
	}
	function C(t) {
		var e = t.tag;
		if (e === 5 || e === 26 || e === 27 || e === 6) return t;
		for (t = t.child; t !== null; ) {
			if (((e = C(t)), e !== null)) return e;
			t = t.sibling;
		}
		return null;
	}
	var R = Object.assign,
		H = Symbol.for("react.element"),
		w = Symbol.for("react.transitional.element"),
		K = Symbol.for("react.portal"),
		L = Symbol.for("react.fragment"),
		G = Symbol.for("react.strict_mode"),
		j = Symbol.for("react.profiler"),
		I = Symbol.for("react.consumer"),
		$ = Symbol.for("react.context"),
		yt = Symbol.for("react.forward_ref"),
		dt = Symbol.for("react.suspense"),
		Tt = Symbol.for("react.suspense_list"),
		k = Symbol.for("react.memo"),
		Dt = Symbol.for("react.lazy"),
		wt = Symbol.for("react.activity"),
		Ne = Symbol.for("react.memo_cache_sentinel"),
		ye = Symbol.iterator;
	function Jt(t) {
		return t === null || typeof t != "object"
			? null
			: ((t = (ye && t[ye]) || t["@@iterator"]),
				typeof t == "function" ? t : null);
	}
	var xe = Symbol.for("react.client.reference");
	function ve(t) {
		if (t == null) return null;
		if (typeof t == "function")
			return t.$$typeof === xe ? null : t.displayName || t.name || null;
		if (typeof t == "string") return t;
		switch (t) {
			case L:
				return "Fragment";
			case j:
				return "Profiler";
			case G:
				return "StrictMode";
			case dt:
				return "Suspense";
			case Tt:
				return "SuspenseList";
			case wt:
				return "Activity";
		}
		if (typeof t == "object")
			switch (t.$$typeof) {
				case K:
					return "Portal";
				case $:
					return t.displayName || "Context";
				case I:
					return (t._context.displayName || "Context") + ".Consumer";
				case yt:
					var e = t.render;
					return (
						(t = t.displayName),
						t ||
							((t = e.displayName || e.name || ""),
							(t = t !== "" ? "ForwardRef(" + t + ")" : "ForwardRef")),
						t
					);
				case k:
					return (
						(e = t.displayName || null),
						e !== null ? e : ve(t.type) || "Memo"
					);
				case Dt:
					((e = t._payload), (t = t._init));
					try {
						return ve(t(e));
					} catch {}
			}
		return null;
	}
	var xt = Array.isArray,
		D = o.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
		B = s.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE,
		W = { pending: !1, data: null, method: null, action: null },
		vt = [],
		bt = -1;
	function g(t) {
		return { current: t };
	}
	function x(t) {
		0 > bt || ((t.current = vt[bt]), (vt[bt] = null), bt--);
	}
	function q(t, e) {
		(bt++, (vt[bt] = t.current), (t.current = e));
	}
	var X = g(null),
		P = g(null),
		lt = g(null),
		st = g(null);
	function Wt(t, e) {
		switch ((q(lt, e), q(P, t), q(X, null), e.nodeType)) {
			case 9:
			case 11:
				t = (t = e.documentElement) && (t = t.namespaceURI) ? Td(t) : 0;
				break;
			default:
				if (((t = e.tagName), (e = e.namespaceURI)))
					((e = Td(e)), (t = zd(e, t)));
				else
					switch (t) {
						case "svg":
							t = 1;
							break;
						case "math":
							t = 2;
							break;
						default:
							t = 0;
					}
		}
		(x(X), q(X, t));
	}
	function Ct() {
		(x(X), x(P), x(lt));
	}
	function ja(t) {
		t.memoizedState !== null && q(st, t);
		var e = X.current,
			l = zd(e, t.type);
		e !== l && (q(P, t), q(X, l));
	}
	function ju(t) {
		(P.current === t && (x(X), x(P)),
			st.current === t && (x(st), (Ru._currentValue = W)));
	}
	var ri, Ff;
	function Cl(t) {
		if (ri === void 0)
			try {
				throw Error();
			} catch (l) {
				var e = l.stack.trim().match(/\n( *(at )?)/);
				((ri = (e && e[1]) || ""),
					(Ff =
						-1 <
						l.stack.indexOf(`
    at`)
							? " (<anonymous>)"
							: -1 < l.stack.indexOf("@")
								? "@unknown:0:0"
								: ""));
			}
		return (
			`
` +
			ri +
			t +
			Ff
		);
	}
	var oi = !1;
	function si(t, e) {
		if (!t || oi) return "";
		oi = !0;
		var l = Error.prepareStackTrace;
		Error.prepareStackTrace = void 0;
		try {
			var a = {
				DetermineComponentFrameRoot: function () {
					try {
						if (e) {
							var N = function () {
								throw Error();
							};
							if (
								(Object.defineProperty(N.prototype, "props", {
									set: function () {
										throw Error();
									},
								}),
								typeof Reflect == "object" && Reflect.construct)
							) {
								try {
									Reflect.construct(N, []);
								} catch (_) {
									var z = _;
								}
								Reflect.construct(t, [], N);
							} else {
								try {
									N.call();
								} catch (_) {
									z = _;
								}
								t.call(N.prototype);
							}
						} else {
							try {
								throw Error();
							} catch (_) {
								z = _;
							}
							(N = t()) &&
								typeof N.catch == "function" &&
								N.catch(function () {});
						}
					} catch (_) {
						if (_ && z && typeof _.stack == "string") return [_.stack, z.stack];
					}
					return [null, null];
				},
			};
			a.DetermineComponentFrameRoot.displayName = "DetermineComponentFrameRoot";
			var u = Object.getOwnPropertyDescriptor(
				a.DetermineComponentFrameRoot,
				"name",
			);
			u &&
				u.configurable &&
				Object.defineProperty(a.DetermineComponentFrameRoot, "name", {
					value: "DetermineComponentFrameRoot",
				});
			var n = a.DetermineComponentFrameRoot(),
				i = n[0],
				r = n[1];
			if (i && r) {
				var h = i.split(`
`),
					T = r.split(`
`);
				for (
					u = a = 0;
					a < h.length && !h[a].includes("DetermineComponentFrameRoot");
				)
					a++;
				for (; u < T.length && !T[u].includes("DetermineComponentFrameRoot"); )
					u++;
				if (a === h.length || u === T.length)
					for (
						a = h.length - 1, u = T.length - 1;
						1 <= a && 0 <= u && h[a] !== T[u];
					)
						u--;
				for (; 1 <= a && 0 <= u; a--, u--)
					if (h[a] !== T[u]) {
						if (a !== 1 || u !== 1)
							do
								if ((a--, u--, 0 > u || h[a] !== T[u])) {
									var O =
										`
` + h[a].replace(" at new ", " at ");
									return (
										t.displayName &&
											O.includes("<anonymous>") &&
											(O = O.replace("<anonymous>", t.displayName)),
										O
									);
								}
							while (1 <= a && 0 <= u);
						break;
					}
			}
		} finally {
			((oi = !1), (Error.prepareStackTrace = l));
		}
		return (l = t ? t.displayName || t.name : "") ? Cl(l) : "";
	}
	function Xh(t, e) {
		switch (t.tag) {
			case 26:
			case 27:
			case 5:
				return Cl(t.type);
			case 16:
				return Cl("Lazy");
			case 13:
				return t.child !== e && e !== null
					? Cl("Suspense Fallback")
					: Cl("Suspense");
			case 19:
				return Cl("SuspenseList");
			case 0:
			case 15:
				return si(t.type, !1);
			case 11:
				return si(t.type.render, !1);
			case 1:
				return si(t.type, !0);
			case 31:
				return Cl("Activity");
			default:
				return "";
		}
	}
	function If(t) {
		try {
			var e = "",
				l = null;
			do ((e += Xh(t, l)), (l = t), (t = t.return));
			while (t);
			return e;
		} catch (a) {
			return (
				`
Error generating stack: ` +
				a.message +
				`
` +
				a.stack
			);
		}
	}
	var di = Object.prototype.hasOwnProperty,
		hi = c.unstable_scheduleCallback,
		mi = c.unstable_cancelCallback,
		Qh = c.unstable_shouldYield,
		Zh = c.unstable_requestPaint,
		ue = c.unstable_now,
		Vh = c.unstable_getCurrentPriorityLevel,
		Pf = c.unstable_ImmediatePriority,
		tr = c.unstable_UserBlockingPriority,
		qu = c.unstable_NormalPriority,
		Kh = c.unstable_LowPriority,
		er = c.unstable_IdlePriority,
		wh = c.log,
		Jh = c.unstable_setDisableYieldValue,
		qa = null,
		ne = null;
	function nl(t) {
		if (
			(typeof wh == "function" && Jh(t),
			ne && typeof ne.setStrictMode == "function")
		)
			try {
				ne.setStrictMode(qa, t);
			} catch {}
	}
	var ie = Math.clz32 ? Math.clz32 : kh,
		$h = Math.log,
		Wh = Math.LN2;
	function kh(t) {
		return ((t >>>= 0), t === 0 ? 32 : (31 - (($h(t) / Wh) | 0)) | 0);
	}
	var Yu = 256,
		Lu = 262144,
		Gu = 4194304;
	function Ul(t) {
		var e = t & 42;
		if (e !== 0) return e;
		switch (t & -t) {
			case 1:
				return 1;
			case 2:
				return 2;
			case 4:
				return 4;
			case 8:
				return 8;
			case 16:
				return 16;
			case 32:
				return 32;
			case 64:
				return 64;
			case 128:
				return 128;
			case 256:
			case 512:
			case 1024:
			case 2048:
			case 4096:
			case 8192:
			case 16384:
			case 32768:
			case 65536:
			case 131072:
				return t & 261888;
			case 262144:
			case 524288:
			case 1048576:
			case 2097152:
				return t & 3932160;
			case 4194304:
			case 8388608:
			case 16777216:
			case 33554432:
				return t & 62914560;
			case 67108864:
				return 67108864;
			case 134217728:
				return 134217728;
			case 268435456:
				return 268435456;
			case 536870912:
				return 536870912;
			case 1073741824:
				return 0;
			default:
				return t;
		}
	}
	function Xu(t, e, l) {
		var a = t.pendingLanes;
		if (a === 0) return 0;
		var u = 0,
			n = t.suspendedLanes,
			i = t.pingedLanes;
		t = t.warmLanes;
		var r = a & 134217727;
		return (
			r !== 0
				? ((a = r & ~n),
					a !== 0
						? (u = Ul(a))
						: ((i &= r),
							i !== 0
								? (u = Ul(i))
								: l || ((l = r & ~t), l !== 0 && (u = Ul(l)))))
				: ((r = a & ~n),
					r !== 0
						? (u = Ul(r))
						: i !== 0
							? (u = Ul(i))
							: l || ((l = a & ~t), l !== 0 && (u = Ul(l)))),
			u === 0
				? 0
				: e !== 0 &&
					  e !== u &&
					  (e & n) === 0 &&
					  ((n = u & -u),
					  (l = e & -e),
					  n >= l || (n === 32 && (l & 4194048) !== 0))
					? e
					: u
		);
	}
	function Ya(t, e) {
		return (t.pendingLanes & ~(t.suspendedLanes & ~t.pingedLanes) & e) === 0;
	}
	function Fh(t, e) {
		switch (t) {
			case 1:
			case 2:
			case 4:
			case 8:
			case 64:
				return e + 250;
			case 16:
			case 32:
			case 128:
			case 256:
			case 512:
			case 1024:
			case 2048:
			case 4096:
			case 8192:
			case 16384:
			case 32768:
			case 65536:
			case 131072:
			case 262144:
			case 524288:
			case 1048576:
			case 2097152:
				return e + 5e3;
			case 4194304:
			case 8388608:
			case 16777216:
			case 33554432:
				return -1;
			case 67108864:
			case 134217728:
			case 268435456:
			case 536870912:
			case 1073741824:
				return -1;
			default:
				return -1;
		}
	}
	function lr() {
		var t = Gu;
		return ((Gu <<= 1), (Gu & 62914560) === 0 && (Gu = 4194304), t);
	}
	function yi(t) {
		for (var e = [], l = 0; 31 > l; l++) e.push(t);
		return e;
	}
	function La(t, e) {
		((t.pendingLanes |= e),
			e !== 268435456 &&
				((t.suspendedLanes = 0), (t.pingedLanes = 0), (t.warmLanes = 0)));
	}
	function Ih(t, e, l, a, u, n) {
		var i = t.pendingLanes;
		((t.pendingLanes = l),
			(t.suspendedLanes = 0),
			(t.pingedLanes = 0),
			(t.warmLanes = 0),
			(t.expiredLanes &= l),
			(t.entangledLanes &= l),
			(t.errorRecoveryDisabledLanes &= l),
			(t.shellSuspendCounter = 0));
		var r = t.entanglements,
			h = t.expirationTimes,
			T = t.hiddenUpdates;
		for (l = i & ~l; 0 < l; ) {
			var O = 31 - ie(l),
				N = 1 << O;
			((r[O] = 0), (h[O] = -1));
			var z = T[O];
			if (z !== null)
				for (T[O] = null, O = 0; O < z.length; O++) {
					var _ = z[O];
					_ !== null && (_.lane &= -536870913);
				}
			l &= ~N;
		}
		(a !== 0 && ar(t, a, 0),
			n !== 0 && u === 0 && t.tag !== 0 && (t.suspendedLanes |= n & ~(i & ~e)));
	}
	function ar(t, e, l) {
		((t.pendingLanes |= e), (t.suspendedLanes &= ~e));
		var a = 31 - ie(e);
		((t.entangledLanes |= e),
			(t.entanglements[a] = t.entanglements[a] | 1073741824 | (l & 261930)));
	}
	function ur(t, e) {
		var l = (t.entangledLanes |= e);
		for (t = t.entanglements; l; ) {
			var a = 31 - ie(l),
				u = 1 << a;
			((u & e) | (t[a] & e) && (t[a] |= e), (l &= ~u));
		}
	}
	function nr(t, e) {
		var l = e & -e;
		return (
			(l = (l & 42) !== 0 ? 1 : vi(l)),
			(l & (t.suspendedLanes | e)) !== 0 ? 0 : l
		);
	}
	function vi(t) {
		switch (t) {
			case 2:
				t = 1;
				break;
			case 8:
				t = 4;
				break;
			case 32:
				t = 16;
				break;
			case 256:
			case 512:
			case 1024:
			case 2048:
			case 4096:
			case 8192:
			case 16384:
			case 32768:
			case 65536:
			case 131072:
			case 262144:
			case 524288:
			case 1048576:
			case 2097152:
			case 4194304:
			case 8388608:
			case 16777216:
			case 33554432:
				t = 128;
				break;
			case 268435456:
				t = 134217728;
				break;
			default:
				t = 0;
		}
		return t;
	}
	function gi(t) {
		return (
			(t &= -t),
			2 < t ? (8 < t ? ((t & 134217727) !== 0 ? 32 : 268435456) : 8) : 2
		);
	}
	function ir() {
		var t = B.p;
		return t !== 0 ? t : ((t = window.event), t === void 0 ? 32 : wd(t.type));
	}
	function cr(t, e) {
		var l = B.p;
		try {
			return ((B.p = t), e());
		} finally {
			B.p = l;
		}
	}
	var il = Math.random().toString(36).slice(2),
		Xt = "__reactFiber$" + il,
		Ft = "__reactProps$" + il,
		Wl = "__reactContainer$" + il,
		pi = "__reactEvents$" + il,
		Ph = "__reactListeners$" + il,
		tm = "__reactHandles$" + il,
		fr = "__reactResources$" + il,
		Ga = "__reactMarker$" + il;
	function Si(t) {
		(delete t[Xt], delete t[Ft], delete t[pi], delete t[Ph], delete t[tm]);
	}
	function kl(t) {
		var e = t[Xt];
		if (e) return e;
		for (var l = t.parentNode; l; ) {
			if ((e = l[Wl] || l[Xt])) {
				if (
					((l = e.alternate),
					e.child !== null || (l !== null && l.child !== null))
				)
					for (t = Cd(t); t !== null; ) {
						if ((l = t[Xt])) return l;
						t = Cd(t);
					}
				return e;
			}
			((t = l), (l = t.parentNode));
		}
		return null;
	}
	function Fl(t) {
		if ((t = t[Xt] || t[Wl])) {
			var e = t.tag;
			if (
				e === 5 ||
				e === 6 ||
				e === 13 ||
				e === 31 ||
				e === 26 ||
				e === 27 ||
				e === 3
			)
				return t;
		}
		return null;
	}
	function Xa(t) {
		var e = t.tag;
		if (e === 5 || e === 26 || e === 27 || e === 6) return t.stateNode;
		throw Error(f(33));
	}
	function Il(t) {
		var e = t[fr];
		return (
			e ||
				(e = t[fr] =
					{ hoistableStyles: new Map(), hoistableScripts: new Map() }),
			e
		);
	}
	function Lt(t) {
		t[Ga] = !0;
	}
	var rr = new Set(),
		or = {};
	function Nl(t, e) {
		(Pl(t, e), Pl(t + "Capture", e));
	}
	function Pl(t, e) {
		for (or[t] = e, t = 0; t < e.length; t++) rr.add(e[t]);
	}
	var em = RegExp(
			"^[:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD][:A-Z_a-z\\u00C0-\\u00D6\\u00D8-\\u00F6\\u00F8-\\u02FF\\u0370-\\u037D\\u037F-\\u1FFF\\u200C-\\u200D\\u2070-\\u218F\\u2C00-\\u2FEF\\u3001-\\uD7FF\\uF900-\\uFDCF\\uFDF0-\\uFFFD\\-.0-9\\u00B7\\u0300-\\u036F\\u203F-\\u2040]*$",
		),
		sr = {},
		dr = {};
	function lm(t) {
		return di.call(dr, t)
			? !0
			: di.call(sr, t)
				? !1
				: em.test(t)
					? (dr[t] = !0)
					: ((sr[t] = !0), !1);
	}
	function Qu(t, e, l) {
		if (lm(e))
			if (l === null) t.removeAttribute(e);
			else {
				switch (typeof l) {
					case "undefined":
					case "function":
					case "symbol":
						t.removeAttribute(e);
						return;
					case "boolean":
						var a = e.toLowerCase().slice(0, 5);
						if (a !== "data-" && a !== "aria-") {
							t.removeAttribute(e);
							return;
						}
				}
				t.setAttribute(e, "" + l);
			}
	}
	function Zu(t, e, l) {
		if (l === null) t.removeAttribute(e);
		else {
			switch (typeof l) {
				case "undefined":
				case "function":
				case "symbol":
				case "boolean":
					t.removeAttribute(e);
					return;
			}
			t.setAttribute(e, "" + l);
		}
	}
	function Ge(t, e, l, a) {
		if (a === null) t.removeAttribute(l);
		else {
			switch (typeof a) {
				case "undefined":
				case "function":
				case "symbol":
				case "boolean":
					t.removeAttribute(l);
					return;
			}
			t.setAttributeNS(e, l, "" + a);
		}
	}
	function ge(t) {
		switch (typeof t) {
			case "bigint":
			case "boolean":
			case "number":
			case "string":
			case "undefined":
				return t;
			case "object":
				return t;
			default:
				return "";
		}
	}
	function hr(t) {
		var e = t.type;
		return (
			(t = t.nodeName) &&
			t.toLowerCase() === "input" &&
			(e === "checkbox" || e === "radio")
		);
	}
	function am(t, e, l) {
		var a = Object.getOwnPropertyDescriptor(t.constructor.prototype, e);
		if (
			!t.hasOwnProperty(e) &&
			typeof a < "u" &&
			typeof a.get == "function" &&
			typeof a.set == "function"
		) {
			var u = a.get,
				n = a.set;
			return (
				Object.defineProperty(t, e, {
					configurable: !0,
					get: function () {
						return u.call(this);
					},
					set: function (i) {
						((l = "" + i), n.call(this, i));
					},
				}),
				Object.defineProperty(t, e, { enumerable: a.enumerable }),
				{
					getValue: function () {
						return l;
					},
					setValue: function (i) {
						l = "" + i;
					},
					stopTracking: function () {
						((t._valueTracker = null), delete t[e]);
					},
				}
			);
		}
	}
	function bi(t) {
		if (!t._valueTracker) {
			var e = hr(t) ? "checked" : "value";
			t._valueTracker = am(t, e, "" + t[e]);
		}
	}
	function mr(t) {
		if (!t) return !1;
		var e = t._valueTracker;
		if (!e) return !0;
		var l = e.getValue(),
			a = "";
		return (
			t && (a = hr(t) ? (t.checked ? "true" : "false") : t.value),
			(t = a),
			t !== l ? (e.setValue(t), !0) : !1
		);
	}
	function Vu(t) {
		if (
			((t = t || (typeof document < "u" ? document : void 0)), typeof t > "u")
		)
			return null;
		try {
			return t.activeElement || t.body;
		} catch {
			return t.body;
		}
	}
	var um = /[\n"\\]/g;
	function pe(t) {
		return t.replace(um, function (e) {
			return "\\" + e.charCodeAt(0).toString(16) + " ";
		});
	}
	function Ei(t, e, l, a, u, n, i, r) {
		((t.name = ""),
			i != null &&
			typeof i != "function" &&
			typeof i != "symbol" &&
			typeof i != "boolean"
				? (t.type = i)
				: t.removeAttribute("type"),
			e != null
				? i === "number"
					? ((e === 0 && t.value === "") || t.value != e) &&
						(t.value = "" + ge(e))
					: t.value !== "" + ge(e) && (t.value = "" + ge(e))
				: (i !== "submit" && i !== "reset") || t.removeAttribute("value"),
			e != null
				? Ti(t, i, ge(e))
				: l != null
					? Ti(t, i, ge(l))
					: a != null && t.removeAttribute("value"),
			u == null && n != null && (t.defaultChecked = !!n),
			u != null &&
				(t.checked = u && typeof u != "function" && typeof u != "symbol"),
			r != null &&
			typeof r != "function" &&
			typeof r != "symbol" &&
			typeof r != "boolean"
				? (t.name = "" + ge(r))
				: t.removeAttribute("name"));
	}
	function yr(t, e, l, a, u, n, i, r) {
		if (
			(n != null &&
				typeof n != "function" &&
				typeof n != "symbol" &&
				typeof n != "boolean" &&
				(t.type = n),
			e != null || l != null)
		) {
			if (!((n !== "submit" && n !== "reset") || e != null)) {
				bi(t);
				return;
			}
			((l = l != null ? "" + ge(l) : ""),
				(e = e != null ? "" + ge(e) : l),
				r || e === t.value || (t.value = e),
				(t.defaultValue = e));
		}
		((a = a ?? u),
			(a = typeof a != "function" && typeof a != "symbol" && !!a),
			(t.checked = r ? t.checked : !!a),
			(t.defaultChecked = !!a),
			i != null &&
				typeof i != "function" &&
				typeof i != "symbol" &&
				typeof i != "boolean" &&
				(t.name = i),
			bi(t));
	}
	function Ti(t, e, l) {
		(e === "number" && Vu(t.ownerDocument) === t) ||
			t.defaultValue === "" + l ||
			(t.defaultValue = "" + l);
	}
	function ta(t, e, l, a) {
		if (((t = t.options), e)) {
			e = {};
			for (var u = 0; u < l.length; u++) e["$" + l[u]] = !0;
			for (l = 0; l < t.length; l++)
				((u = e.hasOwnProperty("$" + t[l].value)),
					t[l].selected !== u && (t[l].selected = u),
					u && a && (t[l].defaultSelected = !0));
		} else {
			for (l = "" + ge(l), e = null, u = 0; u < t.length; u++) {
				if (t[u].value === l) {
					((t[u].selected = !0), a && (t[u].defaultSelected = !0));
					return;
				}
				e !== null || t[u].disabled || (e = t[u]);
			}
			e !== null && (e.selected = !0);
		}
	}
	function vr(t, e, l) {
		if (
			e != null &&
			((e = "" + ge(e)), e !== t.value && (t.value = e), l == null)
		) {
			t.defaultValue !== e && (t.defaultValue = e);
			return;
		}
		t.defaultValue = l != null ? "" + ge(l) : "";
	}
	function gr(t, e, l, a) {
		if (e == null) {
			if (a != null) {
				if (l != null) throw Error(f(92));
				if (xt(a)) {
					if (1 < a.length) throw Error(f(93));
					a = a[0];
				}
				l = a;
			}
			(l == null && (l = ""), (e = l));
		}
		((l = ge(e)),
			(t.defaultValue = l),
			(a = t.textContent),
			a === l && a !== "" && a !== null && (t.value = a),
			bi(t));
	}
	function ea(t, e) {
		if (e) {
			var l = t.firstChild;
			if (l && l === t.lastChild && l.nodeType === 3) {
				l.nodeValue = e;
				return;
			}
		}
		t.textContent = e;
	}
	var nm = new Set(
		"animationIterationCount aspectRatio borderImageOutset borderImageSlice borderImageWidth boxFlex boxFlexGroup boxOrdinalGroup columnCount columns flex flexGrow flexPositive flexShrink flexNegative flexOrder gridArea gridRow gridRowEnd gridRowSpan gridRowStart gridColumn gridColumnEnd gridColumnSpan gridColumnStart fontWeight lineClamp lineHeight opacity order orphans scale tabSize widows zIndex zoom fillOpacity floodOpacity stopOpacity strokeDasharray strokeDashoffset strokeMiterlimit strokeOpacity strokeWidth MozAnimationIterationCount MozBoxFlex MozBoxFlexGroup MozLineClamp msAnimationIterationCount msFlex msZoom msFlexGrow msFlexNegative msFlexOrder msFlexPositive msFlexShrink msGridColumn msGridColumnSpan msGridRow msGridRowSpan WebkitAnimationIterationCount WebkitBoxFlex WebKitBoxFlexGroup WebkitBoxOrdinalGroup WebkitColumnCount WebkitColumns WebkitFlex WebkitFlexGrow WebkitFlexPositive WebkitFlexShrink WebkitLineClamp".split(
			" ",
		),
	);
	function pr(t, e, l) {
		var a = e.indexOf("--") === 0;
		l == null || typeof l == "boolean" || l === ""
			? a
				? t.setProperty(e, "")
				: e === "float"
					? (t.cssFloat = "")
					: (t[e] = "")
			: a
				? t.setProperty(e, l)
				: typeof l != "number" || l === 0 || nm.has(e)
					? e === "float"
						? (t.cssFloat = l)
						: (t[e] = ("" + l).trim())
					: (t[e] = l + "px");
	}
	function Sr(t, e, l) {
		if (e != null && typeof e != "object") throw Error(f(62));
		if (((t = t.style), l != null)) {
			for (var a in l)
				!l.hasOwnProperty(a) ||
					(e != null && e.hasOwnProperty(a)) ||
					(a.indexOf("--") === 0
						? t.setProperty(a, "")
						: a === "float"
							? (t.cssFloat = "")
							: (t[a] = ""));
			for (var u in e)
				((a = e[u]), e.hasOwnProperty(u) && l[u] !== a && pr(t, u, a));
		} else for (var n in e) e.hasOwnProperty(n) && pr(t, n, e[n]);
	}
	function zi(t) {
		if (t.indexOf("-") === -1) return !1;
		switch (t) {
			case "annotation-xml":
			case "color-profile":
			case "font-face":
			case "font-face-src":
			case "font-face-uri":
			case "font-face-format":
			case "font-face-name":
			case "missing-glyph":
				return !1;
			default:
				return !0;
		}
	}
	var im = new Map([
			["acceptCharset", "accept-charset"],
			["htmlFor", "for"],
			["httpEquiv", "http-equiv"],
			["crossOrigin", "crossorigin"],
			["accentHeight", "accent-height"],
			["alignmentBaseline", "alignment-baseline"],
			["arabicForm", "arabic-form"],
			["baselineShift", "baseline-shift"],
			["capHeight", "cap-height"],
			["clipPath", "clip-path"],
			["clipRule", "clip-rule"],
			["colorInterpolation", "color-interpolation"],
			["colorInterpolationFilters", "color-interpolation-filters"],
			["colorProfile", "color-profile"],
			["colorRendering", "color-rendering"],
			["dominantBaseline", "dominant-baseline"],
			["enableBackground", "enable-background"],
			["fillOpacity", "fill-opacity"],
			["fillRule", "fill-rule"],
			["floodColor", "flood-color"],
			["floodOpacity", "flood-opacity"],
			["fontFamily", "font-family"],
			["fontSize", "font-size"],
			["fontSizeAdjust", "font-size-adjust"],
			["fontStretch", "font-stretch"],
			["fontStyle", "font-style"],
			["fontVariant", "font-variant"],
			["fontWeight", "font-weight"],
			["glyphName", "glyph-name"],
			["glyphOrientationHorizontal", "glyph-orientation-horizontal"],
			["glyphOrientationVertical", "glyph-orientation-vertical"],
			["horizAdvX", "horiz-adv-x"],
			["horizOriginX", "horiz-origin-x"],
			["imageRendering", "image-rendering"],
			["letterSpacing", "letter-spacing"],
			["lightingColor", "lighting-color"],
			["markerEnd", "marker-end"],
			["markerMid", "marker-mid"],
			["markerStart", "marker-start"],
			["overlinePosition", "overline-position"],
			["overlineThickness", "overline-thickness"],
			["paintOrder", "paint-order"],
			["panose-1", "panose-1"],
			["pointerEvents", "pointer-events"],
			["renderingIntent", "rendering-intent"],
			["shapeRendering", "shape-rendering"],
			["stopColor", "stop-color"],
			["stopOpacity", "stop-opacity"],
			["strikethroughPosition", "strikethrough-position"],
			["strikethroughThickness", "strikethrough-thickness"],
			["strokeDasharray", "stroke-dasharray"],
			["strokeDashoffset", "stroke-dashoffset"],
			["strokeLinecap", "stroke-linecap"],
			["strokeLinejoin", "stroke-linejoin"],
			["strokeMiterlimit", "stroke-miterlimit"],
			["strokeOpacity", "stroke-opacity"],
			["strokeWidth", "stroke-width"],
			["textAnchor", "text-anchor"],
			["textDecoration", "text-decoration"],
			["textRendering", "text-rendering"],
			["transformOrigin", "transform-origin"],
			["underlinePosition", "underline-position"],
			["underlineThickness", "underline-thickness"],
			["unicodeBidi", "unicode-bidi"],
			["unicodeRange", "unicode-range"],
			["unitsPerEm", "units-per-em"],
			["vAlphabetic", "v-alphabetic"],
			["vHanging", "v-hanging"],
			["vIdeographic", "v-ideographic"],
			["vMathematical", "v-mathematical"],
			["vectorEffect", "vector-effect"],
			["vertAdvY", "vert-adv-y"],
			["vertOriginX", "vert-origin-x"],
			["vertOriginY", "vert-origin-y"],
			["wordSpacing", "word-spacing"],
			["writingMode", "writing-mode"],
			["xmlnsXlink", "xmlns:xlink"],
			["xHeight", "x-height"],
		]),
		cm =
			/^[\u0000-\u001F ]*j[\r\n\t]*a[\r\n\t]*v[\r\n\t]*a[\r\n\t]*s[\r\n\t]*c[\r\n\t]*r[\r\n\t]*i[\r\n\t]*p[\r\n\t]*t[\r\n\t]*:/i;
	function Ku(t) {
		return cm.test("" + t)
			? "javascript:throw new Error('React has blocked a javascript: URL as a security precaution.')"
			: t;
	}
	function Xe() {}
	var Ai = null;
	function Ri(t) {
		return (
			(t = t.target || t.srcElement || window),
			t.correspondingUseElement && (t = t.correspondingUseElement),
			t.nodeType === 3 ? t.parentNode : t
		);
	}
	var la = null,
		aa = null;
	function br(t) {
		var e = Fl(t);
		if (e && (t = e.stateNode)) {
			var l = t[Ft] || null;
			t: switch (((t = e.stateNode), e.type)) {
				case "input":
					if (
						(Ei(
							t,
							l.value,
							l.defaultValue,
							l.defaultValue,
							l.checked,
							l.defaultChecked,
							l.type,
							l.name,
						),
						(e = l.name),
						l.type === "radio" && e != null)
					) {
						for (l = t; l.parentNode; ) l = l.parentNode;
						for (
							l = l.querySelectorAll(
								'input[name="' + pe("" + e) + '"][type="radio"]',
							),
								e = 0;
							e < l.length;
							e++
						) {
							var a = l[e];
							if (a !== t && a.form === t.form) {
								var u = a[Ft] || null;
								if (!u) throw Error(f(90));
								Ei(
									a,
									u.value,
									u.defaultValue,
									u.defaultValue,
									u.checked,
									u.defaultChecked,
									u.type,
									u.name,
								);
							}
						}
						for (e = 0; e < l.length; e++)
							((a = l[e]), a.form === t.form && mr(a));
					}
					break t;
				case "textarea":
					vr(t, l.value, l.defaultValue);
					break t;
				case "select":
					((e = l.value), e != null && ta(t, !!l.multiple, e, !1));
			}
		}
	}
	var _i = !1;
	function Er(t, e, l) {
		if (_i) return t(e, l);
		_i = !0;
		try {
			var a = t(e);
			return a;
		} finally {
			if (
				((_i = !1),
				(la !== null || aa !== null) &&
					(xn(), la && ((e = la), (t = aa), (aa = la = null), br(e), t)))
			)
				for (e = 0; e < t.length; e++) br(t[e]);
		}
	}
	function Qa(t, e) {
		var l = t.stateNode;
		if (l === null) return null;
		var a = l[Ft] || null;
		if (a === null) return null;
		l = a[e];
		t: switch (e) {
			case "onClick":
			case "onClickCapture":
			case "onDoubleClick":
			case "onDoubleClickCapture":
			case "onMouseDown":
			case "onMouseDownCapture":
			case "onMouseMove":
			case "onMouseMoveCapture":
			case "onMouseUp":
			case "onMouseUpCapture":
			case "onMouseEnter":
				((a = !a.disabled) ||
					((t = t.type),
					(a = !(
						t === "button" ||
						t === "input" ||
						t === "select" ||
						t === "textarea"
					))),
					(t = !a));
				break t;
			default:
				t = !1;
		}
		if (t) return null;
		if (l && typeof l != "function") throw Error(f(231, e, typeof l));
		return l;
	}
	var Qe = !(
			typeof window > "u" ||
			typeof window.document > "u" ||
			typeof window.document.createElement > "u"
		),
		Mi = !1;
	if (Qe)
		try {
			var Za = {};
			(Object.defineProperty(Za, "passive", {
				get: function () {
					Mi = !0;
				},
			}),
				window.addEventListener("test", Za, Za),
				window.removeEventListener("test", Za, Za));
		} catch {
			Mi = !1;
		}
	var cl = null,
		Oi = null,
		wu = null;
	function Tr() {
		if (wu) return wu;
		var t,
			e = Oi,
			l = e.length,
			a,
			u = "value" in cl ? cl.value : cl.textContent,
			n = u.length;
		for (t = 0; t < l && e[t] === u[t]; t++);
		var i = l - t;
		for (a = 1; a <= i && e[l - a] === u[n - a]; a++);
		return (wu = u.slice(t, 1 < a ? 1 - a : void 0));
	}
	function Ju(t) {
		var e = t.keyCode;
		return (
			"charCode" in t
				? ((t = t.charCode), t === 0 && e === 13 && (t = 13))
				: (t = e),
			t === 10 && (t = 13),
			32 <= t || t === 13 ? t : 0
		);
	}
	function $u() {
		return !0;
	}
	function zr() {
		return !1;
	}
	function It(t) {
		function e(l, a, u, n, i) {
			((this._reactName = l),
				(this._targetInst = u),
				(this.type = a),
				(this.nativeEvent = n),
				(this.target = i),
				(this.currentTarget = null));
			for (var r in t)
				t.hasOwnProperty(r) && ((l = t[r]), (this[r] = l ? l(n) : n[r]));
			return (
				(this.isDefaultPrevented = (
					n.defaultPrevented != null ? n.defaultPrevented : n.returnValue === !1
				)
					? $u
					: zr),
				(this.isPropagationStopped = zr),
				this
			);
		}
		return (
			R(e.prototype, {
				preventDefault: function () {
					this.defaultPrevented = !0;
					var l = this.nativeEvent;
					l &&
						(l.preventDefault
							? l.preventDefault()
							: typeof l.returnValue != "unknown" && (l.returnValue = !1),
						(this.isDefaultPrevented = $u));
				},
				stopPropagation: function () {
					var l = this.nativeEvent;
					l &&
						(l.stopPropagation
							? l.stopPropagation()
							: typeof l.cancelBubble != "unknown" && (l.cancelBubble = !0),
						(this.isPropagationStopped = $u));
				},
				persist: function () {},
				isPersistent: $u,
			}),
			e
		);
	}
	var xl = {
			eventPhase: 0,
			bubbles: 0,
			cancelable: 0,
			timeStamp: function (t) {
				return t.timeStamp || Date.now();
			},
			defaultPrevented: 0,
			isTrusted: 0,
		},
		Wu = It(xl),
		Va = R({}, xl, { view: 0, detail: 0 }),
		fm = It(Va),
		Di,
		Ci,
		Ka,
		ku = R({}, Va, {
			screenX: 0,
			screenY: 0,
			clientX: 0,
			clientY: 0,
			pageX: 0,
			pageY: 0,
			ctrlKey: 0,
			shiftKey: 0,
			altKey: 0,
			metaKey: 0,
			getModifierState: Ni,
			button: 0,
			buttons: 0,
			relatedTarget: function (t) {
				return t.relatedTarget === void 0
					? t.fromElement === t.srcElement
						? t.toElement
						: t.fromElement
					: t.relatedTarget;
			},
			movementX: function (t) {
				return "movementX" in t
					? t.movementX
					: (t !== Ka &&
							(Ka && t.type === "mousemove"
								? ((Di = t.screenX - Ka.screenX), (Ci = t.screenY - Ka.screenY))
								: (Ci = Di = 0),
							(Ka = t)),
						Di);
			},
			movementY: function (t) {
				return "movementY" in t ? t.movementY : Ci;
			},
		}),
		Ar = It(ku),
		rm = R({}, ku, { dataTransfer: 0 }),
		om = It(rm),
		sm = R({}, Va, { relatedTarget: 0 }),
		Ui = It(sm),
		dm = R({}, xl, { animationName: 0, elapsedTime: 0, pseudoElement: 0 }),
		hm = It(dm),
		mm = R({}, xl, {
			clipboardData: function (t) {
				return "clipboardData" in t ? t.clipboardData : window.clipboardData;
			},
		}),
		ym = It(mm),
		vm = R({}, xl, { data: 0 }),
		Rr = It(vm),
		gm = {
			Esc: "Escape",
			Spacebar: " ",
			Left: "ArrowLeft",
			Up: "ArrowUp",
			Right: "ArrowRight",
			Down: "ArrowDown",
			Del: "Delete",
			Win: "OS",
			Menu: "ContextMenu",
			Apps: "ContextMenu",
			Scroll: "ScrollLock",
			MozPrintableKey: "Unidentified",
		},
		pm = {
			8: "Backspace",
			9: "Tab",
			12: "Clear",
			13: "Enter",
			16: "Shift",
			17: "Control",
			18: "Alt",
			19: "Pause",
			20: "CapsLock",
			27: "Escape",
			32: " ",
			33: "PageUp",
			34: "PageDown",
			35: "End",
			36: "Home",
			37: "ArrowLeft",
			38: "ArrowUp",
			39: "ArrowRight",
			40: "ArrowDown",
			45: "Insert",
			46: "Delete",
			112: "F1",
			113: "F2",
			114: "F3",
			115: "F4",
			116: "F5",
			117: "F6",
			118: "F7",
			119: "F8",
			120: "F9",
			121: "F10",
			122: "F11",
			123: "F12",
			144: "NumLock",
			145: "ScrollLock",
			224: "Meta",
		},
		Sm = {
			Alt: "altKey",
			Control: "ctrlKey",
			Meta: "metaKey",
			Shift: "shiftKey",
		};
	function bm(t) {
		var e = this.nativeEvent;
		return e.getModifierState
			? e.getModifierState(t)
			: (t = Sm[t])
				? !!e[t]
				: !1;
	}
	function Ni() {
		return bm;
	}
	var Em = R({}, Va, {
			key: function (t) {
				if (t.key) {
					var e = gm[t.key] || t.key;
					if (e !== "Unidentified") return e;
				}
				return t.type === "keypress"
					? ((t = Ju(t)), t === 13 ? "Enter" : String.fromCharCode(t))
					: t.type === "keydown" || t.type === "keyup"
						? pm[t.keyCode] || "Unidentified"
						: "";
			},
			code: 0,
			location: 0,
			ctrlKey: 0,
			shiftKey: 0,
			altKey: 0,
			metaKey: 0,
			repeat: 0,
			locale: 0,
			getModifierState: Ni,
			charCode: function (t) {
				return t.type === "keypress" ? Ju(t) : 0;
			},
			keyCode: function (t) {
				return t.type === "keydown" || t.type === "keyup" ? t.keyCode : 0;
			},
			which: function (t) {
				return t.type === "keypress"
					? Ju(t)
					: t.type === "keydown" || t.type === "keyup"
						? t.keyCode
						: 0;
			},
		}),
		Tm = It(Em),
		zm = R({}, ku, {
			pointerId: 0,
			width: 0,
			height: 0,
			pressure: 0,
			tangentialPressure: 0,
			tiltX: 0,
			tiltY: 0,
			twist: 0,
			pointerType: 0,
			isPrimary: 0,
		}),
		_r = It(zm),
		Am = R({}, Va, {
			touches: 0,
			targetTouches: 0,
			changedTouches: 0,
			altKey: 0,
			metaKey: 0,
			ctrlKey: 0,
			shiftKey: 0,
			getModifierState: Ni,
		}),
		Rm = It(Am),
		_m = R({}, xl, { propertyName: 0, elapsedTime: 0, pseudoElement: 0 }),
		Mm = It(_m),
		Om = R({}, ku, {
			deltaX: function (t) {
				return "deltaX" in t
					? t.deltaX
					: "wheelDeltaX" in t
						? -t.wheelDeltaX
						: 0;
			},
			deltaY: function (t) {
				return "deltaY" in t
					? t.deltaY
					: "wheelDeltaY" in t
						? -t.wheelDeltaY
						: "wheelDelta" in t
							? -t.wheelDelta
							: 0;
			},
			deltaZ: 0,
			deltaMode: 0,
		}),
		Dm = It(Om),
		Cm = R({}, xl, { newState: 0, oldState: 0 }),
		Um = It(Cm),
		Nm = [9, 13, 27, 32],
		xi = Qe && "CompositionEvent" in window,
		wa = null;
	Qe && "documentMode" in document && (wa = document.documentMode);
	var xm = Qe && "TextEvent" in window && !wa,
		Mr = Qe && (!xi || (wa && 8 < wa && 11 >= wa)),
		Or = " ",
		Dr = !1;
	function Cr(t, e) {
		switch (t) {
			case "keyup":
				return Nm.indexOf(e.keyCode) !== -1;
			case "keydown":
				return e.keyCode !== 229;
			case "keypress":
			case "mousedown":
			case "focusout":
				return !0;
			default:
				return !1;
		}
	}
	function Ur(t) {
		return (
			(t = t.detail),
			typeof t == "object" && "data" in t ? t.data : null
		);
	}
	var ua = !1;
	function Hm(t, e) {
		switch (t) {
			case "compositionend":
				return Ur(e);
			case "keypress":
				return e.which !== 32 ? null : ((Dr = !0), Or);
			case "textInput":
				return ((t = e.data), t === Or && Dr ? null : t);
			default:
				return null;
		}
	}
	function Bm(t, e) {
		if (ua)
			return t === "compositionend" || (!xi && Cr(t, e))
				? ((t = Tr()), (wu = Oi = cl = null), (ua = !1), t)
				: null;
		switch (t) {
			case "paste":
				return null;
			case "keypress":
				if (!(e.ctrlKey || e.altKey || e.metaKey) || (e.ctrlKey && e.altKey)) {
					if (e.char && 1 < e.char.length) return e.char;
					if (e.which) return String.fromCharCode(e.which);
				}
				return null;
			case "compositionend":
				return Mr && e.locale !== "ko" ? null : e.data;
			default:
				return null;
		}
	}
	var jm = {
		color: !0,
		date: !0,
		datetime: !0,
		"datetime-local": !0,
		email: !0,
		month: !0,
		number: !0,
		password: !0,
		range: !0,
		search: !0,
		tel: !0,
		text: !0,
		time: !0,
		url: !0,
		week: !0,
	};
	function Nr(t) {
		var e = t && t.nodeName && t.nodeName.toLowerCase();
		return e === "input" ? !!jm[t.type] : e === "textarea";
	}
	function xr(t, e, l, a) {
		(la ? (aa ? aa.push(a) : (aa = [a])) : (la = a),
			(e = Gn(e, "onChange")),
			0 < e.length &&
				((l = new Wu("onChange", "change", null, l, a)),
				t.push({ event: l, listeners: e })));
	}
	var Ja = null,
		$a = null;
	function qm(t) {
		vd(t, 0);
	}
	function Fu(t) {
		var e = Xa(t);
		if (mr(e)) return t;
	}
	function Hr(t, e) {
		if (t === "change") return e;
	}
	var Br = !1;
	if (Qe) {
		var Hi;
		if (Qe) {
			var Bi = "oninput" in document;
			if (!Bi) {
				var jr = document.createElement("div");
				(jr.setAttribute("oninput", "return;"),
					(Bi = typeof jr.oninput == "function"));
			}
			Hi = Bi;
		} else Hi = !1;
		Br = Hi && (!document.documentMode || 9 < document.documentMode);
	}
	function qr() {
		Ja && (Ja.detachEvent("onpropertychange", Yr), ($a = Ja = null));
	}
	function Yr(t) {
		if (t.propertyName === "value" && Fu($a)) {
			var e = [];
			(xr(e, $a, t, Ri(t)), Er(qm, e));
		}
	}
	function Ym(t, e, l) {
		t === "focusin"
			? (qr(), (Ja = e), ($a = l), Ja.attachEvent("onpropertychange", Yr))
			: t === "focusout" && qr();
	}
	function Lm(t) {
		if (t === "selectionchange" || t === "keyup" || t === "keydown")
			return Fu($a);
	}
	function Gm(t, e) {
		if (t === "click") return Fu(e);
	}
	function Xm(t, e) {
		if (t === "input" || t === "change") return Fu(e);
	}
	function Qm(t, e) {
		return (t === e && (t !== 0 || 1 / t === 1 / e)) || (t !== t && e !== e);
	}
	var ce = typeof Object.is == "function" ? Object.is : Qm;
	function Wa(t, e) {
		if (ce(t, e)) return !0;
		if (
			typeof t != "object" ||
			t === null ||
			typeof e != "object" ||
			e === null
		)
			return !1;
		var l = Object.keys(t),
			a = Object.keys(e);
		if (l.length !== a.length) return !1;
		for (a = 0; a < l.length; a++) {
			var u = l[a];
			if (!di.call(e, u) || !ce(t[u], e[u])) return !1;
		}
		return !0;
	}
	function Lr(t) {
		for (; t && t.firstChild; ) t = t.firstChild;
		return t;
	}
	function Gr(t, e) {
		var l = Lr(t);
		t = 0;
		for (var a; l; ) {
			if (l.nodeType === 3) {
				if (((a = t + l.textContent.length), t <= e && a >= e))
					return { node: l, offset: e - t };
				t = a;
			}
			t: {
				for (; l; ) {
					if (l.nextSibling) {
						l = l.nextSibling;
						break t;
					}
					l = l.parentNode;
				}
				l = void 0;
			}
			l = Lr(l);
		}
	}
	function Xr(t, e) {
		return t && e
			? t === e
				? !0
				: t && t.nodeType === 3
					? !1
					: e && e.nodeType === 3
						? Xr(t, e.parentNode)
						: "contains" in t
							? t.contains(e)
							: t.compareDocumentPosition
								? !!(t.compareDocumentPosition(e) & 16)
								: !1
			: !1;
	}
	function Qr(t) {
		t =
			t != null &&
			t.ownerDocument != null &&
			t.ownerDocument.defaultView != null
				? t.ownerDocument.defaultView
				: window;
		for (var e = Vu(t.document); e instanceof t.HTMLIFrameElement; ) {
			try {
				var l = typeof e.contentWindow.location.href == "string";
			} catch {
				l = !1;
			}
			if (l) t = e.contentWindow;
			else break;
			e = Vu(t.document);
		}
		return e;
	}
	function ji(t) {
		var e = t && t.nodeName && t.nodeName.toLowerCase();
		return (
			e &&
			((e === "input" &&
				(t.type === "text" ||
					t.type === "search" ||
					t.type === "tel" ||
					t.type === "url" ||
					t.type === "password")) ||
				e === "textarea" ||
				t.contentEditable === "true")
		);
	}
	var Zm = Qe && "documentMode" in document && 11 >= document.documentMode,
		na = null,
		qi = null,
		ka = null,
		Yi = !1;
	function Zr(t, e, l) {
		var a =
			l.window === l ? l.document : l.nodeType === 9 ? l : l.ownerDocument;
		Yi ||
			na == null ||
			na !== Vu(a) ||
			((a = na),
			"selectionStart" in a && ji(a)
				? (a = { start: a.selectionStart, end: a.selectionEnd })
				: ((a = (
						(a.ownerDocument && a.ownerDocument.defaultView) ||
						window
					).getSelection()),
					(a = {
						anchorNode: a.anchorNode,
						anchorOffset: a.anchorOffset,
						focusNode: a.focusNode,
						focusOffset: a.focusOffset,
					})),
			(ka && Wa(ka, a)) ||
				((ka = a),
				(a = Gn(qi, "onSelect")),
				0 < a.length &&
					((e = new Wu("onSelect", "select", null, e, l)),
					t.push({ event: e, listeners: a }),
					(e.target = na))));
	}
	function Hl(t, e) {
		var l = {};
		return (
			(l[t.toLowerCase()] = e.toLowerCase()),
			(l["Webkit" + t] = "webkit" + e),
			(l["Moz" + t] = "moz" + e),
			l
		);
	}
	var ia = {
			animationend: Hl("Animation", "AnimationEnd"),
			animationiteration: Hl("Animation", "AnimationIteration"),
			animationstart: Hl("Animation", "AnimationStart"),
			transitionrun: Hl("Transition", "TransitionRun"),
			transitionstart: Hl("Transition", "TransitionStart"),
			transitioncancel: Hl("Transition", "TransitionCancel"),
			transitionend: Hl("Transition", "TransitionEnd"),
		},
		Li = {},
		Vr = {};
	Qe &&
		((Vr = document.createElement("div").style),
		"AnimationEvent" in window ||
			(delete ia.animationend.animation,
			delete ia.animationiteration.animation,
			delete ia.animationstart.animation),
		"TransitionEvent" in window || delete ia.transitionend.transition);
	function Bl(t) {
		if (Li[t]) return Li[t];
		if (!ia[t]) return t;
		var e = ia[t],
			l;
		for (l in e) if (e.hasOwnProperty(l) && l in Vr) return (Li[t] = e[l]);
		return t;
	}
	var Kr = Bl("animationend"),
		wr = Bl("animationiteration"),
		Jr = Bl("animationstart"),
		Vm = Bl("transitionrun"),
		Km = Bl("transitionstart"),
		wm = Bl("transitioncancel"),
		$r = Bl("transitionend"),
		Wr = new Map(),
		Gi =
			"abort auxClick beforeToggle cancel canPlay canPlayThrough click close contextMenu copy cut drag dragEnd dragEnter dragExit dragLeave dragOver dragStart drop durationChange emptied encrypted ended error gotPointerCapture input invalid keyDown keyPress keyUp load loadedData loadedMetadata loadStart lostPointerCapture mouseDown mouseMove mouseOut mouseOver mouseUp paste pause play playing pointerCancel pointerDown pointerMove pointerOut pointerOver pointerUp progress rateChange reset resize seeked seeking stalled submit suspend timeUpdate touchCancel touchEnd touchStart volumeChange scroll toggle touchMove waiting wheel".split(
				" ",
			);
	Gi.push("scrollEnd");
	function Me(t, e) {
		(Wr.set(t, e), Nl(e, [t]));
	}
	var Iu =
			typeof reportError == "function"
				? reportError
				: function (t) {
						if (
							typeof window == "object" &&
							typeof window.ErrorEvent == "function"
						) {
							var e = new window.ErrorEvent("error", {
								bubbles: !0,
								cancelable: !0,
								message:
									typeof t == "object" &&
									t !== null &&
									typeof t.message == "string"
										? String(t.message)
										: String(t),
								error: t,
							});
							if (!window.dispatchEvent(e)) return;
						} else if (
							typeof process == "object" &&
							typeof process.emit == "function"
						) {
							process.emit("uncaughtException", t);
							return;
						}
						console.error(t);
					},
		Se = [],
		ca = 0,
		Xi = 0;
	function Pu() {
		for (var t = ca, e = (Xi = ca = 0); e < t; ) {
			var l = Se[e];
			Se[e++] = null;
			var a = Se[e];
			Se[e++] = null;
			var u = Se[e];
			Se[e++] = null;
			var n = Se[e];
			if (((Se[e++] = null), a !== null && u !== null)) {
				var i = a.pending;
				(i === null ? (u.next = u) : ((u.next = i.next), (i.next = u)),
					(a.pending = u));
			}
			n !== 0 && kr(l, u, n);
		}
	}
	function tn(t, e, l, a) {
		((Se[ca++] = t),
			(Se[ca++] = e),
			(Se[ca++] = l),
			(Se[ca++] = a),
			(Xi |= a),
			(t.lanes |= a),
			(t = t.alternate),
			t !== null && (t.lanes |= a));
	}
	function Qi(t, e, l, a) {
		return (tn(t, e, l, a), en(t));
	}
	function jl(t, e) {
		return (tn(t, null, null, e), en(t));
	}
	function kr(t, e, l) {
		t.lanes |= l;
		var a = t.alternate;
		a !== null && (a.lanes |= l);
		for (var u = !1, n = t.return; n !== null; )
			((n.childLanes |= l),
				(a = n.alternate),
				a !== null && (a.childLanes |= l),
				n.tag === 22 &&
					((t = n.stateNode), t === null || t._visibility & 1 || (u = !0)),
				(t = n),
				(n = n.return));
		return t.tag === 3
			? ((n = t.stateNode),
				u &&
					e !== null &&
					((u = 31 - ie(l)),
					(t = n.hiddenUpdates),
					(a = t[u]),
					a === null ? (t[u] = [e]) : a.push(e),
					(e.lane = l | 536870912)),
				n)
			: null;
	}
	function en(t) {
		if (50 < pu) throw ((pu = 0), (Fc = null), Error(f(185)));
		for (var e = t.return; e !== null; ) ((t = e), (e = t.return));
		return t.tag === 3 ? t.stateNode : null;
	}
	var fa = {};
	function Jm(t, e, l, a) {
		((this.tag = t),
			(this.key = l),
			(this.sibling =
				this.child =
				this.return =
				this.stateNode =
				this.type =
				this.elementType =
					null),
			(this.index = 0),
			(this.refCleanup = this.ref = null),
			(this.pendingProps = e),
			(this.dependencies =
				this.memoizedState =
				this.updateQueue =
				this.memoizedProps =
					null),
			(this.mode = a),
			(this.subtreeFlags = this.flags = 0),
			(this.deletions = null),
			(this.childLanes = this.lanes = 0),
			(this.alternate = null));
	}
	function fe(t, e, l, a) {
		return new Jm(t, e, l, a);
	}
	function Zi(t) {
		return ((t = t.prototype), !(!t || !t.isReactComponent));
	}
	function Ze(t, e) {
		var l = t.alternate;
		return (
			l === null
				? ((l = fe(t.tag, e, t.key, t.mode)),
					(l.elementType = t.elementType),
					(l.type = t.type),
					(l.stateNode = t.stateNode),
					(l.alternate = t),
					(t.alternate = l))
				: ((l.pendingProps = e),
					(l.type = t.type),
					(l.flags = 0),
					(l.subtreeFlags = 0),
					(l.deletions = null)),
			(l.flags = t.flags & 65011712),
			(l.childLanes = t.childLanes),
			(l.lanes = t.lanes),
			(l.child = t.child),
			(l.memoizedProps = t.memoizedProps),
			(l.memoizedState = t.memoizedState),
			(l.updateQueue = t.updateQueue),
			(e = t.dependencies),
			(l.dependencies =
				e === null ? null : { lanes: e.lanes, firstContext: e.firstContext }),
			(l.sibling = t.sibling),
			(l.index = t.index),
			(l.ref = t.ref),
			(l.refCleanup = t.refCleanup),
			l
		);
	}
	function Fr(t, e) {
		t.flags &= 65011714;
		var l = t.alternate;
		return (
			l === null
				? ((t.childLanes = 0),
					(t.lanes = e),
					(t.child = null),
					(t.subtreeFlags = 0),
					(t.memoizedProps = null),
					(t.memoizedState = null),
					(t.updateQueue = null),
					(t.dependencies = null),
					(t.stateNode = null))
				: ((t.childLanes = l.childLanes),
					(t.lanes = l.lanes),
					(t.child = l.child),
					(t.subtreeFlags = 0),
					(t.deletions = null),
					(t.memoizedProps = l.memoizedProps),
					(t.memoizedState = l.memoizedState),
					(t.updateQueue = l.updateQueue),
					(t.type = l.type),
					(e = l.dependencies),
					(t.dependencies =
						e === null
							? null
							: { lanes: e.lanes, firstContext: e.firstContext })),
			t
		);
	}
	function ln(t, e, l, a, u, n) {
		var i = 0;
		if (((a = t), typeof t == "function")) Zi(t) && (i = 1);
		else if (typeof t == "string")
			i = Iy(t, l, X.current)
				? 26
				: t === "html" || t === "head" || t === "body"
					? 27
					: 5;
		else
			t: switch (t) {
				case wt:
					return (
						(t = fe(31, l, e, u)),
						(t.elementType = wt),
						(t.lanes = n),
						t
					);
				case L:
					return ql(l.children, u, n, e);
				case G:
					((i = 8), (u |= 24));
					break;
				case j:
					return (
						(t = fe(12, l, e, u | 2)),
						(t.elementType = j),
						(t.lanes = n),
						t
					);
				case dt:
					return (
						(t = fe(13, l, e, u)),
						(t.elementType = dt),
						(t.lanes = n),
						t
					);
				case Tt:
					return (
						(t = fe(19, l, e, u)),
						(t.elementType = Tt),
						(t.lanes = n),
						t
					);
				default:
					if (typeof t == "object" && t !== null)
						switch (t.$$typeof) {
							case $:
								i = 10;
								break t;
							case I:
								i = 9;
								break t;
							case yt:
								i = 11;
								break t;
							case k:
								i = 14;
								break t;
							case Dt:
								((i = 16), (a = null));
								break t;
						}
					((i = 29),
						(l = Error(f(130, t === null ? "null" : typeof t, ""))),
						(a = null));
			}
		return (
			(e = fe(i, l, e, u)),
			(e.elementType = t),
			(e.type = a),
			(e.lanes = n),
			e
		);
	}
	function ql(t, e, l, a) {
		return ((t = fe(7, t, a, e)), (t.lanes = l), t);
	}
	function Vi(t, e, l) {
		return ((t = fe(6, t, null, e)), (t.lanes = l), t);
	}
	function Ir(t) {
		var e = fe(18, null, null, 0);
		return ((e.stateNode = t), e);
	}
	function Ki(t, e, l) {
		return (
			(e = fe(4, t.children !== null ? t.children : [], t.key, e)),
			(e.lanes = l),
			(e.stateNode = {
				containerInfo: t.containerInfo,
				pendingChildren: null,
				implementation: t.implementation,
			}),
			e
		);
	}
	var Pr = new WeakMap();
	function be(t, e) {
		if (typeof t == "object" && t !== null) {
			var l = Pr.get(t);
			return l !== void 0
				? l
				: ((e = { value: t, source: e, stack: If(e) }), Pr.set(t, e), e);
		}
		return { value: t, source: e, stack: If(e) };
	}
	var ra = [],
		oa = 0,
		an = null,
		Fa = 0,
		Ee = [],
		Te = 0,
		fl = null,
		He = 1,
		Be = "";
	function Ve(t, e) {
		((ra[oa++] = Fa), (ra[oa++] = an), (an = t), (Fa = e));
	}
	function to(t, e, l) {
		((Ee[Te++] = He), (Ee[Te++] = Be), (Ee[Te++] = fl), (fl = t));
		var a = He;
		t = Be;
		var u = 32 - ie(a) - 1;
		((a &= ~(1 << u)), (l += 1));
		var n = 32 - ie(e) + u;
		if (30 < n) {
			var i = u - (u % 5);
			((n = (a & ((1 << i) - 1)).toString(32)),
				(a >>= i),
				(u -= i),
				(He = (1 << (32 - ie(e) + u)) | (l << u) | a),
				(Be = n + t));
		} else ((He = (1 << n) | (l << u) | a), (Be = t));
	}
	function wi(t) {
		t.return !== null && (Ve(t, 1), to(t, 1, 0));
	}
	function Ji(t) {
		for (; t === an; )
			((an = ra[--oa]), (ra[oa] = null), (Fa = ra[--oa]), (ra[oa] = null));
		for (; t === fl; )
			((fl = Ee[--Te]),
				(Ee[Te] = null),
				(Be = Ee[--Te]),
				(Ee[Te] = null),
				(He = Ee[--Te]),
				(Ee[Te] = null));
	}
	function eo(t, e) {
		((Ee[Te++] = He),
			(Ee[Te++] = Be),
			(Ee[Te++] = fl),
			(He = e.id),
			(Be = e.overflow),
			(fl = t));
	}
	var Qt = null,
		zt = null,
		ct = !1,
		rl = null,
		ze = !1,
		$i = Error(f(519));
	function ol(t) {
		var e = Error(
			f(
				418,
				1 < arguments.length && arguments[1] !== void 0 && arguments[1]
					? "text"
					: "HTML",
				"",
			),
		);
		throw (Ia(be(e, t)), $i);
	}
	function lo(t) {
		var e = t.stateNode,
			l = t.type,
			a = t.memoizedProps;
		switch (((e[Xt] = t), (e[Ft] = a), l)) {
			case "dialog":
				(ut("cancel", e), ut("close", e));
				break;
			case "iframe":
			case "object":
			case "embed":
				ut("load", e);
				break;
			case "video":
			case "audio":
				for (l = 0; l < bu.length; l++) ut(bu[l], e);
				break;
			case "source":
				ut("error", e);
				break;
			case "img":
			case "image":
			case "link":
				(ut("error", e), ut("load", e));
				break;
			case "details":
				ut("toggle", e);
				break;
			case "input":
				(ut("invalid", e),
					yr(
						e,
						a.value,
						a.defaultValue,
						a.checked,
						a.defaultChecked,
						a.type,
						a.name,
						!0,
					));
				break;
			case "select":
				ut("invalid", e);
				break;
			case "textarea":
				(ut("invalid", e), gr(e, a.value, a.defaultValue, a.children));
		}
		((l = a.children),
			(typeof l != "string" && typeof l != "number" && typeof l != "bigint") ||
			e.textContent === "" + l ||
			a.suppressHydrationWarning === !0 ||
			bd(e.textContent, l)
				? (a.popover != null && (ut("beforetoggle", e), ut("toggle", e)),
					a.onScroll != null && ut("scroll", e),
					a.onScrollEnd != null && ut("scrollend", e),
					a.onClick != null && (e.onclick = Xe),
					(e = !0))
				: (e = !1),
			e || ol(t, !0));
	}
	function ao(t) {
		for (Qt = t.return; Qt; )
			switch (Qt.tag) {
				case 5:
				case 31:
				case 13:
					ze = !1;
					return;
				case 27:
				case 3:
					ze = !0;
					return;
				default:
					Qt = Qt.return;
			}
	}
	function sa(t) {
		if (t !== Qt) return !1;
		if (!ct) return (ao(t), (ct = !0), !1);
		var e = t.tag,
			l;
		if (
			((l = e !== 3 && e !== 27) &&
				((l = e === 5) &&
					((l = t.type),
					(l =
						!(l !== "form" && l !== "button") || hf(t.type, t.memoizedProps))),
				(l = !l)),
			l && zt && ol(t),
			ao(t),
			e === 13)
		) {
			if (((t = t.memoizedState), (t = t !== null ? t.dehydrated : null), !t))
				throw Error(f(317));
			zt = Dd(t);
		} else if (e === 31) {
			if (((t = t.memoizedState), (t = t !== null ? t.dehydrated : null), !t))
				throw Error(f(317));
			zt = Dd(t);
		} else
			e === 27
				? ((e = zt), Al(t.type) ? ((t = pf), (pf = null), (zt = t)) : (zt = e))
				: (zt = Qt ? Re(t.stateNode.nextSibling) : null);
		return !0;
	}
	function Yl() {
		((zt = Qt = null), (ct = !1));
	}
	function Wi() {
		var t = rl;
		return (
			t !== null &&
				(le === null ? (le = t) : le.push.apply(le, t), (rl = null)),
			t
		);
	}
	function Ia(t) {
		rl === null ? (rl = [t]) : rl.push(t);
	}
	var ki = g(null),
		Ll = null,
		Ke = null;
	function sl(t, e, l) {
		(q(ki, e._currentValue), (e._currentValue = l));
	}
	function we(t) {
		((t._currentValue = ki.current), x(ki));
	}
	function Fi(t, e, l) {
		for (; t !== null; ) {
			var a = t.alternate;
			if (
				((t.childLanes & e) !== e
					? ((t.childLanes |= e), a !== null && (a.childLanes |= e))
					: a !== null && (a.childLanes & e) !== e && (a.childLanes |= e),
				t === l)
			)
				break;
			t = t.return;
		}
	}
	function Ii(t, e, l, a) {
		var u = t.child;
		for (u !== null && (u.return = t); u !== null; ) {
			var n = u.dependencies;
			if (n !== null) {
				var i = u.child;
				n = n.firstContext;
				t: for (; n !== null; ) {
					var r = n;
					n = u;
					for (var h = 0; h < e.length; h++)
						if (r.context === e[h]) {
							((n.lanes |= l),
								(r = n.alternate),
								r !== null && (r.lanes |= l),
								Fi(n.return, l, t),
								a || (i = null));
							break t;
						}
					n = r.next;
				}
			} else if (u.tag === 18) {
				if (((i = u.return), i === null)) throw Error(f(341));
				((i.lanes |= l),
					(n = i.alternate),
					n !== null && (n.lanes |= l),
					Fi(i, l, t),
					(i = null));
			} else i = u.child;
			if (i !== null) i.return = u;
			else
				for (i = u; i !== null; ) {
					if (i === t) {
						i = null;
						break;
					}
					if (((u = i.sibling), u !== null)) {
						((u.return = i.return), (i = u));
						break;
					}
					i = i.return;
				}
			u = i;
		}
	}
	function da(t, e, l, a) {
		t = null;
		for (var u = e, n = !1; u !== null; ) {
			if (!n) {
				if ((u.flags & 524288) !== 0) n = !0;
				else if ((u.flags & 262144) !== 0) break;
			}
			if (u.tag === 10) {
				var i = u.alternate;
				if (i === null) throw Error(f(387));
				if (((i = i.memoizedProps), i !== null)) {
					var r = u.type;
					ce(u.pendingProps.value, i.value) ||
						(t !== null ? t.push(r) : (t = [r]));
				}
			} else if (u === st.current) {
				if (((i = u.alternate), i === null)) throw Error(f(387));
				i.memoizedState.memoizedState !== u.memoizedState.memoizedState &&
					(t !== null ? t.push(Ru) : (t = [Ru]));
			}
			u = u.return;
		}
		(t !== null && Ii(e, t, l, a), (e.flags |= 262144));
	}
	function un(t) {
		for (t = t.firstContext; t !== null; ) {
			if (!ce(t.context._currentValue, t.memoizedValue)) return !0;
			t = t.next;
		}
		return !1;
	}
	function Gl(t) {
		((Ll = t),
			(Ke = null),
			(t = t.dependencies),
			t !== null && (t.firstContext = null));
	}
	function Zt(t) {
		return uo(Ll, t);
	}
	function nn(t, e) {
		return (Ll === null && Gl(t), uo(t, e));
	}
	function uo(t, e) {
		var l = e._currentValue;
		if (((e = { context: e, memoizedValue: l, next: null }), Ke === null)) {
			if (t === null) throw Error(f(308));
			((Ke = e),
				(t.dependencies = { lanes: 0, firstContext: e }),
				(t.flags |= 524288));
		} else Ke = Ke.next = e;
		return l;
	}
	var $m =
			typeof AbortController < "u"
				? AbortController
				: function () {
						var t = [],
							e = (this.signal = {
								aborted: !1,
								addEventListener: function (l, a) {
									t.push(a);
								},
							});
						this.abort = function () {
							((e.aborted = !0),
								t.forEach(function (l) {
									return l();
								}));
						};
					},
		Wm = c.unstable_scheduleCallback,
		km = c.unstable_NormalPriority,
		Ht = {
			$$typeof: $,
			Consumer: null,
			Provider: null,
			_currentValue: null,
			_currentValue2: null,
			_threadCount: 0,
		};
	function Pi() {
		return { controller: new $m(), data: new Map(), refCount: 0 };
	}
	function Pa(t) {
		(t.refCount--,
			t.refCount === 0 &&
				Wm(km, function () {
					t.controller.abort();
				}));
	}
	var tu = null,
		tc = 0,
		ha = 0,
		ma = null;
	function Fm(t, e) {
		if (tu === null) {
			var l = (tu = []);
			((tc = 0),
				(ha = af()),
				(ma = {
					status: "pending",
					value: void 0,
					then: function (a) {
						l.push(a);
					},
				}));
		}
		return (tc++, e.then(no, no), e);
	}
	function no() {
		if (--tc === 0 && tu !== null) {
			ma !== null && (ma.status = "fulfilled");
			var t = tu;
			((tu = null), (ha = 0), (ma = null));
			for (var e = 0; e < t.length; e++) (0, t[e])();
		}
	}
	function Im(t, e) {
		var l = [],
			a = {
				status: "pending",
				value: null,
				reason: null,
				then: function (u) {
					l.push(u);
				},
			};
		return (
			t.then(
				function () {
					((a.status = "fulfilled"), (a.value = e));
					for (var u = 0; u < l.length; u++) (0, l[u])(e);
				},
				function (u) {
					for (a.status = "rejected", a.reason = u, u = 0; u < l.length; u++)
						(0, l[u])(void 0);
				},
			),
			a
		);
	}
	var io = D.S;
	D.S = function (t, e) {
		((Vs = ue()),
			typeof e == "object" &&
				e !== null &&
				typeof e.then == "function" &&
				Fm(t, e),
			io !== null && io(t, e));
	};
	var Xl = g(null);
	function ec() {
		var t = Xl.current;
		return t !== null ? t : Et.pooledCache;
	}
	function cn(t, e) {
		e === null ? q(Xl, Xl.current) : q(Xl, e.pool);
	}
	function co() {
		var t = ec();
		return t === null ? null : { parent: Ht._currentValue, pool: t };
	}
	var ya = Error(f(460)),
		lc = Error(f(474)),
		fn = Error(f(542)),
		rn = { then: function () {} };
	function fo(t) {
		return ((t = t.status), t === "fulfilled" || t === "rejected");
	}
	function ro(t, e, l) {
		switch (
			((l = t[l]),
			l === void 0 ? t.push(e) : l !== e && (e.then(Xe, Xe), (e = l)),
			e.status)
		) {
			case "fulfilled":
				return e.value;
			case "rejected":
				throw ((t = e.reason), so(t), t);
			default:
				if (typeof e.status == "string") e.then(Xe, Xe);
				else {
					if (((t = Et), t !== null && 100 < t.shellSuspendCounter))
						throw Error(f(482));
					((t = e),
						(t.status = "pending"),
						t.then(
							function (a) {
								if (e.status === "pending") {
									var u = e;
									((u.status = "fulfilled"), (u.value = a));
								}
							},
							function (a) {
								if (e.status === "pending") {
									var u = e;
									((u.status = "rejected"), (u.reason = a));
								}
							},
						));
				}
				switch (e.status) {
					case "fulfilled":
						return e.value;
					case "rejected":
						throw ((t = e.reason), so(t), t);
				}
				throw ((Zl = e), ya);
		}
	}
	function Ql(t) {
		try {
			var e = t._init;
			return e(t._payload);
		} catch (l) {
			throw l !== null && typeof l == "object" && typeof l.then == "function"
				? ((Zl = l), ya)
				: l;
		}
	}
	var Zl = null;
	function oo() {
		if (Zl === null) throw Error(f(459));
		var t = Zl;
		return ((Zl = null), t);
	}
	function so(t) {
		if (t === ya || t === fn) throw Error(f(483));
	}
	var va = null,
		eu = 0;
	function on(t) {
		var e = eu;
		return ((eu += 1), va === null && (va = []), ro(va, t, e));
	}
	function lu(t, e) {
		((e = e.props.ref), (t.ref = e !== void 0 ? e : null));
	}
	function sn(t, e) {
		throw e.$$typeof === H
			? Error(f(525))
			: ((t = Object.prototype.toString.call(e)),
				Error(
					f(
						31,
						t === "[object Object]"
							? "object with keys {" + Object.keys(e).join(", ") + "}"
							: t,
					),
				));
	}
	function ho(t) {
		function e(p, y) {
			if (t) {
				var E = p.deletions;
				E === null ? ((p.deletions = [y]), (p.flags |= 16)) : E.push(y);
			}
		}
		function l(p, y) {
			if (!t) return null;
			for (; y !== null; ) (e(p, y), (y = y.sibling));
			return null;
		}
		function a(p) {
			for (var y = new Map(); p !== null; )
				(p.key !== null ? y.set(p.key, p) : y.set(p.index, p), (p = p.sibling));
			return y;
		}
		function u(p, y) {
			return ((p = Ze(p, y)), (p.index = 0), (p.sibling = null), p);
		}
		function n(p, y, E) {
			return (
				(p.index = E),
				t
					? ((E = p.alternate),
						E !== null
							? ((E = E.index), E < y ? ((p.flags |= 67108866), y) : E)
							: ((p.flags |= 67108866), y))
					: ((p.flags |= 1048576), y)
			);
		}
		function i(p) {
			return (t && p.alternate === null && (p.flags |= 67108866), p);
		}
		function r(p, y, E, U) {
			return y === null || y.tag !== 6
				? ((y = Vi(E, p.mode, U)), (y.return = p), y)
				: ((y = u(y, E)), (y.return = p), y);
		}
		function h(p, y, E, U) {
			var Z = E.type;
			return Z === L
				? O(p, y, E.props.children, U, E.key)
				: y !== null &&
					  (y.elementType === Z ||
							(typeof Z == "object" &&
								Z !== null &&
								Z.$$typeof === Dt &&
								Ql(Z) === y.type))
					? ((y = u(y, E.props)), lu(y, E), (y.return = p), y)
					: ((y = ln(E.type, E.key, E.props, null, p.mode, U)),
						lu(y, E),
						(y.return = p),
						y);
		}
		function T(p, y, E, U) {
			return y === null ||
				y.tag !== 4 ||
				y.stateNode.containerInfo !== E.containerInfo ||
				y.stateNode.implementation !== E.implementation
				? ((y = Ki(E, p.mode, U)), (y.return = p), y)
				: ((y = u(y, E.children || [])), (y.return = p), y);
		}
		function O(p, y, E, U, Z) {
			return y === null || y.tag !== 7
				? ((y = ql(E, p.mode, U, Z)), (y.return = p), y)
				: ((y = u(y, E)), (y.return = p), y);
		}
		function N(p, y, E) {
			if (
				(typeof y == "string" && y !== "") ||
				typeof y == "number" ||
				typeof y == "bigint"
			)
				return ((y = Vi("" + y, p.mode, E)), (y.return = p), y);
			if (typeof y == "object" && y !== null) {
				switch (y.$$typeof) {
					case w:
						return (
							(E = ln(y.type, y.key, y.props, null, p.mode, E)),
							lu(E, y),
							(E.return = p),
							E
						);
					case K:
						return ((y = Ki(y, p.mode, E)), (y.return = p), y);
					case Dt:
						return ((y = Ql(y)), N(p, y, E));
				}
				if (xt(y) || Jt(y))
					return ((y = ql(y, p.mode, E, null)), (y.return = p), y);
				if (typeof y.then == "function") return N(p, on(y), E);
				if (y.$$typeof === $) return N(p, nn(p, y), E);
				sn(p, y);
			}
			return null;
		}
		function z(p, y, E, U) {
			var Z = y !== null ? y.key : null;
			if (
				(typeof E == "string" && E !== "") ||
				typeof E == "number" ||
				typeof E == "bigint"
			)
				return Z !== null ? null : r(p, y, "" + E, U);
			if (typeof E == "object" && E !== null) {
				switch (E.$$typeof) {
					case w:
						return E.key === Z ? h(p, y, E, U) : null;
					case K:
						return E.key === Z ? T(p, y, E, U) : null;
					case Dt:
						return ((E = Ql(E)), z(p, y, E, U));
				}
				if (xt(E) || Jt(E)) return Z !== null ? null : O(p, y, E, U, null);
				if (typeof E.then == "function") return z(p, y, on(E), U);
				if (E.$$typeof === $) return z(p, y, nn(p, E), U);
				sn(p, E);
			}
			return null;
		}
		function _(p, y, E, U, Z) {
			if (
				(typeof U == "string" && U !== "") ||
				typeof U == "number" ||
				typeof U == "bigint"
			)
				return ((p = p.get(E) || null), r(y, p, "" + U, Z));
			if (typeof U == "object" && U !== null) {
				switch (U.$$typeof) {
					case w:
						return (
							(p = p.get(U.key === null ? E : U.key) || null),
							h(y, p, U, Z)
						);
					case K:
						return (
							(p = p.get(U.key === null ? E : U.key) || null),
							T(y, p, U, Z)
						);
					case Dt:
						return ((U = Ql(U)), _(p, y, E, U, Z));
				}
				if (xt(U) || Jt(U))
					return ((p = p.get(E) || null), O(y, p, U, Z, null));
				if (typeof U.then == "function") return _(p, y, E, on(U), Z);
				if (U.$$typeof === $) return _(p, y, E, nn(y, U), Z);
				sn(y, U);
			}
			return null;
		}
		function Y(p, y, E, U) {
			for (
				var Z = null, ft = null, Q = y, et = (y = 0), it = null;
				Q !== null && et < E.length;
				et++
			) {
				Q.index > et ? ((it = Q), (Q = null)) : (it = Q.sibling);
				var rt = z(p, Q, E[et], U);
				if (rt === null) {
					Q === null && (Q = it);
					break;
				}
				(t && Q && rt.alternate === null && e(p, Q),
					(y = n(rt, y, et)),
					ft === null ? (Z = rt) : (ft.sibling = rt),
					(ft = rt),
					(Q = it));
			}
			if (et === E.length) return (l(p, Q), ct && Ve(p, et), Z);
			if (Q === null) {
				for (; et < E.length; et++)
					((Q = N(p, E[et], U)),
						Q !== null &&
							((y = n(Q, y, et)),
							ft === null ? (Z = Q) : (ft.sibling = Q),
							(ft = Q)));
				return (ct && Ve(p, et), Z);
			}
			for (Q = a(Q); et < E.length; et++)
				((it = _(Q, p, et, E[et], U)),
					it !== null &&
						(t &&
							it.alternate !== null &&
							Q.delete(it.key === null ? et : it.key),
						(y = n(it, y, et)),
						ft === null ? (Z = it) : (ft.sibling = it),
						(ft = it)));
			return (
				t &&
					Q.forEach(function (Dl) {
						return e(p, Dl);
					}),
				ct && Ve(p, et),
				Z
			);
		}
		function J(p, y, E, U) {
			if (E == null) throw Error(f(151));
			for (
				var Z = null, ft = null, Q = y, et = (y = 0), it = null, rt = E.next();
				Q !== null && !rt.done;
				et++, rt = E.next()
			) {
				Q.index > et ? ((it = Q), (Q = null)) : (it = Q.sibling);
				var Dl = z(p, Q, rt.value, U);
				if (Dl === null) {
					Q === null && (Q = it);
					break;
				}
				(t && Q && Dl.alternate === null && e(p, Q),
					(y = n(Dl, y, et)),
					ft === null ? (Z = Dl) : (ft.sibling = Dl),
					(ft = Dl),
					(Q = it));
			}
			if (rt.done) return (l(p, Q), ct && Ve(p, et), Z);
			if (Q === null) {
				for (; !rt.done; et++, rt = E.next())
					((rt = N(p, rt.value, U)),
						rt !== null &&
							((y = n(rt, y, et)),
							ft === null ? (Z = rt) : (ft.sibling = rt),
							(ft = rt)));
				return (ct && Ve(p, et), Z);
			}
			for (Q = a(Q); !rt.done; et++, rt = E.next())
				((rt = _(Q, p, et, rt.value, U)),
					rt !== null &&
						(t &&
							rt.alternate !== null &&
							Q.delete(rt.key === null ? et : rt.key),
						(y = n(rt, y, et)),
						ft === null ? (Z = rt) : (ft.sibling = rt),
						(ft = rt)));
			return (
				t &&
					Q.forEach(function (rv) {
						return e(p, rv);
					}),
				ct && Ve(p, et),
				Z
			);
		}
		function St(p, y, E, U) {
			if (
				(typeof E == "object" &&
					E !== null &&
					E.type === L &&
					E.key === null &&
					(E = E.props.children),
				typeof E == "object" && E !== null)
			) {
				switch (E.$$typeof) {
					case w:
						t: {
							for (var Z = E.key; y !== null; ) {
								if (y.key === Z) {
									if (((Z = E.type), Z === L)) {
										if (y.tag === 7) {
											(l(p, y.sibling),
												(U = u(y, E.props.children)),
												(U.return = p),
												(p = U));
											break t;
										}
									} else if (
										y.elementType === Z ||
										(typeof Z == "object" &&
											Z !== null &&
											Z.$$typeof === Dt &&
											Ql(Z) === y.type)
									) {
										(l(p, y.sibling),
											(U = u(y, E.props)),
											lu(U, E),
											(U.return = p),
											(p = U));
										break t;
									}
									l(p, y);
									break;
								} else e(p, y);
								y = y.sibling;
							}
							E.type === L
								? ((U = ql(E.props.children, p.mode, U, E.key)),
									(U.return = p),
									(p = U))
								: ((U = ln(E.type, E.key, E.props, null, p.mode, U)),
									lu(U, E),
									(U.return = p),
									(p = U));
						}
						return i(p);
					case K:
						t: {
							for (Z = E.key; y !== null; ) {
								if (y.key === Z)
									if (
										y.tag === 4 &&
										y.stateNode.containerInfo === E.containerInfo &&
										y.stateNode.implementation === E.implementation
									) {
										(l(p, y.sibling),
											(U = u(y, E.children || [])),
											(U.return = p),
											(p = U));
										break t;
									} else {
										l(p, y);
										break;
									}
								else e(p, y);
								y = y.sibling;
							}
							((U = Ki(E, p.mode, U)), (U.return = p), (p = U));
						}
						return i(p);
					case Dt:
						return ((E = Ql(E)), St(p, y, E, U));
				}
				if (xt(E)) return Y(p, y, E, U);
				if (Jt(E)) {
					if (((Z = Jt(E)), typeof Z != "function")) throw Error(f(150));
					return ((E = Z.call(E)), J(p, y, E, U));
				}
				if (typeof E.then == "function") return St(p, y, on(E), U);
				if (E.$$typeof === $) return St(p, y, nn(p, E), U);
				sn(p, E);
			}
			return (typeof E == "string" && E !== "") ||
				typeof E == "number" ||
				typeof E == "bigint"
				? ((E = "" + E),
					y !== null && y.tag === 6
						? (l(p, y.sibling), (U = u(y, E)), (U.return = p), (p = U))
						: (l(p, y), (U = Vi(E, p.mode, U)), (U.return = p), (p = U)),
					i(p))
				: l(p, y);
		}
		return function (p, y, E, U) {
			try {
				eu = 0;
				var Z = St(p, y, E, U);
				return ((va = null), Z);
			} catch (Q) {
				if (Q === ya || Q === fn) throw Q;
				var ft = fe(29, Q, null, p.mode);
				return ((ft.lanes = U), (ft.return = p), ft);
			} finally {
			}
		};
	}
	var Vl = ho(!0),
		mo = ho(!1),
		dl = !1;
	function ac(t) {
		t.updateQueue = {
			baseState: t.memoizedState,
			firstBaseUpdate: null,
			lastBaseUpdate: null,
			shared: { pending: null, lanes: 0, hiddenCallbacks: null },
			callbacks: null,
		};
	}
	function uc(t, e) {
		((t = t.updateQueue),
			e.updateQueue === t &&
				(e.updateQueue = {
					baseState: t.baseState,
					firstBaseUpdate: t.firstBaseUpdate,
					lastBaseUpdate: t.lastBaseUpdate,
					shared: t.shared,
					callbacks: null,
				}));
	}
	function hl(t) {
		return { lane: t, tag: 0, payload: null, callback: null, next: null };
	}
	function ml(t, e, l) {
		var a = t.updateQueue;
		if (a === null) return null;
		if (((a = a.shared), (ot & 2) !== 0)) {
			var u = a.pending;
			return (
				u === null ? (e.next = e) : ((e.next = u.next), (u.next = e)),
				(a.pending = e),
				(e = en(t)),
				kr(t, null, l),
				e
			);
		}
		return (tn(t, a, e, l), en(t));
	}
	function au(t, e, l) {
		if (
			((e = e.updateQueue), e !== null && ((e = e.shared), (l & 4194048) !== 0))
		) {
			var a = e.lanes;
			((a &= t.pendingLanes), (l |= a), (e.lanes = l), ur(t, l));
		}
	}
	function nc(t, e) {
		var l = t.updateQueue,
			a = t.alternate;
		if (a !== null && ((a = a.updateQueue), l === a)) {
			var u = null,
				n = null;
			if (((l = l.firstBaseUpdate), l !== null)) {
				do {
					var i = {
						lane: l.lane,
						tag: l.tag,
						payload: l.payload,
						callback: null,
						next: null,
					};
					(n === null ? (u = n = i) : (n = n.next = i), (l = l.next));
				} while (l !== null);
				n === null ? (u = n = e) : (n = n.next = e);
			} else u = n = e;
			((l = {
				baseState: a.baseState,
				firstBaseUpdate: u,
				lastBaseUpdate: n,
				shared: a.shared,
				callbacks: a.callbacks,
			}),
				(t.updateQueue = l));
			return;
		}
		((t = l.lastBaseUpdate),
			t === null ? (l.firstBaseUpdate = e) : (t.next = e),
			(l.lastBaseUpdate = e));
	}
	var ic = !1;
	function uu() {
		if (ic) {
			var t = ma;
			if (t !== null) throw t;
		}
	}
	function nu(t, e, l, a) {
		ic = !1;
		var u = t.updateQueue;
		dl = !1;
		var n = u.firstBaseUpdate,
			i = u.lastBaseUpdate,
			r = u.shared.pending;
		if (r !== null) {
			u.shared.pending = null;
			var h = r,
				T = h.next;
			((h.next = null), i === null ? (n = T) : (i.next = T), (i = h));
			var O = t.alternate;
			O !== null &&
				((O = O.updateQueue),
				(r = O.lastBaseUpdate),
				r !== i &&
					(r === null ? (O.firstBaseUpdate = T) : (r.next = T),
					(O.lastBaseUpdate = h)));
		}
		if (n !== null) {
			var N = u.baseState;
			((i = 0), (O = T = h = null), (r = n));
			do {
				var z = r.lane & -536870913,
					_ = z !== r.lane;
				if (_ ? (nt & z) === z : (a & z) === z) {
					(z !== 0 && z === ha && (ic = !0),
						O !== null &&
							(O = O.next =
								{
									lane: 0,
									tag: r.tag,
									payload: r.payload,
									callback: null,
									next: null,
								}));
					t: {
						var Y = t,
							J = r;
						z = e;
						var St = l;
						switch (J.tag) {
							case 1:
								if (((Y = J.payload), typeof Y == "function")) {
									N = Y.call(St, N, z);
									break t;
								}
								N = Y;
								break t;
							case 3:
								Y.flags = (Y.flags & -65537) | 128;
							case 0:
								if (
									((Y = J.payload),
									(z = typeof Y == "function" ? Y.call(St, N, z) : Y),
									z == null)
								)
									break t;
								N = R({}, N, z);
								break t;
							case 2:
								dl = !0;
						}
					}
					((z = r.callback),
						z !== null &&
							((t.flags |= 64),
							_ && (t.flags |= 8192),
							(_ = u.callbacks),
							_ === null ? (u.callbacks = [z]) : _.push(z)));
				} else
					((_ = {
						lane: z,
						tag: r.tag,
						payload: r.payload,
						callback: r.callback,
						next: null,
					}),
						O === null ? ((T = O = _), (h = N)) : (O = O.next = _),
						(i |= z));
				if (((r = r.next), r === null)) {
					if (((r = u.shared.pending), r === null)) break;
					((_ = r),
						(r = _.next),
						(_.next = null),
						(u.lastBaseUpdate = _),
						(u.shared.pending = null));
				}
			} while (!0);
			(O === null && (h = N),
				(u.baseState = h),
				(u.firstBaseUpdate = T),
				(u.lastBaseUpdate = O),
				n === null && (u.shared.lanes = 0),
				(Sl |= i),
				(t.lanes = i),
				(t.memoizedState = N));
		}
	}
	function yo(t, e) {
		if (typeof t != "function") throw Error(f(191, t));
		t.call(e);
	}
	function vo(t, e) {
		var l = t.callbacks;
		if (l !== null)
			for (t.callbacks = null, t = 0; t < l.length; t++) yo(l[t], e);
	}
	var ga = g(null),
		dn = g(0);
	function go(t, e) {
		((t = el), q(dn, t), q(ga, e), (el = t | e.baseLanes));
	}
	function cc() {
		(q(dn, el), q(ga, ga.current));
	}
	function fc() {
		((el = dn.current), x(ga), x(dn));
	}
	var re = g(null),
		Ae = null;
	function yl(t) {
		var e = t.alternate;
		(q(Ut, Ut.current & 1),
			q(re, t),
			Ae === null &&
				(e === null || ga.current !== null || e.memoizedState !== null) &&
				(Ae = t));
	}
	function rc(t) {
		(q(Ut, Ut.current), q(re, t), Ae === null && (Ae = t));
	}
	function po(t) {
		t.tag === 22
			? (q(Ut, Ut.current), q(re, t), Ae === null && (Ae = t))
			: vl();
	}
	function vl() {
		(q(Ut, Ut.current), q(re, re.current));
	}
	function oe(t) {
		(x(re), Ae === t && (Ae = null), x(Ut));
	}
	var Ut = g(0);
	function hn(t) {
		for (var e = t; e !== null; ) {
			if (e.tag === 13) {
				var l = e.memoizedState;
				if (l !== null && ((l = l.dehydrated), l === null || vf(l) || gf(l)))
					return e;
			} else if (
				e.tag === 19 &&
				(e.memoizedProps.revealOrder === "forwards" ||
					e.memoizedProps.revealOrder === "backwards" ||
					e.memoizedProps.revealOrder === "unstable_legacy-backwards" ||
					e.memoizedProps.revealOrder === "together")
			) {
				if ((e.flags & 128) !== 0) return e;
			} else if (e.child !== null) {
				((e.child.return = e), (e = e.child));
				continue;
			}
			if (e === t) break;
			for (; e.sibling === null; ) {
				if (e.return === null || e.return === t) return null;
				e = e.return;
			}
			((e.sibling.return = e.return), (e = e.sibling));
		}
		return null;
	}
	var Je = 0,
		tt = null,
		gt = null,
		Bt = null,
		mn = !1,
		pa = !1,
		Kl = !1,
		yn = 0,
		iu = 0,
		Sa = null,
		Pm = 0;
	function Mt() {
		throw Error(f(321));
	}
	function oc(t, e) {
		if (e === null) return !1;
		for (var l = 0; l < e.length && l < t.length; l++)
			if (!ce(t[l], e[l])) return !1;
		return !0;
	}
	function sc(t, e, l, a, u, n) {
		return (
			(Je = n),
			(tt = e),
			(e.memoizedState = null),
			(e.updateQueue = null),
			(e.lanes = 0),
			(D.H = t === null || t.memoizedState === null ? es : _c),
			(Kl = !1),
			(n = l(a, u)),
			(Kl = !1),
			pa && (n = bo(e, l, a, u)),
			So(t),
			n
		);
	}
	function So(t) {
		D.H = ru;
		var e = gt !== null && gt.next !== null;
		if (((Je = 0), (Bt = gt = tt = null), (mn = !1), (iu = 0), (Sa = null), e))
			throw Error(f(300));
		t === null ||
			jt ||
			((t = t.dependencies), t !== null && un(t) && (jt = !0));
	}
	function bo(t, e, l, a) {
		tt = t;
		var u = 0;
		do {
			if ((pa && (Sa = null), (iu = 0), (pa = !1), 25 <= u))
				throw Error(f(301));
			if (((u += 1), (Bt = gt = null), t.updateQueue != null)) {
				var n = t.updateQueue;
				((n.lastEffect = null),
					(n.events = null),
					(n.stores = null),
					n.memoCache != null && (n.memoCache.index = 0));
			}
			((D.H = ls), (n = e(l, a)));
		} while (pa);
		return n;
	}
	function ty() {
		var t = D.H,
			e = t.useState()[0];
		return (
			(e = typeof e.then == "function" ? cu(e) : e),
			(t = t.useState()[0]),
			(gt !== null ? gt.memoizedState : null) !== t && (tt.flags |= 1024),
			e
		);
	}
	function dc() {
		var t = yn !== 0;
		return ((yn = 0), t);
	}
	function hc(t, e, l) {
		((e.updateQueue = t.updateQueue), (e.flags &= -2053), (t.lanes &= ~l));
	}
	function mc(t) {
		if (mn) {
			for (t = t.memoizedState; t !== null; ) {
				var e = t.queue;
				(e !== null && (e.pending = null), (t = t.next));
			}
			mn = !1;
		}
		((Je = 0), (Bt = gt = tt = null), (pa = !1), (iu = yn = 0), (Sa = null));
	}
	function kt() {
		var t = {
			memoizedState: null,
			baseState: null,
			baseQueue: null,
			queue: null,
			next: null,
		};
		return (Bt === null ? (tt.memoizedState = Bt = t) : (Bt = Bt.next = t), Bt);
	}
	function Nt() {
		if (gt === null) {
			var t = tt.alternate;
			t = t !== null ? t.memoizedState : null;
		} else t = gt.next;
		var e = Bt === null ? tt.memoizedState : Bt.next;
		if (e !== null) ((Bt = e), (gt = t));
		else {
			if (t === null)
				throw tt.alternate === null ? Error(f(467)) : Error(f(310));
			((gt = t),
				(t = {
					memoizedState: gt.memoizedState,
					baseState: gt.baseState,
					baseQueue: gt.baseQueue,
					queue: gt.queue,
					next: null,
				}),
				Bt === null ? (tt.memoizedState = Bt = t) : (Bt = Bt.next = t));
		}
		return Bt;
	}
	function vn() {
		return { lastEffect: null, events: null, stores: null, memoCache: null };
	}
	function cu(t) {
		var e = iu;
		return (
			(iu += 1),
			Sa === null && (Sa = []),
			(t = ro(Sa, t, e)),
			(e = tt),
			(Bt === null ? e.memoizedState : Bt.next) === null &&
				((e = e.alternate),
				(D.H = e === null || e.memoizedState === null ? es : _c)),
			t
		);
	}
	function gn(t) {
		if (t !== null && typeof t == "object") {
			if (typeof t.then == "function") return cu(t);
			if (t.$$typeof === $) return Zt(t);
		}
		throw Error(f(438, String(t)));
	}
	function yc(t) {
		var e = null,
			l = tt.updateQueue;
		if ((l !== null && (e = l.memoCache), e == null)) {
			var a = tt.alternate;
			a !== null &&
				((a = a.updateQueue),
				a !== null &&
					((a = a.memoCache),
					a != null &&
						(e = {
							data: a.data.map(function (u) {
								return u.slice();
							}),
							index: 0,
						})));
		}
		if (
			(e == null && (e = { data: [], index: 0 }),
			l === null && ((l = vn()), (tt.updateQueue = l)),
			(l.memoCache = e),
			(l = e.data[e.index]),
			l === void 0)
		)
			for (l = e.data[e.index] = Array(t), a = 0; a < t; a++) l[a] = Ne;
		return (e.index++, l);
	}
	function $e(t, e) {
		return typeof e == "function" ? e(t) : e;
	}
	function pn(t) {
		var e = Nt();
		return vc(e, gt, t);
	}
	function vc(t, e, l) {
		var a = t.queue;
		if (a === null) throw Error(f(311));
		a.lastRenderedReducer = l;
		var u = t.baseQueue,
			n = a.pending;
		if (n !== null) {
			if (u !== null) {
				var i = u.next;
				((u.next = n.next), (n.next = i));
			}
			((e.baseQueue = u = n), (a.pending = null));
		}
		if (((n = t.baseState), u === null)) t.memoizedState = n;
		else {
			e = u.next;
			var r = (i = null),
				h = null,
				T = e,
				O = !1;
			do {
				var N = T.lane & -536870913;
				if (N !== T.lane ? (nt & N) === N : (Je & N) === N) {
					var z = T.revertLane;
					if (z === 0)
						(h !== null &&
							(h = h.next =
								{
									lane: 0,
									revertLane: 0,
									gesture: null,
									action: T.action,
									hasEagerState: T.hasEagerState,
									eagerState: T.eagerState,
									next: null,
								}),
							N === ha && (O = !0));
					else if ((Je & z) === z) {
						((T = T.next), z === ha && (O = !0));
						continue;
					} else
						((N = {
							lane: 0,
							revertLane: T.revertLane,
							gesture: null,
							action: T.action,
							hasEagerState: T.hasEagerState,
							eagerState: T.eagerState,
							next: null,
						}),
							h === null ? ((r = h = N), (i = n)) : (h = h.next = N),
							(tt.lanes |= z),
							(Sl |= z));
					((N = T.action),
						Kl && l(n, N),
						(n = T.hasEagerState ? T.eagerState : l(n, N)));
				} else
					((z = {
						lane: N,
						revertLane: T.revertLane,
						gesture: T.gesture,
						action: T.action,
						hasEagerState: T.hasEagerState,
						eagerState: T.eagerState,
						next: null,
					}),
						h === null ? ((r = h = z), (i = n)) : (h = h.next = z),
						(tt.lanes |= N),
						(Sl |= N));
				T = T.next;
			} while (T !== null && T !== e);
			if (
				(h === null ? (i = n) : (h.next = r),
				!ce(n, t.memoizedState) && ((jt = !0), O && ((l = ma), l !== null)))
			)
				throw l;
			((t.memoizedState = n),
				(t.baseState = i),
				(t.baseQueue = h),
				(a.lastRenderedState = n));
		}
		return (u === null && (a.lanes = 0), [t.memoizedState, a.dispatch]);
	}
	function gc(t) {
		var e = Nt(),
			l = e.queue;
		if (l === null) throw Error(f(311));
		l.lastRenderedReducer = t;
		var a = l.dispatch,
			u = l.pending,
			n = e.memoizedState;
		if (u !== null) {
			l.pending = null;
			var i = (u = u.next);
			do ((n = t(n, i.action)), (i = i.next));
			while (i !== u);
			(ce(n, e.memoizedState) || (jt = !0),
				(e.memoizedState = n),
				e.baseQueue === null && (e.baseState = n),
				(l.lastRenderedState = n));
		}
		return [n, a];
	}
	function Eo(t, e, l) {
		var a = tt,
			u = Nt(),
			n = ct;
		if (n) {
			if (l === void 0) throw Error(f(407));
			l = l();
		} else l = e();
		var i = !ce((gt || u).memoizedState, l);
		if (
			(i && ((u.memoizedState = l), (jt = !0)),
			(u = u.queue),
			bc(Ao.bind(null, a, u, t), [t]),
			u.getSnapshot !== e || i || (Bt !== null && Bt.memoizedState.tag & 1))
		) {
			if (
				((a.flags |= 2048),
				ba(9, { destroy: void 0 }, zo.bind(null, a, u, l, e), null),
				Et === null)
			)
				throw Error(f(349));
			n || (Je & 127) !== 0 || To(a, e, l);
		}
		return l;
	}
	function To(t, e, l) {
		((t.flags |= 16384),
			(t = { getSnapshot: e, value: l }),
			(e = tt.updateQueue),
			e === null
				? ((e = vn()), (tt.updateQueue = e), (e.stores = [t]))
				: ((l = e.stores), l === null ? (e.stores = [t]) : l.push(t)));
	}
	function zo(t, e, l, a) {
		((e.value = l), (e.getSnapshot = a), Ro(e) && _o(t));
	}
	function Ao(t, e, l) {
		return l(function () {
			Ro(e) && _o(t);
		});
	}
	function Ro(t) {
		var e = t.getSnapshot;
		t = t.value;
		try {
			var l = e();
			return !ce(t, l);
		} catch {
			return !0;
		}
	}
	function _o(t) {
		var e = jl(t, 2);
		e !== null && ae(e, t, 2);
	}
	function pc(t) {
		var e = kt();
		if (typeof t == "function") {
			var l = t;
			if (((t = l()), Kl)) {
				nl(!0);
				try {
					l();
				} finally {
					nl(!1);
				}
			}
		}
		return (
			(e.memoizedState = e.baseState = t),
			(e.queue = {
				pending: null,
				lanes: 0,
				dispatch: null,
				lastRenderedReducer: $e,
				lastRenderedState: t,
			}),
			e
		);
	}
	function Mo(t, e, l, a) {
		return ((t.baseState = l), vc(t, gt, typeof a == "function" ? a : $e));
	}
	function ey(t, e, l, a, u) {
		if (En(t)) throw Error(f(485));
		if (((t = e.action), t !== null)) {
			var n = {
				payload: u,
				action: t,
				next: null,
				isTransition: !0,
				status: "pending",
				value: null,
				reason: null,
				listeners: [],
				then: function (i) {
					n.listeners.push(i);
				},
			};
			(D.T !== null ? l(!0) : (n.isTransition = !1),
				a(n),
				(l = e.pending),
				l === null
					? ((n.next = e.pending = n), Oo(e, n))
					: ((n.next = l.next), (e.pending = l.next = n)));
		}
	}
	function Oo(t, e) {
		var l = e.action,
			a = e.payload,
			u = t.state;
		if (e.isTransition) {
			var n = D.T,
				i = {};
			D.T = i;
			try {
				var r = l(u, a),
					h = D.S;
				(h !== null && h(i, r), Do(t, e, r));
			} catch (T) {
				Sc(t, e, T);
			} finally {
				(n !== null && i.types !== null && (n.types = i.types), (D.T = n));
			}
		} else
			try {
				((n = l(u, a)), Do(t, e, n));
			} catch (T) {
				Sc(t, e, T);
			}
	}
	function Do(t, e, l) {
		l !== null && typeof l == "object" && typeof l.then == "function"
			? l.then(
					function (a) {
						Co(t, e, a);
					},
					function (a) {
						return Sc(t, e, a);
					},
				)
			: Co(t, e, l);
	}
	function Co(t, e, l) {
		((e.status = "fulfilled"),
			(e.value = l),
			Uo(e),
			(t.state = l),
			(e = t.pending),
			e !== null &&
				((l = e.next),
				l === e ? (t.pending = null) : ((l = l.next), (e.next = l), Oo(t, l))));
	}
	function Sc(t, e, l) {
		var a = t.pending;
		if (((t.pending = null), a !== null)) {
			a = a.next;
			do ((e.status = "rejected"), (e.reason = l), Uo(e), (e = e.next));
			while (e !== a);
		}
		t.action = null;
	}
	function Uo(t) {
		t = t.listeners;
		for (var e = 0; e < t.length; e++) (0, t[e])();
	}
	function No(t, e) {
		return e;
	}
	function xo(t, e) {
		if (ct) {
			var l = Et.formState;
			if (l !== null) {
				t: {
					var a = tt;
					if (ct) {
						if (zt) {
							e: {
								for (var u = zt, n = ze; u.nodeType !== 8; ) {
									if (!n) {
										u = null;
										break e;
									}
									if (((u = Re(u.nextSibling)), u === null)) {
										u = null;
										break e;
									}
								}
								((n = u.data), (u = n === "F!" || n === "F" ? u : null));
							}
							if (u) {
								((zt = Re(u.nextSibling)), (a = u.data === "F!"));
								break t;
							}
						}
						ol(a);
					}
					a = !1;
				}
				a && (e = l[0]);
			}
		}
		return (
			(l = kt()),
			(l.memoizedState = l.baseState = e),
			(a = {
				pending: null,
				lanes: 0,
				dispatch: null,
				lastRenderedReducer: No,
				lastRenderedState: e,
			}),
			(l.queue = a),
			(l = Io.bind(null, tt, a)),
			(a.dispatch = l),
			(a = pc(!1)),
			(n = Rc.bind(null, tt, !1, a.queue)),
			(a = kt()),
			(u = { state: e, dispatch: null, action: t, pending: null }),
			(a.queue = u),
			(l = ey.bind(null, tt, u, n, l)),
			(u.dispatch = l),
			(a.memoizedState = t),
			[e, l, !1]
		);
	}
	function Ho(t) {
		var e = Nt();
		return Bo(e, gt, t);
	}
	function Bo(t, e, l) {
		if (
			((e = vc(t, e, No)[0]),
			(t = pn($e)[0]),
			typeof e == "object" && e !== null && typeof e.then == "function")
		)
			try {
				var a = cu(e);
			} catch (i) {
				throw i === ya ? fn : i;
			}
		else a = e;
		e = Nt();
		var u = e.queue,
			n = u.dispatch;
		return (
			l !== e.memoizedState &&
				((tt.flags |= 2048),
				ba(9, { destroy: void 0 }, ly.bind(null, u, l), null)),
			[a, n, t]
		);
	}
	function ly(t, e) {
		t.action = e;
	}
	function jo(t) {
		var e = Nt(),
			l = gt;
		if (l !== null) return Bo(e, l, t);
		(Nt(), (e = e.memoizedState), (l = Nt()));
		var a = l.queue.dispatch;
		return ((l.memoizedState = t), [e, a, !1]);
	}
	function ba(t, e, l, a) {
		return (
			(t = { tag: t, create: l, deps: a, inst: e, next: null }),
			(e = tt.updateQueue),
			e === null && ((e = vn()), (tt.updateQueue = e)),
			(l = e.lastEffect),
			l === null
				? (e.lastEffect = t.next = t)
				: ((a = l.next), (l.next = t), (t.next = a), (e.lastEffect = t)),
			t
		);
	}
	function qo() {
		return Nt().memoizedState;
	}
	function Sn(t, e, l, a) {
		var u = kt();
		((tt.flags |= t),
			(u.memoizedState = ba(
				1 | e,
				{ destroy: void 0 },
				l,
				a === void 0 ? null : a,
			)));
	}
	function bn(t, e, l, a) {
		var u = Nt();
		a = a === void 0 ? null : a;
		var n = u.memoizedState.inst;
		gt !== null && a !== null && oc(a, gt.memoizedState.deps)
			? (u.memoizedState = ba(e, n, l, a))
			: ((tt.flags |= t), (u.memoizedState = ba(1 | e, n, l, a)));
	}
	function Yo(t, e) {
		Sn(8390656, 8, t, e);
	}
	function bc(t, e) {
		bn(2048, 8, t, e);
	}
	function ay(t) {
		tt.flags |= 4;
		var e = tt.updateQueue;
		if (e === null) ((e = vn()), (tt.updateQueue = e), (e.events = [t]));
		else {
			var l = e.events;
			l === null ? (e.events = [t]) : l.push(t);
		}
	}
	function Lo(t) {
		var e = Nt().memoizedState;
		return (
			ay({ ref: e, nextImpl: t }),
			function () {
				if ((ot & 2) !== 0) throw Error(f(440));
				return e.impl.apply(void 0, arguments);
			}
		);
	}
	function Go(t, e) {
		return bn(4, 2, t, e);
	}
	function Xo(t, e) {
		return bn(4, 4, t, e);
	}
	function Qo(t, e) {
		if (typeof e == "function") {
			t = t();
			var l = e(t);
			return function () {
				typeof l == "function" ? l() : e(null);
			};
		}
		if (e != null)
			return (
				(t = t()),
				(e.current = t),
				function () {
					e.current = null;
				}
			);
	}
	function Zo(t, e, l) {
		((l = l != null ? l.concat([t]) : null), bn(4, 4, Qo.bind(null, e, t), l));
	}
	function Ec() {}
	function Vo(t, e) {
		var l = Nt();
		e = e === void 0 ? null : e;
		var a = l.memoizedState;
		return e !== null && oc(e, a[1]) ? a[0] : ((l.memoizedState = [t, e]), t);
	}
	function Ko(t, e) {
		var l = Nt();
		e = e === void 0 ? null : e;
		var a = l.memoizedState;
		if (e !== null && oc(e, a[1])) return a[0];
		if (((a = t()), Kl)) {
			nl(!0);
			try {
				t();
			} finally {
				nl(!1);
			}
		}
		return ((l.memoizedState = [a, e]), a);
	}
	function Tc(t, e, l) {
		return l === void 0 || ((Je & 1073741824) !== 0 && (nt & 261930) === 0)
			? (t.memoizedState = e)
			: ((t.memoizedState = l), (t = ws()), (tt.lanes |= t), (Sl |= t), l);
	}
	function wo(t, e, l, a) {
		return ce(l, e)
			? l
			: ga.current !== null
				? ((t = Tc(t, l, a)), ce(t, e) || (jt = !0), t)
				: (Je & 42) === 0 || ((Je & 1073741824) !== 0 && (nt & 261930) === 0)
					? ((jt = !0), (t.memoizedState = l))
					: ((t = ws()), (tt.lanes |= t), (Sl |= t), e);
	}
	function Jo(t, e, l, a, u) {
		var n = B.p;
		B.p = n !== 0 && 8 > n ? n : 8;
		var i = D.T,
			r = {};
		((D.T = r), Rc(t, !1, e, l));
		try {
			var h = u(),
				T = D.S;
			if (
				(T !== null && T(r, h),
				h !== null && typeof h == "object" && typeof h.then == "function")
			) {
				var O = Im(h, a);
				fu(t, e, O, he(t));
			} else fu(t, e, a, he(t));
		} catch (N) {
			fu(t, e, { then: function () {}, status: "rejected", reason: N }, he());
		} finally {
			((B.p = n),
				i !== null && r.types !== null && (i.types = r.types),
				(D.T = i));
		}
	}
	function uy() {}
	function zc(t, e, l, a) {
		if (t.tag !== 5) throw Error(f(476));
		var u = $o(t).queue;
		Jo(
			t,
			u,
			e,
			W,
			l === null
				? uy
				: function () {
						return (Wo(t), l(a));
					},
		);
	}
	function $o(t) {
		var e = t.memoizedState;
		if (e !== null) return e;
		e = {
			memoizedState: W,
			baseState: W,
			baseQueue: null,
			queue: {
				pending: null,
				lanes: 0,
				dispatch: null,
				lastRenderedReducer: $e,
				lastRenderedState: W,
			},
			next: null,
		};
		var l = {};
		return (
			(e.next = {
				memoizedState: l,
				baseState: l,
				baseQueue: null,
				queue: {
					pending: null,
					lanes: 0,
					dispatch: null,
					lastRenderedReducer: $e,
					lastRenderedState: l,
				},
				next: null,
			}),
			(t.memoizedState = e),
			(t = t.alternate),
			t !== null && (t.memoizedState = e),
			e
		);
	}
	function Wo(t) {
		var e = $o(t);
		(e.next === null && (e = t.alternate.memoizedState),
			fu(t, e.next.queue, {}, he()));
	}
	function Ac() {
		return Zt(Ru);
	}
	function ko() {
		return Nt().memoizedState;
	}
	function Fo() {
		return Nt().memoizedState;
	}
	function ny(t) {
		for (var e = t.return; e !== null; ) {
			switch (e.tag) {
				case 24:
				case 3:
					var l = he();
					t = hl(l);
					var a = ml(e, t, l);
					(a !== null && (ae(a, e, l), au(a, e, l)),
						(e = { cache: Pi() }),
						(t.payload = e));
					return;
			}
			e = e.return;
		}
	}
	function iy(t, e, l) {
		var a = he();
		((l = {
			lane: a,
			revertLane: 0,
			gesture: null,
			action: l,
			hasEagerState: !1,
			eagerState: null,
			next: null,
		}),
			En(t)
				? Po(e, l)
				: ((l = Qi(t, e, l, a)), l !== null && (ae(l, t, a), ts(l, e, a))));
	}
	function Io(t, e, l) {
		var a = he();
		fu(t, e, l, a);
	}
	function fu(t, e, l, a) {
		var u = {
			lane: a,
			revertLane: 0,
			gesture: null,
			action: l,
			hasEagerState: !1,
			eagerState: null,
			next: null,
		};
		if (En(t)) Po(e, u);
		else {
			var n = t.alternate;
			if (
				t.lanes === 0 &&
				(n === null || n.lanes === 0) &&
				((n = e.lastRenderedReducer), n !== null)
			)
				try {
					var i = e.lastRenderedState,
						r = n(i, l);
					if (((u.hasEagerState = !0), (u.eagerState = r), ce(r, i)))
						return (tn(t, e, u, 0), Et === null && Pu(), !1);
				} catch {
				} finally {
				}
			if (((l = Qi(t, e, u, a)), l !== null))
				return (ae(l, t, a), ts(l, e, a), !0);
		}
		return !1;
	}
	function Rc(t, e, l, a) {
		if (
			((a = {
				lane: 2,
				revertLane: af(),
				gesture: null,
				action: a,
				hasEagerState: !1,
				eagerState: null,
				next: null,
			}),
			En(t))
		) {
			if (e) throw Error(f(479));
		} else ((e = Qi(t, l, a, 2)), e !== null && ae(e, t, 2));
	}
	function En(t) {
		var e = t.alternate;
		return t === tt || (e !== null && e === tt);
	}
	function Po(t, e) {
		pa = mn = !0;
		var l = t.pending;
		(l === null ? (e.next = e) : ((e.next = l.next), (l.next = e)),
			(t.pending = e));
	}
	function ts(t, e, l) {
		if ((l & 4194048) !== 0) {
			var a = e.lanes;
			((a &= t.pendingLanes), (l |= a), (e.lanes = l), ur(t, l));
		}
	}
	var ru = {
		readContext: Zt,
		use: gn,
		useCallback: Mt,
		useContext: Mt,
		useEffect: Mt,
		useImperativeHandle: Mt,
		useLayoutEffect: Mt,
		useInsertionEffect: Mt,
		useMemo: Mt,
		useReducer: Mt,
		useRef: Mt,
		useState: Mt,
		useDebugValue: Mt,
		useDeferredValue: Mt,
		useTransition: Mt,
		useSyncExternalStore: Mt,
		useId: Mt,
		useHostTransitionStatus: Mt,
		useFormState: Mt,
		useActionState: Mt,
		useOptimistic: Mt,
		useMemoCache: Mt,
		useCacheRefresh: Mt,
	};
	ru.useEffectEvent = Mt;
	var es = {
			readContext: Zt,
			use: gn,
			useCallback: function (t, e) {
				return ((kt().memoizedState = [t, e === void 0 ? null : e]), t);
			},
			useContext: Zt,
			useEffect: Yo,
			useImperativeHandle: function (t, e, l) {
				((l = l != null ? l.concat([t]) : null),
					Sn(4194308, 4, Qo.bind(null, e, t), l));
			},
			useLayoutEffect: function (t, e) {
				return Sn(4194308, 4, t, e);
			},
			useInsertionEffect: function (t, e) {
				Sn(4, 2, t, e);
			},
			useMemo: function (t, e) {
				var l = kt();
				e = e === void 0 ? null : e;
				var a = t();
				if (Kl) {
					nl(!0);
					try {
						t();
					} finally {
						nl(!1);
					}
				}
				return ((l.memoizedState = [a, e]), a);
			},
			useReducer: function (t, e, l) {
				var a = kt();
				if (l !== void 0) {
					var u = l(e);
					if (Kl) {
						nl(!0);
						try {
							l(e);
						} finally {
							nl(!1);
						}
					}
				} else u = e;
				return (
					(a.memoizedState = a.baseState = u),
					(t = {
						pending: null,
						lanes: 0,
						dispatch: null,
						lastRenderedReducer: t,
						lastRenderedState: u,
					}),
					(a.queue = t),
					(t = t.dispatch = iy.bind(null, tt, t)),
					[a.memoizedState, t]
				);
			},
			useRef: function (t) {
				var e = kt();
				return ((t = { current: t }), (e.memoizedState = t));
			},
			useState: function (t) {
				t = pc(t);
				var e = t.queue,
					l = Io.bind(null, tt, e);
				return ((e.dispatch = l), [t.memoizedState, l]);
			},
			useDebugValue: Ec,
			useDeferredValue: function (t, e) {
				var l = kt();
				return Tc(l, t, e);
			},
			useTransition: function () {
				var t = pc(!1);
				return (
					(t = Jo.bind(null, tt, t.queue, !0, !1)),
					(kt().memoizedState = t),
					[!1, t]
				);
			},
			useSyncExternalStore: function (t, e, l) {
				var a = tt,
					u = kt();
				if (ct) {
					if (l === void 0) throw Error(f(407));
					l = l();
				} else {
					if (((l = e()), Et === null)) throw Error(f(349));
					(nt & 127) !== 0 || To(a, e, l);
				}
				u.memoizedState = l;
				var n = { value: l, getSnapshot: e };
				return (
					(u.queue = n),
					Yo(Ao.bind(null, a, n, t), [t]),
					(a.flags |= 2048),
					ba(9, { destroy: void 0 }, zo.bind(null, a, n, l, e), null),
					l
				);
			},
			useId: function () {
				var t = kt(),
					e = Et.identifierPrefix;
				if (ct) {
					var l = Be,
						a = He;
					((l = (a & ~(1 << (32 - ie(a) - 1))).toString(32) + l),
						(e = "_" + e + "R_" + l),
						(l = yn++),
						0 < l && (e += "H" + l.toString(32)),
						(e += "_"));
				} else ((l = Pm++), (e = "_" + e + "r_" + l.toString(32) + "_"));
				return (t.memoizedState = e);
			},
			useHostTransitionStatus: Ac,
			useFormState: xo,
			useActionState: xo,
			useOptimistic: function (t) {
				var e = kt();
				e.memoizedState = e.baseState = t;
				var l = {
					pending: null,
					lanes: 0,
					dispatch: null,
					lastRenderedReducer: null,
					lastRenderedState: null,
				};
				return (
					(e.queue = l),
					(e = Rc.bind(null, tt, !0, l)),
					(l.dispatch = e),
					[t, e]
				);
			},
			useMemoCache: yc,
			useCacheRefresh: function () {
				return (kt().memoizedState = ny.bind(null, tt));
			},
			useEffectEvent: function (t) {
				var e = kt(),
					l = { impl: t };
				return (
					(e.memoizedState = l),
					function () {
						if ((ot & 2) !== 0) throw Error(f(440));
						return l.impl.apply(void 0, arguments);
					}
				);
			},
		},
		_c = {
			readContext: Zt,
			use: gn,
			useCallback: Vo,
			useContext: Zt,
			useEffect: bc,
			useImperativeHandle: Zo,
			useInsertionEffect: Go,
			useLayoutEffect: Xo,
			useMemo: Ko,
			useReducer: pn,
			useRef: qo,
			useState: function () {
				return pn($e);
			},
			useDebugValue: Ec,
			useDeferredValue: function (t, e) {
				var l = Nt();
				return wo(l, gt.memoizedState, t, e);
			},
			useTransition: function () {
				var t = pn($e)[0],
					e = Nt().memoizedState;
				return [typeof t == "boolean" ? t : cu(t), e];
			},
			useSyncExternalStore: Eo,
			useId: ko,
			useHostTransitionStatus: Ac,
			useFormState: Ho,
			useActionState: Ho,
			useOptimistic: function (t, e) {
				var l = Nt();
				return Mo(l, gt, t, e);
			},
			useMemoCache: yc,
			useCacheRefresh: Fo,
		};
	_c.useEffectEvent = Lo;
	var ls = {
		readContext: Zt,
		use: gn,
		useCallback: Vo,
		useContext: Zt,
		useEffect: bc,
		useImperativeHandle: Zo,
		useInsertionEffect: Go,
		useLayoutEffect: Xo,
		useMemo: Ko,
		useReducer: gc,
		useRef: qo,
		useState: function () {
			return gc($e);
		},
		useDebugValue: Ec,
		useDeferredValue: function (t, e) {
			var l = Nt();
			return gt === null ? Tc(l, t, e) : wo(l, gt.memoizedState, t, e);
		},
		useTransition: function () {
			var t = gc($e)[0],
				e = Nt().memoizedState;
			return [typeof t == "boolean" ? t : cu(t), e];
		},
		useSyncExternalStore: Eo,
		useId: ko,
		useHostTransitionStatus: Ac,
		useFormState: jo,
		useActionState: jo,
		useOptimistic: function (t, e) {
			var l = Nt();
			return gt !== null
				? Mo(l, gt, t, e)
				: ((l.baseState = t), [t, l.queue.dispatch]);
		},
		useMemoCache: yc,
		useCacheRefresh: Fo,
	};
	ls.useEffectEvent = Lo;
	function Mc(t, e, l, a) {
		((e = t.memoizedState),
			(l = l(a, e)),
			(l = l == null ? e : R({}, e, l)),
			(t.memoizedState = l),
			t.lanes === 0 && (t.updateQueue.baseState = l));
	}
	var Oc = {
		enqueueSetState: function (t, e, l) {
			t = t._reactInternals;
			var a = he(),
				u = hl(a);
			((u.payload = e),
				l != null && (u.callback = l),
				(e = ml(t, u, a)),
				e !== null && (ae(e, t, a), au(e, t, a)));
		},
		enqueueReplaceState: function (t, e, l) {
			t = t._reactInternals;
			var a = he(),
				u = hl(a);
			((u.tag = 1),
				(u.payload = e),
				l != null && (u.callback = l),
				(e = ml(t, u, a)),
				e !== null && (ae(e, t, a), au(e, t, a)));
		},
		enqueueForceUpdate: function (t, e) {
			t = t._reactInternals;
			var l = he(),
				a = hl(l);
			((a.tag = 2),
				e != null && (a.callback = e),
				(e = ml(t, a, l)),
				e !== null && (ae(e, t, l), au(e, t, l)));
		},
	};
	function as(t, e, l, a, u, n, i) {
		return (
			(t = t.stateNode),
			typeof t.shouldComponentUpdate == "function"
				? t.shouldComponentUpdate(a, n, i)
				: e.prototype && e.prototype.isPureReactComponent
					? !Wa(l, a) || !Wa(u, n)
					: !0
		);
	}
	function us(t, e, l, a) {
		((t = e.state),
			typeof e.componentWillReceiveProps == "function" &&
				e.componentWillReceiveProps(l, a),
			typeof e.UNSAFE_componentWillReceiveProps == "function" &&
				e.UNSAFE_componentWillReceiveProps(l, a),
			e.state !== t && Oc.enqueueReplaceState(e, e.state, null));
	}
	function wl(t, e) {
		var l = e;
		if ("ref" in e) {
			l = {};
			for (var a in e) a !== "ref" && (l[a] = e[a]);
		}
		if ((t = t.defaultProps)) {
			l === e && (l = R({}, l));
			for (var u in t) l[u] === void 0 && (l[u] = t[u]);
		}
		return l;
	}
	function ns(t) {
		Iu(t);
	}
	function is(t) {
		console.error(t);
	}
	function cs(t) {
		Iu(t);
	}
	function Tn(t, e) {
		try {
			var l = t.onUncaughtError;
			l(e.value, { componentStack: e.stack });
		} catch (a) {
			setTimeout(function () {
				throw a;
			});
		}
	}
	function fs(t, e, l) {
		try {
			var a = t.onCaughtError;
			a(l.value, {
				componentStack: l.stack,
				errorBoundary: e.tag === 1 ? e.stateNode : null,
			});
		} catch (u) {
			setTimeout(function () {
				throw u;
			});
		}
	}
	function Dc(t, e, l) {
		return (
			(l = hl(l)),
			(l.tag = 3),
			(l.payload = { element: null }),
			(l.callback = function () {
				Tn(t, e);
			}),
			l
		);
	}
	function rs(t) {
		return ((t = hl(t)), (t.tag = 3), t);
	}
	function os(t, e, l, a) {
		var u = l.type.getDerivedStateFromError;
		if (typeof u == "function") {
			var n = a.value;
			((t.payload = function () {
				return u(n);
			}),
				(t.callback = function () {
					fs(e, l, a);
				}));
		}
		var i = l.stateNode;
		i !== null &&
			typeof i.componentDidCatch == "function" &&
			(t.callback = function () {
				(fs(e, l, a),
					typeof u != "function" &&
						(bl === null ? (bl = new Set([this])) : bl.add(this)));
				var r = a.stack;
				this.componentDidCatch(a.value, {
					componentStack: r !== null ? r : "",
				});
			});
	}
	function cy(t, e, l, a, u) {
		if (
			((l.flags |= 32768),
			a !== null && typeof a == "object" && typeof a.then == "function")
		) {
			if (
				((e = l.alternate),
				e !== null && da(e, l, u, !0),
				(l = re.current),
				l !== null)
			) {
				switch (l.tag) {
					case 31:
					case 13:
						return (
							Ae === null ? Hn() : l.alternate === null && Ot === 0 && (Ot = 3),
							(l.flags &= -257),
							(l.flags |= 65536),
							(l.lanes = u),
							a === rn
								? (l.flags |= 16384)
								: ((e = l.updateQueue),
									e === null ? (l.updateQueue = new Set([a])) : e.add(a),
									tf(t, a, u)),
							!1
						);
					case 22:
						return (
							(l.flags |= 65536),
							a === rn
								? (l.flags |= 16384)
								: ((e = l.updateQueue),
									e === null
										? ((e = {
												transitions: null,
												markerInstances: null,
												retryQueue: new Set([a]),
											}),
											(l.updateQueue = e))
										: ((l = e.retryQueue),
											l === null ? (e.retryQueue = new Set([a])) : l.add(a)),
									tf(t, a, u)),
							!1
						);
				}
				throw Error(f(435, l.tag));
			}
			return (tf(t, a, u), Hn(), !1);
		}
		if (ct)
			return (
				(e = re.current),
				e !== null
					? ((e.flags & 65536) === 0 && (e.flags |= 256),
						(e.flags |= 65536),
						(e.lanes = u),
						a !== $i && ((t = Error(f(422), { cause: a })), Ia(be(t, l))))
					: (a !== $i && ((e = Error(f(423), { cause: a })), Ia(be(e, l))),
						(t = t.current.alternate),
						(t.flags |= 65536),
						(u &= -u),
						(t.lanes |= u),
						(a = be(a, l)),
						(u = Dc(t.stateNode, a, u)),
						nc(t, u),
						Ot !== 4 && (Ot = 2)),
				!1
			);
		var n = Error(f(520), { cause: a });
		if (
			((n = be(n, l)),
			gu === null ? (gu = [n]) : gu.push(n),
			Ot !== 4 && (Ot = 2),
			e === null)
		)
			return !0;
		((a = be(a, l)), (l = e));
		do {
			switch (l.tag) {
				case 3:
					return (
						(l.flags |= 65536),
						(t = u & -u),
						(l.lanes |= t),
						(t = Dc(l.stateNode, a, t)),
						nc(l, t),
						!1
					);
				case 1:
					if (
						((e = l.type),
						(n = l.stateNode),
						(l.flags & 128) === 0 &&
							(typeof e.getDerivedStateFromError == "function" ||
								(n !== null &&
									typeof n.componentDidCatch == "function" &&
									(bl === null || !bl.has(n)))))
					)
						return (
							(l.flags |= 65536),
							(u &= -u),
							(l.lanes |= u),
							(u = rs(u)),
							os(u, t, l, a),
							nc(l, u),
							!1
						);
			}
			l = l.return;
		} while (l !== null);
		return !1;
	}
	var Cc = Error(f(461)),
		jt = !1;
	function Vt(t, e, l, a) {
		e.child = t === null ? mo(e, null, l, a) : Vl(e, t.child, l, a);
	}
	function ss(t, e, l, a, u) {
		l = l.render;
		var n = e.ref;
		if ("ref" in a) {
			var i = {};
			for (var r in a) r !== "ref" && (i[r] = a[r]);
		} else i = a;
		return (
			Gl(e),
			(a = sc(t, e, l, i, n, u)),
			(r = dc()),
			t !== null && !jt
				? (hc(t, e, u), We(t, e, u))
				: (ct && r && wi(e), (e.flags |= 1), Vt(t, e, a, u), e.child)
		);
	}
	function ds(t, e, l, a, u) {
		if (t === null) {
			var n = l.type;
			return typeof n == "function" &&
				!Zi(n) &&
				n.defaultProps === void 0 &&
				l.compare === null
				? ((e.tag = 15), (e.type = n), hs(t, e, n, a, u))
				: ((t = ln(l.type, null, a, e, e.mode, u)),
					(t.ref = e.ref),
					(t.return = e),
					(e.child = t));
		}
		if (((n = t.child), !Yc(t, u))) {
			var i = n.memoizedProps;
			if (
				((l = l.compare), (l = l !== null ? l : Wa), l(i, a) && t.ref === e.ref)
			)
				return We(t, e, u);
		}
		return (
			(e.flags |= 1),
			(t = Ze(n, a)),
			(t.ref = e.ref),
			(t.return = e),
			(e.child = t)
		);
	}
	function hs(t, e, l, a, u) {
		if (t !== null) {
			var n = t.memoizedProps;
			if (Wa(n, a) && t.ref === e.ref)
				if (((jt = !1), (e.pendingProps = a = n), Yc(t, u)))
					(t.flags & 131072) !== 0 && (jt = !0);
				else return ((e.lanes = t.lanes), We(t, e, u));
		}
		return Uc(t, e, l, a, u);
	}
	function ms(t, e, l, a) {
		var u = a.children,
			n = t !== null ? t.memoizedState : null;
		if (
			(t === null &&
				e.stateNode === null &&
				(e.stateNode = {
					_visibility: 1,
					_pendingMarkers: null,
					_retryCache: null,
					_transitions: null,
				}),
			a.mode === "hidden")
		) {
			if ((e.flags & 128) !== 0) {
				if (((n = n !== null ? n.baseLanes | l : l), t !== null)) {
					for (a = e.child = t.child, u = 0; a !== null; )
						((u = u | a.lanes | a.childLanes), (a = a.sibling));
					a = u & ~n;
				} else ((a = 0), (e.child = null));
				return ys(t, e, n, l, a);
			}
			if ((l & 536870912) !== 0)
				((e.memoizedState = { baseLanes: 0, cachePool: null }),
					t !== null && cn(e, n !== null ? n.cachePool : null),
					n !== null ? go(e, n) : cc(),
					po(e));
			else
				return (
					(a = e.lanes = 536870912),
					ys(t, e, n !== null ? n.baseLanes | l : l, l, a)
				);
		} else
			n !== null
				? (cn(e, n.cachePool), go(e, n), vl(), (e.memoizedState = null))
				: (t !== null && cn(e, null), cc(), vl());
		return (Vt(t, e, u, l), e.child);
	}
	function ou(t, e) {
		return (
			(t !== null && t.tag === 22) ||
				e.stateNode !== null ||
				(e.stateNode = {
					_visibility: 1,
					_pendingMarkers: null,
					_retryCache: null,
					_transitions: null,
				}),
			e.sibling
		);
	}
	function ys(t, e, l, a, u) {
		var n = ec();
		return (
			(n = n === null ? null : { parent: Ht._currentValue, pool: n }),
			(e.memoizedState = { baseLanes: l, cachePool: n }),
			t !== null && cn(e, null),
			cc(),
			po(e),
			t !== null && da(t, e, a, !0),
			(e.childLanes = u),
			null
		);
	}
	function zn(t, e) {
		return (
			(e = Rn({ mode: e.mode, children: e.children }, t.mode)),
			(e.ref = t.ref),
			(t.child = e),
			(e.return = t),
			e
		);
	}
	function vs(t, e, l) {
		return (
			Vl(e, t.child, null, l),
			(t = zn(e, e.pendingProps)),
			(t.flags |= 2),
			oe(e),
			(e.memoizedState = null),
			t
		);
	}
	function fy(t, e, l) {
		var a = e.pendingProps,
			u = (e.flags & 128) !== 0;
		if (((e.flags &= -129), t === null)) {
			if (ct) {
				if (a.mode === "hidden")
					return ((t = zn(e, a)), (e.lanes = 536870912), ou(null, t));
				if (
					(rc(e),
					(t = zt)
						? ((t = Od(t, ze)),
							(t = t !== null && t.data === "&" ? t : null),
							t !== null &&
								((e.memoizedState = {
									dehydrated: t,
									treeContext: fl !== null ? { id: He, overflow: Be } : null,
									retryLane: 536870912,
									hydrationErrors: null,
								}),
								(l = Ir(t)),
								(l.return = e),
								(e.child = l),
								(Qt = e),
								(zt = null)))
						: (t = null),
					t === null)
				)
					throw ol(e);
				return ((e.lanes = 536870912), null);
			}
			return zn(e, a);
		}
		var n = t.memoizedState;
		if (n !== null) {
			var i = n.dehydrated;
			if ((rc(e), u))
				if (e.flags & 256) ((e.flags &= -257), (e = vs(t, e, l)));
				else if (e.memoizedState !== null)
					((e.child = t.child), (e.flags |= 128), (e = null));
				else throw Error(f(558));
			else if (
				(jt || da(t, e, l, !1), (u = (l & t.childLanes) !== 0), jt || u)
			) {
				if (
					((a = Et),
					a !== null && ((i = nr(a, l)), i !== 0 && i !== n.retryLane))
				)
					throw ((n.retryLane = i), jl(t, i), ae(a, t, i), Cc);
				(Hn(), (e = vs(t, e, l)));
			} else
				((t = n.treeContext),
					(zt = Re(i.nextSibling)),
					(Qt = e),
					(ct = !0),
					(rl = null),
					(ze = !1),
					t !== null && eo(e, t),
					(e = zn(e, a)),
					(e.flags |= 4096));
			return e;
		}
		return (
			(t = Ze(t.child, { mode: a.mode, children: a.children })),
			(t.ref = e.ref),
			(e.child = t),
			(t.return = e),
			t
		);
	}
	function An(t, e) {
		var l = e.ref;
		if (l === null) t !== null && t.ref !== null && (e.flags |= 4194816);
		else {
			if (typeof l != "function" && typeof l != "object") throw Error(f(284));
			(t === null || t.ref !== l) && (e.flags |= 4194816);
		}
	}
	function Uc(t, e, l, a, u) {
		return (
			Gl(e),
			(l = sc(t, e, l, a, void 0, u)),
			(a = dc()),
			t !== null && !jt
				? (hc(t, e, u), We(t, e, u))
				: (ct && a && wi(e), (e.flags |= 1), Vt(t, e, l, u), e.child)
		);
	}
	function gs(t, e, l, a, u, n) {
		return (
			Gl(e),
			(e.updateQueue = null),
			(l = bo(e, a, l, u)),
			So(t),
			(a = dc()),
			t !== null && !jt
				? (hc(t, e, n), We(t, e, n))
				: (ct && a && wi(e), (e.flags |= 1), Vt(t, e, l, n), e.child)
		);
	}
	function ps(t, e, l, a, u) {
		if ((Gl(e), e.stateNode === null)) {
			var n = fa,
				i = l.contextType;
			(typeof i == "object" && i !== null && (n = Zt(i)),
				(n = new l(a, n)),
				(e.memoizedState =
					n.state !== null && n.state !== void 0 ? n.state : null),
				(n.updater = Oc),
				(e.stateNode = n),
				(n._reactInternals = e),
				(n = e.stateNode),
				(n.props = a),
				(n.state = e.memoizedState),
				(n.refs = {}),
				ac(e),
				(i = l.contextType),
				(n.context = typeof i == "object" && i !== null ? Zt(i) : fa),
				(n.state = e.memoizedState),
				(i = l.getDerivedStateFromProps),
				typeof i == "function" && (Mc(e, l, i, a), (n.state = e.memoizedState)),
				typeof l.getDerivedStateFromProps == "function" ||
					typeof n.getSnapshotBeforeUpdate == "function" ||
					(typeof n.UNSAFE_componentWillMount != "function" &&
						typeof n.componentWillMount != "function") ||
					((i = n.state),
					typeof n.componentWillMount == "function" && n.componentWillMount(),
					typeof n.UNSAFE_componentWillMount == "function" &&
						n.UNSAFE_componentWillMount(),
					i !== n.state && Oc.enqueueReplaceState(n, n.state, null),
					nu(e, a, n, u),
					uu(),
					(n.state = e.memoizedState)),
				typeof n.componentDidMount == "function" && (e.flags |= 4194308),
				(a = !0));
		} else if (t === null) {
			n = e.stateNode;
			var r = e.memoizedProps,
				h = wl(l, r);
			n.props = h;
			var T = n.context,
				O = l.contextType;
			((i = fa), typeof O == "object" && O !== null && (i = Zt(O)));
			var N = l.getDerivedStateFromProps;
			((O =
				typeof N == "function" ||
				typeof n.getSnapshotBeforeUpdate == "function"),
				(r = e.pendingProps !== r),
				O ||
					(typeof n.UNSAFE_componentWillReceiveProps != "function" &&
						typeof n.componentWillReceiveProps != "function") ||
					((r || T !== i) && us(e, n, a, i)),
				(dl = !1));
			var z = e.memoizedState;
			((n.state = z),
				nu(e, a, n, u),
				uu(),
				(T = e.memoizedState),
				r || z !== T || dl
					? (typeof N == "function" && (Mc(e, l, N, a), (T = e.memoizedState)),
						(h = dl || as(e, l, h, a, z, T, i))
							? (O ||
									(typeof n.UNSAFE_componentWillMount != "function" &&
										typeof n.componentWillMount != "function") ||
									(typeof n.componentWillMount == "function" &&
										n.componentWillMount(),
									typeof n.UNSAFE_componentWillMount == "function" &&
										n.UNSAFE_componentWillMount()),
								typeof n.componentDidMount == "function" &&
									(e.flags |= 4194308))
							: (typeof n.componentDidMount == "function" &&
									(e.flags |= 4194308),
								(e.memoizedProps = a),
								(e.memoizedState = T)),
						(n.props = a),
						(n.state = T),
						(n.context = i),
						(a = h))
					: (typeof n.componentDidMount == "function" && (e.flags |= 4194308),
						(a = !1)));
		} else {
			((n = e.stateNode),
				uc(t, e),
				(i = e.memoizedProps),
				(O = wl(l, i)),
				(n.props = O),
				(N = e.pendingProps),
				(z = n.context),
				(T = l.contextType),
				(h = fa),
				typeof T == "object" && T !== null && (h = Zt(T)),
				(r = l.getDerivedStateFromProps),
				(T =
					typeof r == "function" ||
					typeof n.getSnapshotBeforeUpdate == "function") ||
					(typeof n.UNSAFE_componentWillReceiveProps != "function" &&
						typeof n.componentWillReceiveProps != "function") ||
					((i !== N || z !== h) && us(e, n, a, h)),
				(dl = !1),
				(z = e.memoizedState),
				(n.state = z),
				nu(e, a, n, u),
				uu());
			var _ = e.memoizedState;
			i !== N ||
			z !== _ ||
			dl ||
			(t !== null && t.dependencies !== null && un(t.dependencies))
				? (typeof r == "function" && (Mc(e, l, r, a), (_ = e.memoizedState)),
					(O =
						dl ||
						as(e, l, O, a, z, _, h) ||
						(t !== null && t.dependencies !== null && un(t.dependencies)))
						? (T ||
								(typeof n.UNSAFE_componentWillUpdate != "function" &&
									typeof n.componentWillUpdate != "function") ||
								(typeof n.componentWillUpdate == "function" &&
									n.componentWillUpdate(a, _, h),
								typeof n.UNSAFE_componentWillUpdate == "function" &&
									n.UNSAFE_componentWillUpdate(a, _, h)),
							typeof n.componentDidUpdate == "function" && (e.flags |= 4),
							typeof n.getSnapshotBeforeUpdate == "function" &&
								(e.flags |= 1024))
						: (typeof n.componentDidUpdate != "function" ||
								(i === t.memoizedProps && z === t.memoizedState) ||
								(e.flags |= 4),
							typeof n.getSnapshotBeforeUpdate != "function" ||
								(i === t.memoizedProps && z === t.memoizedState) ||
								(e.flags |= 1024),
							(e.memoizedProps = a),
							(e.memoizedState = _)),
					(n.props = a),
					(n.state = _),
					(n.context = h),
					(a = O))
				: (typeof n.componentDidUpdate != "function" ||
						(i === t.memoizedProps && z === t.memoizedState) ||
						(e.flags |= 4),
					typeof n.getSnapshotBeforeUpdate != "function" ||
						(i === t.memoizedProps && z === t.memoizedState) ||
						(e.flags |= 1024),
					(a = !1));
		}
		return (
			(n = a),
			An(t, e),
			(a = (e.flags & 128) !== 0),
			n || a
				? ((n = e.stateNode),
					(l =
						a && typeof l.getDerivedStateFromError != "function"
							? null
							: n.render()),
					(e.flags |= 1),
					t !== null && a
						? ((e.child = Vl(e, t.child, null, u)),
							(e.child = Vl(e, null, l, u)))
						: Vt(t, e, l, u),
					(e.memoizedState = n.state),
					(t = e.child))
				: (t = We(t, e, u)),
			t
		);
	}
	function Ss(t, e, l, a) {
		return (Yl(), (e.flags |= 256), Vt(t, e, l, a), e.child);
	}
	var Nc = {
		dehydrated: null,
		treeContext: null,
		retryLane: 0,
		hydrationErrors: null,
	};
	function xc(t) {
		return { baseLanes: t, cachePool: co() };
	}
	function Hc(t, e, l) {
		return ((t = t !== null ? t.childLanes & ~l : 0), e && (t |= de), t);
	}
	function bs(t, e, l) {
		var a = e.pendingProps,
			u = !1,
			n = (e.flags & 128) !== 0,
			i;
		if (
			((i = n) ||
				(i =
					t !== null && t.memoizedState === null ? !1 : (Ut.current & 2) !== 0),
			i && ((u = !0), (e.flags &= -129)),
			(i = (e.flags & 32) !== 0),
			(e.flags &= -33),
			t === null)
		) {
			if (ct) {
				if (
					(u ? yl(e) : vl(),
					(t = zt)
						? ((t = Od(t, ze)),
							(t = t !== null && t.data !== "&" ? t : null),
							t !== null &&
								((e.memoizedState = {
									dehydrated: t,
									treeContext: fl !== null ? { id: He, overflow: Be } : null,
									retryLane: 536870912,
									hydrationErrors: null,
								}),
								(l = Ir(t)),
								(l.return = e),
								(e.child = l),
								(Qt = e),
								(zt = null)))
						: (t = null),
					t === null)
				)
					throw ol(e);
				return (gf(t) ? (e.lanes = 32) : (e.lanes = 536870912), null);
			}
			var r = a.children;
			return (
				(a = a.fallback),
				u
					? (vl(),
						(u = e.mode),
						(r = Rn({ mode: "hidden", children: r }, u)),
						(a = ql(a, u, l, null)),
						(r.return = e),
						(a.return = e),
						(r.sibling = a),
						(e.child = r),
						(a = e.child),
						(a.memoizedState = xc(l)),
						(a.childLanes = Hc(t, i, l)),
						(e.memoizedState = Nc),
						ou(null, a))
					: (yl(e), Bc(e, r))
			);
		}
		var h = t.memoizedState;
		if (h !== null && ((r = h.dehydrated), r !== null)) {
			if (n)
				e.flags & 256
					? (yl(e), (e.flags &= -257), (e = jc(t, e, l)))
					: e.memoizedState !== null
						? (vl(), (e.child = t.child), (e.flags |= 128), (e = null))
						: (vl(),
							(r = a.fallback),
							(u = e.mode),
							(a = Rn({ mode: "visible", children: a.children }, u)),
							(r = ql(r, u, l, null)),
							(r.flags |= 2),
							(a.return = e),
							(r.return = e),
							(a.sibling = r),
							(e.child = a),
							Vl(e, t.child, null, l),
							(a = e.child),
							(a.memoizedState = xc(l)),
							(a.childLanes = Hc(t, i, l)),
							(e.memoizedState = Nc),
							(e = ou(null, a)));
			else if ((yl(e), gf(r))) {
				if (((i = r.nextSibling && r.nextSibling.dataset), i)) var T = i.dgst;
				((i = T),
					(a = Error(f(419))),
					(a.stack = ""),
					(a.digest = i),
					Ia({ value: a, source: null, stack: null }),
					(e = jc(t, e, l)));
			} else if (
				(jt || da(t, e, l, !1), (i = (l & t.childLanes) !== 0), jt || i)
			) {
				if (
					((i = Et),
					i !== null && ((a = nr(i, l)), a !== 0 && a !== h.retryLane))
				)
					throw ((h.retryLane = a), jl(t, a), ae(i, t, a), Cc);
				(vf(r) || Hn(), (e = jc(t, e, l)));
			} else
				vf(r)
					? ((e.flags |= 192), (e.child = t.child), (e = null))
					: ((t = h.treeContext),
						(zt = Re(r.nextSibling)),
						(Qt = e),
						(ct = !0),
						(rl = null),
						(ze = !1),
						t !== null && eo(e, t),
						(e = Bc(e, a.children)),
						(e.flags |= 4096));
			return e;
		}
		return u
			? (vl(),
				(r = a.fallback),
				(u = e.mode),
				(h = t.child),
				(T = h.sibling),
				(a = Ze(h, { mode: "hidden", children: a.children })),
				(a.subtreeFlags = h.subtreeFlags & 65011712),
				T !== null ? (r = Ze(T, r)) : ((r = ql(r, u, l, null)), (r.flags |= 2)),
				(r.return = e),
				(a.return = e),
				(a.sibling = r),
				(e.child = a),
				ou(null, a),
				(a = e.child),
				(r = t.child.memoizedState),
				r === null
					? (r = xc(l))
					: ((u = r.cachePool),
						u !== null
							? ((h = Ht._currentValue),
								(u = u.parent !== h ? { parent: h, pool: h } : u))
							: (u = co()),
						(r = { baseLanes: r.baseLanes | l, cachePool: u })),
				(a.memoizedState = r),
				(a.childLanes = Hc(t, i, l)),
				(e.memoizedState = Nc),
				ou(t.child, a))
			: (yl(e),
				(l = t.child),
				(t = l.sibling),
				(l = Ze(l, { mode: "visible", children: a.children })),
				(l.return = e),
				(l.sibling = null),
				t !== null &&
					((i = e.deletions),
					i === null ? ((e.deletions = [t]), (e.flags |= 16)) : i.push(t)),
				(e.child = l),
				(e.memoizedState = null),
				l);
	}
	function Bc(t, e) {
		return (
			(e = Rn({ mode: "visible", children: e }, t.mode)),
			(e.return = t),
			(t.child = e)
		);
	}
	function Rn(t, e) {
		return ((t = fe(22, t, null, e)), (t.lanes = 0), t);
	}
	function jc(t, e, l) {
		return (
			Vl(e, t.child, null, l),
			(t = Bc(e, e.pendingProps.children)),
			(t.flags |= 2),
			(e.memoizedState = null),
			t
		);
	}
	function Es(t, e, l) {
		t.lanes |= e;
		var a = t.alternate;
		(a !== null && (a.lanes |= e), Fi(t.return, e, l));
	}
	function qc(t, e, l, a, u, n) {
		var i = t.memoizedState;
		i === null
			? (t.memoizedState = {
					isBackwards: e,
					rendering: null,
					renderingStartTime: 0,
					last: a,
					tail: l,
					tailMode: u,
					treeForkCount: n,
				})
			: ((i.isBackwards = e),
				(i.rendering = null),
				(i.renderingStartTime = 0),
				(i.last = a),
				(i.tail = l),
				(i.tailMode = u),
				(i.treeForkCount = n));
	}
	function Ts(t, e, l) {
		var a = e.pendingProps,
			u = a.revealOrder,
			n = a.tail;
		a = a.children;
		var i = Ut.current,
			r = (i & 2) !== 0;
		if (
			(r ? ((i = (i & 1) | 2), (e.flags |= 128)) : (i &= 1),
			q(Ut, i),
			Vt(t, e, a, l),
			(a = ct ? Fa : 0),
			!r && t !== null && (t.flags & 128) !== 0)
		)
			t: for (t = e.child; t !== null; ) {
				if (t.tag === 13) t.memoizedState !== null && Es(t, l, e);
				else if (t.tag === 19) Es(t, l, e);
				else if (t.child !== null) {
					((t.child.return = t), (t = t.child));
					continue;
				}
				if (t === e) break t;
				for (; t.sibling === null; ) {
					if (t.return === null || t.return === e) break t;
					t = t.return;
				}
				((t.sibling.return = t.return), (t = t.sibling));
			}
		switch (u) {
			case "forwards":
				for (l = e.child, u = null; l !== null; )
					((t = l.alternate),
						t !== null && hn(t) === null && (u = l),
						(l = l.sibling));
				((l = u),
					l === null
						? ((u = e.child), (e.child = null))
						: ((u = l.sibling), (l.sibling = null)),
					qc(e, !1, u, l, n, a));
				break;
			case "backwards":
			case "unstable_legacy-backwards":
				for (l = null, u = e.child, e.child = null; u !== null; ) {
					if (((t = u.alternate), t !== null && hn(t) === null)) {
						e.child = u;
						break;
					}
					((t = u.sibling), (u.sibling = l), (l = u), (u = t));
				}
				qc(e, !0, l, null, n, a);
				break;
			case "together":
				qc(e, !1, null, null, void 0, a);
				break;
			default:
				e.memoizedState = null;
		}
		return e.child;
	}
	function We(t, e, l) {
		if (
			(t !== null && (e.dependencies = t.dependencies),
			(Sl |= e.lanes),
			(l & e.childLanes) === 0)
		)
			if (t !== null) {
				if ((da(t, e, l, !1), (l & e.childLanes) === 0)) return null;
			} else return null;
		if (t !== null && e.child !== t.child) throw Error(f(153));
		if (e.child !== null) {
			for (
				t = e.child, l = Ze(t, t.pendingProps), e.child = l, l.return = e;
				t.sibling !== null;
			)
				((t = t.sibling),
					(l = l.sibling = Ze(t, t.pendingProps)),
					(l.return = e));
			l.sibling = null;
		}
		return e.child;
	}
	function Yc(t, e) {
		return (t.lanes & e) !== 0
			? !0
			: ((t = t.dependencies), !!(t !== null && un(t)));
	}
	function ry(t, e, l) {
		switch (e.tag) {
			case 3:
				(Wt(e, e.stateNode.containerInfo),
					sl(e, Ht, t.memoizedState.cache),
					Yl());
				break;
			case 27:
			case 5:
				ja(e);
				break;
			case 4:
				Wt(e, e.stateNode.containerInfo);
				break;
			case 10:
				sl(e, e.type, e.memoizedProps.value);
				break;
			case 31:
				if (e.memoizedState !== null) return ((e.flags |= 128), rc(e), null);
				break;
			case 13:
				var a = e.memoizedState;
				if (a !== null)
					return a.dehydrated !== null
						? (yl(e), (e.flags |= 128), null)
						: (l & e.child.childLanes) !== 0
							? bs(t, e, l)
							: (yl(e), (t = We(t, e, l)), t !== null ? t.sibling : null);
				yl(e);
				break;
			case 19:
				var u = (t.flags & 128) !== 0;
				if (
					((a = (l & e.childLanes) !== 0),
					a || (da(t, e, l, !1), (a = (l & e.childLanes) !== 0)),
					u)
				) {
					if (a) return Ts(t, e, l);
					e.flags |= 128;
				}
				if (
					((u = e.memoizedState),
					u !== null &&
						((u.rendering = null), (u.tail = null), (u.lastEffect = null)),
					q(Ut, Ut.current),
					a)
				)
					break;
				return null;
			case 22:
				return ((e.lanes = 0), ms(t, e, l, e.pendingProps));
			case 24:
				sl(e, Ht, t.memoizedState.cache);
		}
		return We(t, e, l);
	}
	function zs(t, e, l) {
		if (t !== null)
			if (t.memoizedProps !== e.pendingProps) jt = !0;
			else {
				if (!Yc(t, l) && (e.flags & 128) === 0) return ((jt = !1), ry(t, e, l));
				jt = (t.flags & 131072) !== 0;
			}
		else ((jt = !1), ct && (e.flags & 1048576) !== 0 && to(e, Fa, e.index));
		switch (((e.lanes = 0), e.tag)) {
			case 16:
				t: {
					var a = e.pendingProps;
					if (((t = Ql(e.elementType)), (e.type = t), typeof t == "function"))
						Zi(t)
							? ((a = wl(t, a)), (e.tag = 1), (e = ps(null, e, t, a, l)))
							: ((e.tag = 0), (e = Uc(null, e, t, a, l)));
					else {
						if (t != null) {
							var u = t.$$typeof;
							if (u === yt) {
								((e.tag = 11), (e = ss(null, e, t, a, l)));
								break t;
							} else if (u === k) {
								((e.tag = 14), (e = ds(null, e, t, a, l)));
								break t;
							}
						}
						throw ((e = ve(t) || t), Error(f(306, e, "")));
					}
				}
				return e;
			case 0:
				return Uc(t, e, e.type, e.pendingProps, l);
			case 1:
				return ((a = e.type), (u = wl(a, e.pendingProps)), ps(t, e, a, u, l));
			case 3:
				t: {
					if ((Wt(e, e.stateNode.containerInfo), t === null))
						throw Error(f(387));
					a = e.pendingProps;
					var n = e.memoizedState;
					((u = n.element), uc(t, e), nu(e, a, null, l));
					var i = e.memoizedState;
					if (
						((a = i.cache),
						sl(e, Ht, a),
						a !== n.cache && Ii(e, [Ht], l, !0),
						uu(),
						(a = i.element),
						n.isDehydrated)
					)
						if (
							((n = { element: a, isDehydrated: !1, cache: i.cache }),
							(e.updateQueue.baseState = n),
							(e.memoizedState = n),
							e.flags & 256)
						) {
							e = Ss(t, e, a, l);
							break t;
						} else if (a !== u) {
							((u = be(Error(f(424)), e)), Ia(u), (e = Ss(t, e, a, l)));
							break t;
						} else {
							switch (((t = e.stateNode.containerInfo), t.nodeType)) {
								case 9:
									t = t.body;
									break;
								default:
									t = t.nodeName === "HTML" ? t.ownerDocument.body : t;
							}
							for (
								zt = Re(t.firstChild),
									Qt = e,
									ct = !0,
									rl = null,
									ze = !0,
									l = mo(e, null, a, l),
									e.child = l;
								l;
							)
								((l.flags = (l.flags & -3) | 4096), (l = l.sibling));
						}
					else {
						if ((Yl(), a === u)) {
							e = We(t, e, l);
							break t;
						}
						Vt(t, e, a, l);
					}
					e = e.child;
				}
				return e;
			case 26:
				return (
					An(t, e),
					t === null
						? (l = Hd(e.type, null, e.pendingProps, null))
							? (e.memoizedState = l)
							: ct ||
								((l = e.type),
								(t = e.pendingProps),
								(a = Xn(lt.current).createElement(l)),
								(a[Xt] = e),
								(a[Ft] = t),
								Kt(a, l, t),
								Lt(a),
								(e.stateNode = a))
						: (e.memoizedState = Hd(
								e.type,
								t.memoizedProps,
								e.pendingProps,
								t.memoizedState,
							)),
					null
				);
			case 27:
				return (
					ja(e),
					t === null &&
						ct &&
						((a = e.stateNode = Ud(e.type, e.pendingProps, lt.current)),
						(Qt = e),
						(ze = !0),
						(u = zt),
						Al(e.type) ? ((pf = u), (zt = Re(a.firstChild))) : (zt = u)),
					Vt(t, e, e.pendingProps.children, l),
					An(t, e),
					t === null && (e.flags |= 4194304),
					e.child
				);
			case 5:
				return (
					t === null &&
						ct &&
						((u = a = zt) &&
							((a = Ly(a, e.type, e.pendingProps, ze)),
							a !== null
								? ((e.stateNode = a),
									(Qt = e),
									(zt = Re(a.firstChild)),
									(ze = !1),
									(u = !0))
								: (u = !1)),
						u || ol(e)),
					ja(e),
					(u = e.type),
					(n = e.pendingProps),
					(i = t !== null ? t.memoizedProps : null),
					(a = n.children),
					hf(u, n) ? (a = null) : i !== null && hf(u, i) && (e.flags |= 32),
					e.memoizedState !== null &&
						((u = sc(t, e, ty, null, null, l)), (Ru._currentValue = u)),
					An(t, e),
					Vt(t, e, a, l),
					e.child
				);
			case 6:
				return (
					t === null &&
						ct &&
						((t = l = zt) &&
							((l = Gy(l, e.pendingProps, ze)),
							l !== null
								? ((e.stateNode = l), (Qt = e), (zt = null), (t = !0))
								: (t = !1)),
						t || ol(e)),
					null
				);
			case 13:
				return bs(t, e, l);
			case 4:
				return (
					Wt(e, e.stateNode.containerInfo),
					(a = e.pendingProps),
					t === null ? (e.child = Vl(e, null, a, l)) : Vt(t, e, a, l),
					e.child
				);
			case 11:
				return ss(t, e, e.type, e.pendingProps, l);
			case 7:
				return (Vt(t, e, e.pendingProps, l), e.child);
			case 8:
				return (Vt(t, e, e.pendingProps.children, l), e.child);
			case 12:
				return (Vt(t, e, e.pendingProps.children, l), e.child);
			case 10:
				return (
					(a = e.pendingProps),
					sl(e, e.type, a.value),
					Vt(t, e, a.children, l),
					e.child
				);
			case 9:
				return (
					(u = e.type._context),
					(a = e.pendingProps.children),
					Gl(e),
					(u = Zt(u)),
					(a = a(u)),
					(e.flags |= 1),
					Vt(t, e, a, l),
					e.child
				);
			case 14:
				return ds(t, e, e.type, e.pendingProps, l);
			case 15:
				return hs(t, e, e.type, e.pendingProps, l);
			case 19:
				return Ts(t, e, l);
			case 31:
				return fy(t, e, l);
			case 22:
				return ms(t, e, l, e.pendingProps);
			case 24:
				return (
					Gl(e),
					(a = Zt(Ht)),
					t === null
						? ((u = ec()),
							u === null &&
								((u = Et),
								(n = Pi()),
								(u.pooledCache = n),
								n.refCount++,
								n !== null && (u.pooledCacheLanes |= l),
								(u = n)),
							(e.memoizedState = { parent: a, cache: u }),
							ac(e),
							sl(e, Ht, u))
						: ((t.lanes & l) !== 0 && (uc(t, e), nu(e, null, null, l), uu()),
							(u = t.memoizedState),
							(n = e.memoizedState),
							u.parent !== a
								? ((u = { parent: a, cache: a }),
									(e.memoizedState = u),
									e.lanes === 0 &&
										(e.memoizedState = e.updateQueue.baseState = u),
									sl(e, Ht, a))
								: ((a = n.cache),
									sl(e, Ht, a),
									a !== u.cache && Ii(e, [Ht], l, !0))),
					Vt(t, e, e.pendingProps.children, l),
					e.child
				);
			case 29:
				throw e.pendingProps;
		}
		throw Error(f(156, e.tag));
	}
	function ke(t) {
		t.flags |= 4;
	}
	function Lc(t, e, l, a, u) {
		if (((e = (t.mode & 32) !== 0) && (e = !1), e)) {
			if (((t.flags |= 16777216), (u & 335544128) === u))
				if (t.stateNode.complete) t.flags |= 8192;
				else if (ks()) t.flags |= 8192;
				else throw ((Zl = rn), lc);
		} else t.flags &= -16777217;
	}
	function As(t, e) {
		if (e.type !== "stylesheet" || (e.state.loading & 4) !== 0)
			t.flags &= -16777217;
		else if (((t.flags |= 16777216), !Ld(e)))
			if (ks()) t.flags |= 8192;
			else throw ((Zl = rn), lc);
	}
	function _n(t, e) {
		(e !== null && (t.flags |= 4),
			t.flags & 16384 &&
				((e = t.tag !== 22 ? lr() : 536870912), (t.lanes |= e), (Aa |= e)));
	}
	function su(t, e) {
		if (!ct)
			switch (t.tailMode) {
				case "hidden":
					e = t.tail;
					for (var l = null; e !== null; )
						(e.alternate !== null && (l = e), (e = e.sibling));
					l === null ? (t.tail = null) : (l.sibling = null);
					break;
				case "collapsed":
					l = t.tail;
					for (var a = null; l !== null; )
						(l.alternate !== null && (a = l), (l = l.sibling));
					a === null
						? e || t.tail === null
							? (t.tail = null)
							: (t.tail.sibling = null)
						: (a.sibling = null);
			}
	}
	function At(t) {
		var e = t.alternate !== null && t.alternate.child === t.child,
			l = 0,
			a = 0;
		if (e)
			for (var u = t.child; u !== null; )
				((l |= u.lanes | u.childLanes),
					(a |= u.subtreeFlags & 65011712),
					(a |= u.flags & 65011712),
					(u.return = t),
					(u = u.sibling));
		else
			for (u = t.child; u !== null; )
				((l |= u.lanes | u.childLanes),
					(a |= u.subtreeFlags),
					(a |= u.flags),
					(u.return = t),
					(u = u.sibling));
		return ((t.subtreeFlags |= a), (t.childLanes = l), e);
	}
	function oy(t, e, l) {
		var a = e.pendingProps;
		switch ((Ji(e), e.tag)) {
			case 16:
			case 15:
			case 0:
			case 11:
			case 7:
			case 8:
			case 12:
			case 9:
			case 14:
				return (At(e), null);
			case 1:
				return (At(e), null);
			case 3:
				return (
					(l = e.stateNode),
					(a = null),
					t !== null && (a = t.memoizedState.cache),
					e.memoizedState.cache !== a && (e.flags |= 2048),
					we(Ht),
					Ct(),
					l.pendingContext &&
						((l.context = l.pendingContext), (l.pendingContext = null)),
					(t === null || t.child === null) &&
						(sa(e)
							? ke(e)
							: t === null ||
								(t.memoizedState.isDehydrated && (e.flags & 256) === 0) ||
								((e.flags |= 1024), Wi())),
					At(e),
					null
				);
			case 26:
				var u = e.type,
					n = e.memoizedState;
				return (
					t === null
						? (ke(e),
							n !== null ? (At(e), As(e, n)) : (At(e), Lc(e, u, null, a, l)))
						: n
							? n !== t.memoizedState
								? (ke(e), At(e), As(e, n))
								: (At(e), (e.flags &= -16777217))
							: ((t = t.memoizedProps),
								t !== a && ke(e),
								At(e),
								Lc(e, u, t, a, l)),
					null
				);
			case 27:
				if (
					(ju(e),
					(l = lt.current),
					(u = e.type),
					t !== null && e.stateNode != null)
				)
					t.memoizedProps !== a && ke(e);
				else {
					if (!a) {
						if (e.stateNode === null) throw Error(f(166));
						return (At(e), null);
					}
					((t = X.current),
						sa(e) ? lo(e) : ((t = Ud(u, a, l)), (e.stateNode = t), ke(e)));
				}
				return (At(e), null);
			case 5:
				if ((ju(e), (u = e.type), t !== null && e.stateNode != null))
					t.memoizedProps !== a && ke(e);
				else {
					if (!a) {
						if (e.stateNode === null) throw Error(f(166));
						return (At(e), null);
					}
					if (((n = X.current), sa(e))) lo(e);
					else {
						var i = Xn(lt.current);
						switch (n) {
							case 1:
								n = i.createElementNS("http://www.w3.org/2000/svg", u);
								break;
							case 2:
								n = i.createElementNS("http://www.w3.org/1998/Math/MathML", u);
								break;
							default:
								switch (u) {
									case "svg":
										n = i.createElementNS("http://www.w3.org/2000/svg", u);
										break;
									case "math":
										n = i.createElementNS(
											"http://www.w3.org/1998/Math/MathML",
											u,
										);
										break;
									case "script":
										((n = i.createElement("div")),
											(n.innerHTML = "<script><\/script>"),
											(n = n.removeChild(n.firstChild)));
										break;
									case "select":
										((n =
											typeof a.is == "string"
												? i.createElement("select", { is: a.is })
												: i.createElement("select")),
											a.multiple
												? (n.multiple = !0)
												: a.size && (n.size = a.size));
										break;
									default:
										n =
											typeof a.is == "string"
												? i.createElement(u, { is: a.is })
												: i.createElement(u);
								}
						}
						((n[Xt] = e), (n[Ft] = a));
						t: for (i = e.child; i !== null; ) {
							if (i.tag === 5 || i.tag === 6) n.appendChild(i.stateNode);
							else if (i.tag !== 4 && i.tag !== 27 && i.child !== null) {
								((i.child.return = i), (i = i.child));
								continue;
							}
							if (i === e) break t;
							for (; i.sibling === null; ) {
								if (i.return === null || i.return === e) break t;
								i = i.return;
							}
							((i.sibling.return = i.return), (i = i.sibling));
						}
						e.stateNode = n;
						t: switch ((Kt(n, u, a), u)) {
							case "button":
							case "input":
							case "select":
							case "textarea":
								a = !!a.autoFocus;
								break t;
							case "img":
								a = !0;
								break t;
							default:
								a = !1;
						}
						a && ke(e);
					}
				}
				return (
					At(e),
					Lc(e, e.type, t === null ? null : t.memoizedProps, e.pendingProps, l),
					null
				);
			case 6:
				if (t && e.stateNode != null) t.memoizedProps !== a && ke(e);
				else {
					if (typeof a != "string" && e.stateNode === null) throw Error(f(166));
					if (((t = lt.current), sa(e))) {
						if (
							((t = e.stateNode),
							(l = e.memoizedProps),
							(a = null),
							(u = Qt),
							u !== null)
						)
							switch (u.tag) {
								case 27:
								case 5:
									a = u.memoizedProps;
							}
						((t[Xt] = e),
							(t = !!(
								t.nodeValue === l ||
								(a !== null && a.suppressHydrationWarning === !0) ||
								bd(t.nodeValue, l)
							)),
							t || ol(e, !0));
					} else
						((t = Xn(t).createTextNode(a)), (t[Xt] = e), (e.stateNode = t));
				}
				return (At(e), null);
			case 31:
				if (((l = e.memoizedState), t === null || t.memoizedState !== null)) {
					if (((a = sa(e)), l !== null)) {
						if (t === null) {
							if (!a) throw Error(f(318));
							if (
								((t = e.memoizedState),
								(t = t !== null ? t.dehydrated : null),
								!t)
							)
								throw Error(f(557));
							t[Xt] = e;
						} else
							(Yl(),
								(e.flags & 128) === 0 && (e.memoizedState = null),
								(e.flags |= 4));
						(At(e), (t = !1));
					} else
						((l = Wi()),
							t !== null &&
								t.memoizedState !== null &&
								(t.memoizedState.hydrationErrors = l),
							(t = !0));
					if (!t) return e.flags & 256 ? (oe(e), e) : (oe(e), null);
					if ((e.flags & 128) !== 0) throw Error(f(558));
				}
				return (At(e), null);
			case 13:
				if (
					((a = e.memoizedState),
					t === null ||
						(t.memoizedState !== null && t.memoizedState.dehydrated !== null))
				) {
					if (((u = sa(e)), a !== null && a.dehydrated !== null)) {
						if (t === null) {
							if (!u) throw Error(f(318));
							if (
								((u = e.memoizedState),
								(u = u !== null ? u.dehydrated : null),
								!u)
							)
								throw Error(f(317));
							u[Xt] = e;
						} else
							(Yl(),
								(e.flags & 128) === 0 && (e.memoizedState = null),
								(e.flags |= 4));
						(At(e), (u = !1));
					} else
						((u = Wi()),
							t !== null &&
								t.memoizedState !== null &&
								(t.memoizedState.hydrationErrors = u),
							(u = !0));
					if (!u) return e.flags & 256 ? (oe(e), e) : (oe(e), null);
				}
				return (
					oe(e),
					(e.flags & 128) !== 0
						? ((e.lanes = l), e)
						: ((l = a !== null),
							(t = t !== null && t.memoizedState !== null),
							l &&
								((a = e.child),
								(u = null),
								a.alternate !== null &&
									a.alternate.memoizedState !== null &&
									a.alternate.memoizedState.cachePool !== null &&
									(u = a.alternate.memoizedState.cachePool.pool),
								(n = null),
								a.memoizedState !== null &&
									a.memoizedState.cachePool !== null &&
									(n = a.memoizedState.cachePool.pool),
								n !== u && (a.flags |= 2048)),
							l !== t && l && (e.child.flags |= 8192),
							_n(e, e.updateQueue),
							At(e),
							null)
				);
			case 4:
				return (Ct(), t === null && ff(e.stateNode.containerInfo), At(e), null);
			case 10:
				return (we(e.type), At(e), null);
			case 19:
				if ((x(Ut), (a = e.memoizedState), a === null)) return (At(e), null);
				if (((u = (e.flags & 128) !== 0), (n = a.rendering), n === null))
					if (u) su(a, !1);
					else {
						if (Ot !== 0 || (t !== null && (t.flags & 128) !== 0))
							for (t = e.child; t !== null; ) {
								if (((n = hn(t)), n !== null)) {
									for (
										e.flags |= 128,
											su(a, !1),
											t = n.updateQueue,
											e.updateQueue = t,
											_n(e, t),
											e.subtreeFlags = 0,
											t = l,
											l = e.child;
										l !== null;
									)
										(Fr(l, t), (l = l.sibling));
									return (
										q(Ut, (Ut.current & 1) | 2),
										ct && Ve(e, a.treeForkCount),
										e.child
									);
								}
								t = t.sibling;
							}
						a.tail !== null &&
							ue() > Un &&
							((e.flags |= 128), (u = !0), su(a, !1), (e.lanes = 4194304));
					}
				else {
					if (!u)
						if (((t = hn(n)), t !== null)) {
							if (
								((e.flags |= 128),
								(u = !0),
								(t = t.updateQueue),
								(e.updateQueue = t),
								_n(e, t),
								su(a, !0),
								a.tail === null &&
									a.tailMode === "hidden" &&
									!n.alternate &&
									!ct)
							)
								return (At(e), null);
						} else
							2 * ue() - a.renderingStartTime > Un &&
								l !== 536870912 &&
								((e.flags |= 128), (u = !0), su(a, !1), (e.lanes = 4194304));
					a.isBackwards
						? ((n.sibling = e.child), (e.child = n))
						: ((t = a.last),
							t !== null ? (t.sibling = n) : (e.child = n),
							(a.last = n));
				}
				return a.tail !== null
					? ((t = a.tail),
						(a.rendering = t),
						(a.tail = t.sibling),
						(a.renderingStartTime = ue()),
						(t.sibling = null),
						(l = Ut.current),
						q(Ut, u ? (l & 1) | 2 : l & 1),
						ct && Ve(e, a.treeForkCount),
						t)
					: (At(e), null);
			case 22:
			case 23:
				return (
					oe(e),
					fc(),
					(a = e.memoizedState !== null),
					t !== null
						? (t.memoizedState !== null) !== a && (e.flags |= 8192)
						: a && (e.flags |= 8192),
					a
						? (l & 536870912) !== 0 &&
							(e.flags & 128) === 0 &&
							(At(e), e.subtreeFlags & 6 && (e.flags |= 8192))
						: At(e),
					(l = e.updateQueue),
					l !== null && _n(e, l.retryQueue),
					(l = null),
					t !== null &&
						t.memoizedState !== null &&
						t.memoizedState.cachePool !== null &&
						(l = t.memoizedState.cachePool.pool),
					(a = null),
					e.memoizedState !== null &&
						e.memoizedState.cachePool !== null &&
						(a = e.memoizedState.cachePool.pool),
					a !== l && (e.flags |= 2048),
					t !== null && x(Xl),
					null
				);
			case 24:
				return (
					(l = null),
					t !== null && (l = t.memoizedState.cache),
					e.memoizedState.cache !== l && (e.flags |= 2048),
					we(Ht),
					At(e),
					null
				);
			case 25:
				return null;
			case 30:
				return null;
		}
		throw Error(f(156, e.tag));
	}
	function sy(t, e) {
		switch ((Ji(e), e.tag)) {
			case 1:
				return (
					(t = e.flags),
					t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
				);
			case 3:
				return (
					we(Ht),
					Ct(),
					(t = e.flags),
					(t & 65536) !== 0 && (t & 128) === 0
						? ((e.flags = (t & -65537) | 128), e)
						: null
				);
			case 26:
			case 27:
			case 5:
				return (ju(e), null);
			case 31:
				if (e.memoizedState !== null) {
					if ((oe(e), e.alternate === null)) throw Error(f(340));
					Yl();
				}
				return (
					(t = e.flags),
					t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
				);
			case 13:
				if (
					(oe(e), (t = e.memoizedState), t !== null && t.dehydrated !== null)
				) {
					if (e.alternate === null) throw Error(f(340));
					Yl();
				}
				return (
					(t = e.flags),
					t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
				);
			case 19:
				return (x(Ut), null);
			case 4:
				return (Ct(), null);
			case 10:
				return (we(e.type), null);
			case 22:
			case 23:
				return (
					oe(e),
					fc(),
					t !== null && x(Xl),
					(t = e.flags),
					t & 65536 ? ((e.flags = (t & -65537) | 128), e) : null
				);
			case 24:
				return (we(Ht), null);
			case 25:
				return null;
			default:
				return null;
		}
	}
	function Rs(t, e) {
		switch ((Ji(e), e.tag)) {
			case 3:
				(we(Ht), Ct());
				break;
			case 26:
			case 27:
			case 5:
				ju(e);
				break;
			case 4:
				Ct();
				break;
			case 31:
				e.memoizedState !== null && oe(e);
				break;
			case 13:
				oe(e);
				break;
			case 19:
				x(Ut);
				break;
			case 10:
				we(e.type);
				break;
			case 22:
			case 23:
				(oe(e), fc(), t !== null && x(Xl));
				break;
			case 24:
				we(Ht);
		}
	}
	function du(t, e) {
		try {
			var l = e.updateQueue,
				a = l !== null ? l.lastEffect : null;
			if (a !== null) {
				var u = a.next;
				l = u;
				do {
					if ((l.tag & t) === t) {
						a = void 0;
						var n = l.create,
							i = l.inst;
						((a = n()), (i.destroy = a));
					}
					l = l.next;
				} while (l !== u);
			}
		} catch (r) {
			mt(e, e.return, r);
		}
	}
	function gl(t, e, l) {
		try {
			var a = e.updateQueue,
				u = a !== null ? a.lastEffect : null;
			if (u !== null) {
				var n = u.next;
				a = n;
				do {
					if ((a.tag & t) === t) {
						var i = a.inst,
							r = i.destroy;
						if (r !== void 0) {
							((i.destroy = void 0), (u = e));
							var h = l,
								T = r;
							try {
								T();
							} catch (O) {
								mt(u, h, O);
							}
						}
					}
					a = a.next;
				} while (a !== n);
			}
		} catch (O) {
			mt(e, e.return, O);
		}
	}
	function _s(t) {
		var e = t.updateQueue;
		if (e !== null) {
			var l = t.stateNode;
			try {
				vo(e, l);
			} catch (a) {
				mt(t, t.return, a);
			}
		}
	}
	function Ms(t, e, l) {
		((l.props = wl(t.type, t.memoizedProps)), (l.state = t.memoizedState));
		try {
			l.componentWillUnmount();
		} catch (a) {
			mt(t, e, a);
		}
	}
	function hu(t, e) {
		try {
			var l = t.ref;
			if (l !== null) {
				switch (t.tag) {
					case 26:
					case 27:
					case 5:
						var a = t.stateNode;
						break;
					case 30:
						a = t.stateNode;
						break;
					default:
						a = t.stateNode;
				}
				typeof l == "function" ? (t.refCleanup = l(a)) : (l.current = a);
			}
		} catch (u) {
			mt(t, e, u);
		}
	}
	function je(t, e) {
		var l = t.ref,
			a = t.refCleanup;
		if (l !== null)
			if (typeof a == "function")
				try {
					a();
				} catch (u) {
					mt(t, e, u);
				} finally {
					((t.refCleanup = null),
						(t = t.alternate),
						t != null && (t.refCleanup = null));
				}
			else if (typeof l == "function")
				try {
					l(null);
				} catch (u) {
					mt(t, e, u);
				}
			else l.current = null;
	}
	function Os(t) {
		var e = t.type,
			l = t.memoizedProps,
			a = t.stateNode;
		try {
			t: switch (e) {
				case "button":
				case "input":
				case "select":
				case "textarea":
					l.autoFocus && a.focus();
					break t;
				case "img":
					l.src ? (a.src = l.src) : l.srcSet && (a.srcset = l.srcSet);
			}
		} catch (u) {
			mt(t, t.return, u);
		}
	}
	function Gc(t, e, l) {
		try {
			var a = t.stateNode;
			(xy(a, t.type, l, e), (a[Ft] = e));
		} catch (u) {
			mt(t, t.return, u);
		}
	}
	function Ds(t) {
		return (
			t.tag === 5 ||
			t.tag === 3 ||
			t.tag === 26 ||
			(t.tag === 27 && Al(t.type)) ||
			t.tag === 4
		);
	}
	function Xc(t) {
		t: for (;;) {
			for (; t.sibling === null; ) {
				if (t.return === null || Ds(t.return)) return null;
				t = t.return;
			}
			for (
				t.sibling.return = t.return, t = t.sibling;
				t.tag !== 5 && t.tag !== 6 && t.tag !== 18;
			) {
				if (
					(t.tag === 27 && Al(t.type)) ||
					t.flags & 2 ||
					t.child === null ||
					t.tag === 4
				)
					continue t;
				((t.child.return = t), (t = t.child));
			}
			if (!(t.flags & 2)) return t.stateNode;
		}
	}
	function Qc(t, e, l) {
		var a = t.tag;
		if (a === 5 || a === 6)
			((t = t.stateNode),
				e
					? (l.nodeType === 9
							? l.body
							: l.nodeName === "HTML"
								? l.ownerDocument.body
								: l
						).insertBefore(t, e)
					: ((e =
							l.nodeType === 9
								? l.body
								: l.nodeName === "HTML"
									? l.ownerDocument.body
									: l),
						e.appendChild(t),
						(l = l._reactRootContainer),
						l != null || e.onclick !== null || (e.onclick = Xe)));
		else if (
			a !== 4 &&
			(a === 27 && Al(t.type) && ((l = t.stateNode), (e = null)),
			(t = t.child),
			t !== null)
		)
			for (Qc(t, e, l), t = t.sibling; t !== null; )
				(Qc(t, e, l), (t = t.sibling));
	}
	function Mn(t, e, l) {
		var a = t.tag;
		if (a === 5 || a === 6)
			((t = t.stateNode), e ? l.insertBefore(t, e) : l.appendChild(t));
		else if (
			a !== 4 &&
			(a === 27 && Al(t.type) && (l = t.stateNode), (t = t.child), t !== null)
		)
			for (Mn(t, e, l), t = t.sibling; t !== null; )
				(Mn(t, e, l), (t = t.sibling));
	}
	function Cs(t) {
		var e = t.stateNode,
			l = t.memoizedProps;
		try {
			for (var a = t.type, u = e.attributes; u.length; )
				e.removeAttributeNode(u[0]);
			(Kt(e, a, l), (e[Xt] = t), (e[Ft] = l));
		} catch (n) {
			mt(t, t.return, n);
		}
	}
	var Fe = !1,
		qt = !1,
		Zc = !1,
		Us = typeof WeakSet == "function" ? WeakSet : Set,
		Gt = null;
	function dy(t, e) {
		if (((t = t.containerInfo), (sf = $n), (t = Qr(t)), ji(t))) {
			if ("selectionStart" in t)
				var l = { start: t.selectionStart, end: t.selectionEnd };
			else
				t: {
					l = ((l = t.ownerDocument) && l.defaultView) || window;
					var a = l.getSelection && l.getSelection();
					if (a && a.rangeCount !== 0) {
						l = a.anchorNode;
						var u = a.anchorOffset,
							n = a.focusNode;
						a = a.focusOffset;
						try {
							(l.nodeType, n.nodeType);
						} catch {
							l = null;
							break t;
						}
						var i = 0,
							r = -1,
							h = -1,
							T = 0,
							O = 0,
							N = t,
							z = null;
						e: for (;;) {
							for (
								var _;
								N !== l || (u !== 0 && N.nodeType !== 3) || (r = i + u),
									N !== n || (a !== 0 && N.nodeType !== 3) || (h = i + a),
									N.nodeType === 3 && (i += N.nodeValue.length),
									(_ = N.firstChild) !== null;
							)
								((z = N), (N = _));
							for (;;) {
								if (N === t) break e;
								if (
									(z === l && ++T === u && (r = i),
									z === n && ++O === a && (h = i),
									(_ = N.nextSibling) !== null)
								)
									break;
								((N = z), (z = N.parentNode));
							}
							N = _;
						}
						l = r === -1 || h === -1 ? null : { start: r, end: h };
					} else l = null;
				}
			l = l || { start: 0, end: 0 };
		} else l = null;
		for (
			df = { focusedElem: t, selectionRange: l }, $n = !1, Gt = e;
			Gt !== null;
		)
			if (
				((e = Gt), (t = e.child), (e.subtreeFlags & 1028) !== 0 && t !== null)
			)
				((t.return = e), (Gt = t));
			else
				for (; Gt !== null; ) {
					switch (((e = Gt), (n = e.alternate), (t = e.flags), e.tag)) {
						case 0:
							if (
								(t & 4) !== 0 &&
								((t = e.updateQueue),
								(t = t !== null ? t.events : null),
								t !== null)
							)
								for (l = 0; l < t.length; l++)
									((u = t[l]), (u.ref.impl = u.nextImpl));
							break;
						case 11:
						case 15:
							break;
						case 1:
							if ((t & 1024) !== 0 && n !== null) {
								((t = void 0),
									(l = e),
									(u = n.memoizedProps),
									(n = n.memoizedState),
									(a = l.stateNode));
								try {
									var Y = wl(l.type, u);
									((t = a.getSnapshotBeforeUpdate(Y, n)),
										(a.__reactInternalSnapshotBeforeUpdate = t));
								} catch (J) {
									mt(l, l.return, J);
								}
							}
							break;
						case 3:
							if ((t & 1024) !== 0) {
								if (
									((t = e.stateNode.containerInfo), (l = t.nodeType), l === 9)
								)
									yf(t);
								else if (l === 1)
									switch (t.nodeName) {
										case "HEAD":
										case "HTML":
										case "BODY":
											yf(t);
											break;
										default:
											t.textContent = "";
									}
							}
							break;
						case 5:
						case 26:
						case 27:
						case 6:
						case 4:
						case 17:
							break;
						default:
							if ((t & 1024) !== 0) throw Error(f(163));
					}
					if (((t = e.sibling), t !== null)) {
						((t.return = e.return), (Gt = t));
						break;
					}
					Gt = e.return;
				}
	}
	function Ns(t, e, l) {
		var a = l.flags;
		switch (l.tag) {
			case 0:
			case 11:
			case 15:
				(Pe(t, l), a & 4 && du(5, l));
				break;
			case 1:
				if ((Pe(t, l), a & 4))
					if (((t = l.stateNode), e === null))
						try {
							t.componentDidMount();
						} catch (i) {
							mt(l, l.return, i);
						}
					else {
						var u = wl(l.type, e.memoizedProps);
						e = e.memoizedState;
						try {
							t.componentDidUpdate(u, e, t.__reactInternalSnapshotBeforeUpdate);
						} catch (i) {
							mt(l, l.return, i);
						}
					}
				(a & 64 && _s(l), a & 512 && hu(l, l.return));
				break;
			case 3:
				if ((Pe(t, l), a & 64 && ((t = l.updateQueue), t !== null))) {
					if (((e = null), l.child !== null))
						switch (l.child.tag) {
							case 27:
							case 5:
								e = l.child.stateNode;
								break;
							case 1:
								e = l.child.stateNode;
						}
					try {
						vo(t, e);
					} catch (i) {
						mt(l, l.return, i);
					}
				}
				break;
			case 27:
				e === null && a & 4 && Cs(l);
			case 26:
			case 5:
				(Pe(t, l), e === null && a & 4 && Os(l), a & 512 && hu(l, l.return));
				break;
			case 12:
				Pe(t, l);
				break;
			case 31:
				(Pe(t, l), a & 4 && Bs(t, l));
				break;
			case 13:
				(Pe(t, l),
					a & 4 && js(t, l),
					a & 64 &&
						((t = l.memoizedState),
						t !== null &&
							((t = t.dehydrated),
							t !== null && ((l = Ey.bind(null, l)), Xy(t, l)))));
				break;
			case 22:
				if (((a = l.memoizedState !== null || Fe), !a)) {
					((e = (e !== null && e.memoizedState !== null) || qt), (u = Fe));
					var n = qt;
					((Fe = a),
						(qt = e) && !n ? tl(t, l, (l.subtreeFlags & 8772) !== 0) : Pe(t, l),
						(Fe = u),
						(qt = n));
				}
				break;
			case 30:
				break;
			default:
				Pe(t, l);
		}
	}
	function xs(t) {
		var e = t.alternate;
		(e !== null && ((t.alternate = null), xs(e)),
			(t.child = null),
			(t.deletions = null),
			(t.sibling = null),
			t.tag === 5 && ((e = t.stateNode), e !== null && Si(e)),
			(t.stateNode = null),
			(t.return = null),
			(t.dependencies = null),
			(t.memoizedProps = null),
			(t.memoizedState = null),
			(t.pendingProps = null),
			(t.stateNode = null),
			(t.updateQueue = null));
	}
	var Rt = null,
		Pt = !1;
	function Ie(t, e, l) {
		for (l = l.child; l !== null; ) (Hs(t, e, l), (l = l.sibling));
	}
	function Hs(t, e, l) {
		if (ne && typeof ne.onCommitFiberUnmount == "function")
			try {
				ne.onCommitFiberUnmount(qa, l);
			} catch {}
		switch (l.tag) {
			case 26:
				(qt || je(l, e),
					Ie(t, e, l),
					l.memoizedState
						? l.memoizedState.count--
						: l.stateNode && ((l = l.stateNode), l.parentNode.removeChild(l)));
				break;
			case 27:
				qt || je(l, e);
				var a = Rt,
					u = Pt;
				(Al(l.type) && ((Rt = l.stateNode), (Pt = !1)),
					Ie(t, e, l),
					Tu(l.stateNode),
					(Rt = a),
					(Pt = u));
				break;
			case 5:
				qt || je(l, e);
			case 6:
				if (
					((a = Rt),
					(u = Pt),
					(Rt = null),
					Ie(t, e, l),
					(Rt = a),
					(Pt = u),
					Rt !== null)
				)
					if (Pt)
						try {
							(Rt.nodeType === 9
								? Rt.body
								: Rt.nodeName === "HTML"
									? Rt.ownerDocument.body
									: Rt
							).removeChild(l.stateNode);
						} catch (n) {
							mt(l, e, n);
						}
					else
						try {
							Rt.removeChild(l.stateNode);
						} catch (n) {
							mt(l, e, n);
						}
				break;
			case 18:
				Rt !== null &&
					(Pt
						? ((t = Rt),
							_d(
								t.nodeType === 9
									? t.body
									: t.nodeName === "HTML"
										? t.ownerDocument.body
										: t,
								l.stateNode,
							),
							Na(t))
						: _d(Rt, l.stateNode));
				break;
			case 4:
				((a = Rt),
					(u = Pt),
					(Rt = l.stateNode.containerInfo),
					(Pt = !0),
					Ie(t, e, l),
					(Rt = a),
					(Pt = u));
				break;
			case 0:
			case 11:
			case 14:
			case 15:
				(gl(2, l, e), qt || gl(4, l, e), Ie(t, e, l));
				break;
			case 1:
				(qt ||
					(je(l, e),
					(a = l.stateNode),
					typeof a.componentWillUnmount == "function" && Ms(l, e, a)),
					Ie(t, e, l));
				break;
			case 21:
				Ie(t, e, l);
				break;
			case 22:
				((qt = (a = qt) || l.memoizedState !== null), Ie(t, e, l), (qt = a));
				break;
			default:
				Ie(t, e, l);
		}
	}
	function Bs(t, e) {
		if (
			e.memoizedState === null &&
			((t = e.alternate), t !== null && ((t = t.memoizedState), t !== null))
		) {
			t = t.dehydrated;
			try {
				Na(t);
			} catch (l) {
				mt(e, e.return, l);
			}
		}
	}
	function js(t, e) {
		if (
			e.memoizedState === null &&
			((t = e.alternate),
			t !== null &&
				((t = t.memoizedState), t !== null && ((t = t.dehydrated), t !== null)))
		)
			try {
				Na(t);
			} catch (l) {
				mt(e, e.return, l);
			}
	}
	function hy(t) {
		switch (t.tag) {
			case 31:
			case 13:
			case 19:
				var e = t.stateNode;
				return (e === null && (e = t.stateNode = new Us()), e);
			case 22:
				return (
					(t = t.stateNode),
					(e = t._retryCache),
					e === null && (e = t._retryCache = new Us()),
					e
				);
			default:
				throw Error(f(435, t.tag));
		}
	}
	function On(t, e) {
		var l = hy(t);
		e.forEach(function (a) {
			if (!l.has(a)) {
				l.add(a);
				var u = Ty.bind(null, t, a);
				a.then(u, u);
			}
		});
	}
	function te(t, e) {
		var l = e.deletions;
		if (l !== null)
			for (var a = 0; a < l.length; a++) {
				var u = l[a],
					n = t,
					i = e,
					r = i;
				t: for (; r !== null; ) {
					switch (r.tag) {
						case 27:
							if (Al(r.type)) {
								((Rt = r.stateNode), (Pt = !1));
								break t;
							}
							break;
						case 5:
							((Rt = r.stateNode), (Pt = !1));
							break t;
						case 3:
						case 4:
							((Rt = r.stateNode.containerInfo), (Pt = !0));
							break t;
					}
					r = r.return;
				}
				if (Rt === null) throw Error(f(160));
				(Hs(n, i, u),
					(Rt = null),
					(Pt = !1),
					(n = u.alternate),
					n !== null && (n.return = null),
					(u.return = null));
			}
		if (e.subtreeFlags & 13886)
			for (e = e.child; e !== null; ) (qs(e, t), (e = e.sibling));
	}
	var Oe = null;
	function qs(t, e) {
		var l = t.alternate,
			a = t.flags;
		switch (t.tag) {
			case 0:
			case 11:
			case 14:
			case 15:
				(te(e, t),
					ee(t),
					a & 4 && (gl(3, t, t.return), du(3, t), gl(5, t, t.return)));
				break;
			case 1:
				(te(e, t),
					ee(t),
					a & 512 && (qt || l === null || je(l, l.return)),
					a & 64 &&
						Fe &&
						((t = t.updateQueue),
						t !== null &&
							((a = t.callbacks),
							a !== null &&
								((l = t.shared.hiddenCallbacks),
								(t.shared.hiddenCallbacks = l === null ? a : l.concat(a))))));
				break;
			case 26:
				var u = Oe;
				if (
					(te(e, t),
					ee(t),
					a & 512 && (qt || l === null || je(l, l.return)),
					a & 4)
				) {
					var n = l !== null ? l.memoizedState : null;
					if (((a = t.memoizedState), l === null))
						if (a === null)
							if (t.stateNode === null) {
								t: {
									((a = t.type),
										(l = t.memoizedProps),
										(u = u.ownerDocument || u));
									e: switch (a) {
										case "title":
											((n = u.getElementsByTagName("title")[0]),
												(!n ||
													n[Ga] ||
													n[Xt] ||
													n.namespaceURI === "http://www.w3.org/2000/svg" ||
													n.hasAttribute("itemprop")) &&
													((n = u.createElement(a)),
													u.head.insertBefore(
														n,
														u.querySelector("head > title"),
													)),
												Kt(n, a, l),
												(n[Xt] = t),
												Lt(n),
												(a = n));
											break t;
										case "link":
											var i = qd("link", "href", u).get(a + (l.href || ""));
											if (i) {
												for (var r = 0; r < i.length; r++)
													if (
														((n = i[r]),
														n.getAttribute("href") ===
															(l.href == null || l.href === ""
																? null
																: l.href) &&
															n.getAttribute("rel") ===
																(l.rel == null ? null : l.rel) &&
															n.getAttribute("title") ===
																(l.title == null ? null : l.title) &&
															n.getAttribute("crossorigin") ===
																(l.crossOrigin == null ? null : l.crossOrigin))
													) {
														i.splice(r, 1);
														break e;
													}
											}
											((n = u.createElement(a)),
												Kt(n, a, l),
												u.head.appendChild(n));
											break;
										case "meta":
											if (
												(i = qd("meta", "content", u).get(
													a + (l.content || ""),
												))
											) {
												for (r = 0; r < i.length; r++)
													if (
														((n = i[r]),
														n.getAttribute("content") ===
															(l.content == null ? null : "" + l.content) &&
															n.getAttribute("name") ===
																(l.name == null ? null : l.name) &&
															n.getAttribute("property") ===
																(l.property == null ? null : l.property) &&
															n.getAttribute("http-equiv") ===
																(l.httpEquiv == null ? null : l.httpEquiv) &&
															n.getAttribute("charset") ===
																(l.charSet == null ? null : l.charSet))
													) {
														i.splice(r, 1);
														break e;
													}
											}
											((n = u.createElement(a)),
												Kt(n, a, l),
												u.head.appendChild(n));
											break;
										default:
											throw Error(f(468, a));
									}
									((n[Xt] = t), Lt(n), (a = n));
								}
								t.stateNode = a;
							} else Yd(u, t.type, t.stateNode);
						else t.stateNode = jd(u, a, t.memoizedProps);
					else
						n !== a
							? (n === null
									? l.stateNode !== null &&
										((l = l.stateNode), l.parentNode.removeChild(l))
									: n.count--,
								a === null
									? Yd(u, t.type, t.stateNode)
									: jd(u, a, t.memoizedProps))
							: a === null &&
								t.stateNode !== null &&
								Gc(t, t.memoizedProps, l.memoizedProps);
				}
				break;
			case 27:
				(te(e, t),
					ee(t),
					a & 512 && (qt || l === null || je(l, l.return)),
					l !== null && a & 4 && Gc(t, t.memoizedProps, l.memoizedProps));
				break;
			case 5:
				if (
					(te(e, t),
					ee(t),
					a & 512 && (qt || l === null || je(l, l.return)),
					t.flags & 32)
				) {
					u = t.stateNode;
					try {
						ea(u, "");
					} catch (Y) {
						mt(t, t.return, Y);
					}
				}
				(a & 4 &&
					t.stateNode != null &&
					((u = t.memoizedProps), Gc(t, u, l !== null ? l.memoizedProps : u)),
					a & 1024 && (Zc = !0));
				break;
			case 6:
				if ((te(e, t), ee(t), a & 4)) {
					if (t.stateNode === null) throw Error(f(162));
					((a = t.memoizedProps), (l = t.stateNode));
					try {
						l.nodeValue = a;
					} catch (Y) {
						mt(t, t.return, Y);
					}
				}
				break;
			case 3:
				if (
					((Vn = null),
					(u = Oe),
					(Oe = Qn(e.containerInfo)),
					te(e, t),
					(Oe = u),
					ee(t),
					a & 4 && l !== null && l.memoizedState.isDehydrated)
				)
					try {
						Na(e.containerInfo);
					} catch (Y) {
						mt(t, t.return, Y);
					}
				Zc && ((Zc = !1), Ys(t));
				break;
			case 4:
				((a = Oe),
					(Oe = Qn(t.stateNode.containerInfo)),
					te(e, t),
					ee(t),
					(Oe = a));
				break;
			case 12:
				(te(e, t), ee(t));
				break;
			case 31:
				(te(e, t),
					ee(t),
					a & 4 &&
						((a = t.updateQueue),
						a !== null && ((t.updateQueue = null), On(t, a))));
				break;
			case 13:
				(te(e, t),
					ee(t),
					t.child.flags & 8192 &&
						(t.memoizedState !== null) !=
							(l !== null && l.memoizedState !== null) &&
						(Cn = ue()),
					a & 4 &&
						((a = t.updateQueue),
						a !== null && ((t.updateQueue = null), On(t, a))));
				break;
			case 22:
				u = t.memoizedState !== null;
				var h = l !== null && l.memoizedState !== null,
					T = Fe,
					O = qt;
				if (
					((Fe = T || u),
					(qt = O || h),
					te(e, t),
					(qt = O),
					(Fe = T),
					ee(t),
					a & 8192)
				)
					t: for (
						e = t.stateNode,
							e._visibility = u ? e._visibility & -2 : e._visibility | 1,
							u && (l === null || h || Fe || qt || Jl(t)),
							l = null,
							e = t;
						;
					) {
						if (e.tag === 5 || e.tag === 26) {
							if (l === null) {
								h = l = e;
								try {
									if (((n = h.stateNode), u))
										((i = n.style),
											typeof i.setProperty == "function"
												? i.setProperty("display", "none", "important")
												: (i.display = "none"));
									else {
										r = h.stateNode;
										var N = h.memoizedProps.style,
											z =
												N != null && N.hasOwnProperty("display")
													? N.display
													: null;
										r.style.display =
											z == null || typeof z == "boolean" ? "" : ("" + z).trim();
									}
								} catch (Y) {
									mt(h, h.return, Y);
								}
							}
						} else if (e.tag === 6) {
							if (l === null) {
								h = e;
								try {
									h.stateNode.nodeValue = u ? "" : h.memoizedProps;
								} catch (Y) {
									mt(h, h.return, Y);
								}
							}
						} else if (e.tag === 18) {
							if (l === null) {
								h = e;
								try {
									var _ = h.stateNode;
									u ? Md(_, !0) : Md(h.stateNode, !1);
								} catch (Y) {
									mt(h, h.return, Y);
								}
							}
						} else if (
							((e.tag !== 22 && e.tag !== 23) ||
								e.memoizedState === null ||
								e === t) &&
							e.child !== null
						) {
							((e.child.return = e), (e = e.child));
							continue;
						}
						if (e === t) break t;
						for (; e.sibling === null; ) {
							if (e.return === null || e.return === t) break t;
							(l === e && (l = null), (e = e.return));
						}
						(l === e && (l = null),
							(e.sibling.return = e.return),
							(e = e.sibling));
					}
				a & 4 &&
					((a = t.updateQueue),
					a !== null &&
						((l = a.retryQueue),
						l !== null && ((a.retryQueue = null), On(t, l))));
				break;
			case 19:
				(te(e, t),
					ee(t),
					a & 4 &&
						((a = t.updateQueue),
						a !== null && ((t.updateQueue = null), On(t, a))));
				break;
			case 30:
				break;
			case 21:
				break;
			default:
				(te(e, t), ee(t));
		}
	}
	function ee(t) {
		var e = t.flags;
		if (e & 2) {
			try {
				for (var l, a = t.return; a !== null; ) {
					if (Ds(a)) {
						l = a;
						break;
					}
					a = a.return;
				}
				if (l == null) throw Error(f(160));
				switch (l.tag) {
					case 27:
						var u = l.stateNode,
							n = Xc(t);
						Mn(t, n, u);
						break;
					case 5:
						var i = l.stateNode;
						l.flags & 32 && (ea(i, ""), (l.flags &= -33));
						var r = Xc(t);
						Mn(t, r, i);
						break;
					case 3:
					case 4:
						var h = l.stateNode.containerInfo,
							T = Xc(t);
						Qc(t, T, h);
						break;
					default:
						throw Error(f(161));
				}
			} catch (O) {
				mt(t, t.return, O);
			}
			t.flags &= -3;
		}
		e & 4096 && (t.flags &= -4097);
	}
	function Ys(t) {
		if (t.subtreeFlags & 1024)
			for (t = t.child; t !== null; ) {
				var e = t;
				(Ys(e),
					e.tag === 5 && e.flags & 1024 && e.stateNode.reset(),
					(t = t.sibling));
			}
	}
	function Pe(t, e) {
		if (e.subtreeFlags & 8772)
			for (e = e.child; e !== null; ) (Ns(t, e.alternate, e), (e = e.sibling));
	}
	function Jl(t) {
		for (t = t.child; t !== null; ) {
			var e = t;
			switch (e.tag) {
				case 0:
				case 11:
				case 14:
				case 15:
					(gl(4, e, e.return), Jl(e));
					break;
				case 1:
					je(e, e.return);
					var l = e.stateNode;
					(typeof l.componentWillUnmount == "function" && Ms(e, e.return, l),
						Jl(e));
					break;
				case 27:
					Tu(e.stateNode);
				case 26:
				case 5:
					(je(e, e.return), Jl(e));
					break;
				case 22:
					e.memoizedState === null && Jl(e);
					break;
				case 30:
					Jl(e);
					break;
				default:
					Jl(e);
			}
			t = t.sibling;
		}
	}
	function tl(t, e, l) {
		for (l = l && (e.subtreeFlags & 8772) !== 0, e = e.child; e !== null; ) {
			var a = e.alternate,
				u = t,
				n = e,
				i = n.flags;
			switch (n.tag) {
				case 0:
				case 11:
				case 15:
					(tl(u, n, l), du(4, n));
					break;
				case 1:
					if (
						(tl(u, n, l),
						(a = n),
						(u = a.stateNode),
						typeof u.componentDidMount == "function")
					)
						try {
							u.componentDidMount();
						} catch (T) {
							mt(a, a.return, T);
						}
					if (((a = n), (u = a.updateQueue), u !== null)) {
						var r = a.stateNode;
						try {
							var h = u.shared.hiddenCallbacks;
							if (h !== null)
								for (u.shared.hiddenCallbacks = null, u = 0; u < h.length; u++)
									yo(h[u], r);
						} catch (T) {
							mt(a, a.return, T);
						}
					}
					(l && i & 64 && _s(n), hu(n, n.return));
					break;
				case 27:
					Cs(n);
				case 26:
				case 5:
					(tl(u, n, l), l && a === null && i & 4 && Os(n), hu(n, n.return));
					break;
				case 12:
					tl(u, n, l);
					break;
				case 31:
					(tl(u, n, l), l && i & 4 && Bs(u, n));
					break;
				case 13:
					(tl(u, n, l), l && i & 4 && js(u, n));
					break;
				case 22:
					(n.memoizedState === null && tl(u, n, l), hu(n, n.return));
					break;
				case 30:
					break;
				default:
					tl(u, n, l);
			}
			e = e.sibling;
		}
	}
	function Vc(t, e) {
		var l = null;
		(t !== null &&
			t.memoizedState !== null &&
			t.memoizedState.cachePool !== null &&
			(l = t.memoizedState.cachePool.pool),
			(t = null),
			e.memoizedState !== null &&
				e.memoizedState.cachePool !== null &&
				(t = e.memoizedState.cachePool.pool),
			t !== l && (t != null && t.refCount++, l != null && Pa(l)));
	}
	function Kc(t, e) {
		((t = null),
			e.alternate !== null && (t = e.alternate.memoizedState.cache),
			(e = e.memoizedState.cache),
			e !== t && (e.refCount++, t != null && Pa(t)));
	}
	function De(t, e, l, a) {
		if (e.subtreeFlags & 10256)
			for (e = e.child; e !== null; ) (Ls(t, e, l, a), (e = e.sibling));
	}
	function Ls(t, e, l, a) {
		var u = e.flags;
		switch (e.tag) {
			case 0:
			case 11:
			case 15:
				(De(t, e, l, a), u & 2048 && du(9, e));
				break;
			case 1:
				De(t, e, l, a);
				break;
			case 3:
				(De(t, e, l, a),
					u & 2048 &&
						((t = null),
						e.alternate !== null && (t = e.alternate.memoizedState.cache),
						(e = e.memoizedState.cache),
						e !== t && (e.refCount++, t != null && Pa(t))));
				break;
			case 12:
				if (u & 2048) {
					(De(t, e, l, a), (t = e.stateNode));
					try {
						var n = e.memoizedProps,
							i = n.id,
							r = n.onPostCommit;
						typeof r == "function" &&
							r(
								i,
								e.alternate === null ? "mount" : "update",
								t.passiveEffectDuration,
								-0,
							);
					} catch (h) {
						mt(e, e.return, h);
					}
				} else De(t, e, l, a);
				break;
			case 31:
				De(t, e, l, a);
				break;
			case 13:
				De(t, e, l, a);
				break;
			case 23:
				break;
			case 22:
				((n = e.stateNode),
					(i = e.alternate),
					e.memoizedState !== null
						? n._visibility & 2
							? De(t, e, l, a)
							: mu(t, e)
						: n._visibility & 2
							? De(t, e, l, a)
							: ((n._visibility |= 2),
								Ea(t, e, l, a, (e.subtreeFlags & 10256) !== 0 || !1)),
					u & 2048 && Vc(i, e));
				break;
			case 24:
				(De(t, e, l, a), u & 2048 && Kc(e.alternate, e));
				break;
			default:
				De(t, e, l, a);
		}
	}
	function Ea(t, e, l, a, u) {
		for (
			u = u && ((e.subtreeFlags & 10256) !== 0 || !1), e = e.child;
			e !== null;
		) {
			var n = t,
				i = e,
				r = l,
				h = a,
				T = i.flags;
			switch (i.tag) {
				case 0:
				case 11:
				case 15:
					(Ea(n, i, r, h, u), du(8, i));
					break;
				case 23:
					break;
				case 22:
					var O = i.stateNode;
					(i.memoizedState !== null
						? O._visibility & 2
							? Ea(n, i, r, h, u)
							: mu(n, i)
						: ((O._visibility |= 2), Ea(n, i, r, h, u)),
						u && T & 2048 && Vc(i.alternate, i));
					break;
				case 24:
					(Ea(n, i, r, h, u), u && T & 2048 && Kc(i.alternate, i));
					break;
				default:
					Ea(n, i, r, h, u);
			}
			e = e.sibling;
		}
	}
	function mu(t, e) {
		if (e.subtreeFlags & 10256)
			for (e = e.child; e !== null; ) {
				var l = t,
					a = e,
					u = a.flags;
				switch (a.tag) {
					case 22:
						(mu(l, a), u & 2048 && Vc(a.alternate, a));
						break;
					case 24:
						(mu(l, a), u & 2048 && Kc(a.alternate, a));
						break;
					default:
						mu(l, a);
				}
				e = e.sibling;
			}
	}
	var yu = 8192;
	function Ta(t, e, l) {
		if (t.subtreeFlags & yu)
			for (t = t.child; t !== null; ) (Gs(t, e, l), (t = t.sibling));
	}
	function Gs(t, e, l) {
		switch (t.tag) {
			case 26:
				(Ta(t, e, l),
					t.flags & yu &&
						t.memoizedState !== null &&
						Py(l, Oe, t.memoizedState, t.memoizedProps));
				break;
			case 5:
				Ta(t, e, l);
				break;
			case 3:
			case 4:
				var a = Oe;
				((Oe = Qn(t.stateNode.containerInfo)), Ta(t, e, l), (Oe = a));
				break;
			case 22:
				t.memoizedState === null &&
					((a = t.alternate),
					a !== null && a.memoizedState !== null
						? ((a = yu), (yu = 16777216), Ta(t, e, l), (yu = a))
						: Ta(t, e, l));
				break;
			default:
				Ta(t, e, l);
		}
	}
	function Xs(t) {
		var e = t.alternate;
		if (e !== null && ((t = e.child), t !== null)) {
			e.child = null;
			do ((e = t.sibling), (t.sibling = null), (t = e));
			while (t !== null);
		}
	}
	function vu(t) {
		var e = t.deletions;
		if ((t.flags & 16) !== 0) {
			if (e !== null)
				for (var l = 0; l < e.length; l++) {
					var a = e[l];
					((Gt = a), Zs(a, t));
				}
			Xs(t);
		}
		if (t.subtreeFlags & 10256)
			for (t = t.child; t !== null; ) (Qs(t), (t = t.sibling));
	}
	function Qs(t) {
		switch (t.tag) {
			case 0:
			case 11:
			case 15:
				(vu(t), t.flags & 2048 && gl(9, t, t.return));
				break;
			case 3:
				vu(t);
				break;
			case 12:
				vu(t);
				break;
			case 22:
				var e = t.stateNode;
				t.memoizedState !== null &&
				e._visibility & 2 &&
				(t.return === null || t.return.tag !== 13)
					? ((e._visibility &= -3), Dn(t))
					: vu(t);
				break;
			default:
				vu(t);
		}
	}
	function Dn(t) {
		var e = t.deletions;
		if ((t.flags & 16) !== 0) {
			if (e !== null)
				for (var l = 0; l < e.length; l++) {
					var a = e[l];
					((Gt = a), Zs(a, t));
				}
			Xs(t);
		}
		for (t = t.child; t !== null; ) {
			switch (((e = t), e.tag)) {
				case 0:
				case 11:
				case 15:
					(gl(8, e, e.return), Dn(e));
					break;
				case 22:
					((l = e.stateNode),
						l._visibility & 2 && ((l._visibility &= -3), Dn(e)));
					break;
				default:
					Dn(e);
			}
			t = t.sibling;
		}
	}
	function Zs(t, e) {
		for (; Gt !== null; ) {
			var l = Gt;
			switch (l.tag) {
				case 0:
				case 11:
				case 15:
					gl(8, l, e);
					break;
				case 23:
				case 22:
					if (l.memoizedState !== null && l.memoizedState.cachePool !== null) {
						var a = l.memoizedState.cachePool.pool;
						a != null && a.refCount++;
					}
					break;
				case 24:
					Pa(l.memoizedState.cache);
			}
			if (((a = l.child), a !== null)) ((a.return = l), (Gt = a));
			else
				t: for (l = t; Gt !== null; ) {
					a = Gt;
					var u = a.sibling,
						n = a.return;
					if ((xs(a), a === l)) {
						Gt = null;
						break t;
					}
					if (u !== null) {
						((u.return = n), (Gt = u));
						break t;
					}
					Gt = n;
				}
		}
	}
	var my = {
			getCacheForType: function (t) {
				var e = Zt(Ht),
					l = e.data.get(t);
				return (l === void 0 && ((l = t()), e.data.set(t, l)), l);
			},
			cacheSignal: function () {
				return Zt(Ht).controller.signal;
			},
		},
		yy = typeof WeakMap == "function" ? WeakMap : Map,
		ot = 0,
		Et = null,
		at = null,
		nt = 0,
		ht = 0,
		se = null,
		pl = !1,
		za = !1,
		wc = !1,
		el = 0,
		Ot = 0,
		Sl = 0,
		$l = 0,
		Jc = 0,
		de = 0,
		Aa = 0,
		gu = null,
		le = null,
		$c = !1,
		Cn = 0,
		Vs = 0,
		Un = 1 / 0,
		Nn = null,
		bl = null,
		Yt = 0,
		El = null,
		Ra = null,
		ll = 0,
		Wc = 0,
		kc = null,
		Ks = null,
		pu = 0,
		Fc = null;
	function he() {
		return (ot & 2) !== 0 && nt !== 0 ? nt & -nt : D.T !== null ? af() : ir();
	}
	function ws() {
		if (de === 0)
			if ((nt & 536870912) === 0 || ct) {
				var t = Lu;
				((Lu <<= 1), (Lu & 3932160) === 0 && (Lu = 262144), (de = t));
			} else de = 536870912;
		return ((t = re.current), t !== null && (t.flags |= 32), de);
	}
	function ae(t, e, l) {
		(((t === Et && (ht === 2 || ht === 9)) || t.cancelPendingCommit !== null) &&
			(_a(t, 0), Tl(t, nt, de, !1)),
			La(t, l),
			((ot & 2) === 0 || t !== Et) &&
				(t === Et &&
					((ot & 2) === 0 && ($l |= l), Ot === 4 && Tl(t, nt, de, !1)),
				qe(t)));
	}
	function Js(t, e, l) {
		if ((ot & 6) !== 0) throw Error(f(327));
		var a = (!l && (e & 127) === 0 && (e & t.expiredLanes) === 0) || Ya(t, e),
			u = a ? py(t, e) : Pc(t, e, !0),
			n = a;
		do {
			if (u === 0) {
				za && !a && Tl(t, e, 0, !1);
				break;
			} else {
				if (((l = t.current.alternate), n && !vy(l))) {
					((u = Pc(t, e, !1)), (n = !1));
					continue;
				}
				if (u === 2) {
					if (((n = e), t.errorRecoveryDisabledLanes & n)) var i = 0;
					else
						((i = t.pendingLanes & -536870913),
							(i = i !== 0 ? i : i & 536870912 ? 536870912 : 0));
					if (i !== 0) {
						e = i;
						t: {
							var r = t;
							u = gu;
							var h = r.current.memoizedState.isDehydrated;
							if ((h && (_a(r, i).flags |= 256), (i = Pc(r, i, !1)), i !== 2)) {
								if (wc && !h) {
									((r.errorRecoveryDisabledLanes |= n), ($l |= n), (u = 4));
									break t;
								}
								((n = le),
									(le = u),
									n !== null &&
										(le === null ? (le = n) : le.push.apply(le, n)));
							}
							u = i;
						}
						if (((n = !1), u !== 2)) continue;
					}
				}
				if (u === 1) {
					(_a(t, 0), Tl(t, e, 0, !0));
					break;
				}
				t: {
					switch (((a = t), (n = u), n)) {
						case 0:
						case 1:
							throw Error(f(345));
						case 4:
							if ((e & 4194048) !== e) break;
						case 6:
							Tl(a, e, de, !pl);
							break t;
						case 2:
							le = null;
							break;
						case 3:
						case 5:
							break;
						default:
							throw Error(f(329));
					}
					if ((e & 62914560) === e && ((u = Cn + 300 - ue()), 10 < u)) {
						if ((Tl(a, e, de, !pl), Xu(a, 0, !0) !== 0)) break t;
						((ll = e),
							(a.timeoutHandle = Ad(
								$s.bind(
									null,
									a,
									l,
									le,
									Nn,
									$c,
									e,
									de,
									$l,
									Aa,
									pl,
									n,
									"Throttled",
									-0,
									0,
								),
								u,
							)));
						break t;
					}
					$s(a, l, le, Nn, $c, e, de, $l, Aa, pl, n, null, -0, 0);
				}
			}
			break;
		} while (!0);
		qe(t);
	}
	function $s(t, e, l, a, u, n, i, r, h, T, O, N, z, _) {
		if (
			((t.timeoutHandle = -1),
			(N = e.subtreeFlags),
			N & 8192 || (N & 16785408) === 16785408)
		) {
			((N = {
				stylesheets: null,
				count: 0,
				imgCount: 0,
				imgBytes: 0,
				suspenseyImages: [],
				waitingForImages: !0,
				waitingForViewTransition: !1,
				unsuspend: Xe,
			}),
				Gs(e, n, N));
			var Y =
				(n & 62914560) === n ? Cn - ue() : (n & 4194048) === n ? Vs - ue() : 0;
			if (((Y = tv(N, Y)), Y !== null)) {
				((ll = n),
					(t.cancelPendingCommit = Y(
						ld.bind(null, t, e, n, l, a, u, i, r, h, O, N, null, z, _),
					)),
					Tl(t, n, i, !T));
				return;
			}
		}
		ld(t, e, n, l, a, u, i, r, h);
	}
	function vy(t) {
		for (var e = t; ; ) {
			var l = e.tag;
			if (
				(l === 0 || l === 11 || l === 15) &&
				e.flags & 16384 &&
				((l = e.updateQueue), l !== null && ((l = l.stores), l !== null))
			)
				for (var a = 0; a < l.length; a++) {
					var u = l[a],
						n = u.getSnapshot;
					u = u.value;
					try {
						if (!ce(n(), u)) return !1;
					} catch {
						return !1;
					}
				}
			if (((l = e.child), e.subtreeFlags & 16384 && l !== null))
				((l.return = e), (e = l));
			else {
				if (e === t) break;
				for (; e.sibling === null; ) {
					if (e.return === null || e.return === t) return !0;
					e = e.return;
				}
				((e.sibling.return = e.return), (e = e.sibling));
			}
		}
		return !0;
	}
	function Tl(t, e, l, a) {
		((e &= ~Jc),
			(e &= ~$l),
			(t.suspendedLanes |= e),
			(t.pingedLanes &= ~e),
			a && (t.warmLanes |= e),
			(a = t.expirationTimes));
		for (var u = e; 0 < u; ) {
			var n = 31 - ie(u),
				i = 1 << n;
			((a[n] = -1), (u &= ~i));
		}
		l !== 0 && ar(t, l, e);
	}
	function xn() {
		return (ot & 6) === 0 ? (Su(0), !1) : !0;
	}
	function Ic() {
		if (at !== null) {
			if (ht === 0) var t = at.return;
			else ((t = at), (Ke = Ll = null), mc(t), (va = null), (eu = 0), (t = at));
			for (; t !== null; ) (Rs(t.alternate, t), (t = t.return));
			at = null;
		}
	}
	function _a(t, e) {
		var l = t.timeoutHandle;
		(l !== -1 && ((t.timeoutHandle = -1), jy(l)),
			(l = t.cancelPendingCommit),
			l !== null && ((t.cancelPendingCommit = null), l()),
			(ll = 0),
			Ic(),
			(Et = t),
			(at = l = Ze(t.current, null)),
			(nt = e),
			(ht = 0),
			(se = null),
			(pl = !1),
			(za = Ya(t, e)),
			(wc = !1),
			(Aa = de = Jc = $l = Sl = Ot = 0),
			(le = gu = null),
			($c = !1),
			(e & 8) !== 0 && (e |= e & 32));
		var a = t.entangledLanes;
		if (a !== 0)
			for (t = t.entanglements, a &= e; 0 < a; ) {
				var u = 31 - ie(a),
					n = 1 << u;
				((e |= t[u]), (a &= ~n));
			}
		return ((el = e), Pu(), l);
	}
	function Ws(t, e) {
		((tt = null),
			(D.H = ru),
			e === ya || e === fn
				? ((e = oo()), (ht = 3))
				: e === lc
					? ((e = oo()), (ht = 4))
					: (ht =
							e === Cc
								? 8
								: e !== null &&
									  typeof e == "object" &&
									  typeof e.then == "function"
									? 6
									: 1),
			(se = e),
			at === null && ((Ot = 1), Tn(t, be(e, t.current))));
	}
	function ks() {
		var t = re.current;
		return t === null
			? !0
			: (nt & 4194048) === nt
				? Ae === null
				: (nt & 62914560) === nt || (nt & 536870912) !== 0
					? t === Ae
					: !1;
	}
	function Fs() {
		var t = D.H;
		return ((D.H = ru), t === null ? ru : t);
	}
	function Is() {
		var t = D.A;
		return ((D.A = my), t);
	}
	function Hn() {
		((Ot = 4),
			pl || ((nt & 4194048) !== nt && re.current !== null) || (za = !0),
			((Sl & 134217727) === 0 && ($l & 134217727) === 0) ||
				Et === null ||
				Tl(Et, nt, de, !1));
	}
	function Pc(t, e, l) {
		var a = ot;
		ot |= 2;
		var u = Fs(),
			n = Is();
		((Et !== t || nt !== e) && ((Nn = null), _a(t, e)), (e = !1));
		var i = Ot;
		t: do
			try {
				if (ht !== 0 && at !== null) {
					var r = at,
						h = se;
					switch (ht) {
						case 8:
							(Ic(), (i = 6));
							break t;
						case 3:
						case 2:
						case 9:
						case 6:
							re.current === null && (e = !0);
							var T = ht;
							if (((ht = 0), (se = null), Ma(t, r, h, T), l && za)) {
								i = 0;
								break t;
							}
							break;
						default:
							((T = ht), (ht = 0), (se = null), Ma(t, r, h, T));
					}
				}
				(gy(), (i = Ot));
				break;
			} catch (O) {
				Ws(t, O);
			}
		while (!0);
		return (
			e && t.shellSuspendCounter++,
			(Ke = Ll = null),
			(ot = a),
			(D.H = u),
			(D.A = n),
			at === null && ((Et = null), (nt = 0), Pu()),
			i
		);
	}
	function gy() {
		for (; at !== null; ) Ps(at);
	}
	function py(t, e) {
		var l = ot;
		ot |= 2;
		var a = Fs(),
			u = Is();
		Et !== t || nt !== e
			? ((Nn = null), (Un = ue() + 500), _a(t, e))
			: (za = Ya(t, e));
		t: do
			try {
				if (ht !== 0 && at !== null) {
					e = at;
					var n = se;
					e: switch (ht) {
						case 1:
							((ht = 0), (se = null), Ma(t, e, n, 1));
							break;
						case 2:
						case 9:
							if (fo(n)) {
								((ht = 0), (se = null), td(e));
								break;
							}
							((e = function () {
								((ht !== 2 && ht !== 9) || Et !== t || (ht = 7), qe(t));
							}),
								n.then(e, e));
							break t;
						case 3:
							ht = 7;
							break t;
						case 4:
							ht = 5;
							break t;
						case 7:
							fo(n)
								? ((ht = 0), (se = null), td(e))
								: ((ht = 0), (se = null), Ma(t, e, n, 7));
							break;
						case 5:
							var i = null;
							switch (at.tag) {
								case 26:
									i = at.memoizedState;
								case 5:
								case 27:
									var r = at;
									if (i ? Ld(i) : r.stateNode.complete) {
										((ht = 0), (se = null));
										var h = r.sibling;
										if (h !== null) at = h;
										else {
											var T = r.return;
											T !== null ? ((at = T), Bn(T)) : (at = null);
										}
										break e;
									}
							}
							((ht = 0), (se = null), Ma(t, e, n, 5));
							break;
						case 6:
							((ht = 0), (se = null), Ma(t, e, n, 6));
							break;
						case 8:
							(Ic(), (Ot = 6));
							break t;
						default:
							throw Error(f(462));
					}
				}
				Sy();
				break;
			} catch (O) {
				Ws(t, O);
			}
		while (!0);
		return (
			(Ke = Ll = null),
			(D.H = a),
			(D.A = u),
			(ot = l),
			at !== null ? 0 : ((Et = null), (nt = 0), Pu(), Ot)
		);
	}
	function Sy() {
		for (; at !== null && !Qh(); ) Ps(at);
	}
	function Ps(t) {
		var e = zs(t.alternate, t, el);
		((t.memoizedProps = t.pendingProps), e === null ? Bn(t) : (at = e));
	}
	function td(t) {
		var e = t,
			l = e.alternate;
		switch (e.tag) {
			case 15:
			case 0:
				e = gs(l, e, e.pendingProps, e.type, void 0, nt);
				break;
			case 11:
				e = gs(l, e, e.pendingProps, e.type.render, e.ref, nt);
				break;
			case 5:
				mc(e);
			default:
				(Rs(l, e), (e = at = Fr(e, el)), (e = zs(l, e, el)));
		}
		((t.memoizedProps = t.pendingProps), e === null ? Bn(t) : (at = e));
	}
	function Ma(t, e, l, a) {
		((Ke = Ll = null), mc(e), (va = null), (eu = 0));
		var u = e.return;
		try {
			if (cy(t, u, e, l, nt)) {
				((Ot = 1), Tn(t, be(l, t.current)), (at = null));
				return;
			}
		} catch (n) {
			if (u !== null) throw ((at = u), n);
			((Ot = 1), Tn(t, be(l, t.current)), (at = null));
			return;
		}
		e.flags & 32768
			? (ct || a === 1
					? (t = !0)
					: za || (nt & 536870912) !== 0
						? (t = !1)
						: ((pl = t = !0),
							(a === 2 || a === 9 || a === 3 || a === 6) &&
								((a = re.current),
								a !== null && a.tag === 13 && (a.flags |= 16384))),
				ed(e, t))
			: Bn(e);
	}
	function Bn(t) {
		var e = t;
		do {
			if ((e.flags & 32768) !== 0) {
				ed(e, pl);
				return;
			}
			t = e.return;
			var l = oy(e.alternate, e, el);
			if (l !== null) {
				at = l;
				return;
			}
			if (((e = e.sibling), e !== null)) {
				at = e;
				return;
			}
			at = e = t;
		} while (e !== null);
		Ot === 0 && (Ot = 5);
	}
	function ed(t, e) {
		do {
			var l = sy(t.alternate, t);
			if (l !== null) {
				((l.flags &= 32767), (at = l));
				return;
			}
			if (
				((l = t.return),
				l !== null &&
					((l.flags |= 32768), (l.subtreeFlags = 0), (l.deletions = null)),
				!e && ((t = t.sibling), t !== null))
			) {
				at = t;
				return;
			}
			at = t = l;
		} while (t !== null);
		((Ot = 6), (at = null));
	}
	function ld(t, e, l, a, u, n, i, r, h) {
		t.cancelPendingCommit = null;
		do jn();
		while (Yt !== 0);
		if ((ot & 6) !== 0) throw Error(f(327));
		if (e !== null) {
			if (e === t.current) throw Error(f(177));
			if (
				((n = e.lanes | e.childLanes),
				(n |= Xi),
				Ih(t, l, n, i, r, h),
				t === Et && ((at = Et = null), (nt = 0)),
				(Ra = e),
				(El = t),
				(ll = l),
				(Wc = n),
				(kc = u),
				(Ks = a),
				(e.subtreeFlags & 10256) !== 0 || (e.flags & 10256) !== 0
					? ((t.callbackNode = null),
						(t.callbackPriority = 0),
						zy(qu, function () {
							return (cd(), null);
						}))
					: ((t.callbackNode = null), (t.callbackPriority = 0)),
				(a = (e.flags & 13878) !== 0),
				(e.subtreeFlags & 13878) !== 0 || a)
			) {
				((a = D.T), (D.T = null), (u = B.p), (B.p = 2), (i = ot), (ot |= 4));
				try {
					dy(t, e, l);
				} finally {
					((ot = i), (B.p = u), (D.T = a));
				}
			}
			((Yt = 1), ad(), ud(), nd());
		}
	}
	function ad() {
		if (Yt === 1) {
			Yt = 0;
			var t = El,
				e = Ra,
				l = (e.flags & 13878) !== 0;
			if ((e.subtreeFlags & 13878) !== 0 || l) {
				((l = D.T), (D.T = null));
				var a = B.p;
				B.p = 2;
				var u = ot;
				ot |= 4;
				try {
					qs(e, t);
					var n = df,
						i = Qr(t.containerInfo),
						r = n.focusedElem,
						h = n.selectionRange;
					if (
						i !== r &&
						r &&
						r.ownerDocument &&
						Xr(r.ownerDocument.documentElement, r)
					) {
						if (h !== null && ji(r)) {
							var T = h.start,
								O = h.end;
							if ((O === void 0 && (O = T), "selectionStart" in r))
								((r.selectionStart = T),
									(r.selectionEnd = Math.min(O, r.value.length)));
							else {
								var N = r.ownerDocument || document,
									z = (N && N.defaultView) || window;
								if (z.getSelection) {
									var _ = z.getSelection(),
										Y = r.textContent.length,
										J = Math.min(h.start, Y),
										St = h.end === void 0 ? J : Math.min(h.end, Y);
									!_.extend && J > St && ((i = St), (St = J), (J = i));
									var p = Gr(r, J),
										y = Gr(r, St);
									if (
										p &&
										y &&
										(_.rangeCount !== 1 ||
											_.anchorNode !== p.node ||
											_.anchorOffset !== p.offset ||
											_.focusNode !== y.node ||
											_.focusOffset !== y.offset)
									) {
										var E = N.createRange();
										(E.setStart(p.node, p.offset),
											_.removeAllRanges(),
											J > St
												? (_.addRange(E), _.extend(y.node, y.offset))
												: (E.setEnd(y.node, y.offset), _.addRange(E)));
									}
								}
							}
						}
						for (N = [], _ = r; (_ = _.parentNode); )
							_.nodeType === 1 &&
								N.push({ element: _, left: _.scrollLeft, top: _.scrollTop });
						for (
							typeof r.focus == "function" && r.focus(), r = 0;
							r < N.length;
							r++
						) {
							var U = N[r];
							((U.element.scrollLeft = U.left), (U.element.scrollTop = U.top));
						}
					}
					(($n = !!sf), (df = sf = null));
				} finally {
					((ot = u), (B.p = a), (D.T = l));
				}
			}
			((t.current = e), (Yt = 2));
		}
	}
	function ud() {
		if (Yt === 2) {
			Yt = 0;
			var t = El,
				e = Ra,
				l = (e.flags & 8772) !== 0;
			if ((e.subtreeFlags & 8772) !== 0 || l) {
				((l = D.T), (D.T = null));
				var a = B.p;
				B.p = 2;
				var u = ot;
				ot |= 4;
				try {
					Ns(t, e.alternate, e);
				} finally {
					((ot = u), (B.p = a), (D.T = l));
				}
			}
			Yt = 3;
		}
	}
	function nd() {
		if (Yt === 4 || Yt === 3) {
			((Yt = 0), Zh());
			var t = El,
				e = Ra,
				l = ll,
				a = Ks;
			(e.subtreeFlags & 10256) !== 0 || (e.flags & 10256) !== 0
				? (Yt = 5)
				: ((Yt = 0), (Ra = El = null), id(t, t.pendingLanes));
			var u = t.pendingLanes;
			if (
				(u === 0 && (bl = null),
				gi(l),
				(e = e.stateNode),
				ne && typeof ne.onCommitFiberRoot == "function")
			)
				try {
					ne.onCommitFiberRoot(qa, e, void 0, (e.current.flags & 128) === 128);
				} catch {}
			if (a !== null) {
				((e = D.T), (u = B.p), (B.p = 2), (D.T = null));
				try {
					for (var n = t.onRecoverableError, i = 0; i < a.length; i++) {
						var r = a[i];
						n(r.value, { componentStack: r.stack });
					}
				} finally {
					((D.T = e), (B.p = u));
				}
			}
			((ll & 3) !== 0 && jn(),
				qe(t),
				(u = t.pendingLanes),
				(l & 261930) !== 0 && (u & 42) !== 0
					? t === Fc
						? pu++
						: ((pu = 0), (Fc = t))
					: (pu = 0),
				Su(0));
		}
	}
	function id(t, e) {
		(t.pooledCacheLanes &= e) === 0 &&
			((e = t.pooledCache), e != null && ((t.pooledCache = null), Pa(e)));
	}
	function jn() {
		return (ad(), ud(), nd(), cd());
	}
	function cd() {
		if (Yt !== 5) return !1;
		var t = El,
			e = Wc;
		Wc = 0;
		var l = gi(ll),
			a = D.T,
			u = B.p;
		try {
			((B.p = 32 > l ? 32 : l), (D.T = null), (l = kc), (kc = null));
			var n = El,
				i = ll;
			if (((Yt = 0), (Ra = El = null), (ll = 0), (ot & 6) !== 0))
				throw Error(f(331));
			var r = ot;
			if (
				((ot |= 4),
				Qs(n.current),
				Ls(n, n.current, i, l),
				(ot = r),
				Su(0, !1),
				ne && typeof ne.onPostCommitFiberRoot == "function")
			)
				try {
					ne.onPostCommitFiberRoot(qa, n);
				} catch {}
			return !0;
		} finally {
			((B.p = u), (D.T = a), id(t, e));
		}
	}
	function fd(t, e, l) {
		((e = be(l, e)),
			(e = Dc(t.stateNode, e, 2)),
			(t = ml(t, e, 2)),
			t !== null && (La(t, 2), qe(t)));
	}
	function mt(t, e, l) {
		if (t.tag === 3) fd(t, t, l);
		else
			for (; e !== null; ) {
				if (e.tag === 3) {
					fd(e, t, l);
					break;
				} else if (e.tag === 1) {
					var a = e.stateNode;
					if (
						typeof e.type.getDerivedStateFromError == "function" ||
						(typeof a.componentDidCatch == "function" &&
							(bl === null || !bl.has(a)))
					) {
						((t = be(l, t)),
							(l = rs(2)),
							(a = ml(e, l, 2)),
							a !== null && (os(l, a, e, t), La(a, 2), qe(a)));
						break;
					}
				}
				e = e.return;
			}
	}
	function tf(t, e, l) {
		var a = t.pingCache;
		if (a === null) {
			a = t.pingCache = new yy();
			var u = new Set();
			a.set(e, u);
		} else ((u = a.get(e)), u === void 0 && ((u = new Set()), a.set(e, u)));
		u.has(l) ||
			((wc = !0), u.add(l), (t = by.bind(null, t, e, l)), e.then(t, t));
	}
	function by(t, e, l) {
		var a = t.pingCache;
		(a !== null && a.delete(e),
			(t.pingedLanes |= t.suspendedLanes & l),
			(t.warmLanes &= ~l),
			Et === t &&
				(nt & l) === l &&
				(Ot === 4 || (Ot === 3 && (nt & 62914560) === nt && 300 > ue() - Cn)
					? (ot & 2) === 0 && _a(t, 0)
					: (Jc |= l),
				Aa === nt && (Aa = 0)),
			qe(t));
	}
	function rd(t, e) {
		(e === 0 && (e = lr()), (t = jl(t, e)), t !== null && (La(t, e), qe(t)));
	}
	function Ey(t) {
		var e = t.memoizedState,
			l = 0;
		(e !== null && (l = e.retryLane), rd(t, l));
	}
	function Ty(t, e) {
		var l = 0;
		switch (t.tag) {
			case 31:
			case 13:
				var a = t.stateNode,
					u = t.memoizedState;
				u !== null && (l = u.retryLane);
				break;
			case 19:
				a = t.stateNode;
				break;
			case 22:
				a = t.stateNode._retryCache;
				break;
			default:
				throw Error(f(314));
		}
		(a !== null && a.delete(e), rd(t, l));
	}
	function zy(t, e) {
		return hi(t, e);
	}
	var qn = null,
		Oa = null,
		ef = !1,
		Yn = !1,
		lf = !1,
		zl = 0;
	function qe(t) {
		(t !== Oa &&
			t.next === null &&
			(Oa === null ? (qn = Oa = t) : (Oa = Oa.next = t)),
			(Yn = !0),
			ef || ((ef = !0), Ry()));
	}
	function Su(t, e) {
		if (!lf && Yn) {
			lf = !0;
			do
				for (var l = !1, a = qn; a !== null; ) {
					if (t !== 0) {
						var u = a.pendingLanes;
						if (u === 0) var n = 0;
						else {
							var i = a.suspendedLanes,
								r = a.pingedLanes;
							((n = (1 << (31 - ie(42 | t) + 1)) - 1),
								(n &= u & ~(i & ~r)),
								(n = n & 201326741 ? (n & 201326741) | 1 : n ? n | 2 : 0));
						}
						n !== 0 && ((l = !0), hd(a, n));
					} else
						((n = nt),
							(n = Xu(
								a,
								a === Et ? n : 0,
								a.cancelPendingCommit !== null || a.timeoutHandle !== -1,
							)),
							(n & 3) === 0 || Ya(a, n) || ((l = !0), hd(a, n)));
					a = a.next;
				}
			while (l);
			lf = !1;
		}
	}
	function Ay() {
		od();
	}
	function od() {
		Yn = ef = !1;
		var t = 0;
		zl !== 0 && By() && (t = zl);
		for (var e = ue(), l = null, a = qn; a !== null; ) {
			var u = a.next,
				n = sd(a, e);
			(n === 0
				? ((a.next = null),
					l === null ? (qn = u) : (l.next = u),
					u === null && (Oa = l))
				: ((l = a), (t !== 0 || (n & 3) !== 0) && (Yn = !0)),
				(a = u));
		}
		((Yt !== 0 && Yt !== 5) || Su(t), zl !== 0 && (zl = 0));
	}
	function sd(t, e) {
		for (
			var l = t.suspendedLanes,
				a = t.pingedLanes,
				u = t.expirationTimes,
				n = t.pendingLanes & -62914561;
			0 < n;
		) {
			var i = 31 - ie(n),
				r = 1 << i,
				h = u[i];
			(h === -1
				? ((r & l) === 0 || (r & a) !== 0) && (u[i] = Fh(r, e))
				: h <= e && (t.expiredLanes |= r),
				(n &= ~r));
		}
		if (
			((e = Et),
			(l = nt),
			(l = Xu(
				t,
				t === e ? l : 0,
				t.cancelPendingCommit !== null || t.timeoutHandle !== -1,
			)),
			(a = t.callbackNode),
			l === 0 ||
				(t === e && (ht === 2 || ht === 9)) ||
				t.cancelPendingCommit !== null)
		)
			return (
				a !== null && a !== null && mi(a),
				(t.callbackNode = null),
				(t.callbackPriority = 0)
			);
		if ((l & 3) === 0 || Ya(t, l)) {
			if (((e = l & -l), e === t.callbackPriority)) return e;
			switch ((a !== null && mi(a), gi(l))) {
				case 2:
				case 8:
					l = tr;
					break;
				case 32:
					l = qu;
					break;
				case 268435456:
					l = er;
					break;
				default:
					l = qu;
			}
			return (
				(a = dd.bind(null, t)),
				(l = hi(l, a)),
				(t.callbackPriority = e),
				(t.callbackNode = l),
				e
			);
		}
		return (
			a !== null && a !== null && mi(a),
			(t.callbackPriority = 2),
			(t.callbackNode = null),
			2
		);
	}
	function dd(t, e) {
		if (Yt !== 0 && Yt !== 5)
			return ((t.callbackNode = null), (t.callbackPriority = 0), null);
		var l = t.callbackNode;
		if (jn() && t.callbackNode !== l) return null;
		var a = nt;
		return (
			(a = Xu(
				t,
				t === Et ? a : 0,
				t.cancelPendingCommit !== null || t.timeoutHandle !== -1,
			)),
			a === 0
				? null
				: (Js(t, a, e),
					sd(t, ue()),
					t.callbackNode != null && t.callbackNode === l
						? dd.bind(null, t)
						: null)
		);
	}
	function hd(t, e) {
		if (jn()) return null;
		Js(t, e, !0);
	}
	function Ry() {
		qy(function () {
			(ot & 6) !== 0 ? hi(Pf, Ay) : od();
		});
	}
	function af() {
		if (zl === 0) {
			var t = ha;
			(t === 0 && ((t = Yu), (Yu <<= 1), (Yu & 261888) === 0 && (Yu = 256)),
				(zl = t));
		}
		return zl;
	}
	function md(t) {
		return t == null || typeof t == "symbol" || typeof t == "boolean"
			? null
			: typeof t == "function"
				? t
				: Ku("" + t);
	}
	function yd(t, e) {
		var l = e.ownerDocument.createElement("input");
		return (
			(l.name = e.name),
			(l.value = e.value),
			t.id && l.setAttribute("form", t.id),
			e.parentNode.insertBefore(l, e),
			(t = new FormData(t)),
			l.parentNode.removeChild(l),
			t
		);
	}
	function _y(t, e, l, a, u) {
		if (e === "submit" && l && l.stateNode === u) {
			var n = md((u[Ft] || null).action),
				i = a.submitter;
			i &&
				((e = (e = i[Ft] || null)
					? md(e.formAction)
					: i.getAttribute("formAction")),
				e !== null && ((n = e), (i = null)));
			var r = new Wu("action", "action", null, a, u);
			t.push({
				event: r,
				listeners: [
					{
						instance: null,
						listener: function () {
							if (a.defaultPrevented) {
								if (zl !== 0) {
									var h = i ? yd(u, i) : new FormData(u);
									zc(
										l,
										{ pending: !0, data: h, method: u.method, action: n },
										null,
										h,
									);
								}
							} else
								typeof n == "function" &&
									(r.preventDefault(),
									(h = i ? yd(u, i) : new FormData(u)),
									zc(
										l,
										{ pending: !0, data: h, method: u.method, action: n },
										n,
										h,
									));
						},
						currentTarget: u,
					},
				],
			});
		}
	}
	for (var uf = 0; uf < Gi.length; uf++) {
		var nf = Gi[uf],
			My = nf.toLowerCase(),
			Oy = nf[0].toUpperCase() + nf.slice(1);
		Me(My, "on" + Oy);
	}
	(Me(Kr, "onAnimationEnd"),
		Me(wr, "onAnimationIteration"),
		Me(Jr, "onAnimationStart"),
		Me("dblclick", "onDoubleClick"),
		Me("focusin", "onFocus"),
		Me("focusout", "onBlur"),
		Me(Vm, "onTransitionRun"),
		Me(Km, "onTransitionStart"),
		Me(wm, "onTransitionCancel"),
		Me($r, "onTransitionEnd"),
		Pl("onMouseEnter", ["mouseout", "mouseover"]),
		Pl("onMouseLeave", ["mouseout", "mouseover"]),
		Pl("onPointerEnter", ["pointerout", "pointerover"]),
		Pl("onPointerLeave", ["pointerout", "pointerover"]),
		Nl(
			"onChange",
			"change click focusin focusout input keydown keyup selectionchange".split(
				" ",
			),
		),
		Nl(
			"onSelect",
			"focusout contextmenu dragend focusin keydown keyup mousedown mouseup selectionchange".split(
				" ",
			),
		),
		Nl("onBeforeInput", ["compositionend", "keypress", "textInput", "paste"]),
		Nl(
			"onCompositionEnd",
			"compositionend focusout keydown keypress keyup mousedown".split(" "),
		),
		Nl(
			"onCompositionStart",
			"compositionstart focusout keydown keypress keyup mousedown".split(" "),
		),
		Nl(
			"onCompositionUpdate",
			"compositionupdate focusout keydown keypress keyup mousedown".split(" "),
		));
	var bu =
			"abort canplay canplaythrough durationchange emptied encrypted ended error loadeddata loadedmetadata loadstart pause play playing progress ratechange resize seeked seeking stalled suspend timeupdate volumechange waiting".split(
				" ",
			),
		Dy = new Set(
			"beforetoggle cancel close invalid load scroll scrollend toggle"
				.split(" ")
				.concat(bu),
		);
	function vd(t, e) {
		e = (e & 4) !== 0;
		for (var l = 0; l < t.length; l++) {
			var a = t[l],
				u = a.event;
			a = a.listeners;
			t: {
				var n = void 0;
				if (e)
					for (var i = a.length - 1; 0 <= i; i--) {
						var r = a[i],
							h = r.instance,
							T = r.currentTarget;
						if (((r = r.listener), h !== n && u.isPropagationStopped()))
							break t;
						((n = r), (u.currentTarget = T));
						try {
							n(u);
						} catch (O) {
							Iu(O);
						}
						((u.currentTarget = null), (n = h));
					}
				else
					for (i = 0; i < a.length; i++) {
						if (
							((r = a[i]),
							(h = r.instance),
							(T = r.currentTarget),
							(r = r.listener),
							h !== n && u.isPropagationStopped())
						)
							break t;
						((n = r), (u.currentTarget = T));
						try {
							n(u);
						} catch (O) {
							Iu(O);
						}
						((u.currentTarget = null), (n = h));
					}
			}
		}
	}
	function ut(t, e) {
		var l = e[pi];
		l === void 0 && (l = e[pi] = new Set());
		var a = t + "__bubble";
		l.has(a) || (gd(e, t, 2, !1), l.add(a));
	}
	function cf(t, e, l) {
		var a = 0;
		(e && (a |= 4), gd(l, t, a, e));
	}
	var Ln = "_reactListening" + Math.random().toString(36).slice(2);
	function ff(t) {
		if (!t[Ln]) {
			((t[Ln] = !0),
				rr.forEach(function (l) {
					l !== "selectionchange" && (Dy.has(l) || cf(l, !1, t), cf(l, !0, t));
				}));
			var e = t.nodeType === 9 ? t : t.ownerDocument;
			e === null || e[Ln] || ((e[Ln] = !0), cf("selectionchange", !1, e));
		}
	}
	function gd(t, e, l, a) {
		switch (wd(e)) {
			case 2:
				var u = av;
				break;
			case 8:
				u = uv;
				break;
			default:
				u = zf;
		}
		((l = u.bind(null, e, l, t)),
			(u = void 0),
			!Mi ||
				(e !== "touchstart" && e !== "touchmove" && e !== "wheel") ||
				(u = !0),
			a
				? u !== void 0
					? t.addEventListener(e, l, { capture: !0, passive: u })
					: t.addEventListener(e, l, !0)
				: u !== void 0
					? t.addEventListener(e, l, { passive: u })
					: t.addEventListener(e, l, !1));
	}
	function rf(t, e, l, a, u) {
		var n = a;
		if ((e & 1) === 0 && (e & 2) === 0 && a !== null)
			t: for (;;) {
				if (a === null) return;
				var i = a.tag;
				if (i === 3 || i === 4) {
					var r = a.stateNode.containerInfo;
					if (r === u) break;
					if (i === 4)
						for (i = a.return; i !== null; ) {
							var h = i.tag;
							if ((h === 3 || h === 4) && i.stateNode.containerInfo === u)
								return;
							i = i.return;
						}
					for (; r !== null; ) {
						if (((i = kl(r)), i === null)) return;
						if (((h = i.tag), h === 5 || h === 6 || h === 26 || h === 27)) {
							a = n = i;
							continue t;
						}
						r = r.parentNode;
					}
				}
				a = a.return;
			}
		Er(function () {
			var T = n,
				O = Ri(l),
				N = [];
			t: {
				var z = Wr.get(t);
				if (z !== void 0) {
					var _ = Wu,
						Y = t;
					switch (t) {
						case "keypress":
							if (Ju(l) === 0) break t;
						case "keydown":
						case "keyup":
							_ = Tm;
							break;
						case "focusin":
							((Y = "focus"), (_ = Ui));
							break;
						case "focusout":
							((Y = "blur"), (_ = Ui));
							break;
						case "beforeblur":
						case "afterblur":
							_ = Ui;
							break;
						case "click":
							if (l.button === 2) break t;
						case "auxclick":
						case "dblclick":
						case "mousedown":
						case "mousemove":
						case "mouseup":
						case "mouseout":
						case "mouseover":
						case "contextmenu":
							_ = Ar;
							break;
						case "drag":
						case "dragend":
						case "dragenter":
						case "dragexit":
						case "dragleave":
						case "dragover":
						case "dragstart":
						case "drop":
							_ = om;
							break;
						case "touchcancel":
						case "touchend":
						case "touchmove":
						case "touchstart":
							_ = Rm;
							break;
						case Kr:
						case wr:
						case Jr:
							_ = hm;
							break;
						case $r:
							_ = Mm;
							break;
						case "scroll":
						case "scrollend":
							_ = fm;
							break;
						case "wheel":
							_ = Dm;
							break;
						case "copy":
						case "cut":
						case "paste":
							_ = ym;
							break;
						case "gotpointercapture":
						case "lostpointercapture":
						case "pointercancel":
						case "pointerdown":
						case "pointermove":
						case "pointerout":
						case "pointerover":
						case "pointerup":
							_ = _r;
							break;
						case "toggle":
						case "beforetoggle":
							_ = Um;
					}
					var J = (e & 4) !== 0,
						St = !J && (t === "scroll" || t === "scrollend"),
						p = J ? (z !== null ? z + "Capture" : null) : z;
					J = [];
					for (var y = T, E; y !== null; ) {
						var U = y;
						if (
							((E = U.stateNode),
							(U = U.tag),
							(U !== 5 && U !== 26 && U !== 27) ||
								E === null ||
								p === null ||
								((U = Qa(y, p)), U != null && J.push(Eu(y, U, E))),
							St)
						)
							break;
						y = y.return;
					}
					0 < J.length &&
						((z = new _(z, Y, null, l, O)), N.push({ event: z, listeners: J }));
				}
			}
			if ((e & 7) === 0) {
				t: {
					if (
						((z = t === "mouseover" || t === "pointerover"),
						(_ = t === "mouseout" || t === "pointerout"),
						z &&
							l !== Ai &&
							(Y = l.relatedTarget || l.fromElement) &&
							(kl(Y) || Y[Wl]))
					)
						break t;
					if (
						(_ || z) &&
						((z =
							O.window === O
								? O
								: (z = O.ownerDocument)
									? z.defaultView || z.parentWindow
									: window),
						_
							? ((Y = l.relatedTarget || l.toElement),
								(_ = T),
								(Y = Y ? kl(Y) : null),
								Y !== null &&
									((St = m(Y)),
									(J = Y.tag),
									Y !== St || (J !== 5 && J !== 27 && J !== 6)) &&
									(Y = null))
							: ((_ = null), (Y = T)),
						_ !== Y)
					) {
						if (
							((J = Ar),
							(U = "onMouseLeave"),
							(p = "onMouseEnter"),
							(y = "mouse"),
							(t === "pointerout" || t === "pointerover") &&
								((J = _r),
								(U = "onPointerLeave"),
								(p = "onPointerEnter"),
								(y = "pointer")),
							(St = _ == null ? z : Xa(_)),
							(E = Y == null ? z : Xa(Y)),
							(z = new J(U, y + "leave", _, l, O)),
							(z.target = St),
							(z.relatedTarget = E),
							(U = null),
							kl(O) === T &&
								((J = new J(p, y + "enter", Y, l, O)),
								(J.target = E),
								(J.relatedTarget = St),
								(U = J)),
							(St = U),
							_ && Y)
						)
							e: {
								for (J = Cy, p = _, y = Y, E = 0, U = p; U; U = J(U)) E++;
								U = 0;
								for (var Z = y; Z; Z = J(Z)) U++;
								for (; 0 < E - U; ) ((p = J(p)), E--);
								for (; 0 < U - E; ) ((y = J(y)), U--);
								for (; E--; ) {
									if (p === y || (y !== null && p === y.alternate)) {
										J = p;
										break e;
									}
									((p = J(p)), (y = J(y)));
								}
								J = null;
							}
						else J = null;
						(_ !== null && pd(N, z, _, J, !1),
							Y !== null && St !== null && pd(N, St, Y, J, !0));
					}
				}
				t: {
					if (
						((z = T ? Xa(T) : window),
						(_ = z.nodeName && z.nodeName.toLowerCase()),
						_ === "select" || (_ === "input" && z.type === "file"))
					)
						var ft = Hr;
					else if (Nr(z))
						if (Br) ft = Xm;
						else {
							ft = Lm;
							var Q = Ym;
						}
					else
						((_ = z.nodeName),
							!_ ||
							_.toLowerCase() !== "input" ||
							(z.type !== "checkbox" && z.type !== "radio")
								? T && zi(T.elementType) && (ft = Hr)
								: (ft = Gm));
					if (ft && (ft = ft(t, T))) {
						xr(N, ft, l, O);
						break t;
					}
					(Q && Q(t, z, T),
						t === "focusout" &&
							T &&
							z.type === "number" &&
							T.memoizedProps.value != null &&
							Ti(z, "number", z.value));
				}
				switch (((Q = T ? Xa(T) : window), t)) {
					case "focusin":
						(Nr(Q) || Q.contentEditable === "true") &&
							((na = Q), (qi = T), (ka = null));
						break;
					case "focusout":
						ka = qi = na = null;
						break;
					case "mousedown":
						Yi = !0;
						break;
					case "contextmenu":
					case "mouseup":
					case "dragend":
						((Yi = !1), Zr(N, l, O));
						break;
					case "selectionchange":
						if (Zm) break;
					case "keydown":
					case "keyup":
						Zr(N, l, O);
				}
				var et;
				if (xi)
					t: {
						switch (t) {
							case "compositionstart":
								var it = "onCompositionStart";
								break t;
							case "compositionend":
								it = "onCompositionEnd";
								break t;
							case "compositionupdate":
								it = "onCompositionUpdate";
								break t;
						}
						it = void 0;
					}
				else
					ua
						? Cr(t, l) && (it = "onCompositionEnd")
						: t === "keydown" &&
							l.keyCode === 229 &&
							(it = "onCompositionStart");
				(it &&
					(Mr &&
						l.locale !== "ko" &&
						(ua || it !== "onCompositionStart"
							? it === "onCompositionEnd" && ua && (et = Tr())
							: ((cl = O),
								(Oi = "value" in cl ? cl.value : cl.textContent),
								(ua = !0))),
					(Q = Gn(T, it)),
					0 < Q.length &&
						((it = new Rr(it, t, null, l, O)),
						N.push({ event: it, listeners: Q }),
						et
							? (it.data = et)
							: ((et = Ur(l)), et !== null && (it.data = et)))),
					(et = xm ? Hm(t, l) : Bm(t, l)) &&
						((it = Gn(T, "onBeforeInput")),
						0 < it.length &&
							((Q = new Rr("onBeforeInput", "beforeinput", null, l, O)),
							N.push({ event: Q, listeners: it }),
							(Q.data = et))),
					_y(N, t, T, l, O));
			}
			vd(N, e);
		});
	}
	function Eu(t, e, l) {
		return { instance: t, listener: e, currentTarget: l };
	}
	function Gn(t, e) {
		for (var l = e + "Capture", a = []; t !== null; ) {
			var u = t,
				n = u.stateNode;
			if (
				((u = u.tag),
				(u !== 5 && u !== 26 && u !== 27) ||
					n === null ||
					((u = Qa(t, l)),
					u != null && a.unshift(Eu(t, u, n)),
					(u = Qa(t, e)),
					u != null && a.push(Eu(t, u, n))),
				t.tag === 3)
			)
				return a;
			t = t.return;
		}
		return [];
	}
	function Cy(t) {
		if (t === null) return null;
		do t = t.return;
		while (t && t.tag !== 5 && t.tag !== 27);
		return t || null;
	}
	function pd(t, e, l, a, u) {
		for (var n = e._reactName, i = []; l !== null && l !== a; ) {
			var r = l,
				h = r.alternate,
				T = r.stateNode;
			if (((r = r.tag), h !== null && h === a)) break;
			((r !== 5 && r !== 26 && r !== 27) ||
				T === null ||
				((h = T),
				u
					? ((T = Qa(l, n)), T != null && i.unshift(Eu(l, T, h)))
					: u || ((T = Qa(l, n)), T != null && i.push(Eu(l, T, h)))),
				(l = l.return));
		}
		i.length !== 0 && t.push({ event: e, listeners: i });
	}
	var Uy = /\r\n?/g,
		Ny = /\u0000|\uFFFD/g;
	function Sd(t) {
		return (typeof t == "string" ? t : "" + t)
			.replace(
				Uy,
				`
`,
			)
			.replace(Ny, "");
	}
	function bd(t, e) {
		return ((e = Sd(e)), Sd(t) === e);
	}
	function pt(t, e, l, a, u, n) {
		switch (l) {
			case "children":
				typeof a == "string"
					? e === "body" || (e === "textarea" && a === "") || ea(t, a)
					: (typeof a == "number" || typeof a == "bigint") &&
						e !== "body" &&
						ea(t, "" + a);
				break;
			case "className":
				Zu(t, "class", a);
				break;
			case "tabIndex":
				Zu(t, "tabindex", a);
				break;
			case "dir":
			case "role":
			case "viewBox":
			case "width":
			case "height":
				Zu(t, l, a);
				break;
			case "style":
				Sr(t, a, n);
				break;
			case "data":
				if (e !== "object") {
					Zu(t, "data", a);
					break;
				}
			case "src":
			case "href":
				if (a === "" && (e !== "a" || l !== "href")) {
					t.removeAttribute(l);
					break;
				}
				if (
					a == null ||
					typeof a == "function" ||
					typeof a == "symbol" ||
					typeof a == "boolean"
				) {
					t.removeAttribute(l);
					break;
				}
				((a = Ku("" + a)), t.setAttribute(l, a));
				break;
			case "action":
			case "formAction":
				if (typeof a == "function") {
					t.setAttribute(
						l,
						"javascript:throw new Error('A React form was unexpectedly submitted. If you called form.submit() manually, consider using form.requestSubmit() instead. If you\\'re trying to use event.stopPropagation() in a submit event handler, consider also calling event.preventDefault().')",
					);
					break;
				} else
					typeof n == "function" &&
						(l === "formAction"
							? (e !== "input" && pt(t, e, "name", u.name, u, null),
								pt(t, e, "formEncType", u.formEncType, u, null),
								pt(t, e, "formMethod", u.formMethod, u, null),
								pt(t, e, "formTarget", u.formTarget, u, null))
							: (pt(t, e, "encType", u.encType, u, null),
								pt(t, e, "method", u.method, u, null),
								pt(t, e, "target", u.target, u, null)));
				if (a == null || typeof a == "symbol" || typeof a == "boolean") {
					t.removeAttribute(l);
					break;
				}
				((a = Ku("" + a)), t.setAttribute(l, a));
				break;
			case "onClick":
				a != null && (t.onclick = Xe);
				break;
			case "onScroll":
				a != null && ut("scroll", t);
				break;
			case "onScrollEnd":
				a != null && ut("scrollend", t);
				break;
			case "dangerouslySetInnerHTML":
				if (a != null) {
					if (typeof a != "object" || !("__html" in a)) throw Error(f(61));
					if (((l = a.__html), l != null)) {
						if (u.children != null) throw Error(f(60));
						t.innerHTML = l;
					}
				}
				break;
			case "multiple":
				t.multiple = a && typeof a != "function" && typeof a != "symbol";
				break;
			case "muted":
				t.muted = a && typeof a != "function" && typeof a != "symbol";
				break;
			case "suppressContentEditableWarning":
			case "suppressHydrationWarning":
			case "defaultValue":
			case "defaultChecked":
			case "innerHTML":
			case "ref":
				break;
			case "autoFocus":
				break;
			case "xlinkHref":
				if (
					a == null ||
					typeof a == "function" ||
					typeof a == "boolean" ||
					typeof a == "symbol"
				) {
					t.removeAttribute("xlink:href");
					break;
				}
				((l = Ku("" + a)),
					t.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", l));
				break;
			case "contentEditable":
			case "spellCheck":
			case "draggable":
			case "value":
			case "autoReverse":
			case "externalResourcesRequired":
			case "focusable":
			case "preserveAlpha":
				a != null && typeof a != "function" && typeof a != "symbol"
					? t.setAttribute(l, "" + a)
					: t.removeAttribute(l);
				break;
			case "inert":
			case "allowFullScreen":
			case "async":
			case "autoPlay":
			case "controls":
			case "default":
			case "defer":
			case "disabled":
			case "disablePictureInPicture":
			case "disableRemotePlayback":
			case "formNoValidate":
			case "hidden":
			case "loop":
			case "noModule":
			case "noValidate":
			case "open":
			case "playsInline":
			case "readOnly":
			case "required":
			case "reversed":
			case "scoped":
			case "seamless":
			case "itemScope":
				a && typeof a != "function" && typeof a != "symbol"
					? t.setAttribute(l, "")
					: t.removeAttribute(l);
				break;
			case "capture":
			case "download":
				a === !0
					? t.setAttribute(l, "")
					: a !== !1 &&
						  a != null &&
						  typeof a != "function" &&
						  typeof a != "symbol"
						? t.setAttribute(l, a)
						: t.removeAttribute(l);
				break;
			case "cols":
			case "rows":
			case "size":
			case "span":
				a != null &&
				typeof a != "function" &&
				typeof a != "symbol" &&
				!isNaN(a) &&
				1 <= a
					? t.setAttribute(l, a)
					: t.removeAttribute(l);
				break;
			case "rowSpan":
			case "start":
				a == null || typeof a == "function" || typeof a == "symbol" || isNaN(a)
					? t.removeAttribute(l)
					: t.setAttribute(l, a);
				break;
			case "popover":
				(ut("beforetoggle", t), ut("toggle", t), Qu(t, "popover", a));
				break;
			case "xlinkActuate":
				Ge(t, "http://www.w3.org/1999/xlink", "xlink:actuate", a);
				break;
			case "xlinkArcrole":
				Ge(t, "http://www.w3.org/1999/xlink", "xlink:arcrole", a);
				break;
			case "xlinkRole":
				Ge(t, "http://www.w3.org/1999/xlink", "xlink:role", a);
				break;
			case "xlinkShow":
				Ge(t, "http://www.w3.org/1999/xlink", "xlink:show", a);
				break;
			case "xlinkTitle":
				Ge(t, "http://www.w3.org/1999/xlink", "xlink:title", a);
				break;
			case "xlinkType":
				Ge(t, "http://www.w3.org/1999/xlink", "xlink:type", a);
				break;
			case "xmlBase":
				Ge(t, "http://www.w3.org/XML/1998/namespace", "xml:base", a);
				break;
			case "xmlLang":
				Ge(t, "http://www.w3.org/XML/1998/namespace", "xml:lang", a);
				break;
			case "xmlSpace":
				Ge(t, "http://www.w3.org/XML/1998/namespace", "xml:space", a);
				break;
			case "is":
				Qu(t, "is", a);
				break;
			case "innerText":
			case "textContent":
				break;
			default:
				(!(2 < l.length) ||
					(l[0] !== "o" && l[0] !== "O") ||
					(l[1] !== "n" && l[1] !== "N")) &&
					((l = im.get(l) || l), Qu(t, l, a));
		}
	}
	function of(t, e, l, a, u, n) {
		switch (l) {
			case "style":
				Sr(t, a, n);
				break;
			case "dangerouslySetInnerHTML":
				if (a != null) {
					if (typeof a != "object" || !("__html" in a)) throw Error(f(61));
					if (((l = a.__html), l != null)) {
						if (u.children != null) throw Error(f(60));
						t.innerHTML = l;
					}
				}
				break;
			case "children":
				typeof a == "string"
					? ea(t, a)
					: (typeof a == "number" || typeof a == "bigint") && ea(t, "" + a);
				break;
			case "onScroll":
				a != null && ut("scroll", t);
				break;
			case "onScrollEnd":
				a != null && ut("scrollend", t);
				break;
			case "onClick":
				a != null && (t.onclick = Xe);
				break;
			case "suppressContentEditableWarning":
			case "suppressHydrationWarning":
			case "innerHTML":
			case "ref":
				break;
			case "innerText":
			case "textContent":
				break;
			default:
				if (!or.hasOwnProperty(l))
					t: {
						if (
							l[0] === "o" &&
							l[1] === "n" &&
							((u = l.endsWith("Capture")),
							(e = l.slice(2, u ? l.length - 7 : void 0)),
							(n = t[Ft] || null),
							(n = n != null ? n[l] : null),
							typeof n == "function" && t.removeEventListener(e, n, u),
							typeof a == "function")
						) {
							(typeof n != "function" &&
								n !== null &&
								(l in t
									? (t[l] = null)
									: t.hasAttribute(l) && t.removeAttribute(l)),
								t.addEventListener(e, a, u));
							break t;
						}
						l in t
							? (t[l] = a)
							: a === !0
								? t.setAttribute(l, "")
								: Qu(t, l, a);
					}
		}
	}
	function Kt(t, e, l) {
		switch (e) {
			case "div":
			case "span":
			case "svg":
			case "path":
			case "a":
			case "g":
			case "p":
			case "li":
				break;
			case "img":
				(ut("error", t), ut("load", t));
				var a = !1,
					u = !1,
					n;
				for (n in l)
					if (l.hasOwnProperty(n)) {
						var i = l[n];
						if (i != null)
							switch (n) {
								case "src":
									a = !0;
									break;
								case "srcSet":
									u = !0;
									break;
								case "children":
								case "dangerouslySetInnerHTML":
									throw Error(f(137, e));
								default:
									pt(t, e, n, i, l, null);
							}
					}
				(u && pt(t, e, "srcSet", l.srcSet, l, null),
					a && pt(t, e, "src", l.src, l, null));
				return;
			case "input":
				ut("invalid", t);
				var r = (n = i = u = null),
					h = null,
					T = null;
				for (a in l)
					if (l.hasOwnProperty(a)) {
						var O = l[a];
						if (O != null)
							switch (a) {
								case "name":
									u = O;
									break;
								case "type":
									i = O;
									break;
								case "checked":
									h = O;
									break;
								case "defaultChecked":
									T = O;
									break;
								case "value":
									n = O;
									break;
								case "defaultValue":
									r = O;
									break;
								case "children":
								case "dangerouslySetInnerHTML":
									if (O != null) throw Error(f(137, e));
									break;
								default:
									pt(t, e, a, O, l, null);
							}
					}
				yr(t, n, r, h, T, i, u, !1);
				return;
			case "select":
				(ut("invalid", t), (a = i = n = null));
				for (u in l)
					if (l.hasOwnProperty(u) && ((r = l[u]), r != null))
						switch (u) {
							case "value":
								n = r;
								break;
							case "defaultValue":
								i = r;
								break;
							case "multiple":
								a = r;
							default:
								pt(t, e, u, r, l, null);
						}
				((e = n),
					(l = i),
					(t.multiple = !!a),
					e != null ? ta(t, !!a, e, !1) : l != null && ta(t, !!a, l, !0));
				return;
			case "textarea":
				(ut("invalid", t), (n = u = a = null));
				for (i in l)
					if (l.hasOwnProperty(i) && ((r = l[i]), r != null))
						switch (i) {
							case "value":
								a = r;
								break;
							case "defaultValue":
								u = r;
								break;
							case "children":
								n = r;
								break;
							case "dangerouslySetInnerHTML":
								if (r != null) throw Error(f(91));
								break;
							default:
								pt(t, e, i, r, l, null);
						}
				gr(t, a, u, n);
				return;
			case "option":
				for (h in l)
					if (l.hasOwnProperty(h) && ((a = l[h]), a != null))
						switch (h) {
							case "selected":
								t.selected =
									a && typeof a != "function" && typeof a != "symbol";
								break;
							default:
								pt(t, e, h, a, l, null);
						}
				return;
			case "dialog":
				(ut("beforetoggle", t),
					ut("toggle", t),
					ut("cancel", t),
					ut("close", t));
				break;
			case "iframe":
			case "object":
				ut("load", t);
				break;
			case "video":
			case "audio":
				for (a = 0; a < bu.length; a++) ut(bu[a], t);
				break;
			case "image":
				(ut("error", t), ut("load", t));
				break;
			case "details":
				ut("toggle", t);
				break;
			case "embed":
			case "source":
			case "link":
				(ut("error", t), ut("load", t));
			case "area":
			case "base":
			case "br":
			case "col":
			case "hr":
			case "keygen":
			case "meta":
			case "param":
			case "track":
			case "wbr":
			case "menuitem":
				for (T in l)
					if (l.hasOwnProperty(T) && ((a = l[T]), a != null))
						switch (T) {
							case "children":
							case "dangerouslySetInnerHTML":
								throw Error(f(137, e));
							default:
								pt(t, e, T, a, l, null);
						}
				return;
			default:
				if (zi(e)) {
					for (O in l)
						l.hasOwnProperty(O) &&
							((a = l[O]), a !== void 0 && of(t, e, O, a, l, void 0));
					return;
				}
		}
		for (r in l)
			l.hasOwnProperty(r) && ((a = l[r]), a != null && pt(t, e, r, a, l, null));
	}
	function xy(t, e, l, a) {
		switch (e) {
			case "div":
			case "span":
			case "svg":
			case "path":
			case "a":
			case "g":
			case "p":
			case "li":
				break;
			case "input":
				var u = null,
					n = null,
					i = null,
					r = null,
					h = null,
					T = null,
					O = null;
				for (_ in l) {
					var N = l[_];
					if (l.hasOwnProperty(_) && N != null)
						switch (_) {
							case "checked":
								break;
							case "value":
								break;
							case "defaultValue":
								h = N;
							default:
								a.hasOwnProperty(_) || pt(t, e, _, null, a, N);
						}
				}
				for (var z in a) {
					var _ = a[z];
					if (((N = l[z]), a.hasOwnProperty(z) && (_ != null || N != null)))
						switch (z) {
							case "type":
								n = _;
								break;
							case "name":
								u = _;
								break;
							case "checked":
								T = _;
								break;
							case "defaultChecked":
								O = _;
								break;
							case "value":
								i = _;
								break;
							case "defaultValue":
								r = _;
								break;
							case "children":
							case "dangerouslySetInnerHTML":
								if (_ != null) throw Error(f(137, e));
								break;
							default:
								_ !== N && pt(t, e, z, _, a, N);
						}
				}
				Ei(t, i, r, h, T, O, n, u);
				return;
			case "select":
				_ = i = r = z = null;
				for (n in l)
					if (((h = l[n]), l.hasOwnProperty(n) && h != null))
						switch (n) {
							case "value":
								break;
							case "multiple":
								_ = h;
							default:
								a.hasOwnProperty(n) || pt(t, e, n, null, a, h);
						}
				for (u in a)
					if (
						((n = a[u]),
						(h = l[u]),
						a.hasOwnProperty(u) && (n != null || h != null))
					)
						switch (u) {
							case "value":
								z = n;
								break;
							case "defaultValue":
								r = n;
								break;
							case "multiple":
								i = n;
							default:
								n !== h && pt(t, e, u, n, a, h);
						}
				((e = r),
					(l = i),
					(a = _),
					z != null
						? ta(t, !!l, z, !1)
						: !!a != !!l &&
							(e != null ? ta(t, !!l, e, !0) : ta(t, !!l, l ? [] : "", !1)));
				return;
			case "textarea":
				_ = z = null;
				for (r in l)
					if (
						((u = l[r]),
						l.hasOwnProperty(r) && u != null && !a.hasOwnProperty(r))
					)
						switch (r) {
							case "value":
								break;
							case "children":
								break;
							default:
								pt(t, e, r, null, a, u);
						}
				for (i in a)
					if (
						((u = a[i]),
						(n = l[i]),
						a.hasOwnProperty(i) && (u != null || n != null))
					)
						switch (i) {
							case "value":
								z = u;
								break;
							case "defaultValue":
								_ = u;
								break;
							case "children":
								break;
							case "dangerouslySetInnerHTML":
								if (u != null) throw Error(f(91));
								break;
							default:
								u !== n && pt(t, e, i, u, a, n);
						}
				vr(t, z, _);
				return;
			case "option":
				for (var Y in l)
					if (
						((z = l[Y]),
						l.hasOwnProperty(Y) && z != null && !a.hasOwnProperty(Y))
					)
						switch (Y) {
							case "selected":
								t.selected = !1;
								break;
							default:
								pt(t, e, Y, null, a, z);
						}
				for (h in a)
					if (
						((z = a[h]),
						(_ = l[h]),
						a.hasOwnProperty(h) && z !== _ && (z != null || _ != null))
					)
						switch (h) {
							case "selected":
								t.selected =
									z && typeof z != "function" && typeof z != "symbol";
								break;
							default:
								pt(t, e, h, z, a, _);
						}
				return;
			case "img":
			case "link":
			case "area":
			case "base":
			case "br":
			case "col":
			case "embed":
			case "hr":
			case "keygen":
			case "meta":
			case "param":
			case "source":
			case "track":
			case "wbr":
			case "menuitem":
				for (var J in l)
					((z = l[J]),
						l.hasOwnProperty(J) &&
							z != null &&
							!a.hasOwnProperty(J) &&
							pt(t, e, J, null, a, z));
				for (T in a)
					if (
						((z = a[T]),
						(_ = l[T]),
						a.hasOwnProperty(T) && z !== _ && (z != null || _ != null))
					)
						switch (T) {
							case "children":
							case "dangerouslySetInnerHTML":
								if (z != null) throw Error(f(137, e));
								break;
							default:
								pt(t, e, T, z, a, _);
						}
				return;
			default:
				if (zi(e)) {
					for (var St in l)
						((z = l[St]),
							l.hasOwnProperty(St) &&
								z !== void 0 &&
								!a.hasOwnProperty(St) &&
								of(t, e, St, void 0, a, z));
					for (O in a)
						((z = a[O]),
							(_ = l[O]),
							!a.hasOwnProperty(O) ||
								z === _ ||
								(z === void 0 && _ === void 0) ||
								of(t, e, O, z, a, _));
					return;
				}
		}
		for (var p in l)
			((z = l[p]),
				l.hasOwnProperty(p) &&
					z != null &&
					!a.hasOwnProperty(p) &&
					pt(t, e, p, null, a, z));
		for (N in a)
			((z = a[N]),
				(_ = l[N]),
				!a.hasOwnProperty(N) ||
					z === _ ||
					(z == null && _ == null) ||
					pt(t, e, N, z, a, _));
	}
	function Ed(t) {
		switch (t) {
			case "css":
			case "script":
			case "font":
			case "img":
			case "image":
			case "input":
			case "link":
				return !0;
			default:
				return !1;
		}
	}
	function Hy() {
		if (typeof performance.getEntriesByType == "function") {
			for (
				var t = 0, e = 0, l = performance.getEntriesByType("resource"), a = 0;
				a < l.length;
				a++
			) {
				var u = l[a],
					n = u.transferSize,
					i = u.initiatorType,
					r = u.duration;
				if (n && r && Ed(i)) {
					for (i = 0, r = u.responseEnd, a += 1; a < l.length; a++) {
						var h = l[a],
							T = h.startTime;
						if (T > r) break;
						var O = h.transferSize,
							N = h.initiatorType;
						O &&
							Ed(N) &&
							((h = h.responseEnd), (i += O * (h < r ? 1 : (r - T) / (h - T))));
					}
					if ((--a, (e += (8 * (n + i)) / (u.duration / 1e3)), t++, 10 < t))
						break;
				}
			}
			if (0 < t) return e / t / 1e6;
		}
		return navigator.connection &&
			((t = navigator.connection.downlink), typeof t == "number")
			? t
			: 5;
	}
	var sf = null,
		df = null;
	function Xn(t) {
		return t.nodeType === 9 ? t : t.ownerDocument;
	}
	function Td(t) {
		switch (t) {
			case "http://www.w3.org/2000/svg":
				return 1;
			case "http://www.w3.org/1998/Math/MathML":
				return 2;
			default:
				return 0;
		}
	}
	function zd(t, e) {
		if (t === 0)
			switch (e) {
				case "svg":
					return 1;
				case "math":
					return 2;
				default:
					return 0;
			}
		return t === 1 && e === "foreignObject" ? 0 : t;
	}
	function hf(t, e) {
		return (
			t === "textarea" ||
			t === "noscript" ||
			typeof e.children == "string" ||
			typeof e.children == "number" ||
			typeof e.children == "bigint" ||
			(typeof e.dangerouslySetInnerHTML == "object" &&
				e.dangerouslySetInnerHTML !== null &&
				e.dangerouslySetInnerHTML.__html != null)
		);
	}
	var mf = null;
	function By() {
		var t = window.event;
		return t && t.type === "popstate"
			? t === mf
				? !1
				: ((mf = t), !0)
			: ((mf = null), !1);
	}
	var Ad = typeof setTimeout == "function" ? setTimeout : void 0,
		jy = typeof clearTimeout == "function" ? clearTimeout : void 0,
		Rd = typeof Promise == "function" ? Promise : void 0,
		qy =
			typeof queueMicrotask == "function"
				? queueMicrotask
				: typeof Rd < "u"
					? function (t) {
							return Rd.resolve(null).then(t).catch(Yy);
						}
					: Ad;
	function Yy(t) {
		setTimeout(function () {
			throw t;
		});
	}
	function Al(t) {
		return t === "head";
	}
	function _d(t, e) {
		var l = e,
			a = 0;
		do {
			var u = l.nextSibling;
			if ((t.removeChild(l), u && u.nodeType === 8))
				if (((l = u.data), l === "/$" || l === "/&")) {
					if (a === 0) {
						(t.removeChild(u), Na(e));
						return;
					}
					a--;
				} else if (
					l === "$" ||
					l === "$?" ||
					l === "$~" ||
					l === "$!" ||
					l === "&"
				)
					a++;
				else if (l === "html") Tu(t.ownerDocument.documentElement);
				else if (l === "head") {
					((l = t.ownerDocument.head), Tu(l));
					for (var n = l.firstChild; n; ) {
						var i = n.nextSibling,
							r = n.nodeName;
						(n[Ga] ||
							r === "SCRIPT" ||
							r === "STYLE" ||
							(r === "LINK" && n.rel.toLowerCase() === "stylesheet") ||
							l.removeChild(n),
							(n = i));
					}
				} else l === "body" && Tu(t.ownerDocument.body);
			l = u;
		} while (l);
		Na(e);
	}
	function Md(t, e) {
		var l = t;
		t = 0;
		do {
			var a = l.nextSibling;
			if (
				(l.nodeType === 1
					? e
						? ((l._stashedDisplay = l.style.display),
							(l.style.display = "none"))
						: ((l.style.display = l._stashedDisplay || ""),
							l.getAttribute("style") === "" && l.removeAttribute("style"))
					: l.nodeType === 3 &&
						(e
							? ((l._stashedText = l.nodeValue), (l.nodeValue = ""))
							: (l.nodeValue = l._stashedText || "")),
				a && a.nodeType === 8)
			)
				if (((l = a.data), l === "/$")) {
					if (t === 0) break;
					t--;
				} else (l !== "$" && l !== "$?" && l !== "$~" && l !== "$!") || t++;
			l = a;
		} while (l);
	}
	function yf(t) {
		var e = t.firstChild;
		for (e && e.nodeType === 10 && (e = e.nextSibling); e; ) {
			var l = e;
			switch (((e = e.nextSibling), l.nodeName)) {
				case "HTML":
				case "HEAD":
				case "BODY":
					(yf(l), Si(l));
					continue;
				case "SCRIPT":
				case "STYLE":
					continue;
				case "LINK":
					if (l.rel.toLowerCase() === "stylesheet") continue;
			}
			t.removeChild(l);
		}
	}
	function Ly(t, e, l, a) {
		for (; t.nodeType === 1; ) {
			var u = l;
			if (t.nodeName.toLowerCase() !== e.toLowerCase()) {
				if (!a && (t.nodeName !== "INPUT" || t.type !== "hidden")) break;
			} else if (a) {
				if (!t[Ga])
					switch (e) {
						case "meta":
							if (!t.hasAttribute("itemprop")) break;
							return t;
						case "link":
							if (
								((n = t.getAttribute("rel")),
								n === "stylesheet" && t.hasAttribute("data-precedence"))
							)
								break;
							if (
								n !== u.rel ||
								t.getAttribute("href") !==
									(u.href == null || u.href === "" ? null : u.href) ||
								t.getAttribute("crossorigin") !==
									(u.crossOrigin == null ? null : u.crossOrigin) ||
								t.getAttribute("title") !== (u.title == null ? null : u.title)
							)
								break;
							return t;
						case "style":
							if (t.hasAttribute("data-precedence")) break;
							return t;
						case "script":
							if (
								((n = t.getAttribute("src")),
								(n !== (u.src == null ? null : u.src) ||
									t.getAttribute("type") !== (u.type == null ? null : u.type) ||
									t.getAttribute("crossorigin") !==
										(u.crossOrigin == null ? null : u.crossOrigin)) &&
									n &&
									t.hasAttribute("async") &&
									!t.hasAttribute("itemprop"))
							)
								break;
							return t;
						default:
							return t;
					}
			} else if (e === "input" && t.type === "hidden") {
				var n = u.name == null ? null : "" + u.name;
				if (u.type === "hidden" && t.getAttribute("name") === n) return t;
			} else return t;
			if (((t = Re(t.nextSibling)), t === null)) break;
		}
		return null;
	}
	function Gy(t, e, l) {
		if (e === "") return null;
		for (; t.nodeType !== 3; )
			if (
				((t.nodeType !== 1 || t.nodeName !== "INPUT" || t.type !== "hidden") &&
					!l) ||
				((t = Re(t.nextSibling)), t === null)
			)
				return null;
		return t;
	}
	function Od(t, e) {
		for (; t.nodeType !== 8; )
			if (
				((t.nodeType !== 1 || t.nodeName !== "INPUT" || t.type !== "hidden") &&
					!e) ||
				((t = Re(t.nextSibling)), t === null)
			)
				return null;
		return t;
	}
	function vf(t) {
		return t.data === "$?" || t.data === "$~";
	}
	function gf(t) {
		return (
			t.data === "$!" ||
			(t.data === "$?" && t.ownerDocument.readyState !== "loading")
		);
	}
	function Xy(t, e) {
		var l = t.ownerDocument;
		if (t.data === "$~") t._reactRetry = e;
		else if (t.data !== "$?" || l.readyState !== "loading") e();
		else {
			var a = function () {
				(e(), l.removeEventListener("DOMContentLoaded", a));
			};
			(l.addEventListener("DOMContentLoaded", a), (t._reactRetry = a));
		}
	}
	function Re(t) {
		for (; t != null; t = t.nextSibling) {
			var e = t.nodeType;
			if (e === 1 || e === 3) break;
			if (e === 8) {
				if (
					((e = t.data),
					e === "$" ||
						e === "$!" ||
						e === "$?" ||
						e === "$~" ||
						e === "&" ||
						e === "F!" ||
						e === "F")
				)
					break;
				if (e === "/$" || e === "/&") return null;
			}
		}
		return t;
	}
	var pf = null;
	function Dd(t) {
		t = t.nextSibling;
		for (var e = 0; t; ) {
			if (t.nodeType === 8) {
				var l = t.data;
				if (l === "/$" || l === "/&") {
					if (e === 0) return Re(t.nextSibling);
					e--;
				} else
					(l !== "$" && l !== "$!" && l !== "$?" && l !== "$~" && l !== "&") ||
						e++;
			}
			t = t.nextSibling;
		}
		return null;
	}
	function Cd(t) {
		t = t.previousSibling;
		for (var e = 0; t; ) {
			if (t.nodeType === 8) {
				var l = t.data;
				if (l === "$" || l === "$!" || l === "$?" || l === "$~" || l === "&") {
					if (e === 0) return t;
					e--;
				} else (l !== "/$" && l !== "/&") || e++;
			}
			t = t.previousSibling;
		}
		return null;
	}
	function Ud(t, e, l) {
		switch (((e = Xn(l)), t)) {
			case "html":
				if (((t = e.documentElement), !t)) throw Error(f(452));
				return t;
			case "head":
				if (((t = e.head), !t)) throw Error(f(453));
				return t;
			case "body":
				if (((t = e.body), !t)) throw Error(f(454));
				return t;
			default:
				throw Error(f(451));
		}
	}
	function Tu(t) {
		for (var e = t.attributes; e.length; ) t.removeAttributeNode(e[0]);
		Si(t);
	}
	var _e = new Map(),
		Nd = new Set();
	function Qn(t) {
		return typeof t.getRootNode == "function"
			? t.getRootNode()
			: t.nodeType === 9
				? t
				: t.ownerDocument;
	}
	var al = B.d;
	B.d = { f: Qy, r: Zy, D: Vy, C: Ky, L: wy, m: Jy, X: Wy, S: $y, M: ky };
	function Qy() {
		var t = al.f(),
			e = xn();
		return t || e;
	}
	function Zy(t) {
		var e = Fl(t);
		e !== null && e.tag === 5 && e.type === "form" ? Wo(e) : al.r(t);
	}
	var Da = typeof document > "u" ? null : document;
	function xd(t, e, l) {
		var a = Da;
		if (a && typeof e == "string" && e) {
			var u = pe(e);
			((u = 'link[rel="' + t + '"][href="' + u + '"]'),
				typeof l == "string" && (u += '[crossorigin="' + l + '"]'),
				Nd.has(u) ||
					(Nd.add(u),
					(t = { rel: t, crossOrigin: l, href: e }),
					a.querySelector(u) === null &&
						((e = a.createElement("link")),
						Kt(e, "link", t),
						Lt(e),
						a.head.appendChild(e))));
		}
	}
	function Vy(t) {
		(al.D(t), xd("dns-prefetch", t, null));
	}
	function Ky(t, e) {
		(al.C(t, e), xd("preconnect", t, e));
	}
	function wy(t, e, l) {
		al.L(t, e, l);
		var a = Da;
		if (a && t && e) {
			var u = 'link[rel="preload"][as="' + pe(e) + '"]';
			e === "image" && l && l.imageSrcSet
				? ((u += '[imagesrcset="' + pe(l.imageSrcSet) + '"]'),
					typeof l.imageSizes == "string" &&
						(u += '[imagesizes="' + pe(l.imageSizes) + '"]'))
				: (u += '[href="' + pe(t) + '"]');
			var n = u;
			switch (e) {
				case "style":
					n = Ca(t);
					break;
				case "script":
					n = Ua(t);
			}
			_e.has(n) ||
				((t = R(
					{
						rel: "preload",
						href: e === "image" && l && l.imageSrcSet ? void 0 : t,
						as: e,
					},
					l,
				)),
				_e.set(n, t),
				a.querySelector(u) !== null ||
					(e === "style" && a.querySelector(zu(n))) ||
					(e === "script" && a.querySelector(Au(n))) ||
					((e = a.createElement("link")),
					Kt(e, "link", t),
					Lt(e),
					a.head.appendChild(e)));
		}
	}
	function Jy(t, e) {
		al.m(t, e);
		var l = Da;
		if (l && t) {
			var a = e && typeof e.as == "string" ? e.as : "script",
				u =
					'link[rel="modulepreload"][as="' + pe(a) + '"][href="' + pe(t) + '"]',
				n = u;
			switch (a) {
				case "audioworklet":
				case "paintworklet":
				case "serviceworker":
				case "sharedworker":
				case "worker":
				case "script":
					n = Ua(t);
			}
			if (
				!_e.has(n) &&
				((t = R({ rel: "modulepreload", href: t }, e)),
				_e.set(n, t),
				l.querySelector(u) === null)
			) {
				switch (a) {
					case "audioworklet":
					case "paintworklet":
					case "serviceworker":
					case "sharedworker":
					case "worker":
					case "script":
						if (l.querySelector(Au(n))) return;
				}
				((a = l.createElement("link")),
					Kt(a, "link", t),
					Lt(a),
					l.head.appendChild(a));
			}
		}
	}
	function $y(t, e, l) {
		al.S(t, e, l);
		var a = Da;
		if (a && t) {
			var u = Il(a).hoistableStyles,
				n = Ca(t);
			e = e || "default";
			var i = u.get(n);
			if (!i) {
				var r = { loading: 0, preload: null };
				if ((i = a.querySelector(zu(n)))) r.loading = 5;
				else {
					((t = R({ rel: "stylesheet", href: t, "data-precedence": e }, l)),
						(l = _e.get(n)) && Sf(t, l));
					var h = (i = a.createElement("link"));
					(Lt(h),
						Kt(h, "link", t),
						(h._p = new Promise(function (T, O) {
							((h.onload = T), (h.onerror = O));
						})),
						h.addEventListener("load", function () {
							r.loading |= 1;
						}),
						h.addEventListener("error", function () {
							r.loading |= 2;
						}),
						(r.loading |= 4),
						Zn(i, e, a));
				}
				((i = { type: "stylesheet", instance: i, count: 1, state: r }),
					u.set(n, i));
			}
		}
	}
	function Wy(t, e) {
		al.X(t, e);
		var l = Da;
		if (l && t) {
			var a = Il(l).hoistableScripts,
				u = Ua(t),
				n = a.get(u);
			n ||
				((n = l.querySelector(Au(u))),
				n ||
					((t = R({ src: t, async: !0 }, e)),
					(e = _e.get(u)) && bf(t, e),
					(n = l.createElement("script")),
					Lt(n),
					Kt(n, "link", t),
					l.head.appendChild(n)),
				(n = { type: "script", instance: n, count: 1, state: null }),
				a.set(u, n));
		}
	}
	function ky(t, e) {
		al.M(t, e);
		var l = Da;
		if (l && t) {
			var a = Il(l).hoistableScripts,
				u = Ua(t),
				n = a.get(u);
			n ||
				((n = l.querySelector(Au(u))),
				n ||
					((t = R({ src: t, async: !0, type: "module" }, e)),
					(e = _e.get(u)) && bf(t, e),
					(n = l.createElement("script")),
					Lt(n),
					Kt(n, "link", t),
					l.head.appendChild(n)),
				(n = { type: "script", instance: n, count: 1, state: null }),
				a.set(u, n));
		}
	}
	function Hd(t, e, l, a) {
		var u = (u = lt.current) ? Qn(u) : null;
		if (!u) throw Error(f(446));
		switch (t) {
			case "meta":
			case "title":
				return null;
			case "style":
				return typeof l.precedence == "string" && typeof l.href == "string"
					? ((e = Ca(l.href)),
						(l = Il(u).hoistableStyles),
						(a = l.get(e)),
						a ||
							((a = { type: "style", instance: null, count: 0, state: null }),
							l.set(e, a)),
						a)
					: { type: "void", instance: null, count: 0, state: null };
			case "link":
				if (
					l.rel === "stylesheet" &&
					typeof l.href == "string" &&
					typeof l.precedence == "string"
				) {
					t = Ca(l.href);
					var n = Il(u).hoistableStyles,
						i = n.get(t);
					if (
						(i ||
							((u = u.ownerDocument || u),
							(i = {
								type: "stylesheet",
								instance: null,
								count: 0,
								state: { loading: 0, preload: null },
							}),
							n.set(t, i),
							(n = u.querySelector(zu(t))) &&
								!n._p &&
								((i.instance = n), (i.state.loading = 5)),
							_e.has(t) ||
								((l = {
									rel: "preload",
									as: "style",
									href: l.href,
									crossOrigin: l.crossOrigin,
									integrity: l.integrity,
									media: l.media,
									hrefLang: l.hrefLang,
									referrerPolicy: l.referrerPolicy,
								}),
								_e.set(t, l),
								n || Fy(u, t, l, i.state))),
						e && a === null)
					)
						throw Error(f(528, ""));
					return i;
				}
				if (e && a !== null) throw Error(f(529, ""));
				return null;
			case "script":
				return (
					(e = l.async),
					(l = l.src),
					typeof l == "string" &&
					e &&
					typeof e != "function" &&
					typeof e != "symbol"
						? ((e = Ua(l)),
							(l = Il(u).hoistableScripts),
							(a = l.get(e)),
							a ||
								((a = {
									type: "script",
									instance: null,
									count: 0,
									state: null,
								}),
								l.set(e, a)),
							a)
						: { type: "void", instance: null, count: 0, state: null }
				);
			default:
				throw Error(f(444, t));
		}
	}
	function Ca(t) {
		return 'href="' + pe(t) + '"';
	}
	function zu(t) {
		return 'link[rel="stylesheet"][' + t + "]";
	}
	function Bd(t) {
		return R({}, t, { "data-precedence": t.precedence, precedence: null });
	}
	function Fy(t, e, l, a) {
		t.querySelector('link[rel="preload"][as="style"][' + e + "]")
			? (a.loading = 1)
			: ((e = t.createElement("link")),
				(a.preload = e),
				e.addEventListener("load", function () {
					return (a.loading |= 1);
				}),
				e.addEventListener("error", function () {
					return (a.loading |= 2);
				}),
				Kt(e, "link", l),
				Lt(e),
				t.head.appendChild(e));
	}
	function Ua(t) {
		return '[src="' + pe(t) + '"]';
	}
	function Au(t) {
		return "script[async]" + t;
	}
	function jd(t, e, l) {
		if ((e.count++, e.instance === null))
			switch (e.type) {
				case "style":
					var a = t.querySelector('style[data-href~="' + pe(l.href) + '"]');
					if (a) return ((e.instance = a), Lt(a), a);
					var u = R({}, l, {
						"data-href": l.href,
						"data-precedence": l.precedence,
						href: null,
						precedence: null,
					});
					return (
						(a = (t.ownerDocument || t).createElement("style")),
						Lt(a),
						Kt(a, "style", u),
						Zn(a, l.precedence, t),
						(e.instance = a)
					);
				case "stylesheet":
					u = Ca(l.href);
					var n = t.querySelector(zu(u));
					if (n) return ((e.state.loading |= 4), (e.instance = n), Lt(n), n);
					((a = Bd(l)),
						(u = _e.get(u)) && Sf(a, u),
						(n = (t.ownerDocument || t).createElement("link")),
						Lt(n));
					var i = n;
					return (
						(i._p = new Promise(function (r, h) {
							((i.onload = r), (i.onerror = h));
						})),
						Kt(n, "link", a),
						(e.state.loading |= 4),
						Zn(n, l.precedence, t),
						(e.instance = n)
					);
				case "script":
					return (
						(n = Ua(l.src)),
						(u = t.querySelector(Au(n)))
							? ((e.instance = u), Lt(u), u)
							: ((a = l),
								(u = _e.get(n)) && ((a = R({}, l)), bf(a, u)),
								(t = t.ownerDocument || t),
								(u = t.createElement("script")),
								Lt(u),
								Kt(u, "link", a),
								t.head.appendChild(u),
								(e.instance = u))
					);
				case "void":
					return null;
				default:
					throw Error(f(443, e.type));
			}
		else
			e.type === "stylesheet" &&
				(e.state.loading & 4) === 0 &&
				((a = e.instance), (e.state.loading |= 4), Zn(a, l.precedence, t));
		return e.instance;
	}
	function Zn(t, e, l) {
		for (
			var a = l.querySelectorAll(
					'link[rel="stylesheet"][data-precedence],style[data-precedence]',
				),
				u = a.length ? a[a.length - 1] : null,
				n = u,
				i = 0;
			i < a.length;
			i++
		) {
			var r = a[i];
			if (r.dataset.precedence === e) n = r;
			else if (n !== u) break;
		}
		n
			? n.parentNode.insertBefore(t, n.nextSibling)
			: ((e = l.nodeType === 9 ? l.head : l), e.insertBefore(t, e.firstChild));
	}
	function Sf(t, e) {
		(t.crossOrigin == null && (t.crossOrigin = e.crossOrigin),
			t.referrerPolicy == null && (t.referrerPolicy = e.referrerPolicy),
			t.title == null && (t.title = e.title));
	}
	function bf(t, e) {
		(t.crossOrigin == null && (t.crossOrigin = e.crossOrigin),
			t.referrerPolicy == null && (t.referrerPolicy = e.referrerPolicy),
			t.integrity == null && (t.integrity = e.integrity));
	}
	var Vn = null;
	function qd(t, e, l) {
		if (Vn === null) {
			var a = new Map(),
				u = (Vn = new Map());
			u.set(l, a);
		} else ((u = Vn), (a = u.get(l)), a || ((a = new Map()), u.set(l, a)));
		if (a.has(t)) return a;
		for (
			a.set(t, null), l = l.getElementsByTagName(t), u = 0;
			u < l.length;
			u++
		) {
			var n = l[u];
			if (
				!(
					n[Ga] ||
					n[Xt] ||
					(t === "link" && n.getAttribute("rel") === "stylesheet")
				) &&
				n.namespaceURI !== "http://www.w3.org/2000/svg"
			) {
				var i = n.getAttribute(e) || "";
				i = t + i;
				var r = a.get(i);
				r ? r.push(n) : a.set(i, [n]);
			}
		}
		return a;
	}
	function Yd(t, e, l) {
		((t = t.ownerDocument || t),
			t.head.insertBefore(
				l,
				e === "title" ? t.querySelector("head > title") : null,
			));
	}
	function Iy(t, e, l) {
		if (l === 1 || e.itemProp != null) return !1;
		switch (t) {
			case "meta":
			case "title":
				return !0;
			case "style":
				if (
					typeof e.precedence != "string" ||
					typeof e.href != "string" ||
					e.href === ""
				)
					break;
				return !0;
			case "link":
				if (
					typeof e.rel != "string" ||
					typeof e.href != "string" ||
					e.href === "" ||
					e.onLoad ||
					e.onError
				)
					break;
				switch (e.rel) {
					case "stylesheet":
						return (
							(t = e.disabled),
							typeof e.precedence == "string" && t == null
						);
					default:
						return !0;
				}
			case "script":
				if (
					e.async &&
					typeof e.async != "function" &&
					typeof e.async != "symbol" &&
					!e.onLoad &&
					!e.onError &&
					e.src &&
					typeof e.src == "string"
				)
					return !0;
		}
		return !1;
	}
	function Ld(t) {
		return !(t.type === "stylesheet" && (t.state.loading & 3) === 0);
	}
	function Py(t, e, l, a) {
		if (
			l.type === "stylesheet" &&
			(typeof a.media != "string" || matchMedia(a.media).matches !== !1) &&
			(l.state.loading & 4) === 0
		) {
			if (l.instance === null) {
				var u = Ca(a.href),
					n = e.querySelector(zu(u));
				if (n) {
					((e = n._p),
						e !== null &&
							typeof e == "object" &&
							typeof e.then == "function" &&
							(t.count++, (t = Kn.bind(t)), e.then(t, t)),
						(l.state.loading |= 4),
						(l.instance = n),
						Lt(n));
					return;
				}
				((n = e.ownerDocument || e),
					(a = Bd(a)),
					(u = _e.get(u)) && Sf(a, u),
					(n = n.createElement("link")),
					Lt(n));
				var i = n;
				((i._p = new Promise(function (r, h) {
					((i.onload = r), (i.onerror = h));
				})),
					Kt(n, "link", a),
					(l.instance = n));
			}
			(t.stylesheets === null && (t.stylesheets = new Map()),
				t.stylesheets.set(l, e),
				(e = l.state.preload) &&
					(l.state.loading & 3) === 0 &&
					(t.count++,
					(l = Kn.bind(t)),
					e.addEventListener("load", l),
					e.addEventListener("error", l)));
		}
	}
	var Ef = 0;
	function tv(t, e) {
		return (
			t.stylesheets && t.count === 0 && Jn(t, t.stylesheets),
			0 < t.count || 0 < t.imgCount
				? function (l) {
						var a = setTimeout(function () {
							if ((t.stylesheets && Jn(t, t.stylesheets), t.unsuspend)) {
								var n = t.unsuspend;
								((t.unsuspend = null), n());
							}
						}, 6e4 + e);
						0 < t.imgBytes && Ef === 0 && (Ef = 62500 * Hy());
						var u = setTimeout(
							function () {
								if (
									((t.waitingForImages = !1),
									t.count === 0 &&
										(t.stylesheets && Jn(t, t.stylesheets), t.unsuspend))
								) {
									var n = t.unsuspend;
									((t.unsuspend = null), n());
								}
							},
							(t.imgBytes > Ef ? 50 : 800) + e,
						);
						return (
							(t.unsuspend = l),
							function () {
								((t.unsuspend = null), clearTimeout(a), clearTimeout(u));
							}
						);
					}
				: null
		);
	}
	function Kn() {
		if (
			(this.count--,
			this.count === 0 && (this.imgCount === 0 || !this.waitingForImages))
		) {
			if (this.stylesheets) Jn(this, this.stylesheets);
			else if (this.unsuspend) {
				var t = this.unsuspend;
				((this.unsuspend = null), t());
			}
		}
	}
	var wn = null;
	function Jn(t, e) {
		((t.stylesheets = null),
			t.unsuspend !== null &&
				(t.count++,
				(wn = new Map()),
				e.forEach(ev, t),
				(wn = null),
				Kn.call(t)));
	}
	function ev(t, e) {
		if (!(e.state.loading & 4)) {
			var l = wn.get(t);
			if (l) var a = l.get(null);
			else {
				((l = new Map()), wn.set(t, l));
				for (
					var u = t.querySelectorAll(
							"link[data-precedence],style[data-precedence]",
						),
						n = 0;
					n < u.length;
					n++
				) {
					var i = u[n];
					(i.nodeName === "LINK" || i.getAttribute("media") !== "not all") &&
						(l.set(i.dataset.precedence, i), (a = i));
				}
				a && l.set(null, a);
			}
			((u = e.instance),
				(i = u.getAttribute("data-precedence")),
				(n = l.get(i) || a),
				n === a && l.set(null, u),
				l.set(i, u),
				this.count++,
				(a = Kn.bind(this)),
				u.addEventListener("load", a),
				u.addEventListener("error", a),
				n
					? n.parentNode.insertBefore(u, n.nextSibling)
					: ((t = t.nodeType === 9 ? t.head : t),
						t.insertBefore(u, t.firstChild)),
				(e.state.loading |= 4));
		}
	}
	var Ru = {
		$$typeof: $,
		Provider: null,
		Consumer: null,
		_currentValue: W,
		_currentValue2: W,
		_threadCount: 0,
	};
	function lv(t, e, l, a, u, n, i, r, h) {
		((this.tag = 1),
			(this.containerInfo = t),
			(this.pingCache = this.current = this.pendingChildren = null),
			(this.timeoutHandle = -1),
			(this.callbackNode =
				this.next =
				this.pendingContext =
				this.context =
				this.cancelPendingCommit =
					null),
			(this.callbackPriority = 0),
			(this.expirationTimes = yi(-1)),
			(this.entangledLanes =
				this.shellSuspendCounter =
				this.errorRecoveryDisabledLanes =
				this.expiredLanes =
				this.warmLanes =
				this.pingedLanes =
				this.suspendedLanes =
				this.pendingLanes =
					0),
			(this.entanglements = yi(0)),
			(this.hiddenUpdates = yi(null)),
			(this.identifierPrefix = a),
			(this.onUncaughtError = u),
			(this.onCaughtError = n),
			(this.onRecoverableError = i),
			(this.pooledCache = null),
			(this.pooledCacheLanes = 0),
			(this.formState = h),
			(this.incompleteTransitions = new Map()));
	}
	function Gd(t, e, l, a, u, n, i, r, h, T, O, N) {
		return (
			(t = new lv(t, e, l, i, h, T, O, N, r)),
			(e = 1),
			n === !0 && (e |= 24),
			(n = fe(3, null, null, e)),
			(t.current = n),
			(n.stateNode = t),
			(e = Pi()),
			e.refCount++,
			(t.pooledCache = e),
			e.refCount++,
			(n.memoizedState = { element: a, isDehydrated: l, cache: e }),
			ac(n),
			t
		);
	}
	function Xd(t) {
		return t ? ((t = fa), t) : fa;
	}
	function Qd(t, e, l, a, u, n) {
		((u = Xd(u)),
			a.context === null ? (a.context = u) : (a.pendingContext = u),
			(a = hl(e)),
			(a.payload = { element: l }),
			(n = n === void 0 ? null : n),
			n !== null && (a.callback = n),
			(l = ml(t, a, e)),
			l !== null && (ae(l, t, e), au(l, t, e)));
	}
	function Zd(t, e) {
		if (((t = t.memoizedState), t !== null && t.dehydrated !== null)) {
			var l = t.retryLane;
			t.retryLane = l !== 0 && l < e ? l : e;
		}
	}
	function Tf(t, e) {
		(Zd(t, e), (t = t.alternate) && Zd(t, e));
	}
	function Vd(t) {
		if (t.tag === 13 || t.tag === 31) {
			var e = jl(t, 67108864);
			(e !== null && ae(e, t, 67108864), Tf(t, 67108864));
		}
	}
	function Kd(t) {
		if (t.tag === 13 || t.tag === 31) {
			var e = he();
			e = vi(e);
			var l = jl(t, e);
			(l !== null && ae(l, t, e), Tf(t, e));
		}
	}
	var $n = !0;
	function av(t, e, l, a) {
		var u = D.T;
		D.T = null;
		var n = B.p;
		try {
			((B.p = 2), zf(t, e, l, a));
		} finally {
			((B.p = n), (D.T = u));
		}
	}
	function uv(t, e, l, a) {
		var u = D.T;
		D.T = null;
		var n = B.p;
		try {
			((B.p = 8), zf(t, e, l, a));
		} finally {
			((B.p = n), (D.T = u));
		}
	}
	function zf(t, e, l, a) {
		if ($n) {
			var u = Af(a);
			if (u === null) (rf(t, e, a, Wn, l), Jd(t, a));
			else if (iv(u, t, e, l, a)) a.stopPropagation();
			else if ((Jd(t, a), e & 4 && -1 < nv.indexOf(t))) {
				for (; u !== null; ) {
					var n = Fl(u);
					if (n !== null)
						switch (n.tag) {
							case 3:
								if (((n = n.stateNode), n.current.memoizedState.isDehydrated)) {
									var i = Ul(n.pendingLanes);
									if (i !== 0) {
										var r = n;
										for (r.pendingLanes |= 2, r.entangledLanes |= 2; i; ) {
											var h = 1 << (31 - ie(i));
											((r.entanglements[1] |= h), (i &= ~h));
										}
										(qe(n), (ot & 6) === 0 && ((Un = ue() + 500), Su(0)));
									}
								}
								break;
							case 31:
							case 13:
								((r = jl(n, 2)), r !== null && ae(r, n, 2), xn(), Tf(n, 2));
						}
					if (((n = Af(a)), n === null && rf(t, e, a, Wn, l), n === u)) break;
					u = n;
				}
				u !== null && a.stopPropagation();
			} else rf(t, e, a, null, l);
		}
	}
	function Af(t) {
		return ((t = Ri(t)), Rf(t));
	}
	var Wn = null;
	function Rf(t) {
		if (((Wn = null), (t = kl(t)), t !== null)) {
			var e = m(t);
			if (e === null) t = null;
			else {
				var l = e.tag;
				if (l === 13) {
					if (((t = S(e)), t !== null)) return t;
					t = null;
				} else if (l === 31) {
					if (((t = A(e)), t !== null)) return t;
					t = null;
				} else if (l === 3) {
					if (e.stateNode.current.memoizedState.isDehydrated)
						return e.tag === 3 ? e.stateNode.containerInfo : null;
					t = null;
				} else e !== t && (t = null);
			}
		}
		return ((Wn = t), null);
	}
	function wd(t) {
		switch (t) {
			case "beforetoggle":
			case "cancel":
			case "click":
			case "close":
			case "contextmenu":
			case "copy":
			case "cut":
			case "auxclick":
			case "dblclick":
			case "dragend":
			case "dragstart":
			case "drop":
			case "focusin":
			case "focusout":
			case "input":
			case "invalid":
			case "keydown":
			case "keypress":
			case "keyup":
			case "mousedown":
			case "mouseup":
			case "paste":
			case "pause":
			case "play":
			case "pointercancel":
			case "pointerdown":
			case "pointerup":
			case "ratechange":
			case "reset":
			case "resize":
			case "seeked":
			case "submit":
			case "toggle":
			case "touchcancel":
			case "touchend":
			case "touchstart":
			case "volumechange":
			case "change":
			case "selectionchange":
			case "textInput":
			case "compositionstart":
			case "compositionend":
			case "compositionupdate":
			case "beforeblur":
			case "afterblur":
			case "beforeinput":
			case "blur":
			case "fullscreenchange":
			case "focus":
			case "hashchange":
			case "popstate":
			case "select":
			case "selectstart":
				return 2;
			case "drag":
			case "dragenter":
			case "dragexit":
			case "dragleave":
			case "dragover":
			case "mousemove":
			case "mouseout":
			case "mouseover":
			case "pointermove":
			case "pointerout":
			case "pointerover":
			case "scroll":
			case "touchmove":
			case "wheel":
			case "mouseenter":
			case "mouseleave":
			case "pointerenter":
			case "pointerleave":
				return 8;
			case "message":
				switch (Vh()) {
					case Pf:
						return 2;
					case tr:
						return 8;
					case qu:
					case Kh:
						return 32;
					case er:
						return 268435456;
					default:
						return 32;
				}
			default:
				return 32;
		}
	}
	var _f = !1,
		Rl = null,
		_l = null,
		Ml = null,
		_u = new Map(),
		Mu = new Map(),
		Ol = [],
		nv =
			"mousedown mouseup touchcancel touchend touchstart auxclick dblclick pointercancel pointerdown pointerup dragend dragstart drop compositionend compositionstart keydown keypress keyup input textInput copy cut paste click change contextmenu reset".split(
				" ",
			);
	function Jd(t, e) {
		switch (t) {
			case "focusin":
			case "focusout":
				Rl = null;
				break;
			case "dragenter":
			case "dragleave":
				_l = null;
				break;
			case "mouseover":
			case "mouseout":
				Ml = null;
				break;
			case "pointerover":
			case "pointerout":
				_u.delete(e.pointerId);
				break;
			case "gotpointercapture":
			case "lostpointercapture":
				Mu.delete(e.pointerId);
		}
	}
	function Ou(t, e, l, a, u, n) {
		return t === null || t.nativeEvent !== n
			? ((t = {
					blockedOn: e,
					domEventName: l,
					eventSystemFlags: a,
					nativeEvent: n,
					targetContainers: [u],
				}),
				e !== null && ((e = Fl(e)), e !== null && Vd(e)),
				t)
			: ((t.eventSystemFlags |= a),
				(e = t.targetContainers),
				u !== null && e.indexOf(u) === -1 && e.push(u),
				t);
	}
	function iv(t, e, l, a, u) {
		switch (e) {
			case "focusin":
				return ((Rl = Ou(Rl, t, e, l, a, u)), !0);
			case "dragenter":
				return ((_l = Ou(_l, t, e, l, a, u)), !0);
			case "mouseover":
				return ((Ml = Ou(Ml, t, e, l, a, u)), !0);
			case "pointerover":
				var n = u.pointerId;
				return (_u.set(n, Ou(_u.get(n) || null, t, e, l, a, u)), !0);
			case "gotpointercapture":
				return (
					(n = u.pointerId),
					Mu.set(n, Ou(Mu.get(n) || null, t, e, l, a, u)),
					!0
				);
		}
		return !1;
	}
	function $d(t) {
		var e = kl(t.target);
		if (e !== null) {
			var l = m(e);
			if (l !== null) {
				if (((e = l.tag), e === 13)) {
					if (((e = S(l)), e !== null)) {
						((t.blockedOn = e),
							cr(t.priority, function () {
								Kd(l);
							}));
						return;
					}
				} else if (e === 31) {
					if (((e = A(l)), e !== null)) {
						((t.blockedOn = e),
							cr(t.priority, function () {
								Kd(l);
							}));
						return;
					}
				} else if (e === 3 && l.stateNode.current.memoizedState.isDehydrated) {
					t.blockedOn = l.tag === 3 ? l.stateNode.containerInfo : null;
					return;
				}
			}
		}
		t.blockedOn = null;
	}
	function kn(t) {
		if (t.blockedOn !== null) return !1;
		for (var e = t.targetContainers; 0 < e.length; ) {
			var l = Af(t.nativeEvent);
			if (l === null) {
				l = t.nativeEvent;
				var a = new l.constructor(l.type, l);
				((Ai = a), l.target.dispatchEvent(a), (Ai = null));
			} else return ((e = Fl(l)), e !== null && Vd(e), (t.blockedOn = l), !1);
			e.shift();
		}
		return !0;
	}
	function Wd(t, e, l) {
		kn(t) && l.delete(e);
	}
	function cv() {
		((_f = !1),
			Rl !== null && kn(Rl) && (Rl = null),
			_l !== null && kn(_l) && (_l = null),
			Ml !== null && kn(Ml) && (Ml = null),
			_u.forEach(Wd),
			Mu.forEach(Wd));
	}
	function Fn(t, e) {
		t.blockedOn === e &&
			((t.blockedOn = null),
			_f ||
				((_f = !0),
				c.unstable_scheduleCallback(c.unstable_NormalPriority, cv)));
	}
	var In = null;
	function kd(t) {
		In !== t &&
			((In = t),
			c.unstable_scheduleCallback(c.unstable_NormalPriority, function () {
				In === t && (In = null);
				for (var e = 0; e < t.length; e += 3) {
					var l = t[e],
						a = t[e + 1],
						u = t[e + 2];
					if (typeof a != "function") {
						if (Rf(a || l) === null) continue;
						break;
					}
					var n = Fl(l);
					n !== null &&
						(t.splice(e, 3),
						(e -= 3),
						zc(n, { pending: !0, data: u, method: l.method, action: a }, a, u));
				}
			}));
	}
	function Na(t) {
		function e(h) {
			return Fn(h, t);
		}
		(Rl !== null && Fn(Rl, t),
			_l !== null && Fn(_l, t),
			Ml !== null && Fn(Ml, t),
			_u.forEach(e),
			Mu.forEach(e));
		for (var l = 0; l < Ol.length; l++) {
			var a = Ol[l];
			a.blockedOn === t && (a.blockedOn = null);
		}
		for (; 0 < Ol.length && ((l = Ol[0]), l.blockedOn === null); )
			($d(l), l.blockedOn === null && Ol.shift());
		if (((l = (t.ownerDocument || t).$$reactFormReplay), l != null))
			for (a = 0; a < l.length; a += 3) {
				var u = l[a],
					n = l[a + 1],
					i = u[Ft] || null;
				if (typeof n == "function") i || kd(l);
				else if (i) {
					var r = null;
					if (n && n.hasAttribute("formAction")) {
						if (((u = n), (i = n[Ft] || null))) r = i.formAction;
						else if (Rf(u) !== null) continue;
					} else r = i.action;
					(typeof r == "function" ? (l[a + 1] = r) : (l.splice(a, 3), (a -= 3)),
						kd(l));
				}
			}
	}
	function Fd() {
		function t(n) {
			n.canIntercept &&
				n.info === "react-transition" &&
				n.intercept({
					handler: function () {
						return new Promise(function (i) {
							return (u = i);
						});
					},
					focusReset: "manual",
					scroll: "manual",
				});
		}
		function e() {
			(u !== null && (u(), (u = null)), a || setTimeout(l, 20));
		}
		function l() {
			if (!a && !navigation.transition) {
				var n = navigation.currentEntry;
				n &&
					n.url != null &&
					navigation.navigate(n.url, {
						state: n.getState(),
						info: "react-transition",
						history: "replace",
					});
			}
		}
		if (typeof navigation == "object") {
			var a = !1,
				u = null;
			return (
				navigation.addEventListener("navigate", t),
				navigation.addEventListener("navigatesuccess", e),
				navigation.addEventListener("navigateerror", e),
				setTimeout(l, 100),
				function () {
					((a = !0),
						navigation.removeEventListener("navigate", t),
						navigation.removeEventListener("navigatesuccess", e),
						navigation.removeEventListener("navigateerror", e),
						u !== null && (u(), (u = null)));
				}
			);
		}
	}
	function Mf(t) {
		this._internalRoot = t;
	}
	((Pn.prototype.render = Mf.prototype.render =
		function (t) {
			var e = this._internalRoot;
			if (e === null) throw Error(f(409));
			var l = e.current,
				a = he();
			Qd(l, a, t, e, null, null);
		}),
		(Pn.prototype.unmount = Mf.prototype.unmount =
			function () {
				var t = this._internalRoot;
				if (t !== null) {
					this._internalRoot = null;
					var e = t.containerInfo;
					(Qd(t.current, 2, null, t, null, null), xn(), (e[Wl] = null));
				}
			}));
	function Pn(t) {
		this._internalRoot = t;
	}
	Pn.prototype.unstable_scheduleHydration = function (t) {
		if (t) {
			var e = ir();
			t = { blockedOn: null, target: t, priority: e };
			for (var l = 0; l < Ol.length && e !== 0 && e < Ol[l].priority; l++);
			(Ol.splice(l, 0, t), l === 0 && $d(t));
		}
	};
	var Id = o.version;
	if (Id !== "19.2.6") throw Error(f(527, Id, "19.2.6"));
	B.findDOMNode = function (t) {
		var e = t._reactInternals;
		if (e === void 0)
			throw typeof t.render == "function"
				? Error(f(188))
				: ((t = Object.keys(t).join(",")), Error(f(268, t)));
		return (
			(t = v(e)),
			(t = t !== null ? C(t) : null),
			(t = t === null ? null : t.stateNode),
			t
		);
	};
	var fv = {
		bundleType: 0,
		version: "19.2.6",
		rendererPackageName: "react-dom",
		currentDispatcherRef: D,
		reconcilerVersion: "19.2.6",
	};
	if (typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ < "u") {
		var ti = __REACT_DEVTOOLS_GLOBAL_HOOK__;
		if (!ti.isDisabled && ti.supportsFiber)
			try {
				((qa = ti.inject(fv)), (ne = ti));
			} catch {}
	}
	return (
		(Cu.createRoot = function (t, e) {
			if (!d(t)) throw Error(f(299));
			var l = !1,
				a = "",
				u = ns,
				n = is,
				i = cs;
			return (
				e != null &&
					(e.unstable_strictMode === !0 && (l = !0),
					e.identifierPrefix !== void 0 && (a = e.identifierPrefix),
					e.onUncaughtError !== void 0 && (u = e.onUncaughtError),
					e.onCaughtError !== void 0 && (n = e.onCaughtError),
					e.onRecoverableError !== void 0 && (i = e.onRecoverableError)),
				(e = Gd(t, 1, !1, null, null, l, a, null, u, n, i, Fd)),
				(t[Wl] = e.current),
				ff(t),
				new Mf(e)
			);
		}),
		(Cu.hydrateRoot = function (t, e, l) {
			if (!d(t)) throw Error(f(299));
			var a = !1,
				u = "",
				n = ns,
				i = is,
				r = cs,
				h = null;
			return (
				l != null &&
					(l.unstable_strictMode === !0 && (a = !0),
					l.identifierPrefix !== void 0 && (u = l.identifierPrefix),
					l.onUncaughtError !== void 0 && (n = l.onUncaughtError),
					l.onCaughtError !== void 0 && (i = l.onCaughtError),
					l.onRecoverableError !== void 0 && (r = l.onRecoverableError),
					l.formState !== void 0 && (h = l.formState)),
				(e = Gd(t, 1, !0, e, l ?? null, a, u, h, n, i, r, Fd)),
				(e.context = Xd(null)),
				(l = e.current),
				(a = he()),
				(a = vi(a)),
				(u = hl(a)),
				(u.callback = null),
				ml(l, u, a),
				(l = a),
				(e.current.lanes = l),
				La(e, l),
				qe(e),
				(t[Wl] = e.current),
				ff(t),
				new Pn(e)
			);
		}),
		(Cu.version = "19.2.6"),
		Cu
	);
}
var fh;
function Sv() {
	if (fh) return Cf.exports;
	fh = 1;
	function c() {
		if (
			!(
				typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ > "u" ||
				typeof __REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE != "function"
			)
		)
			try {
				__REACT_DEVTOOLS_GLOBAL_HOOK__.checkDCE(c);
			} catch (o) {
				console.error(o);
			}
	}
	return (c(), (Cf.exports = pv()), Cf.exports);
}
var bv = Sv();
const Ev = bh(bv);
/**
 * react-router v7.15.1
 *
 * Copyright (c) Remix Software Inc.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE.md file in the root directory of this source tree.
 *
 * @license MIT
 */ var rh = "popstate";
function oh(c) {
	return (
		typeof c == "object" &&
		c != null &&
		"pathname" in c &&
		"search" in c &&
		"hash" in c &&
		"state" in c &&
		"key" in c
	);
}
function Tv(c = {}) {
	function o(f, d) {
		var v;
		let m = (v = d.state) == null ? void 0 : v.masked,
			{ pathname: S, search: A, hash: b } = m || f.location;
		return qf(
			"",
			{ pathname: S, search: A, hash: b },
			(d.state && d.state.usr) || null,
			(d.state && d.state.key) || "default",
			m
				? {
						pathname: f.location.pathname,
						search: f.location.search,
						hash: f.location.hash,
					}
				: void 0,
		);
	}
	function s(f, d) {
		return typeof d == "string" ? d : xu(d);
	}
	return Av(o, s, null, c);
}
function _t(c, o) {
	if (c === !1 || c === null || typeof c > "u") throw new Error(o);
}
function Ue(c, o) {
	if (!c) {
		typeof console < "u" && console.warn(o);
		try {
			throw new Error(o);
		} catch {}
	}
}
function zv() {
	return Math.random().toString(36).substring(2, 10);
}
function sh(c, o) {
	return {
		usr: c.state,
		key: c.key,
		idx: o,
		masked: c.mask
			? { pathname: c.pathname, search: c.search, hash: c.hash }
			: void 0,
	};
}
function qf(c, o, s = null, f, d) {
	return {
		pathname: typeof c == "string" ? c : c.pathname,
		search: "",
		hash: "",
		...(typeof o == "string" ? xa(o) : o),
		state: s,
		key: (o && o.key) || f || zv(),
		mask: d,
	};
}
function xu({ pathname: c = "/", search: o = "", hash: s = "" }) {
	return (
		o && o !== "?" && (c += o.charAt(0) === "?" ? o : "?" + o),
		s && s !== "#" && (c += s.charAt(0) === "#" ? s : "#" + s),
		c
	);
}
function xa(c) {
	let o = {};
	if (c) {
		let s = c.indexOf("#");
		s >= 0 && ((o.hash = c.substring(s)), (c = c.substring(0, s)));
		let f = c.indexOf("?");
		(f >= 0 && ((o.search = c.substring(f)), (c = c.substring(0, f))),
			c && (o.pathname = c));
	}
	return o;
}
function Av(c, o, s, f = {}) {
	let { window: d = document.defaultView, v5Compat: m = !1 } = f,
		S = d.history,
		A = "POP",
		b = null,
		v = C();
	v == null && ((v = 0), S.replaceState({ ...S.state, idx: v }, ""));
	function C() {
		return (S.state || { idx: null }).idx;
	}
	function R() {
		A = "POP";
		let G = C(),
			j = G == null ? null : G - v;
		((v = G), b && b({ action: A, location: L.location, delta: j }));
	}
	function H(G, j) {
		A = "PUSH";
		let I = oh(G) ? G : qf(L.location, G, j);
		v = C() + 1;
		let $ = sh(I, v),
			yt = L.createHref(I.mask || I);
		try {
			S.pushState($, "", yt);
		} catch (dt) {
			if (dt instanceof DOMException && dt.name === "DataCloneError") throw dt;
			d.location.assign(yt);
		}
		m && b && b({ action: A, location: L.location, delta: 1 });
	}
	function w(G, j) {
		A = "REPLACE";
		let I = oh(G) ? G : qf(L.location, G, j);
		v = C();
		let $ = sh(I, v),
			yt = L.createHref(I.mask || I);
		(S.replaceState($, "", yt),
			m && b && b({ action: A, location: L.location, delta: 0 }));
	}
	function K(G) {
		return Rv(G);
	}
	let L = {
		get action() {
			return A;
		},
		get location() {
			return c(d, S);
		},
		listen(G) {
			if (b) throw new Error("A history only accepts one active listener");
			return (
				d.addEventListener(rh, R),
				(b = G),
				() => {
					(d.removeEventListener(rh, R), (b = null));
				}
			);
		},
		createHref(G) {
			return o(d, G);
		},
		createURL: K,
		encodeLocation(G) {
			let j = K(G);
			return { pathname: j.pathname, search: j.search, hash: j.hash };
		},
		push: H,
		replace: w,
		go(G) {
			return S.go(G);
		},
	};
	return L;
}
function Rv(c, o = !1) {
	let s = "http://localhost";
	(typeof window < "u" &&
		(s =
			window.location.origin !== "null"
				? window.location.origin
				: window.location.href),
		_t(s, "No window.location.(origin|href) available to create URL"));
	let f = typeof c == "string" ? c : xu(c);
	return (
		(f = f.replace(/ $/, "%20")),
		!o && f.startsWith("//") && (f = s + f),
		new URL(f, s)
	);
}
function Eh(c, o, s = "/") {
	return _v(c, o, s, !1);
}
function _v(c, o, s, f, d) {
	let m = typeof o == "string" ? xa(o) : o,
		S = ul(m.pathname || "/", s);
	if (S == null) return null;
	let A = Mv(c),
		b = null,
		v = Lv(S);
	for (let C = 0; b == null && C < A.length; ++C) b = qv(A[C], v, f);
	return b;
}
function Mv(c) {
	let o = Th(c);
	return (Ov(o), o);
}
function Th(c, o = [], s = [], f = "", d = !1) {
	let m = (S, A, b = d, v) => {
		let C = {
			relativePath: v === void 0 ? S.path || "" : v,
			caseSensitive: S.caseSensitive === !0,
			childrenIndex: A,
			route: S,
		};
		if (C.relativePath.startsWith("/")) {
			if (!C.relativePath.startsWith(f) && b) return;
			(_t(
				C.relativePath.startsWith(f),
				`Absolute route path "${C.relativePath}" nested under path "${f}" is not valid. An absolute child route path must start with the combined path of all its parent routes.`,
			),
				(C.relativePath = C.relativePath.slice(f.length)));
		}
		let R = Ce([f, C.relativePath]),
			H = s.concat(C);
		(S.children &&
			S.children.length > 0 &&
			(_t(
				S.index !== !0,
				`Index routes must not have child routes. Please remove all child routes from route path "${R}".`,
			),
			Th(S.children, o, H, R, b)),
			!(S.path == null && !S.index) &&
				o.push({ path: R, score: Bv(R, S.index), routesMeta: H }));
	};
	return (
		c.forEach((S, A) => {
			var b;
			if (S.path === "" || !((b = S.path) != null && b.includes("?"))) m(S, A);
			else for (let v of zh(S.path)) m(S, A, !0, v);
		}),
		o
	);
}
function zh(c) {
	let o = c.split("/");
	if (o.length === 0) return [];
	let [s, ...f] = o,
		d = s.endsWith("?"),
		m = s.replace(/\?$/, "");
	if (f.length === 0) return d ? [m, ""] : [m];
	let S = zh(f.join("/")),
		A = [];
	return (
		A.push(...S.map((b) => (b === "" ? m : [m, b].join("/")))),
		d && A.push(...S),
		A.map((b) => (c.startsWith("/") && b === "" ? "/" : b))
	);
}
function Ov(c) {
	c.sort((o, s) =>
		o.score !== s.score
			? s.score - o.score
			: jv(
					o.routesMeta.map((f) => f.childrenIndex),
					s.routesMeta.map((f) => f.childrenIndex),
				),
	);
}
var Dv = /^:[\w-]+$/,
	Cv = 3,
	Uv = 2,
	Nv = 1,
	xv = 10,
	Hv = -2,
	dh = (c) => c === "*";
function Bv(c, o) {
	let s = c.split("/"),
		f = s.length;
	return (
		s.some(dh) && (f += Hv),
		o && (f += Uv),
		s
			.filter((d) => !dh(d))
			.reduce((d, m) => d + (Dv.test(m) ? Cv : m === "" ? Nv : xv), f)
	);
}
function jv(c, o) {
	return c.length === o.length && c.slice(0, -1).every((f, d) => f === o[d])
		? c[c.length - 1] - o[o.length - 1]
		: 0;
}
function qv(c, o, s = !1) {
	let { routesMeta: f } = c,
		d = {},
		m = "/",
		S = [];
	for (let A = 0; A < f.length; ++A) {
		let b = f[A],
			v = A === f.length - 1,
			C = m === "/" ? o : o.slice(m.length) || "/",
			R = ui(
				{ path: b.relativePath, caseSensitive: b.caseSensitive, end: v },
				C,
			),
			H = b.route;
		if (
			(!R &&
				v &&
				s &&
				!f[f.length - 1].route.index &&
				(R = ui(
					{ path: b.relativePath, caseSensitive: b.caseSensitive, end: !1 },
					C,
				)),
			!R)
		)
			return null;
		(Object.assign(d, R.params),
			S.push({
				params: d,
				pathname: Ce([m, R.pathname]),
				pathnameBase: Zv(Ce([m, R.pathnameBase])),
				route: H,
			}),
			R.pathnameBase !== "/" && (m = Ce([m, R.pathnameBase])));
	}
	return S;
}
function ui(c, o) {
	typeof c == "string" && (c = { path: c, caseSensitive: !1, end: !0 });
	let [s, f] = Yv(c.path, c.caseSensitive, c.end),
		d = o.match(s);
	if (!d) return null;
	let m = d[0],
		S = m.replace(/(.)\/+$/, "$1"),
		A = d.slice(1);
	return {
		params: f.reduce((v, { paramName: C, isOptional: R }, H) => {
			if (C === "*") {
				let K = A[H] || "";
				S = m.slice(0, m.length - K.length).replace(/(.)\/+$/, "$1");
			}
			const w = A[H];
			return (
				R && !w ? (v[C] = void 0) : (v[C] = (w || "").replace(/%2F/g, "/")),
				v
			);
		}, {}),
		pathname: m,
		pathnameBase: S,
		pattern: c,
	};
}
function Yv(c, o = !1, s = !0) {
	Ue(
		c === "*" || !c.endsWith("*") || c.endsWith("/*"),
		`Route path "${c}" will be treated as if it were "${c.replace(/\*$/, "/*")}" because the \`*\` character must always follow a \`/\` in the pattern. To get rid of this warning, please change the route path to "${c.replace(/\*$/, "/*")}".`,
	);
	let f = [],
		d =
			"^" +
			c
				.replace(/\/*\*?$/, "")
				.replace(/^\/*/, "/")
				.replace(/[\\.*+^${}|()[\]]/g, "\\$&")
				.replace(/\/:([\w-]+)(\?)?/g, (S, A, b, v, C) => {
					if ((f.push({ paramName: A, isOptional: b != null }), b)) {
						let R = C.charAt(v + S.length);
						return R && R !== "/" ? "/([^\\/]*)" : "(?:/([^\\/]*))?";
					}
					return "/([^\\/]+)";
				})
				.replace(/\/([\w-]+)\?(\/|$)/g, "(/$1)?$2");
	return (
		c.endsWith("*")
			? (f.push({ paramName: "*" }),
				(d += c === "*" || c === "/*" ? "(.*)$" : "(?:\\/(.+)|\\/*)$"))
			: s
				? (d += "\\/*$")
				: c !== "" && c !== "/" && (d += "(?:(?=\\/|$))"),
		[new RegExp(d, o ? void 0 : "i"), f]
	);
}
function Lv(c) {
	try {
		return c
			.split("/")
			.map((o) => decodeURIComponent(o).replace(/\//g, "%2F"))
			.join("/");
	} catch (o) {
		return (
			Ue(
				!1,
				`The URL path "${c}" could not be decoded because it is a malformed URL segment. This is probably due to a bad percent encoding (${o}).`,
			),
			c
		);
	}
}
function ul(c, o) {
	if (o === "/") return c;
	if (!c.toLowerCase().startsWith(o.toLowerCase())) return null;
	let s = o.endsWith("/") ? o.length - 1 : o.length,
		f = c.charAt(s);
	return f && f !== "/" ? null : c.slice(s) || "/";
}
var Gv = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i;
function Xv(c, o = "/") {
	let {
			pathname: s,
			search: f = "",
			hash: d = "",
		} = typeof c == "string" ? xa(c) : c,
		m;
	return (
		s
			? ((s = Ah(s)),
				s.startsWith("/") ? (m = hh(s.substring(1), "/")) : (m = hh(s, o)))
			: (m = o),
		{ pathname: m, search: Vv(f), hash: Kv(d) }
	);
}
function hh(c, o) {
	let s = ni(o).split("/");
	return (
		c.split("/").forEach((d) => {
			d === ".." ? s.length > 1 && s.pop() : d !== "." && s.push(d);
		}),
		s.length > 1 ? s.join("/") : "/"
	);
}
function Hf(c, o, s, f) {
	return `Cannot include a '${c}' character in a manually specified \`to.${o}\` field [${JSON.stringify(f)}].  Please separate it out to the \`to.${s}\` field. Alternatively you may provide the full path as a string in <Link to="..."> and the router will parse it for you.`;
}
function Qv(c) {
	return c.filter(
		(o, s) => s === 0 || (o.route.path && o.route.path.length > 0),
	);
}
function Qf(c) {
	let o = Qv(c);
	return o.map((s, f) => (f === o.length - 1 ? s.pathname : s.pathnameBase));
}
function ii(c, o, s, f = !1) {
	let d;
	typeof c == "string"
		? (d = xa(c))
		: ((d = { ...c }),
			_t(
				!d.pathname || !d.pathname.includes("?"),
				Hf("?", "pathname", "search", d),
			),
			_t(
				!d.pathname || !d.pathname.includes("#"),
				Hf("#", "pathname", "hash", d),
			),
			_t(!d.search || !d.search.includes("#"), Hf("#", "search", "hash", d)));
	let m = c === "" || d.pathname === "",
		S = m ? "/" : d.pathname,
		A;
	if (S == null) A = s;
	else {
		let R = o.length - 1;
		if (!f && S.startsWith("..")) {
			let H = S.split("/");
			for (; H[0] === ".."; ) (H.shift(), (R -= 1));
			d.pathname = H.join("/");
		}
		A = R >= 0 ? o[R] : "/";
	}
	let b = Xv(d, A),
		v = S && S !== "/" && S.endsWith("/"),
		C = (m || S === ".") && s.endsWith("/");
	return (!b.pathname.endsWith("/") && (v || C) && (b.pathname += "/"), b);
}
var Ah = (c) => c.replace(/\/\/+/g, "/"),
	Ce = (c) => Ah(c.join("/")),
	ni = (c) => c.replace(/\/+$/, ""),
	Zv = (c) => ni(c).replace(/^\/*/, "/"),
	Vv = (c) => (!c || c === "?" ? "" : c.startsWith("?") ? c : "?" + c),
	Kv = (c) => (!c || c === "#" ? "" : c.startsWith("#") ? c : "#" + c),
	wv = class {
		constructor(c, o, s, f = !1) {
			((this.status = c),
				(this.statusText = o || ""),
				(this.internal = f),
				s instanceof Error
					? ((this.data = s.toString()), (this.error = s))
					: (this.data = s));
		}
	};
function Jv(c) {
	return (
		c != null &&
		typeof c.status == "number" &&
		typeof c.statusText == "string" &&
		typeof c.internal == "boolean" &&
		"data" in c
	);
}
function $v(c) {
	let o = c.map((s) => s.route.path).filter(Boolean);
	return Ce(o) || "/";
}
var Rh =
	typeof window < "u" &&
	typeof window.document < "u" &&
	typeof window.document.createElement < "u";
function _h(c, o) {
	let s = c;
	if (typeof s != "string" || !Gv.test(s))
		return { absoluteURL: void 0, isExternal: !1, to: s };
	let f = s,
		d = !1;
	if (Rh)
		try {
			let m = new URL(window.location.href),
				S = s.startsWith("//") ? new URL(m.protocol + s) : new URL(s),
				A = ul(S.pathname, o);
			S.origin === m.origin && A != null
				? (s = A + S.search + S.hash)
				: (d = !0);
		} catch {
			Ue(
				!1,
				`<Link to="${s}"> contains an invalid URL which will probably break when clicked - please update to a valid URL path.`,
			);
		}
	return { absoluteURL: f, isExternal: d, to: s };
}
Object.getOwnPropertyNames(Object.prototype).sort().join("\0");
var Mh = ["POST", "PUT", "PATCH", "DELETE"];
new Set(Mh);
var Wv = ["GET", ...Mh];
new Set(Wv);
var Ha = M.createContext(null);
Ha.displayName = "DataRouter";
var ci = M.createContext(null);
ci.displayName = "DataRouterState";
var Oh = M.createContext(!1);
function kv() {
	return M.useContext(Oh);
}
var Dh = M.createContext({ isTransitioning: !1 });
Dh.displayName = "ViewTransition";
var Fv = M.createContext(new Map());
Fv.displayName = "Fetchers";
var Iv = M.createContext(null);
Iv.displayName = "Await";
var me = M.createContext(null);
me.displayName = "Navigation";
var Hu = M.createContext(null);
Hu.displayName = "Location";
var Ye = M.createContext({ outlet: null, matches: [], isDataRoute: !1 });
Ye.displayName = "Route";
var Zf = M.createContext(null);
Zf.displayName = "RouteError";
var Ch = "REACT_ROUTER_ERROR",
	Pv = "REDIRECT",
	t0 = "ROUTE_ERROR_RESPONSE";
function e0(c) {
	if (c.startsWith(`${Ch}:${Pv}:{`))
		try {
			let o = JSON.parse(c.slice(28));
			if (
				typeof o == "object" &&
				o &&
				typeof o.status == "number" &&
				typeof o.statusText == "string" &&
				typeof o.location == "string" &&
				typeof o.reloadDocument == "boolean" &&
				typeof o.replace == "boolean"
			)
				return o;
		} catch {}
}
function l0(c) {
	if (c.startsWith(`${Ch}:${t0}:{`))
		try {
			let o = JSON.parse(c.slice(40));
			if (
				typeof o == "object" &&
				o &&
				typeof o.status == "number" &&
				typeof o.statusText == "string"
			)
				return new wv(o.status, o.statusText, o.data);
		} catch {}
}
function a0(c, { relative: o } = {}) {
	_t(
		Ba(),
		"useHref() may be used only in the context of a <Router> component.",
	);
	let { basename: s, navigator: f } = M.useContext(me),
		{ hash: d, pathname: m, search: S } = Bu(c, { relative: o }),
		A = m;
	return (
		s !== "/" && (A = m === "/" ? s : Ce([s, m])),
		f.createHref({ pathname: A, search: S, hash: d })
	);
}
function Ba() {
	return M.useContext(Hu) != null;
}
function Le() {
	return (
		_t(
			Ba(),
			"useLocation() may be used only in the context of a <Router> component.",
		),
		M.useContext(Hu).location
	);
}
var Uh =
	"You should call navigate() in a React.useEffect(), not when your component is first rendered.";
function Nh(c) {
	M.useContext(me).static || M.useLayoutEffect(c);
}
function xh() {
	let { isDataRoute: c } = M.useContext(Ye);
	return c ? v0() : u0();
}
function u0() {
	_t(
		Ba(),
		"useNavigate() may be used only in the context of a <Router> component.",
	);
	let c = M.useContext(Ha),
		{ basename: o, navigator: s } = M.useContext(me),
		{ matches: f } = M.useContext(Ye),
		{ pathname: d } = Le(),
		m = JSON.stringify(Qf(f)),
		S = M.useRef(!1);
	return (
		Nh(() => {
			S.current = !0;
		}),
		M.useCallback(
			(b, v = {}) => {
				if ((Ue(S.current, Uh), !S.current)) return;
				if (typeof b == "number") {
					s.go(b);
					return;
				}
				let C = ii(b, JSON.parse(m), d, v.relative === "path");
				(c == null &&
					o !== "/" &&
					(C.pathname = C.pathname === "/" ? o : Ce([o, C.pathname])),
					(v.replace ? s.replace : s.push)(C, v.state, v));
			},
			[o, s, m, d, c],
		)
	);
}
M.createContext(null);
function Bu(c, { relative: o } = {}) {
	let { matches: s } = M.useContext(Ye),
		{ pathname: f } = Le(),
		d = JSON.stringify(Qf(s));
	return M.useMemo(() => ii(c, JSON.parse(d), f, o === "path"), [c, d, f, o]);
}
function n0(c, o) {
	return Hh(c, o);
}
function Hh(c, o, s) {
	var G;
	_t(
		Ba(),
		"useRoutes() may be used only in the context of a <Router> component.",
	);
	let { navigator: f } = M.useContext(me),
		{ matches: d } = M.useContext(Ye),
		m = d[d.length - 1],
		S = m ? m.params : {},
		A = m ? m.pathname : "/",
		b = m ? m.pathnameBase : "/",
		v = m && m.route;
	{
		let j = (v && v.path) || "";
		jh(
			A,
			!v || j.endsWith("*") || j.endsWith("*?"),
			`You rendered descendant <Routes> (or called \`useRoutes()\`) at "${A}" (under <Route path="${j}">) but the parent route path has no trailing "*". This means if you navigate deeper, the parent won't match anymore and therefore the child routes will never render.

Please change the parent <Route path="${j}"> to <Route path="${j === "/" ? "*" : `${j}/*`}">.`,
		);
	}
	let C = Le(),
		R;
	if (o) {
		let j = typeof o == "string" ? xa(o) : o;
		(_t(
			b === "/" || ((G = j.pathname) == null ? void 0 : G.startsWith(b)),
			`When overriding the location using \`<Routes location>\` or \`useRoutes(routes, location)\`, the location pathname must begin with the portion of the URL pathname that was matched by all parent routes. The current pathname base is "${b}" but pathname "${j.pathname}" was given in the \`location\` prop.`,
		),
			(R = j));
	} else R = C;
	let H = R.pathname || "/",
		w = H;
	if (b !== "/") {
		let j = b.replace(/^\//, "").split("/");
		w = "/" + H.replace(/^\//, "").split("/").slice(j.length).join("/");
	}
	let K =
		s && s.state.matches.length
			? s.state.matches.map((j) =>
					Object.assign(j, { route: s.manifest[j.route.id] || j.route }),
				)
			: Eh(c, { pathname: w });
	(Ue(
		v || K != null,
		`No routes matched location "${R.pathname}${R.search}${R.hash}" `,
	),
		Ue(
			K == null ||
				K[K.length - 1].route.element !== void 0 ||
				K[K.length - 1].route.Component !== void 0 ||
				K[K.length - 1].route.lazy !== void 0,
			`Matched leaf route at location "${R.pathname}${R.search}${R.hash}" does not have an element or Component. This means it will render an <Outlet /> with a null value by default resulting in an "empty" page.`,
		));
	let L = o0(
		K &&
			K.map((j) =>
				Object.assign({}, j, {
					params: Object.assign({}, S, j.params),
					pathname: Ce([
						b,
						f.encodeLocation
							? f.encodeLocation(
									j.pathname
										.replace(/%/g, "%25")
										.replace(/\?/g, "%3F")
										.replace(/#/g, "%23"),
								).pathname
							: j.pathname,
					]),
					pathnameBase:
						j.pathnameBase === "/"
							? b
							: Ce([
									b,
									f.encodeLocation
										? f.encodeLocation(
												j.pathnameBase
													.replace(/%/g, "%25")
													.replace(/\?/g, "%3F")
													.replace(/#/g, "%23"),
											).pathname
										: j.pathnameBase,
								]),
				}),
			),
		d,
		s,
	);
	return o && L
		? M.createElement(
				Hu.Provider,
				{
					value: {
						location: {
							pathname: "/",
							search: "",
							hash: "",
							state: null,
							key: "default",
							mask: void 0,
							...R,
						},
						navigationType: "POP",
					},
				},
				L,
			)
		: L;
}
function i0() {
	let c = y0(),
		o = Jv(c)
			? `${c.status} ${c.statusText}`
			: c instanceof Error
				? c.message
				: JSON.stringify(c),
		s = c instanceof Error ? c.stack : null,
		f = "rgba(200,200,200, 0.5)",
		d = { padding: "0.5rem", backgroundColor: f },
		m = { padding: "2px 4px", backgroundColor: f },
		S = null;
	return (
		console.error("Error handled by React Router default ErrorBoundary:", c),
		(S = M.createElement(
			M.Fragment,
			null,
			M.createElement("p", null, "💿 Hey developer 👋"),
			M.createElement(
				"p",
				null,
				"You can provide a way better UX than this when your app throws errors by providing your own ",
				M.createElement("code", { style: m }, "ErrorBoundary"),
				" or",
				" ",
				M.createElement("code", { style: m }, "errorElement"),
				" prop on your route.",
			),
		)),
		M.createElement(
			M.Fragment,
			null,
			M.createElement("h2", null, "Unexpected Application Error!"),
			M.createElement("h3", { style: { fontStyle: "italic" } }, o),
			s ? M.createElement("pre", { style: d }, s) : null,
			S,
		)
	);
}
var c0 = M.createElement(i0, null),
	Bh = class extends M.Component {
		constructor(c) {
			(super(c),
				(this.state = {
					location: c.location,
					revalidation: c.revalidation,
					error: c.error,
				}));
		}
		static getDerivedStateFromError(c) {
			return { error: c };
		}
		static getDerivedStateFromProps(c, o) {
			return o.location !== c.location ||
				(o.revalidation !== "idle" && c.revalidation === "idle")
				? { error: c.error, location: c.location, revalidation: c.revalidation }
				: {
						error: c.error !== void 0 ? c.error : o.error,
						location: o.location,
						revalidation: c.revalidation || o.revalidation,
					};
		}
		componentDidCatch(c, o) {
			this.props.onError
				? this.props.onError(c, o)
				: console.error(
						"React Router caught the following error during render",
						c,
					);
		}
		render() {
			let c = this.state.error;
			if (
				this.context &&
				typeof c == "object" &&
				c &&
				"digest" in c &&
				typeof c.digest == "string"
			) {
				const s = l0(c.digest);
				s && (c = s);
			}
			let o =
				c !== void 0
					? M.createElement(
							Ye.Provider,
							{ value: this.props.routeContext },
							M.createElement(Zf.Provider, {
								value: c,
								children: this.props.component,
							}),
						)
					: this.props.children;
			return this.context ? M.createElement(f0, { error: c }, o) : o;
		}
	};
Bh.contextType = Oh;
var Bf = new WeakMap();
function f0({ children: c, error: o }) {
	let { basename: s } = M.useContext(me);
	if (
		typeof o == "object" &&
		o &&
		"digest" in o &&
		typeof o.digest == "string"
	) {
		let f = e0(o.digest);
		if (f) {
			let d = Bf.get(o);
			if (d) throw d;
			let m = _h(f.location, s);
			if (Rh && !Bf.get(o))
				if (m.isExternal || f.reloadDocument)
					window.location.href = m.absoluteURL || m.to;
				else {
					const S = Promise.resolve().then(() =>
						window.__reactRouterDataRouter.navigate(m.to, {
							replace: f.replace,
						}),
					);
					throw (Bf.set(o, S), S);
				}
			return M.createElement("meta", {
				httpEquiv: "refresh",
				content: `0;url=${m.absoluteURL || m.to}`,
			});
		}
	}
	return c;
}
function r0({ routeContext: c, match: o, children: s }) {
	let f = M.useContext(Ha);
	return (
		f &&
			f.static &&
			f.staticContext &&
			(o.route.errorElement || o.route.ErrorBoundary) &&
			(f.staticContext._deepestRenderedBoundaryId = o.route.id),
		M.createElement(Ye.Provider, { value: c }, s)
	);
}
function o0(c, o = [], s) {
	let f = s == null ? void 0 : s.state;
	if (c == null) {
		if (!f) return null;
		if (f.errors) c = f.matches;
		else if (o.length === 0 && !f.initialized && f.matches.length > 0)
			c = f.matches;
		else return null;
	}
	let d = c,
		m = f == null ? void 0 : f.errors;
	if (m != null) {
		let C = d.findIndex(
			(R) => R.route.id && (m == null ? void 0 : m[R.route.id]) !== void 0,
		);
		(_t(
			C >= 0,
			`Could not find a matching route for errors on route IDs: ${Object.keys(m).join(",")}`,
		),
			(d = d.slice(0, Math.min(d.length, C + 1))));
	}
	let S = !1,
		A = -1;
	if (s && f) {
		S = f.renderFallback;
		for (let C = 0; C < d.length; C++) {
			let R = d[C];
			if (
				((R.route.HydrateFallback || R.route.hydrateFallbackElement) && (A = C),
				R.route.id)
			) {
				let { loaderData: H, errors: w } = f,
					K =
						R.route.loader &&
						!H.hasOwnProperty(R.route.id) &&
						(!w || w[R.route.id] === void 0);
				if (R.route.lazy || K) {
					(s.isStatic && (S = !0),
						A >= 0 ? (d = d.slice(0, A + 1)) : (d = [d[0]]));
					break;
				}
			}
		}
	}
	let b = s == null ? void 0 : s.onError,
		v =
			f && b
				? (C, R) => {
						var H, w;
						b(C, {
							location: f.location,
							params:
								((w = (H = f.matches) == null ? void 0 : H[0]) == null
									? void 0
									: w.params) ?? {},
							pattern: $v(f.matches),
							errorInfo: R,
						});
					}
				: void 0;
	return d.reduceRight((C, R, H) => {
		let w,
			K = !1,
			L = null,
			G = null;
		f &&
			((w = m && R.route.id ? m[R.route.id] : void 0),
			(L = R.route.errorElement || c0),
			S &&
				(A < 0 && H === 0
					? (jh(
							"route-fallback",
							!1,
							"No `HydrateFallback` element provided to render during initial hydration",
						),
						(K = !0),
						(G = null))
					: A === H &&
						((K = !0), (G = R.route.hydrateFallbackElement || null))));
		let j = o.concat(d.slice(0, H + 1)),
			I = () => {
				let $;
				return (
					w
						? ($ = L)
						: K
							? ($ = G)
							: R.route.Component
								? ($ = M.createElement(R.route.Component, null))
								: R.route.element
									? ($ = R.route.element)
									: ($ = C),
					M.createElement(r0, {
						match: R,
						routeContext: { outlet: C, matches: j, isDataRoute: f != null },
						children: $,
					})
				);
			};
		return f && (R.route.ErrorBoundary || R.route.errorElement || H === 0)
			? M.createElement(Bh, {
					location: f.location,
					revalidation: f.revalidation,
					component: L,
					error: w,
					children: I(),
					routeContext: { outlet: null, matches: j, isDataRoute: !0 },
					onError: v,
				})
			: I();
	}, null);
}
function Vf(c) {
	return `${c} must be used within a data router.  See https://reactrouter.com/en/main/routers/picking-a-router.`;
}
function s0(c) {
	let o = M.useContext(Ha);
	return (_t(o, Vf(c)), o);
}
function d0(c) {
	let o = M.useContext(ci);
	return (_t(o, Vf(c)), o);
}
function h0(c) {
	let o = M.useContext(Ye);
	return (_t(o, Vf(c)), o);
}
function Kf(c) {
	let o = h0(c),
		s = o.matches[o.matches.length - 1];
	return (
		_t(
			s.route.id,
			`${c} can only be used on routes that contain a unique "id"`,
		),
		s.route.id
	);
}
function m0() {
	return Kf("useRouteId");
}
function y0() {
	var f;
	let c = M.useContext(Zf),
		o = d0("useRouteError"),
		s = Kf("useRouteError");
	return c !== void 0 ? c : (f = o.errors) == null ? void 0 : f[s];
}
function v0() {
	let { router: c } = s0("useNavigate"),
		o = Kf("useNavigate"),
		s = M.useRef(!1);
	return (
		Nh(() => {
			s.current = !0;
		}),
		M.useCallback(
			async (d, m = {}) => {
				(Ue(s.current, Uh),
					s.current &&
						(typeof d == "number"
							? await c.navigate(d)
							: await c.navigate(d, { fromRouteId: o, ...m })));
			},
			[c, o],
		)
	);
}
var mh = {};
function jh(c, o, s) {
	!o && !mh[c] && ((mh[c] = !0), Ue(!1, s));
}
M.memo(g0);
function g0({
	routes: c,
	manifest: o,
	future: s,
	state: f,
	isStatic: d,
	onError: m,
}) {
	return Hh(c, void 0, { manifest: o, state: f, isStatic: d, onError: m });
}
function yh({ to: c, replace: o, state: s, relative: f }) {
	_t(
		Ba(),
		"<Navigate> may be used only in the context of a <Router> component.",
	);
	let { static: d } = M.useContext(me);
	Ue(
		!d,
		"<Navigate> must not be used on the initial render in a <StaticRouter>. This is a no-op, but you should modify your code so the <Navigate> is only ever rendered in response to some user interaction or state change.",
	);
	let { matches: m } = M.useContext(Ye),
		{ pathname: S } = Le(),
		A = xh(),
		b = ii(c, Qf(m), S, f === "path"),
		v = JSON.stringify(b);
	return (
		M.useEffect(() => {
			A(JSON.parse(v), { replace: o, state: s, relative: f });
		}, [A, v, f, o, s]),
		null
	);
}
function Nu(c) {
	_t(
		!1,
		"A <Route> is only ever to be used as the child of <Routes> element, never rendered directly. Please wrap your <Route> in a <Routes>.",
	);
}
function p0({
	basename: c = "/",
	children: o = null,
	location: s,
	navigationType: f = "POP",
	navigator: d,
	static: m = !1,
	useTransitions: S,
}) {
	_t(
		!Ba(),
		"You cannot render a <Router> inside another <Router>. You should never have more than one in your app.",
	);
	let A = c.replace(/^\/*/, "/"),
		b = M.useMemo(
			() => ({
				basename: A,
				navigator: d,
				static: m,
				useTransitions: S,
				future: {},
			}),
			[A, d, m, S],
		);
	typeof s == "string" && (s = xa(s));
	let {
			pathname: v = "/",
			search: C = "",
			hash: R = "",
			state: H = null,
			key: w = "default",
			mask: K,
		} = s,
		L = M.useMemo(() => {
			let G = ul(v, A);
			return G == null
				? null
				: {
						location: {
							pathname: G,
							search: C,
							hash: R,
							state: H,
							key: w,
							mask: K,
						},
						navigationType: f,
					};
		}, [A, v, C, R, H, w, f, K]);
	return (
		Ue(
			L != null,
			`<Router basename="${A}"> is not able to match the URL "${v}${C}${R}" because it does not start with the basename, so the <Router> won't render anything.`,
		),
		L == null
			? null
			: M.createElement(
					me.Provider,
					{ value: b },
					M.createElement(Hu.Provider, { children: o, value: L }),
				)
	);
}
function S0({ children: c, location: o }) {
	return n0(Yf(c), o);
}
function Yf(c, o = []) {
	let s = [];
	return (
		M.Children.forEach(c, (f, d) => {
			if (!M.isValidElement(f)) return;
			let m = [...o, d];
			if (f.type === M.Fragment) {
				s.push.apply(s, Yf(f.props.children, m));
				return;
			}
			(_t(
				f.type === Nu,
				`[${typeof f.type == "string" ? f.type : f.type.name}] is not a <Route> component. All component children of <Routes> must be a <Route> or <React.Fragment>`,
			),
				_t(
					!f.props.index || !f.props.children,
					"An index route cannot have child routes.",
				));
			let S = {
				id: f.props.id || m.join("-"),
				caseSensitive: f.props.caseSensitive,
				element: f.props.element,
				Component: f.props.Component,
				index: f.props.index,
				path: f.props.path,
				middleware: f.props.middleware,
				loader: f.props.loader,
				action: f.props.action,
				hydrateFallbackElement: f.props.hydrateFallbackElement,
				HydrateFallback: f.props.HydrateFallback,
				errorElement: f.props.errorElement,
				ErrorBoundary: f.props.ErrorBoundary,
				hasErrorBoundary:
					f.props.hasErrorBoundary === !0 ||
					f.props.ErrorBoundary != null ||
					f.props.errorElement != null,
				shouldRevalidate: f.props.shouldRevalidate,
				handle: f.props.handle,
				lazy: f.props.lazy,
			};
			(f.props.children && (S.children = Yf(f.props.children, m)), s.push(S));
		}),
		s
	);
}
var li = "get",
	ai = "application/x-www-form-urlencoded";
function fi(c) {
	return typeof HTMLElement < "u" && c instanceof HTMLElement;
}
function b0(c) {
	return fi(c) && c.tagName.toLowerCase() === "button";
}
function E0(c) {
	return fi(c) && c.tagName.toLowerCase() === "form";
}
function T0(c) {
	return fi(c) && c.tagName.toLowerCase() === "input";
}
function z0(c) {
	return !!(c.metaKey || c.altKey || c.ctrlKey || c.shiftKey);
}
function A0(c, o) {
	return c.button === 0 && (!o || o === "_self") && !z0(c);
}
var ei = null;
function R0() {
	if (ei === null)
		try {
			(new FormData(document.createElement("form"), 0), (ei = !1));
		} catch {
			ei = !0;
		}
	return ei;
}
var _0 = new Set([
	"application/x-www-form-urlencoded",
	"multipart/form-data",
	"text/plain",
]);
function jf(c) {
	return c != null && !_0.has(c)
		? (Ue(
				!1,
				`"${c}" is not a valid \`encType\` for \`<Form>\`/\`<fetcher.Form>\` and will default to "${ai}"`,
			),
			null)
		: c;
}
function M0(c, o) {
	let s, f, d, m, S;
	if (E0(c)) {
		let A = c.getAttribute("action");
		((f = A ? ul(A, o) : null),
			(s = c.getAttribute("method") || li),
			(d = jf(c.getAttribute("enctype")) || ai),
			(m = new FormData(c)));
	} else if (b0(c) || (T0(c) && (c.type === "submit" || c.type === "image"))) {
		let A = c.form;
		if (A == null)
			throw new Error(
				'Cannot submit a <button> or <input type="submit"> without a <form>',
			);
		let b = c.getAttribute("formaction") || A.getAttribute("action");
		if (
			((f = b ? ul(b, o) : null),
			(s = c.getAttribute("formmethod") || A.getAttribute("method") || li),
			(d =
				jf(c.getAttribute("formenctype")) ||
				jf(A.getAttribute("enctype")) ||
				ai),
			(m = new FormData(A, c)),
			!R0())
		) {
			let { name: v, type: C, value: R } = c;
			if (C === "image") {
				let H = v ? `${v}.` : "";
				(m.append(`${H}x`, "0"), m.append(`${H}y`, "0"));
			} else v && m.append(v, R);
		}
	} else {
		if (fi(c))
			throw new Error(
				'Cannot submit element that is not <form>, <button>, or <input type="submit|image">',
			);
		((s = li), (f = null), (d = ai), (S = c));
	}
	return (
		m && d === "text/plain" && ((S = m), (m = void 0)),
		{ action: f, method: s.toLowerCase(), encType: d, formData: m, body: S }
	);
}
Object.getOwnPropertyNames(Object.prototype).sort().join("\0");
function wf(c, o) {
	if (c === !1 || c === null || typeof c > "u") throw new Error(o);
}
function qh(c, o, s, f) {
	let d =
		typeof c == "string"
			? new URL(
					c,
					typeof window > "u"
						? "server://singlefetch/"
						: window.location.origin,
				)
			: c;
	return (
		s
			? d.pathname.endsWith("/")
				? (d.pathname = `${d.pathname}_.${f}`)
				: (d.pathname = `${d.pathname}.${f}`)
			: d.pathname === "/"
				? (d.pathname = `_root.${f}`)
				: o && ul(d.pathname, o) === "/"
					? (d.pathname = `${ni(o)}/_root.${f}`)
					: (d.pathname = `${ni(d.pathname)}.${f}`),
		d
	);
}
async function O0(c, o) {
	if (c.id in o) return o[c.id];
	try {
		let s = await import(c.module);
		return ((o[c.id] = s), s);
	} catch (s) {
		return (
			console.error(
				`Error loading route module \`${c.module}\`, reloading page...`,
			),
			console.error(s),
			window.__reactRouterContext && window.__reactRouterContext.isSpaMode,
			window.location.reload(),
			new Promise(() => {})
		);
	}
}
function D0(c) {
	return c == null
		? !1
		: c.href == null
			? c.rel === "preload" &&
				typeof c.imageSrcSet == "string" &&
				typeof c.imageSizes == "string"
			: typeof c.rel == "string" && typeof c.href == "string";
}
async function C0(c, o, s) {
	let f = await Promise.all(
		c.map(async (d) => {
			let m = o.routes[d.route.id];
			if (m) {
				let S = await O0(m, s);
				return S.links ? S.links() : [];
			}
			return [];
		}),
	);
	return H0(
		f
			.flat(1)
			.filter(D0)
			.filter((d) => d.rel === "stylesheet" || d.rel === "preload")
			.map((d) =>
				d.rel === "stylesheet"
					? { ...d, rel: "prefetch", as: "style" }
					: { ...d, rel: "prefetch" },
			),
	);
}
function vh(c, o, s, f, d, m) {
	let S = (b, v) => (s[v] ? b.route.id !== s[v].route.id : !0),
		A = (b, v) => {
			var C;
			return (
				s[v].pathname !== b.pathname ||
				(((C = s[v].route.path) == null ? void 0 : C.endsWith("*")) &&
					s[v].params["*"] !== b.params["*"])
			);
		};
	return m === "assets"
		? o.filter((b, v) => S(b, v) || A(b, v))
		: m === "data"
			? o.filter((b, v) => {
					var R;
					let C = f.routes[b.route.id];
					if (!C || !C.hasLoader) return !1;
					if (S(b, v) || A(b, v)) return !0;
					if (b.route.shouldRevalidate) {
						let H = b.route.shouldRevalidate({
							currentUrl: new URL(
								d.pathname + d.search + d.hash,
								window.origin,
							),
							currentParams: ((R = s[0]) == null ? void 0 : R.params) || {},
							nextUrl: new URL(c, window.origin),
							nextParams: b.params,
							defaultShouldRevalidate: !0,
						});
						if (typeof H == "boolean") return H;
					}
					return !0;
				})
			: [];
}
function U0(c, o, { includeHydrateFallback: s } = {}) {
	return N0(
		c
			.map((f) => {
				let d = o.routes[f.route.id];
				if (!d) return [];
				let m = [d.module];
				return (
					d.clientActionModule && (m = m.concat(d.clientActionModule)),
					d.clientLoaderModule && (m = m.concat(d.clientLoaderModule)),
					s &&
						d.hydrateFallbackModule &&
						(m = m.concat(d.hydrateFallbackModule)),
					d.imports && (m = m.concat(d.imports)),
					m
				);
			})
			.flat(1),
	);
}
function N0(c) {
	return [...new Set(c)];
}
function x0(c) {
	let o = {},
		s = Object.keys(c).sort();
	for (let f of s) o[f] = c[f];
	return o;
}
function H0(c, o) {
	let s = new Set();
	return (
		new Set(o),
		c.reduce((f, d) => {
			let m = JSON.stringify(x0(d));
			return (s.has(m) || (s.add(m), f.push({ key: m, link: d })), f);
		}, [])
	);
}
function Jf() {
	let c = M.useContext(Ha);
	return (
		wf(
			c,
			"You must render this element inside a <DataRouterContext.Provider> element",
		),
		c
	);
}
function B0() {
	let c = M.useContext(ci);
	return (
		wf(
			c,
			"You must render this element inside a <DataRouterStateContext.Provider> element",
		),
		c
	);
}
var $f = M.createContext(void 0);
$f.displayName = "FrameworkContext";
function Wf() {
	let c = M.useContext($f);
	return (
		wf(c, "You must render this element inside a <HydratedRouter> element"),
		c
	);
}
function j0(c, o) {
	let s = M.useContext($f),
		[f, d] = M.useState(!1),
		[m, S] = M.useState(!1),
		{
			onFocus: A,
			onBlur: b,
			onMouseEnter: v,
			onMouseLeave: C,
			onTouchStart: R,
		} = o,
		H = M.useRef(null);
	(M.useEffect(() => {
		if ((c === "render" && S(!0), c === "viewport")) {
			let L = (j) => {
					j.forEach((I) => {
						S(I.isIntersecting);
					});
				},
				G = new IntersectionObserver(L, { threshold: 0.5 });
			return (
				H.current && G.observe(H.current),
				() => {
					G.disconnect();
				}
			);
		}
	}, [c]),
		M.useEffect(() => {
			if (f) {
				let L = setTimeout(() => {
					S(!0);
				}, 100);
				return () => {
					clearTimeout(L);
				};
			}
		}, [f]));
	let w = () => {
			d(!0);
		},
		K = () => {
			(d(!1), S(!1));
		};
	return s
		? c !== "intent"
			? [m, H, {}]
			: [
					m,
					H,
					{
						onFocus: Uu(A, w),
						onBlur: Uu(b, K),
						onMouseEnter: Uu(v, w),
						onMouseLeave: Uu(C, K),
						onTouchStart: Uu(R, w),
					},
				]
		: [!1, H, {}];
}
function Uu(c, o) {
	return (s) => {
		(c && c(s), s.defaultPrevented || o(s));
	};
}
function q0({ page: c, ...o }) {
	let s = kv(),
		{ router: f } = Jf(),
		d = M.useMemo(() => Eh(f.routes, c, f.basename), [f.routes, c, f.basename]);
	return d
		? s
			? M.createElement(L0, { page: c, matches: d, ...o })
			: M.createElement(G0, { page: c, matches: d, ...o })
		: null;
}
function Y0(c) {
	let { manifest: o, routeModules: s } = Wf(),
		[f, d] = M.useState([]);
	return (
		M.useEffect(() => {
			let m = !1;
			return (
				C0(c, o, s).then((S) => {
					m || d(S);
				}),
				() => {
					m = !0;
				}
			);
		}, [c, o, s]),
		f
	);
}
function L0({ page: c, matches: o, ...s }) {
	let f = Le(),
		{ future: d } = Wf(),
		{ basename: m } = Jf(),
		S = M.useMemo(() => {
			if (c === f.pathname + f.search + f.hash) return [];
			let A = qh(c, m, d.unstable_trailingSlashAwareDataRequests, "rsc"),
				b = !1,
				v = [];
			for (let C of o)
				typeof C.route.shouldRevalidate == "function"
					? (b = !0)
					: v.push(C.route.id);
			return (
				b && v.length > 0 && A.searchParams.set("_routes", v.join(",")),
				[A.pathname + A.search]
			);
		}, [m, d.unstable_trailingSlashAwareDataRequests, c, f, o]);
	return M.createElement(
		M.Fragment,
		null,
		S.map((A) =>
			M.createElement("link", {
				key: A,
				rel: "prefetch",
				as: "fetch",
				href: A,
				...s,
			}),
		),
	);
}
function G0({ page: c, matches: o, ...s }) {
	let f = Le(),
		{ future: d, manifest: m, routeModules: S } = Wf(),
		{ basename: A } = Jf(),
		{ loaderData: b, matches: v } = B0(),
		C = M.useMemo(() => vh(c, o, v, m, f, "data"), [c, o, v, m, f]),
		R = M.useMemo(() => vh(c, o, v, m, f, "assets"), [c, o, v, m, f]),
		H = M.useMemo(() => {
			if (c === f.pathname + f.search + f.hash) return [];
			let L = new Set(),
				G = !1;
			if (
				(o.forEach((I) => {
					var yt;
					let $ = m.routes[I.route.id];
					!$ ||
						!$.hasLoader ||
						((!C.some((dt) => dt.route.id === I.route.id) &&
							I.route.id in b &&
							(yt = S[I.route.id]) != null &&
							yt.shouldRevalidate) ||
						$.hasClientLoader
							? (G = !0)
							: L.add(I.route.id));
				}),
				L.size === 0)
			)
				return [];
			let j = qh(c, A, d.unstable_trailingSlashAwareDataRequests, "data");
			return (
				G &&
					L.size > 0 &&
					j.searchParams.set(
						"_routes",
						o
							.filter((I) => L.has(I.route.id))
							.map((I) => I.route.id)
							.join(","),
					),
				[j.pathname + j.search]
			);
		}, [A, d.unstable_trailingSlashAwareDataRequests, b, f, m, C, o, c, S]),
		w = M.useMemo(() => U0(R, m), [R, m]),
		K = Y0(R);
	return M.createElement(
		M.Fragment,
		null,
		H.map((L) =>
			M.createElement("link", {
				key: L,
				rel: "prefetch",
				as: "fetch",
				href: L,
				...s,
			}),
		),
		w.map((L) =>
			M.createElement("link", { key: L, rel: "modulepreload", href: L, ...s }),
		),
		K.map(({ key: L, link: G }) =>
			M.createElement("link", {
				key: L,
				nonce: s.nonce,
				...G,
				crossOrigin: G.crossOrigin ?? s.crossOrigin,
			}),
		),
	);
}
function X0(...c) {
	return (o) => {
		c.forEach((s) => {
			typeof s == "function" ? s(o) : s != null && (s.current = o);
		});
	};
}
var Q0 =
	typeof window < "u" &&
	typeof window.document < "u" &&
	typeof window.document.createElement < "u";
try {
	Q0 && (window.__reactRouterVersion = "7.15.1");
} catch {}
function Z0({ basename: c, children: o, useTransitions: s, window: f }) {
	let d = M.useRef();
	d.current == null && (d.current = Tv({ window: f, v5Compat: !0 }));
	let m = d.current,
		[S, A] = M.useState({ action: m.action, location: m.location }),
		b = M.useCallback(
			(v) => {
				s === !1 ? A(v) : M.startTransition(() => A(v));
			},
			[s],
		);
	return (
		M.useLayoutEffect(() => m.listen(b), [m, b]),
		M.createElement(p0, {
			basename: c,
			children: o,
			location: S.location,
			navigationType: S.action,
			navigator: m,
			useTransitions: s,
		})
	);
}
var Yh = /^(?:[a-z][a-z0-9+.-]*:|\/\/)/i,
	Lh = M.forwardRef(function (
		{
			onClick: o,
			discover: s = "render",
			prefetch: f = "none",
			relative: d,
			reloadDocument: m,
			replace: S,
			mask: A,
			state: b,
			target: v,
			to: C,
			preventScrollReset: R,
			viewTransition: H,
			defaultShouldRevalidate: w,
			...K
		},
		L,
	) {
		let { basename: G, navigator: j, useTransitions: I } = M.useContext(me),
			$ = typeof C == "string" && Yh.test(C),
			yt = _h(C, G);
		C = yt.to;
		let dt = a0(C, { relative: d }),
			Tt = Le(),
			k = null;
		if (A) {
			let xt = ii(A, [], Tt.mask ? Tt.mask.pathname : "/", !0);
			(G !== "/" &&
				(xt.pathname = xt.pathname === "/" ? G : Ce([G, xt.pathname])),
				(k = j.createHref(xt)));
		}
		let [Dt, wt, Ne] = j0(f, K),
			ye = w0(C, {
				replace: S,
				mask: A,
				state: b,
				target: v,
				preventScrollReset: R,
				relative: d,
				viewTransition: H,
				defaultShouldRevalidate: w,
				useTransitions: I,
			});
		function Jt(xt) {
			(o && o(xt), xt.defaultPrevented || ye(xt));
		}
		let xe = !(yt.isExternal || m),
			ve = M.createElement("a", {
				...K,
				...Ne,
				href: (xe ? k : void 0) || yt.absoluteURL || dt,
				onClick: xe ? Jt : o,
				ref: X0(L, wt),
				target: v,
				"data-discover": !$ && s === "render" ? "true" : void 0,
			});
		return Dt && !$
			? M.createElement(M.Fragment, null, ve, M.createElement(q0, { page: dt }))
			: ve;
	});
Lh.displayName = "Link";
var Lf = M.forwardRef(function (
	{
		"aria-current": o = "page",
		caseSensitive: s = !1,
		className: f = "",
		end: d = !1,
		style: m,
		to: S,
		viewTransition: A,
		children: b,
		...v
	},
	C,
) {
	let R = Bu(S, { relative: v.relative }),
		H = Le(),
		w = M.useContext(ci),
		{ navigator: K, basename: L } = M.useContext(me),
		G = w != null && F0(R) && A === !0,
		j = K.encodeLocation ? K.encodeLocation(R).pathname : R.pathname,
		I = H.pathname,
		$ =
			w && w.navigation && w.navigation.location
				? w.navigation.location.pathname
				: null;
	(s ||
		((I = I.toLowerCase()),
		($ = $ ? $.toLowerCase() : null),
		(j = j.toLowerCase())),
		$ && L && ($ = ul($, L) || $));
	const yt = j !== "/" && j.endsWith("/") ? j.length - 1 : j.length;
	let dt = I === j || (!d && I.startsWith(j) && I.charAt(yt) === "/"),
		Tt =
			$ != null &&
			($ === j || (!d && $.startsWith(j) && $.charAt(j.length) === "/")),
		k = { isActive: dt, isPending: Tt, isTransitioning: G },
		Dt = dt ? o : void 0,
		wt;
	typeof f == "function"
		? (wt = f(k))
		: (wt = [
				f,
				dt ? "active" : null,
				Tt ? "pending" : null,
				G ? "transitioning" : null,
			]
				.filter(Boolean)
				.join(" "));
	let Ne = typeof m == "function" ? m(k) : m;
	return M.createElement(
		Lh,
		{
			...v,
			"aria-current": Dt,
			className: wt,
			ref: C,
			style: Ne,
			to: S,
			viewTransition: A,
		},
		typeof b == "function" ? b(k) : b,
	);
});
Lf.displayName = "NavLink";
var V0 = M.forwardRef(
	(
		{
			discover: c = "render",
			fetcherKey: o,
			navigate: s,
			reloadDocument: f,
			replace: d,
			state: m,
			method: S = li,
			action: A,
			onSubmit: b,
			relative: v,
			preventScrollReset: C,
			viewTransition: R,
			defaultShouldRevalidate: H,
			...w
		},
		K,
	) => {
		let { useTransitions: L } = M.useContext(me),
			G = W0(),
			j = k0(A, { relative: v }),
			I = S.toLowerCase() === "get" ? "get" : "post",
			$ = typeof A == "string" && Yh.test(A),
			yt = (dt) => {
				if ((b && b(dt), dt.defaultPrevented)) return;
				dt.preventDefault();
				let Tt = dt.nativeEvent.submitter,
					k = (Tt == null ? void 0 : Tt.getAttribute("formmethod")) || S,
					Dt = () =>
						G(Tt || dt.currentTarget, {
							fetcherKey: o,
							method: k,
							navigate: s,
							replace: d,
							state: m,
							relative: v,
							preventScrollReset: C,
							viewTransition: R,
							defaultShouldRevalidate: H,
						});
				L && s !== !1 ? M.startTransition(() => Dt()) : Dt();
			};
		return M.createElement("form", {
			ref: K,
			method: I,
			action: j,
			onSubmit: f ? b : yt,
			...w,
			"data-discover": !$ && c === "render" ? "true" : void 0,
		});
	},
);
V0.displayName = "Form";
function K0(c) {
	return `${c} must be used within a data router.  See https://reactrouter.com/en/main/routers/picking-a-router.`;
}
function Gh(c) {
	let o = M.useContext(Ha);
	return (_t(o, K0(c)), o);
}
function w0(
	c,
	{
		target: o,
		replace: s,
		mask: f,
		state: d,
		preventScrollReset: m,
		relative: S,
		viewTransition: A,
		defaultShouldRevalidate: b,
		useTransitions: v,
	} = {},
) {
	let C = xh(),
		R = Le(),
		H = Bu(c, { relative: S });
	return M.useCallback(
		(w) => {
			if (A0(w, o)) {
				w.preventDefault();
				let K = s !== void 0 ? s : xu(R) === xu(H),
					L = () =>
						C(c, {
							replace: K,
							mask: f,
							state: d,
							preventScrollReset: m,
							relative: S,
							viewTransition: A,
							defaultShouldRevalidate: b,
						});
				v ? M.startTransition(() => L()) : L();
			}
		},
		[R, C, H, s, f, d, o, c, m, S, A, b, v],
	);
}
var J0 = 0,
	$0 = () => `__${String(++J0)}__`;
function W0() {
	let { router: c } = Gh("useSubmit"),
		{ basename: o } = M.useContext(me),
		s = m0(),
		f = c.fetch,
		d = c.navigate;
	return M.useCallback(
		async (m, S = {}) => {
			let { action: A, method: b, encType: v, formData: C, body: R } = M0(m, o);
			if (S.navigate === !1) {
				let H = S.fetcherKey || $0();
				await f(H, s, S.action || A, {
					defaultShouldRevalidate: S.defaultShouldRevalidate,
					preventScrollReset: S.preventScrollReset,
					formData: C,
					body: R,
					formMethod: S.method || b,
					formEncType: S.encType || v,
					flushSync: S.flushSync,
				});
			} else
				await d(S.action || A, {
					defaultShouldRevalidate: S.defaultShouldRevalidate,
					preventScrollReset: S.preventScrollReset,
					formData: C,
					body: R,
					formMethod: S.method || b,
					formEncType: S.encType || v,
					replace: S.replace,
					state: S.state,
					fromRouteId: s,
					flushSync: S.flushSync,
					viewTransition: S.viewTransition,
				});
		},
		[f, d, o, s],
	);
}
function k0(c, { relative: o } = {}) {
	let { basename: s } = M.useContext(me),
		f = M.useContext(Ye);
	_t(f, "useFormAction must be used inside a RouteContext");
	let [d] = f.matches.slice(-1),
		m = { ...Bu(c || ".", { relative: o }) },
		S = Le();
	if (c == null) {
		m.search = S.search;
		let A = new URLSearchParams(m.search),
			b = A.getAll("index");
		if (b.some((C) => C === "")) {
			(A.delete("index"),
				b.filter((R) => R).forEach((R) => A.append("index", R)));
			let C = A.toString();
			m.search = C ? `?${C}` : "";
		}
	}
	return (
		(!c || c === ".") &&
			d.route.index &&
			(m.search = m.search ? m.search.replace(/^\?/, "?index&") : "?index"),
		s !== "/" && (m.pathname = m.pathname === "/" ? s : Ce([s, m.pathname])),
		xu(m)
	);
}
function F0(c, { relative: o } = {}) {
	let s = M.useContext(Dh);
	_t(
		s != null,
		"`useViewTransitionState` must be used within `react-router-dom`'s `RouterProvider`.  Did you accidentally import `RouterProvider` from `react-router`?",
	);
	let { basename: f } = Gh("useViewTransitionState"),
		d = Bu(c, { relative: o });
	if (!s.isTransitioning) return !1;
	let m = ul(s.currentLocation.pathname, f) || s.currentLocation.pathname,
		S = ul(s.nextLocation.pathname, f) || s.nextLocation.pathname;
	return ui(d.pathname, S) != null || ui(d.pathname, m) != null;
}
function I0({ disabled: c = !1, onClick: o }) {
	return V.jsxs("button", {
		type: "button",
		className: `lever ${c ? "lever--disabled" : ""}`,
		onClick: o,
		disabled: c,
		"aria-label": "Spin lever",
		children: [
			V.jsx("span", { className: "lever__handle", "aria-hidden": "true" }),
			V.jsx("span", { className: "lever__label", children: "Spin" }),
		],
	});
}
function P0({ top: c, center: o, bottom: s, spinning: f, durationMs: d }) {
	return V.jsx("section", {
		className: `topic-stack ${f ? "topic-stack--spinning" : ""}`,
		"aria-label": "Topic display",
		children: V.jsxs("div", {
			className: "topic-stack__track",
			style: { "--spin-duration": `${d}ms` },
			children: [
				V.jsx("div", {
					className: "topic-slot topic-slot--faded",
					children: c,
				}),
				V.jsx("div", {
					className: "topic-slot topic-slot--center",
					children: o,
				}),
				V.jsx("div", {
					className: "topic-slot topic-slot--faded",
					children: s,
				}),
			],
		}),
	});
}
const tg = [
		"General",
		"Technology",
		"Finance",
		"History",
		"Sports",
		"One-Minute Idea Pitch",
	],
	kf = {
		General: [
			"A small habit that improved your daily life",
			"A meaningful lesson from a recent mistake",
			"The best advice you have received",
			"A place that feels like home",
			"A time you surprised yourself",
		],
		Technology: [
			"How AI could change everyday routines",
			"The most useful app on your phone",
			"A technology you think is overrated",
			"What makes a great user experience",
			"How software shapes modern work",
		],
		Finance: [
			"A smart way to manage spending",
			"Why emergency savings matter",
			"The difference between needs and wants",
			"A financial goal worth planning for",
			"How compound interest works in real life",
		],
		History: [
			"A historical event that still matters today",
			"A leader you would study and why",
			"A lesson from the past society should remember",
			"The most important invention in history",
			"How history changes the way people think",
		],
		Sports: [
			"What makes a great team player",
			"A sport you wish more people understood",
			"The role of discipline in athletic success",
			"A memorable comeback in sports",
			"How sports build community",
		],
		"One-Minute Idea Pitch": [
			"Pitch an app that solves one daily annoyance",
			"Sell a product that makes meetings better",
			"Describe a service for busy students",
			"Pitch a tool that saves time every week",
			"Present an idea that improves focus",
		],
	};
function eg(c) {
	return kf[c];
}
const Gf = 1400,
	lg = 2400;
function gh(c) {
	const o = kf[c];
	return {
		top: o[0],
		center: o[1] ?? o[0],
		bottom: o[2] ?? o[1] ?? o[0],
		spinning: !1,
		durationMs: Gf,
	};
}
function ag(c, o = Math.random()) {
	const s = kf[c],
		f = Math.round(Gf + o * (lg - Gf)),
		d = Math.floor(o * s.length) % s.length,
		m = (d - 1 + s.length) % s.length,
		S = (d + 1) % s.length;
	return { top: s[m], center: s[d], bottom: s[S], durationMs: f };
}
function ug() {
	const [c, o] = M.useState("General"),
		[s, f] = M.useState(() => gh("General")),
		d = M.useRef(null),
		m = M.useMemo(() => eg(c), [c]),
		S = () => {
			if (s.spinning) return;
			const A = ag(c);
			(f((b) => ({ ...b, spinning: !0, durationMs: A.durationMs })),
				d.current !== null && window.clearTimeout(d.current),
				(d.current = window.setTimeout(() => {
					(f({
						top: A.top,
						center: A.center,
						bottom: A.bottom,
						spinning: !1,
						durationMs: A.durationMs,
					}),
						(d.current = null));
				}, A.durationMs)));
		};
	return (
		M.useEffect(
			() => () => {
				d.current !== null && window.clearTimeout(d.current);
			},
			[],
		),
		V.jsxs("section", {
			className: "page page--spin",
			children: [
				V.jsxs("div", {
					className: "hero-copy",
					children: [
						V.jsx("h1", { children: "Pick a category and pull the lever." }),
						V.jsx("p", {
							className: "subcopy",
							children:
								"Practice impromptu speaking by spinning for a topic and talking for 1 minute.",
						}),
					],
				}),
				V.jsxs("div", {
					className: "spin-layout",
					children: [
						V.jsxs("div", {
							className: "control-card",
							children: [
								V.jsx("label", {
									className: "field-label",
									htmlFor: "category-select",
									children: "Category",
								}),
								V.jsx("select", {
									id: "category-select",
									className: "select",
									value: c,
									onChange: (A) => {
										const b = A.target.value;
										(o(b), f(gh(b)));
									},
									disabled: s.spinning,
									children: tg.map((A) =>
										V.jsx("option", { value: A, children: A }, A),
									),
								}),
								V.jsxs("div", {
									className: "spin-layout__controls",
									children: [
										V.jsx(P0, {
											top: s.top,
											center: s.center,
											bottom: s.bottom,
											spinning: s.spinning,
											durationMs: s.durationMs,
										}),
										V.jsxs("div", {
											className: "lever-column",
											children: [
												V.jsx("p", {
													className: "lever-hint",
													children: "Press to spin",
												}),
												V.jsx(I0, { disabled: s.spinning, onClick: S }),
											],
										}),
									],
								}),
							],
						}),
						V.jsxs("aside", {
							className: "detail-card",
							"aria-label": "Selected category prompts",
							children: [
								V.jsx("h2", { children: c }),
								V.jsx("ul", {
									className: "topic-list",
									children: m.map((A) => V.jsx("li", { children: A }, A)),
								}),
							],
						}),
					],
				}),
			],
		})
	);
}
function ng({
	displayTime: c,
	secondsLeft: o,
	running: s,
	paused: f,
	onStart: d,
	onPause: m,
	onReset: S,
}) {
	return V.jsxs("section", {
		className: `timer-panel ${f ? "timer-panel--paused" : ""}`,
		"aria-label": "Timer controls",
		children: [
			V.jsx("div", {
				className: "timer-panel__clock",
				"aria-live": "polite",
				children: c,
			}),
			V.jsxs("div", {
				className: "timer-panel__buttons",
				children: [
					V.jsx("button", {
						type: "button",
						className: "button button--primary",
						onClick: d,
						disabled: s,
						children: "Start",
					}),
					V.jsx("button", {
						type: "button",
						className: "button",
						onClick: m,
						disabled: !s,
						children: "Pause",
					}),
					V.jsx("button", {
						type: "button",
						className: "button",
						onClick: S,
						disabled: o === 60 && !s && !f,
						children: "Reset",
					}),
				],
			}),
		],
	});
}
const ig = 60;
function ph() {
	return { secondsLeft: ig, running: !1, paused: !1 };
}
function cg(c) {
	const o = Math.floor(c / 60),
		s = c % 60;
	return `${o}:${String(s).padStart(2, "0")}`;
}
function fg() {
	const [c, o] = M.useState(ph),
		s = M.useRef(null);
	M.useEffect(
		() => () => {
			s.current !== null && window.clearInterval(s.current);
		},
		[],
	);
	const f = () => {
			s.current !== null &&
				(window.clearInterval(s.current), (s.current = null));
		},
		d = () => {
			c.running ||
				c.secondsLeft === 0 ||
				(o((A) => ({ ...A, running: !0, paused: !1 })),
				(s.current = window.setInterval(() => {
					o((A) =>
						A.secondsLeft <= 1
							? (f(), { secondsLeft: 0, running: !1, paused: !1 })
							: { ...A, secondsLeft: A.secondsLeft - 1 },
					);
				}, 1e3)));
		},
		m = () => {
			c.running && (f(), o((A) => ({ ...A, running: !1, paused: !0 })));
		},
		S = () => {
			(f(), o(ph()));
		};
	return V.jsxs("section", {
		className: "page page--timer",
		children: [
			V.jsxs("div", {
				className: "hero-copy",
				children: [
					V.jsx("h1", { children: "Practice for one minute." }),
					V.jsx("p", {
						className: "subcopy",
						children:
							"Start when you are ready, pause if needed, and reset when finished.",
					}),
				],
			}),
			V.jsx(ng, {
				displayTime: cg(c.secondsLeft),
				secondsLeft: c.secondsLeft,
				running: c.running,
				paused: c.paused,
				onStart: d,
				onPause: m,
				onReset: S,
			}),
		],
	});
}
const Sh = ({ isActive: c }) => (c ? "nav-link nav-link--active" : "nav-link");
function rg() {
	return V.jsxs("div", {
		className: "app-shell",
		children: [
			V.jsx("header", {
				className: "top-nav",
				"aria-label": "Primary",
				children: V.jsxs("nav", {
					className: "nav-links",
					children: [
						V.jsx(Lf, {
							to: "/spin",
							className: Sh,
							end: !0,
							children: "Spin",
						}),
						V.jsx(Lf, { to: "/timer", className: Sh, children: "Timer" }),
					],
				}),
			}),
			V.jsx("main", {
				className: "app-main",
				children: V.jsxs(S0, {
					children: [
						V.jsx(Nu, {
							path: "/",
							element: V.jsx(yh, { to: "/spin", replace: !0 }),
						}),
						V.jsx(Nu, { path: "/spin", element: V.jsx(ug, {}) }),
						V.jsx(Nu, { path: "/timer", element: V.jsx(fg, {}) }),
						V.jsx(Nu, {
							path: "*",
							element: V.jsx(yh, { to: "/spin", replace: !0 }),
						}),
					],
				}),
			}),
		],
	});
}
Ev.createRoot(document.getElementById("root")).render(
	V.jsx(hv.StrictMode, { children: V.jsx(Z0, { children: V.jsx(rg, {}) }) }),
);
